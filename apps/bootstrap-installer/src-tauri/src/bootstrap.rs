...
//! CLIO_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for CLIO_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub clio_home: Option<String>,
    /// GitHub personal access token for cloning private repos.
    /// Passed to install.ps1 as -GitHubToken so HTTPS clone and ZIP
    /// download work on private repositories.
...
pub async fn start_bootstrap(
    ...
    // If no token was provided by the frontend, fall back to the CLI arg token.
    if args.github_token.is_none() {
        args.github_token = state.github_token.clone();
    }
...
/// Spawn the locally-built Clio desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_clio_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_clio_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Clio desktop at {}. The desktop build step \
             may have been skipped or failed. Run `clio desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Clio desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Clio
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_clio_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Clio.exe"),
            ("win-arm64-unpacked", "Clio.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Clio.app/Contents/MacOS", "Clio"),
            ("mac-arm64/Clio.app/Contents/MacOS", "Clio"),
        ]
    } else {
        &[("linux-unpacked", "clio")]
    };
...
pub(crate) fn resolve_clio_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_clio_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Clio.app/Contents/MacOS/Clio -> .../Clio.app
...
/// path so a bare re-open just opens Clio instead of re-running setup.
pub(crate) fn clio_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".clio-bootstrap-complete").exists()
        && resolve_clio_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_clio_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Clio desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_clio_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::clio_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    let manifest_args = build_pin_args(&script, args.github_token.as_deref());
    ...
    // explicitly; we infer it from $ClioHome which Stage-Repository clones
    // the repo INTO at $ClioHome\clio-agent. Mirrors clio_constants.
    let clio_home = args
        .clio_home
        .clone()
        .unwrap_or_else(|| crate::paths::clio_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&clio_home).join("clio-agent");

    // Copy ourselves to CLIO_HOME/clio-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_clio_home() {
        tracing::warn!(?err, "failed to copy installer into CLIO_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    powershell::run_script(script_path, args, sink, clio_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
fn build_pin_args(script: &install_script::ResolvedScript, github_token: Option<&str>) -> Vec<String> {
    let mut out = Vec::new();
    ...
    if let Some(token) = github_token {
        if !token.trim().is_empty() {
            out.push("-GitHubToken".to_string());
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Clio"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Clio.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Clio.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("clio");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Clio.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_clio_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_clio_desktop_app(&root)
    ...
    #[test]
    fn resolve_clio_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_clio_desktop_app(&root).is_none(),

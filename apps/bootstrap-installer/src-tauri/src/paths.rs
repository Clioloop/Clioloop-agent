...
//!
//! Mirrors `clio_constants.get_clio_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\clio
//!   macOS:   ~/.clio
//!   Linux:   ~/.clio  (override via $CLIO_HOME)
//!
//! NOTE (macOS): Python's get_clio_home(), scripts/install.sh, and the
//! Electron desktop's resolveClioHome() ALL use ~/.clio on macOS — there
...
/// Returns the canonical Clio home directory, respecting $CLIO_HOME if set.
pub fn clio_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("CLIO_HOME") {
    ...
    {
        // %LOCALAPPDATA%\clio — matches scripts/install.ps1's $ClioHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("clio");
        }
    }

    // macOS + Linux + fallback: ~/.clio (matches Python get_clio_home(),
    // install.sh, and the Electron desktop's resolveClioHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".clio");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    clio_home().join("bootstrap-cache")
}
...
/// CLIO_HOME so it survives repo checkout deletion (unlike anything under
/// clio-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\clio\clio-setup.exe`; on other
...
pub fn copy_self_to_clio_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to CLIO_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_CLIO_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".clio-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under CLIO_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[clio-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("CLIO_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_clio_home() -> String {
    clio_home().to_string_lossy().into_owned()
}

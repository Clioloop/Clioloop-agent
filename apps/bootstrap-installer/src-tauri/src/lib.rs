...
/// desktop app handing off via `Clio-Setup.exe --update`).
///
...
pub struct AppState {
    ...
    pub mode: AppMode,
    /// GitHub token for cloning private repos, passed via --github-token CLI arg.
    pub github_token: Option<String>,
}

impl AppState {
    fn new(mode: AppMode, github_token: Option<String>) -> Self {
        Self {
...
/// Frontend → Rust: whether a GitHub token was provided via CLI args.
#[tauri::command]
fn has_github_token(state: tauri::State<'_, Arc<AppState>>) -> bool {
    state.github_token.is_some()
}

/// Extract `--github-token <value>` from an arg iterator. Returns the token
/// value and the remaining args (with the flag and its value removed).
fn extract_github_token<I, S>(args: I) -> (Option<String>, Vec<String>)
where
    I: IntoIterator<Item = S>,
    S: AsRef<str> + Into<String>,
{
    ...
    let mut iter: Vec<String> = args.into_iter().map(|s| s.into()).collect();
    let mut i = 0;
    while i < iter.len() {
        if iter[i] == "--github-token" && i + 1 < iter.len() {
            token = Some(iter[i + 1].clone());
            i += 2;
        } else {
            remaining.push(iter[i].clone());
            i += 1;
        }
    }
    (token, remaining)
...
pub fn run() {
    // Tracing → bootstrap-installer.log under CLIO_HOME/logs/ so install
    // failures leave a trail for support. Console output also goes here in
    // debug builds.
    let _guard = paths::init_logging();

    let (github_token, remaining_args) = extract_github_token(std::env::args().skip(1));
    let mode = AppMode::from_args(&remaining_args);
    // Escape hatch: `--reinstall`/`--repair` forces the installer UI even when
    // Clio is already installed, so users can re-run setup to repair a broken
    // install instead of the launcher fast path silently relaunching the app.
    let force_setup = force_setup_from_args(&remaining_args);
    tracing::info!(?mode, force_setup, github_token = github_token.is_some(), "Clio installer starting");
    ...
    tauri::Builder::default()
        ...
        .manage(Arc::new(AppState::new(mode, github_token.clone())))
        .setup(move |app| {
            use tauri::Manager;
            // Launcher fast path (macOS only): a bare ("Install") launch when
            // Clio is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Clio" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Clio" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_clio_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::clio_home().join("clio-agent");
                if bootstrap::clio_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_clio_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "clio already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }

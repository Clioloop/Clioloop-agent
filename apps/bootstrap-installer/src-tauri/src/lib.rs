...
/// Filter out any `--github-token <value>` flags from an arg iterator.
/// No longer used by the app, but we silently strip them so old scripts
/// that pass the flag don't break.
fn strip_github_token<I, S>(args: I) -> Vec<String>
where
    I: IntoIterator<Item = S>,
...
pub fn run() {
    ...
    let remaining_args = strip_github_token(std::env::args().skip(1));
    ...
    tracing::info!(?mode, force_setup, "Clio installer starting");

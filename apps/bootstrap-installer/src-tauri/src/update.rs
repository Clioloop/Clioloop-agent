...
//!
//! Driven when the installer is launched as `Clio-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Clio desktop process to fully exit (so the venv
//!      shim is free; otherwise `clio update` aborts with exit code 2),
//!   2. run `clio update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in clio_cli/main.py),
//!   3. run `clio desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `clio update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_clio) and
...
/// `clio update` exit code meaning "another clio process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// clio_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release the venv shim
/// before giving up and letting `clio update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Clio"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let clio_home = crate::paths::clio_home();
    let install_root = clio_home.join("clio-agent");
    ...
    let clio = resolve_clio(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the clio CLI under {}. Is Clio installed? \
    ...
    // async on Windows. If it still holds the venv shim, `clio update`
    // aborts with exit 2. Give it a bounded window to clear.
    wait_for_venv_free(&install_root, &app).await;

    // ---- stage 1: clio update -----------------------------------------
    // Pass --branch so `clio update` targets the branch this installer was
    ...
    // without --branch, `clio update` switches the checkout to `main` (a
    ...
    // --force skips `clio update`'s Windows running-exe guard (which would
    ...
    // time `clio update` runs there is no legitimate clio.exe to protect,
    // and the guard would only produce a false "Clio is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `clio update` lazily imports
    ...
    // second `clio update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Clio" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Clio is still running. Close all Clio windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "clio update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::clio_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: clio desktop --build-only ----------------------------
    // `clio update` deliberately does NOT build apps/desktop (it installs
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `clio desktop` \
    ...
    if let Some(target_app) = launch_target {
        ...
        crate::bootstrap::launch_clio_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    let shim = venv_clio(install_root);
    let deadline = Instant::now() + DESKTOP_EXIT_WAIT;

    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Clio to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend clio.exe (or a grandchild it spawned)
            ...
            // every clio.exe except ourselves, then give the OS a beat to
            // unload the image.
            emit_log(
                app,
                Some("update"),
                LogStream::Stdout,
                "[update] Clio still holding the venv shim; force-killing stragglers…",
            );
            force_kill_other_clio();
...
/// Force-kill any `clio.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `clio.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside `wait_for_venv_free`,
/// which completes BEFORE we spawn `venv\Scripts\clio.exe update`. At this
/// point no update-driven clio.exe exists yet, so the only clio.exe images
...
/// Spawn `clio <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv clio shim under an install root, regardless of existence.
fn venv_clio(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("clio.exe")
    } else {
        install_root.join("venv").join("bin").join("clio")
    }
}

/// Resolve the clio CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `clio` on PATH.
fn resolve_clio(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_clio(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "clio.exe" } else { "clio" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        "CLIO_HOME".to_string(),
        clio_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        clio_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_clio_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Clio.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.clio-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.clio-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::clio_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_clio_is_under_install_root() {
        let root = Path::new("/x/clio-agent");
        let shim = venv_clio(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Clio.app"]),
            Some(PathBuf::from("/Applications/Clio.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Clio.app");
        let tmp = base.join("Clio.app.clio-update-new");
        let old = base.join("Clio.app.clio-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Clio.app");
        let tmp = base.join("Clio.app.clio-update-new"); // intentionally absent
        let old = base.join("Clio.app.clio-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Clio.app");
        let tmp = base.join("Clio.app.clio-update-new"); // absent
        let old = base.join("Clio.app.clio-update-old");
        write_marker(&target, "OLD");

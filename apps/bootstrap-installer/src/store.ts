...
export const $clioHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, clioHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_clio_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $clioHome.set(clioHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Clio" success screen. Update: this is a
...
export async function startInstall(opts?: { branch?: string; githubToken?: string }): Promise<void> {
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Clio-Setup.exe --update);
...
export async function launchClioDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_clio_desktop', { installRoot })
}

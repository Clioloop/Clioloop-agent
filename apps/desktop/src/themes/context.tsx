...
import { BUILTIN_THEME_LIST, BUILTIN_THEMES, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, managedTheme } from './presets'
import type { DesktopTheme, DesktopThemeColors } from './types'

const SKIN_KEY = 'clio-desktop-theme-v2'
const MODE_KEY = 'clio-desktop-mode-v1'
const RETIRED_SKINS = new Set(['managed-light', 'default', 'gold'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = BUILTIN_THEMES[skinName] ?? managedTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...managedTheme.typography, ...theme.typography }
  ...
  root.dataset.clioTheme = skinName
  root.dataset.clioMode = rendered
  ...
  window.clioDesktop?.setTitleBarTheme?.({
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.clioThemeFont = 'true'
...
/** Sync the desktop skin with the active Clio backend theme on connect. */

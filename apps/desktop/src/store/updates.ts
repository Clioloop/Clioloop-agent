...
const UPDATE_TOAST_SNOOZE_KEY = 'clio:update-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: translateNow('notifications.updateClio'), onClick: () => void applyUpdates() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.clioDesktop?.getVersion?.()
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    ...
    // The update check pulls the latest clio_cli + bundled package metadata
...
function onFocus() {
  ...
  // updated Clio from another window/CLI between focuses, and About should

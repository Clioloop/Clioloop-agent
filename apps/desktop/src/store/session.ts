...
import type { SessionInfo, UsageStats } from '@/types/clio'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'clio.desktop.workspace-cwd'
...
export const $connection = atom<ClioConnection | null>(null)
...
export const $fusionPickerOpen = atom(false)

export const setConnection = (next: Updater<ClioConnection | null>) => updateAtom($connection, next)
...
export const setFusionPickerOpen = (next: Updater<boolean>) => updateAtom($fusionPickerOpen, next)

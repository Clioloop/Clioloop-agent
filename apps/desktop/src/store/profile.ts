...
import { getProfiles, setApiRequestProfile } from '@/clio'
...
// /api/profiles/active `current`). "default" is the root ~/.clio. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'clio.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'clio.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.clioDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new CLIO_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.clioDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'clio.desktop.showAllProfiles'
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.clioDesktop?.touchBackend?.(target).catch(() => undefined)
}

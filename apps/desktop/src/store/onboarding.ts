...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/clio'
...
export interface DesktopOnboardingState {
  ...
  /** True until the user has been through the first-run provider picker at all
   *  (connected a provider OR chose "later") on this desktop install. While
   *  true, the overlay is forced open ONCE on launch — even if a provider is
   *  already configured — so every new install lands on provider selection and
   *  sees Omni Loop recommended. Persisted to localStorage; distinct from
   *  `configured` (which can be true via a CLI login / no-key fallback). */
...
const CONFIGURED_CACHE_KEY = 'clio-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'clio-onboarding-skipped-v1'
// Set once the user has been through the first-run picker (connected or
// deferred). Until then the picker is forced open once on launch.
const FIRSTRUN_CACHE_KEY = 'clio-onboarding-firstrun-v1'
const POLL_MS = 2000
...
function readFirstRunSeen(): boolean {
  ...
  try {
    return window.localStorage.getItem(FIRSTRUN_CACHE_KEY) === '1'
  } catch {
    return false
  }
}

function writeFirstRunSeen(value: boolean) {
  if (typeof window === 'undefined') {
    return
  }
  ...
  try {
    if (value) {
      window.localStorage.setItem(FIRSTRUN_CACHE_KEY, '1')
    } else {
      window.localStorage.removeItem(FIRSTRUN_CACHE_KEY)
    }
  } catch {
    // localStorage unavailable — degrade silently.
  }
...
const FIRST_RUN_PENDING = !readFirstRunSeen()
...
const INITIAL: DesktopOnboardingState = {
  ...
  providers: null,
  reason: null,
  // On first run, request the provider list up front so the forced picker is
  // populated as soon as the gateway is reachable.
  requested: FIRST_RUN_PENDING,
...
function isRetiredManagedProvider(provider: OAuthProvider) {
  return RETIRED_PROVIDER_IDS.has(provider.id)
}
...
async function fetchProviderDefaultModel(
  ...
  // `clio model` does (for the managed provider it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Clio still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Clio still cannot resolve a usable provider.'
}
...
async function refreshProviders() {
  ...
  providersRefreshPromise = (async () => {
    try {
      ...
      const visibleProviders = providers.filter(p => !isRetiredManagedProvider(p))
      const state = $desktopOnboarding.get()

      if (state.firstRunPending && visibleProviders.length === 0) {
        patch({ mode: 'oauth', providers: null })
      ...
      if (state.firstRunPending) {
        patch({
          mode: 'oauth',
          providers: state.providers && state.providers.length > 0 ? state.providers : null
        })
...
export function completeDesktopOnboarding() {
  ...
  // The user has now been through the first-run picker — never force it again.
  writeFirstRunSeen(true)
...
export function dismissFirstRunOnboarding() {
  ...
  // The user has been through the first-run picker (deferred) — never force it again.
  writeFirstRunSeen(true)
  patch({
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  // First run: force the provider picker once regardless of whether a provider
  // already resolves (CLI login / no-key fallback). Don't auto-complete — just
  // load the provider list and let the overlay show the picker.
  if ($desktopOnboarding.get().firstRunPending) {
    await refreshProviders()
...
async function openSignInUrl(url: string) {
  if (window.clioDesktop?.openExternal) {
    try {
      await window.clioDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Clio still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Clio still cannot reach ${url}.` }
    }

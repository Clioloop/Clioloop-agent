...
describe('refreshOnboarding', () => {
  ...
  it('keeps live managed provider while hiding only retired managed aliases', async () => {
    const api = vi.fn(async ({ path }: { path: string }) => {
      if (path === '/api/providers/oauth') {
        return {
          providers: [
            provider('anthropic', 'Anthropic Claude'),
            provider('managed', 'managed provider'),
            provider('managed-provider', 'old managed provider'),
            provider('managed_provider', 'old managed provider')
          ]
        }
      }
    ...
    expect($desktopOnboarding.get().providers?.map(p => p.id)).toEqual(['anthropic', 'managed'])
  })
...
describe('first-run onboarding', () => {
  ...
  it('forces the picker on first run even when a provider is already usable', async () => {
    const api = vi.fn(async ({ path }: { path: string }) => {
      if (path === '/api/providers/oauth') {
        return { providers: [provider('managed', 'managed provider')] }
      }
    ...
    installApiMock(api)
    // Provider configured + runtime ready, but first run is pending → must NOT
    // auto-complete; it should load providers and keep the picker open.
    $desktopOnboarding.set(baseState({ firstRunPending: true, configured: true, requested: true }))
    ...
    expect(ready).toBe(false)
    expect($desktopOnboarding.get().firstRunPending).toBe(true)
    expect($desktopOnboarding.get().providers?.map(p => p.id)).toEqual(['managed'])
  })

  it('keeps first-run provider discovery pending when OAuth provider fetch fails', async () => {
    const api = vi.fn(async ({ path }: { path: string }) => {
      if (path === '/api/providers/oauth') {
        throw new Error('provider endpoint warming up')
      }
  ...
  it('keeps first-run provider discovery pending when OAuth provider fetch is empty', async () => {
    const api = vi.fn(async ({ path }: { path: string }) => {
      if (path === '/api/providers/oauth') {
        return { providers: [] }
      }
  ...
  it('dismissFirstRunOnboarding clears the pending flag and persists it', () => {
    $desktopOnboarding.set(baseState({ firstRunPending: true }))

    dismissFirstRunOnboarding()

    expect($desktopOnboarding.get().firstRunPending).toBe(false)
    expect(window.localStorage.getItem('clio-onboarding-firstrun-v1')).toBe('1')
  })

  it('completeDesktopOnboarding clears the pending flag and persists it', () => {
    $desktopOnboarding.set(baseState({ firstRunPending: true }))
...
describe('OAuth onboarding', () => {
  ...
  it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: provider('openai-codex', 'OpenAI OAuth'),
          start: {
    ...
    if (state.flow.status === 'confirming_model') {
      expect(state.flow.label).toBe('OpenAI OAuth')

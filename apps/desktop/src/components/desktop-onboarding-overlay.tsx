...
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    description: 'Point Clio at a local or self-hosted OpenAI-compatible endpoint (vLLM, llama.cpp, Ollama, etc).',
    docsUrl: 'https://github.com/Clioloop/Clioloop-agent#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// The Omni Loop Portal Subscription (backend id "managed") is the recommended first-run
// path: one device login, 300+ models and the Tool Gateway, no API keys.
const PORTAL_PROVIDER_ID = OMNI_PROVIDER_SLUG

const PROVIDER_DISPLAY: Record<string, { order: number; title: string }> = {
  [PORTAL_PROVIDER_ID]: { order: 0, title: OMNI_PROVIDER_LABEL },
...
const FLOW_SUBTITLES: Record<OAuthProvider['flow'], string> = {
  ...
  device_code: 'Opens a verification page in your browser — Clio connects automatically',
  loopback: 'Opens your browser to sign in — Clio connects automatically',
...
// "managed" (Omni Loop Portal Subscription) — NOT retired; it's the recommended option.
...
export const sortProviders = (providers: OAuthProvider[]) =>
  providers
    .filter(p => !isRetiredManagedProvider(p))
...
export function DesktopOnboardingOverlay({ enabled, onCompleted, requestGateway }: DesktopOnboardingOverlayProps) {
  ...
  if (onboarding.configured === true && !onboarding.manual && !onboarding.firstRunPending) {
    return null
  }
  ...
  const ready = onboarding.manual || onboarding.firstRunPending || (enabled && onboarding.configured === false)
  ...
  return (
    <div className="grid gap-3" role="status">
      <p className="text-sm text-muted-foreground">
        {installing
          ? 'Clio is finishing install. This usually takes under a minute on first run.'
          : 'Starting Clio…'}
      </p>
  ...
  return (
    <div className="border-b border-(--ui-stroke-tertiary) bg-(--ui-chat-bubble-background) px-5 py-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 className="text-[0.9375rem] font-semibold tracking-tight">Let's get you setup with Clioloop</h2>
...
function FlowPanel({ ctx, flow }: { ctx: OnboardingContext; flow: OnboardingFlow }) {
  ...
  if (flow.status === 'awaiting_browser') {
    return (
      <Step title={`Sign in with ${title}`}>
        <p className="text-sm text-muted-foreground">
          We opened {title} in your browser. Authorize Clio there and you'll be connected automatically — nothing to
          copy or paste.
        </p>

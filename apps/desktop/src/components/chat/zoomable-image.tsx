...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'clio:saveImageFromUrl'")
}
...
export function ZoomableImage({ className, containerClassName, src, alt, slot, ...props }: ZoomableImageProps) {
  ...
  async function handleDownload() {
    ...
    try {
      if (window.clioDesktop?.saveImageFromUrl) {
        const saved = await window.clioDesktop.saveImageFromUrl(src)
      ...
      if (isMissingIpcHandler(error)) {
        try {
          ...
          notify({
            ...
            message: 'Restart Clio Desktop to use Save Image.'
          })
        } catch (fallbackError) {
          notifyError(fallbackError, 'Restart Clio Desktop to save images')
        }

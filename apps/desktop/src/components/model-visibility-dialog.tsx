...
import type { ModelOptionProvider, ModelOptionsResponse } from '@/types/clio'
...
export function ModelVisibilityDialog({
  ...
  const providers = useMemo(
    () =>
      [...(modelOptions.data?.providers ?? []).filter(provider => (provider.models ?? []).length > 0)].sort(
        compareOmniFirst
      ),
    [modelOptions.data]
  )
  ...
  const providerDisplayName = (provider: ModelOptionProvider) =>
    isOmniProvider(provider.slug) ? OMNI_PROVIDER_LABEL : provider.name

  const matches = (provider: ModelOptionProvider, model: string) =>
    !q ||
    `${model} ${providerDisplayName(provider)} ${provider.name} ${provider.slug} ${displayModelName(model)}`
      .toLowerCase()
      .includes(q)
            ...
            providers.map(provider => {
              ...
              return (
                <div className="py-0.5" key={provider.slug}>
                  <div
                    className="flex items-center gap-1.5 px-3 pb-0.5 pt-1 text-[0.625rem] font-medium uppercase tracking-wide text-(--ui-text-tertiary)"
                    title={omni ? OMNI_PROVIDER_DESCRIPTION : undefined}
                  >
                    <span className="truncate">{providerDisplayName(provider)}</span>
                    {omni ? <span className="shrink-0 text-primary">{OMNI_PROVIDER_BADGE}</span> : null}
                  </div>
                  {models.map(family => {

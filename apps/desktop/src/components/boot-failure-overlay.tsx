...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.clioDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.clioDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.clioDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }
  ...
  const signInRemote = async () => {
    ...
    try {
      const result = await window.clioDesktop?.oauthLoginConnectionConfig(remoteReauth.url)
  ...
  const openLogs = () => void window.clioDesktop?.revealLogs().catch(() => undefined)
  ...
  return (
    <div className="fixed inset-0 z-[1400] flex items-center justify-center bg-[color-mix(in_srgb,var(--ui-chat-surface-background)_82%,transparent)] p-6 backdrop-blur-sm">
      <div className="w-full max-w-[40rem] overflow-hidden rounded-xl border border-(--ui-stroke-secondary) bg-[color-mix(in_srgb,var(--ui-chat-bubble-background)_92%,transparent)] shadow-sm backdrop-blur-xl">

...
function setPickerState(overrides: Partial<DesktopOnboardingState>) {
  $desktopOnboarding.set({
    configured: false,
    flow: { status: 'idle' },
    mode: 'oauth',
...
describe('onboarding Picker', () => {
  it('keeps first-run provider discovery pending instead of showing API-key setup', () => {
    setPickerState({ firstRunPending: true, providers: null })
  ...
  it('surfaces the Omni Loop Portal Subscription (managed) as the recommended first-run option', () => {
    // The backend may still send the legacy "managed provider" name; the
    // display title is overridden to the public subscription name regardless.
    setProviders([provider('anthropic', 'Anthropic Claude'), provider('managed', 'managed provider')])
    render(<Picker ctx={ctx} />)

    const omni = screen.getByText('Omni Loop Portal Subscription')
    const anthropic = screen.getByText('Anthropic API Key')

    expect(omni).toBeTruthy()
    expect(screen.getByText('Recommended')).toBeTruthy()
    expect(
      screen.getByText('Currently the only way to use Model Fusion · one login · no API keys · includes Tool Gateway')
    ).toBeTruthy()
    expect(omni.compareDocumentPosition(anthropic) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  })

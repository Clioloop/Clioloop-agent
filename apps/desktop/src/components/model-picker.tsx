...
export function ModelPickerDialog({
  ...
  // the `clio model` CLI picker, which shows the curated list verbatim.
...
function ModelResults({
  ...
  if (providers.length === 0) {
    return (
      <div className="px-4 py-6 text-sm text-muted-foreground">
        No authenticated providers. Add {OMNI_PROVIDER_LABEL} first for Model Fusion.
      </div>
    )
  }
  ...
  const configured = [...providers.filter(p => (p.models ?? []).length > 0)].sort(compareOmniFirst)
            ...
            >
              ...
              {provider.slug === 'managed' && (
                <span
                  className={cn(
                    'shrink-0 text-[0.62rem] lowercase tracking-wide',
                    isCurrent ? 'text-primary-foreground/70' : 'text-muted-foreground/70'
                  )}
                  title={
                    price?.free
                      ? 'Free model'
              ...
              <ModelPrice hideFree={provider.slug === 'managed'} isCurrent={isCurrent} price={price} />
            </CommandItem>
          )
        }
...
function ModelPrice({
  ...
  if (price.free) {
    // The managed picker shows a "(free)" suffix tag already, so skip the
    // duplicate green badge there.
    if (hideFree) {
      return null
    }
...
function ProviderHeading({ provider }: { provider: ModelOptionProvider }) {
  ...
  return (
    <span className="flex min-w-0 items-center gap-2">
      <span className="truncate" title={omni ? OMNI_PROVIDER_DESCRIPTION : undefined}>
        {omni ? OMNI_PROVIDER_LABEL : provider.name}
      </span>

...
describe('deriveProviderShape', () => {
  ...
  it('OAuth shape when the provider is a redirect IDP', () => {
    expect(deriveProviderShape([{ name: 'managed', displayName: 'Omni Loop Labs', supportsPassword: false }])).toEqual({
      isPassword: false,
      providerLabel: 'Omni Loop Labs'
    })
  })
  ...
  it('mixed deployment keeps generic OAuth copy (not every provider is password)', () => {
    const shape = deriveProviderShape([
      ...
      { name: 'managed', displayName: 'Omni Loop Labs', supportsPassword: false }
    ])

    expect(shape.isPassword).toBe(false)
    expect(shape.providerLabel).toBe('Username & Password / Omni Loop Labs')
  })
...
describe('signInLabel', () => {
  ...
  it('OAuth gateway names the provider', () => {
    expect(signInLabel({ url: 'x', isPassword: false, providerLabel: 'Omni Loop Labs' })).toBe(
      'Sign in with Omni Loop Labs'
    )
  })

...
const TOOL_META: Record<string, ToolMeta> = {
  ...
  music_generate: { done: 'Generated music', pending: 'Generating music', icon: 'music', tone: 'image' },
...
function toolImageUrl(args: Record<string, unknown>, result: Record<string, unknown>): string {
  const candidate =
    firstStringField(result, ['image', 'image_url', 'url', 'path', 'image_path']) ||
  ...
  // Inline-render images the renderer can load: data URLs, remote http(s), or a
  // local filesystem path with an image extension (e.g. image_generate output or
  // vision_analyze's input). Local paths are resolved to a data URL by
  // ResolvedImage; non-image references fall back to the tool's codicon.
  ...
  const isLocalImage = !/^[a-z][a-z0-9+.-]*:/i.test(candidate) && mediaKind(candidate) === 'image'

...
const ApprovalBar: FC<{ request: ApprovalRequest }> = ({ request }) => {
  ...
  // "Always allow" persists the pattern to ~/.clio/config.yaml permanently, so
  ...
  const respond = useCallback(
    async (choice: ApprovalChoice) => {
      ...
      if (!gateway) {
        notifyError(new Error('Clio gateway is not connected'), 'Could not send approval response')
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <code className="font-mono text-xs">~/.clio/config.yaml</code>). Clio won’t ask again for commands

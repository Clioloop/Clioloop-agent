...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.clioDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  return window.clioDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.clioDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>

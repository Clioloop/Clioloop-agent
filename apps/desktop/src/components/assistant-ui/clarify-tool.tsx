...
export const ClarifyTool = (props: ToolCallMessagePartProps) => {
  ...
  // Once Clio records an answer, fall back to the standard tool block so
...
function ClarifyToolPending({ args }: ToolCallMessagePartProps) {
  ...
  const respond = useCallback(
    async (answer: string) => {
      ...
      if (!gateway) {
        notifyError(new Error('Clio gateway is not connected'), 'Could not send clarify response')

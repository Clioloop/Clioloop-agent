...
import { CLIO_PATHS_MIME, extractDroppedFiles } from '@/app/chat/hooks/use-composer-actions'
import type { ClioGateway } from '@/clio'
import { ClarifyTool } from '@/components/assistant-ui/clarify-tool'
import { clioDirectiveFormatter, DirectiveContent } from '@/components/assistant-ui/directive-text'
...
const ResponseLoadingIndicator: FC = () => {
  ...
  return (
    <StatusRow data-slot="aui_response-loading" label="Clio is loading a response">
...
// The image_generate tool returns JSON like {"success": true, "image": <path|url>}.
// Pull out the renderable reference (a local cache path or a remote URL).
function generatedImageSrc(result: unknown): string {
  ...
  for (const key of ['image', 'image_url', 'url', 'image_path', 'path']) {
...
const ImageGenerateTool: FC<ToolCallMessagePartProps> = ({ result }) => {
  ...
  const src = generatedImageSrc(result)
  ...
  return (
    <div className="mt-1.5 max-w-72 overflow-hidden rounded-[0.25rem] border border-(--ui-stroke-tertiary)">
      <ResolvedImage alt="Generated image" className="h-auto w-full object-cover" src={src} />
    </div>
  )
}
...
const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const replaceTriggerWithChip = useCallback(
    (item: Unstable_TriggerItem) => {
      ...
      const serialized = clioDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLElement>) => {
    if (!dragHasAttachments(event.dataTransfer, CLIO_PATHS_MIME)) {
      return
    }

...
const CLIO_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type ClioRefType = (typeof CLIO_REF_TYPES)[number]
...
const ICON_PATHS: Record<ClioRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ClioRefType] ?? ICON_FALLBACK
...
const CLIO_DIRECTIVE_RE = new RegExp(
...
export const clioDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(CLIO_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ClioRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ClioRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Clio directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => clioDirectiveFormatter.parse(cleanedText), [cleanedText])

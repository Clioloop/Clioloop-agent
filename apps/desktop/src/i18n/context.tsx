...
export interface I18nConfigClient {
  getConfig: () => Promise<ClioConfigRecord>
  saveConfig: (config: ClioConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.clioDesktop?.api) {
      return Promise.resolve({})
    }

    return getClioConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.clioDesktop?.api) {
...
export function getConfigDisplayLanguage(config: ClioConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ClioConfigRecord, locale: Locale): ClioConfigRecord {

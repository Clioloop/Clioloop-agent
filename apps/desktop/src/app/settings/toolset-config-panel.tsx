...
import { deleteEnvVar, getToolsetConfig, revealEnvVar, selectToolsetProvider, setEnvVar } from '@/clio'
import { Check, Loader2, Save } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ToolEnvVar, ToolProvider, ToolsetConfig } from '@/types/clio'
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange }: ToolsetConfigPanelProps) {
  ...
  // panel highlighted the first keyless provider (e.g. managed provider) even when
          ...
          <div className="overflow-hidden rounded-xl bg-background/60" key={provider.name}>
            ...
            {isActive && (
              <div className="grid gap-2 bg-muted/20 p-3">
                ...
                {provider.requires_managed_auth && (
                  <p className="text-[0.72rem] text-muted-foreground">
                    This retired gateway provider is no longer available.
                  </p>
                )}
                ...
                {provider.post_setup && (
                  <p className="text-[0.72rem] text-muted-foreground">
                    ...
                    <code className="font-mono">clio tools</code> for now.
                  </p>
                )}
              </div>
            )}

...
import type { ClioConfigRecord } from '@/types/clio'
...
describe('settings helpers', () => {
  it('reads and writes nested config paths', () => {
    const config: ClioConfigRecord = { display: { theme: 'mono' } }
  ...
  describe('providerGroup', () => {
    it('maps a provider env var to its labeled group', () => {
      ...
      expect(providerGroup('CLIO_API_KEY')).toBe('Clio')
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // CLIO_QWEN_ and CLIO_GEMINI_ both share the CLIO_ stem.
      expect(providerGroup('CLIO_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('CLIO_GEMINI_CLIENT_ID')).toBe('Gemini')
    })

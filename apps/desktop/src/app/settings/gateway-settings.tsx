...
export function GatewaySettings() {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.clioDesktop.applyConnectionConfig(payload())
        : await window.clioDesktop.saveConnectionConfig(payload())
      ...
      notify({
        ...
        message: apply ? 'Clio Desktop will reconnect using the saved settings.' : 'Saved for the next restart.'
      })
    } catch (err) {
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.clioDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.clioDesktop.oauthLoginConnectionConfig(trimmedUrl)

      if (result.connected) {
        const refreshed = await window.clioDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.clioDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.clioDesktop.getConnectionConfig(scope)
      setState(refreshed)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.clioDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
      ...
      const message = `Connected to ${result.baseUrl}${result.version ? ` · Clio ${result.version}` : ''}`
      setLastTest(message)
  ...
  if (!window.clioDesktop?.getConnectionConfig) {
    return (
      <EmptyState
    ...
    <SettingsContent>
      <div className="mb-5">
        ...
        <p className="mt-2 max-w-2xl text-[length:var(--conversation-caption-font-size)] leading-(--conversation-caption-line-height) text-(--ui-text-tertiary)">
          Clio Desktop starts its own local gateway by default. Use a remote gateway when you want this app to control
          an already-running Clio backend on another machine or behind a trusted proxy. Pick a profile below to give it
          its own remote host.
        </p>
      </div>
        ...
        <div className="mb-5 flex items-start gap-2 rounded-xl border border-destructive/30 bg-destructive/10 px-3 py-2.5 text-[length:var(--conversation-caption-font-size)] text-destructive">
          ...
          <div>
            ...
            <div className="mt-1 leading-5">
              Unset <code>CLIO_DESKTOP_REMOTE_URL</code> and <code>CLIO_DESKTOP_REMOTE_TOKEN</code> to use the saved
              setting below.
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <ModeCard
          ...
          description="Start a private Clio backend on localhost. This is the default and works offline."
        ...
        <ModeCard
          ...
          description="Connect this desktop shell to a remote Clio backend. Hosted gateways use OAuth or a username and password; self-hosted ones may use a session token."
          disabled={state.envOverride}
          icon={Globe}
        ...
        <ListRow
          action={
            <Input
              ...
              placeholder="https://gateway.example.com/clio"
              value={state.remoteUrl}
            />
          }
          description="Base URL for the remote dashboard backend. Path prefixes are supported, for example /clio."
          title="Remote URL"
        />
      ...
      <div className="mt-6 grid gap-1">
        <ListRow
          action={
            <Button onClick={() => void window.clioDesktop?.revealLogs()} size="sm" variant="textStrong">

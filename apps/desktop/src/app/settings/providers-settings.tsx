...
import type { EnvVarInfo, OAuthProvider } from '@/types/clio'
...
// (`Picker` in desktop-onboarding-overlay): same provider rows, same
// OpenRouter quick-key row, and the same API-key affordance. The leaf cards are
// shared so the two surfaces stay visually identical.
  ...
  return (
    <section className="mb-5 grid gap-2">
      ...
      <p className="-mt-2 mb-1 text-[length:var(--conversation-caption-font-size)] leading-(--conversation-caption-line-height) text-(--ui-text-tertiary)">
        Sign in with a subscription — no API key to copy. Clio runs the browser sign-in for you, right here in the
        app.
      </p>
      {connected.length > 0 && (
        <>
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) {
          setOauthProviders(sortProviders(providers))
        }
      } catch {

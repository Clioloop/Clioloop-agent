...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const providerOptions = useMemo(
    () => [...(providers.length ? providers : NO_PROVIDERS)].sort(compareOmniFirst),
    [providers]
  )
  ...
  const displayProviderName = useCallback(
    (provider: Pick<ModelOptionProvider, 'name' | 'slug'>) =>
      isOmniProvider(provider.slug) ? OMNI_PROVIDER_LABEL : provider.name,
    []
  )

  const displayProviderSlug = useCallback(
    (provider: string) =>
      isOmniProvider(provider) ? OMNI_PROVIDER_LABEL : (providerNameBySlug.get(provider) ?? provider),
    [providerNameBySlug]
  )
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 text-sm font-medium">
              ∞ {OMNI_PROVIDER_LABEL}
              <Pill tone="primary">{OMNI_PROVIDER_BADGE}</Pill>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {portal?.logged_in
                ? OMNI_PROVIDER_DESCRIPTION
                : 'Required for Model Fusion. Connect once for models and Tool Gateway access — no API keys.'}
            </p>
          </div>
              ...
              <ListRow
                ...
                description={
                  <span className="font-mono text-[0.68rem]">
                    {isAuto
                      ? 'auto · use main model'
                      : `${displayProviderSlug(current.provider)} · ${current.model || '(provider default)'}`}
                  </span>
                }
                key={meta.key}
                title={

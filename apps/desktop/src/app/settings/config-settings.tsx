...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ClioConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<ClioConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getClioConfigRecord(), getClioConfigDefaults(), getClioConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: ClioConfigRecord) => {
  ...
  if (!config || !schema) {
    return <LoadingState label="Loading Clio configuration..." />
  }

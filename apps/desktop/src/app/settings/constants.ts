...
export const EMPTY_SELECT_VALUE = '__clio_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'CLIO_',
    name: 'Clio',
    description: 'Clio desktop runtime and local app settings',
    priority: 20
  },
  {
    prefix: 'OPENROUTER_',
  ...
  { prefix: 'CLIO_GEMINI_', name: 'Gemini', priority: 4 },
  {
    prefix: 'DEEPSEEK_',
    name: 'DeepSeek',
  ...
  { prefix: 'CLIO_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = {
  ...
  timezone: 'Used when Clio needs local time context. Blank uses the system timezone.',
  ...
  file_read_max_chars: 'Maximum characters Clio can read from one file request.',
  'approvals.mode': 'How Clio handles commands that need explicit approval.',
  ...
  'agent.max_turns': 'Upper bound for tool-calling turns before Clio stops a run.',
  'updates.non_interactive_local_changes':
    'When Clio updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
}

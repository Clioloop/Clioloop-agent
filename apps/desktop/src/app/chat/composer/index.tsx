...
import { clioDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
export function ChatBar({
  ...
  // cold start ("Starting Clio...") from a dropped connection we're trying to
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, CLIO_PATHS_MIME)) {
      return
    }
  ...
  const handleDrop = (event: ReactDragEvent<HTMLFormElement>) => {
    ...
    if (Array.from(event.dataTransfer.types || []).includes(CLIO_PATHS_MIME)) {
  ...
  useEffect(() => {
    ...
    loadIntoComposer(queueEdit.draft, queueEdit.attachments)
    setQueueEdit(null)
  }, [activeQueueSessionKey, editingQueuedPrompt, queueEdit])

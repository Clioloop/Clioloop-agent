    ...
    <PreviewEmptyState
        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.clioDesktop?.openExternal(error.url)
            }}
          >
      ...
      secondaryAction={
        onRestartServer
          ? {
              disabled: restarting,
              label: restarting ? 'Clio is restarting...' : 'Ask Clio to restart the server',
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Clio to restart the server" looked like it did nothing —
    ...
    try {
      ...
      appendConsoleEntry({
        level: 1,
        message: `Clio is looking for a preview server to restart (${taskId})`
      })

      notify({
        kind: 'info',
        title: 'Restarting preview server',
        message: 'Clio is working in the background. Watch the preview console for progress.',
        durationMs: 4000
      })
    } catch (error) {
      appendConsoleEntry({
  ...
  useEffect(() => {
      ...
      message:
        previewServerRestart.status === 'running'
          ...
          : previewServerRestart.status === 'complete'
            ? `Clio finished restarting the preview server${
    ...
    if (previewServerRestart.status === 'complete') {
      ...
      notify({
        ...
        message: previewServerRestart.message?.slice(0, 200) || 'Clio could not restart the server.',
        durationMs: 6000
      })
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      failPreviewServerRestart(
        taskId,
        'Clio is still working, but no restart result has arrived yet. The server command may be running in the foreground.'
      )
  ...
  useEffect(() => {
    if (
      target.kind !== 'file' ||
      !window.clioDesktop?.watchPreviewFile ||
      !window.clioDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.clioDesktop.onPreviewFileChanged(payload => {
    ...
    void window.clioDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.clioDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.clioDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:clio-preview')

...
import { CLIO_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  if (window.clioDesktop.readFileText) {
    try {
      return await window.clioDesktop.readFileText(filePath)
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)

      if (!message.includes("No handler registered for 'clio:readFileText'")) {
        throw error
      }
    }
  }
  ...
  const dataUrl = await window.clioDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(CLIO_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
      ...
      <PreviewEmptyState
        body={
          binary
            ...
            : `${target.label} is ${formatBytes(size)}. Clio will only show the first 512 KB.`
        }

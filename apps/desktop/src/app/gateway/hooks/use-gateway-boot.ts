...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.clioDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ClioGateway | null) => void
  refreshClioConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting Clio...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // as an opaque "Could not connect to Clio gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshClioConfig().catch(() => undefined)
    ...
    const offExit = desktop.onBackendExit(() => {
      if ($desktopBoot.get().running || $desktopBoot.get().visible) {
        failDesktopBoot('Clio background process exited during startup.')
      }

      notify({
        kind: 'error',
        title: 'Backend stopped',
        message: 'Clio background process exited.',
        durationMs: 0
      })
    })
    ...
    async function boot() {
      try {
        ...
        setDesktopBootStep({
          ...
          message: 'Loading Clio settings',
          progress: 97
        })
        await callbacksRef.current.refreshClioConfig()

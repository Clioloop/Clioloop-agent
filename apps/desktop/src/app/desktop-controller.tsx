...
import { FusionSetupOverlay } from './fusion-setup-overlay'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.clioDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.clioDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.clioDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshClioConfig, sttEnabled, voiceMaxRecordingSeconds } = useClioConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      window.dispatchEvent(new CustomEvent('clio:new-session-shortcut'))
    }
  ...
  const overlays = (
    <>
      ...
      <FusionSetupOverlay gateway={gatewayRef.current || undefined} />

...
import { getProfiles, transcribeAudio } from '@/clio'
  ...
  const executeSlashCommand = useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      const runSlash = async (commandText: string, sessionHint?: string, recordInput = true): Promise<void> => {
        ...
        //
        // Why not the slash worker: it's a separate ClioCLI subprocess whose
        ...
        // Bare `/fusion` opens the desktop fusion setup wizard (counts + model
        // picks), mirroring Telegram. `/fusion off|status|<args>` falls through
        // to the backend slash worker below.
        if (normalizedName === 'fusion' && !arg) {
          try {
            const res = await requestGateway<{ output?: string; warning?: string }>('slash.exec', {
              session_id: sessionId,
              command: 'fusion gate'
            })
            ...
            if (output === 'OK') {
              ...
              renderSlashOutput(output || 'Fusion is not available for this account.')
            }
          } catch (err) {
            renderSlashOutput(`error: ${err instanceof Error ? err.message : String(err)}`)
          }

...
import { $fusionPickerOpen, $sessions, setFusionPickerOpen, setSessions } from '@/store/session'
import type { SessionInfo } from '@/types/clio'
...
describe('usePromptActions /title', () => {
  ...
  it('opens the Fusion picker after the backend gate allows it', async () => {
    const refreshSessions = vi.fn(async () => undefined)
    const requestGateway = vi.fn(async () => ({ output: 'OK' }) as never)

    let handle: HarnessHandle | null = null
    render(<Harness onReady={h => (handle = h)} refreshSessions={refreshSessions} requestGateway={requestGateway} />)

    await handle!.submitText('/fusion')

    expect(requestGateway).toHaveBeenCalledWith('slash.exec', {
      session_id: RUNTIME_SESSION_ID,
      command: 'fusion gate'
    })
    expect($fusionPickerOpen.get()).toBe(true)
  })

...
import { getClioConfig, getClioConfigDefaults } from '@/clio'
...
interface ClioConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useClioConfig({ activeSessionIdRef, refreshProjectBranch }: ClioConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshClioConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getClioConfig(), getClioConfigDefaults().catch(() => ({}))])
  ...
  return { refreshClioConfig, sttEnabled, voiceMaxRecordingSeconds }
}

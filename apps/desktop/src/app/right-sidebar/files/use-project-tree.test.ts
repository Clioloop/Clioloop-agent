...
import type { ClioReadDirResult } from '@/global'

import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<ClioReadDirResult>>()

beforeEach(() => {
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { clioDesktop: { readDir: typeof readDir } }).clioDesktop = { readDir }
})

afterEach(() => {
  resetProjectTreeState()
  delete (window as unknown as { clioDesktop?: unknown }).clioDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ClioReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ClioReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ClioReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.clioDesktop is missing', async () => {
    delete (window as unknown as { clioDesktop?: unknown }).clioDesktop

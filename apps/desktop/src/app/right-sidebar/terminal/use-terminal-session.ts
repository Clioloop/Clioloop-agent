...
const CLIO_PATHS_MIME = 'application/x-clio-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(CLIO_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(CLIO_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.clioDesktop?.terminal
    ...
    try {
      ...
      console.warn('[clio-terminal] WebGL unavailable; falling back to DOM', err)
    }

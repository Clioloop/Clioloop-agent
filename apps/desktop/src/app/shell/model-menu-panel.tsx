            ...
            <DropdownMenuGroup className="py-0.5" key={group.provider.slug}>
              <DropdownMenuLabel className={dropdownMenuSectionLabel}>
                <span className="flex min-w-0 items-center gap-1.5">
                  <span
                    className="truncate"
                    title={isOmniProvider(group.provider.slug) ? OMNI_PROVIDER_DESCRIPTION : undefined}
                  >
                    {isOmniProvider(group.provider.slug) ? OMNI_PROVIDER_LABEL : group.provider.name}
                  </span>
                  {isOmniProvider(group.provider.slug) ? (
                    <span className="text-[0.58rem] font-semibold uppercase tracking-wide text-primary">
                      {OMNI_PROVIDER_BADGE}
                    </span>
                  ) : null}
                </span>
...
function groupModels(
  ...
  // Stable, logical group order: Omni first, then alphabetical by provider name.
  // The backend floats the current provider first, which would reshuffle on every switch.

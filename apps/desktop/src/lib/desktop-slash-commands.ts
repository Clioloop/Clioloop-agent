...
const DESKTOP_COMMAND_META = [
  ...
  ['/fusion', 'Set up Model Fusion — planners & reviewers (Pro plan and up)'],
  ['/goal', 'Manage the standing goal for this session'],
  ['/help', 'Show desktop slash commands'],
  ['/new', 'Start a new desktop chat'],
  ['/profile', 'Switch the active Clio profile'],
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  return DESKTOP_COMMANDS.has(canonical) || !isKnownClioSlashCommand(normalized)
}

/**
 * An "extension" command is anything the backend surfaces that is NOT one of
 * Clio' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownClioSlashCommand(normalized)
}
...
function isKnownClioSlashCommand(command: string): boolean {

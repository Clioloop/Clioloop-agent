 ...
 *
 * Stamps the Clio icon + identity onto the packed Windows Clio.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `clio desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Clio')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Clio'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Clio.exe keeps the stock Electron icon`)
  }
}

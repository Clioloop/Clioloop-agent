 ...
 * product name (`Clio`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Clio'
 *
 * This is a hard failure with no obvious cause for the user — `clio desktop`
 ...
 * (the mac-unpacked Clio.app bundle) and Windows (win-unpacked), so we

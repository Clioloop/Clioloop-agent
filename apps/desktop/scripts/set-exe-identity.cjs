#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Clio icon + version metadata onto the
// built Clio.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Clio.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `clio desktop`, the installer's
...
// path (which rebuilds via `clio desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Clio.exe>
//
...
// Stamp the Clio icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Clio',
      FileDescription: 'Clio',
      CompanyName: 'Omni Loop Labs',
      LegalCopyright: 'Copyright (c) 2026 Omni Loop Labs'
    }
  })

  console.log('[set-exe-identity] done — Clio icon + identity stamped')
}

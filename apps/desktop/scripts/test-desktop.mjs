...
// no longer bundles the Clio Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Clio.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Clio'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Clio.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'clio'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default CLIO_HOME for non-sandboxed runs -- matches main.cjs's
// resolveClioHome(). On Windows it's %LOCALAPPDATA%\clio; elsewhere
// it's ~/.clio. The fresh-install sandbox launchFresh() sets its own
// CLIO_HOME and never touches this.
const DEFAULT_CLIO_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'clio')
  }
  return path.join(os.homedir(), '.clio')
})()
const VENV_ROOT = path.join(DEFAULT_CLIO_HOME, 'clio-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'clio-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.CLIO_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Clio-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Clio-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Clio-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Clio-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.CLIO_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.CLIO_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const clioHome = path.join(sandbox, 'clio-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(clioHome, { recursive: true })
  ...
  env.CLIO_DESKTOP_CWD = cwd
  env.CLIO_DESKTOP_IGNORE_EXISTING = '1'
  env.CLIO_DESKTOP_TEST_MODE = 'fresh-install'
  env.CLIO_DESKTOP_USER_DATA_DIR = userDataDir
  env.CLIO_HOME = clioHome
  delete env.CLIO_DESKTOP_CLIO
  delete env.CLIO_DESKTOP_CLIO_ROOT
  ...
  console.log(`  CLIO_HOME: ${clioHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(clioHome, 'clio-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Clio Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of clio_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'clio-agent', 'clio_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Clio
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + CLIO_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  CLIO_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}

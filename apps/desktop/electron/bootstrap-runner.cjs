 ...
 *
 * Drives apps/desktop's first-launch install of Clio Agent by spawning
...
function bootstrapCacheDir(clioHome) {
  return path.join(clioHome, 'bootstrap-cache')
}

function cachedScriptPath(clioHome, commit) {
  return path.join(bootstrapCacheDir(clioHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/Clioloop/Clioloop-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, clioHome, emit }) {
  ...
  const cached = cachedScriptPath(clioHome, installStamp.commit)
  try {
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, clioHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, {
      ...
      env: {
        ...process.env,
        // Pass CLIO_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        CLIO_HOME: clioHome || process.env.CLIO_HOME || ''
      }
    })
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, clioHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, clioHome }) {
  const args = ['--dir', activeRoot, '--clio-home', clioHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, clioHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, clioHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, clioHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, clioHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(clioHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, clioHome, emit })

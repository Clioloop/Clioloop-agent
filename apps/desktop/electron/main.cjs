...
const USER_DATA_OVERRIDE = process.env.CLIO_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.CLIO_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with CLIO_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[clio] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[clio] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[clio] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[clio] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// CLIO_HOME — the user-facing root for everything Clio-related. Mirrors
// scripts/install.ps1's $ClioHome and scripts/install.sh's $CLIO_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\clio (matches install.ps1)
//   macOS / Linux: ~/.clio (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.clio directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\clio yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// CLIO_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// CLIO_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.clio / %LOCALAPPDATA%\clio.
function resolveClioHome() {
  if (process.env.CLIO_HOME) return path.resolve(process.env.CLIO_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'clio-home')
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'clio')
    const legacy = path.join(app.getPath('home'), '.clio')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.clio setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.clio')
}

const CLIO_HOME = resolveClioHome()
// ACTIVE_CLIO_ROOT — the canonical mutable Clio install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_CLIO_ROOT = path.join(CLIO_HOME, 'clio-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_CLIO_ROOT, 'venv')
...
// boots, going straight to `resolveClioBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_CLIO_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_CLIO_ROOT, '.clio-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Clio profile the desktop launches its
// local backend as. When set, startClio() passes `clio --profile <name>
// dashboard …`, which deterministically pins CLIO_HOME (see
// _apply_profile_override in clio_cli/main.py) and bypasses the sticky
...
// clioDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under CLIO_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by clio_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(CLIO_HOME, 'logs', 'desktop.log')
const DESKTOP_LOG_FLUSH_MS = 120
const DESKTOP_LOG_BUFFER_MAX_CHARS = 64 * 1024
const BOOT_FAKE_MODE = process.env.CLIO_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.CLIO_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 Omni Loop Labs'
})
...
const MEDIA_PROTOCOL = 'clio-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(clio-media://…)`.
...
let clioProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by clioProcess +
// connectionPromise + startClio(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.CLIO_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.CLIO_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startClio() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Clio backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  clioLog.push(...lines)
  if (clioLog.length > 300) {
    clioLog.splice(0, clioLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-clio-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('clio:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the clio:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clio:bootstrap:event', ev)
}
...
function isClioSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'clio_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.CLIO_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Clio deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Clio' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\clio\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'clio', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'clio', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\clio\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'clio', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'clio', 'git', 'bin', 'git.exe'))
  }
...
function recentClioLog() {
  return clioLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's clio root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_CLIO_ROOT.
// CLIO_DESKTOP_CLIO_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.CLIO_DESKTOP_CLIO_ROOT && path.resolve(process.env.CLIO_DESKTOP_CLIO_ROOT),
    !IS_PACKAGED && isClioSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isClioSourceRoot(ACTIVE_CLIO_ROOT) ? ACTIVE_CLIO_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_CLIO_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('clio:updates:progress', merged)
  }
}
...
// CLIO_HOME/clio-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_clio_home). That binary owns
// ALL repo mutation — running `clio update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'clio-setup.exe' : 'clio-setup'
  const candidate = path.join(CLIO_HOME, name)
  return fileExists(candidate) ? candidate : null
}

// Path to the venv shim whose lock decides whether `clio update` can write
// fresh entry points. On Windows this is the file the running backend
// `clio.exe` holds open; on POSIX it's never mandatory-locked.
function venvClioShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'clio.exe')
    : path.join(updateRoot, 'venv', 'bin', 'clio')
}
...
// only signals the direct child, so on Windows a backend `clio.exe` that
// spawned its own grandchildren (a `clio` REPL, a pty terminal session, the
...
// `clioProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `clio.exe`, the quarantine
...
async function releaseBackendLockForUpdate(updateRoot) {
  ...
  const pids = []
  if (clioProcess && Number.isInteger(clioProcess.pid)) pids.push(clioProcess.pid)
  ...
  if (clioProcess && !clioProcess.killed) {
    try {
      clioProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvClioShimPath(updateRoot)
...
// `clio update`). Instead we spawn the staged Clio-Setup binary with
// --update and quit, so it can run `clio update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri clio-setup. Unlike Windows
      ...
      // whole update itself: `clio update` (backend) + `clio desktop
    ...
    if (!updater) {
      ...
      // `clio desktop`, never the Tauri installer that self-copies
      // clio-setup.exe into CLIO_HOME). They DO have a working `clio`
      ...
      // checkout they're on — bare `clio update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'clio update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `clio update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `clio update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, clioRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Clio updater…', percent: 100 })
    ...
    // clio.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `clio update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: CLIO_HOME,
      env: {
        ...process.env,
        CLIO_HOME,
        PATH: [path.join(CLIO_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
// Resolve the clio CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `clio` on PATH.
function resolveClioCliBinary(updateRoot) {
  const venvClio = path.join(updateRoot, 'venv', 'bin', 'clio')
  if (fileExists(venvClio)) return venvClio
  return findOnPath('clio') || null
}
...
// macOS/Linux in-app update: backend (`clio update`) + OS-aware GUI rebuild
// (`clio desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const clio = resolveClioCliBinary(updateRoot)
  if (!clio) {
    emitUpdateProgress({ stage: 'manual', message: 'clio update', percent: null })
    return { ok: true, manual: true, command: 'clio update', clioRoot: updateRoot }
  }

  // Put the Clio-managed Node and the venv on PATH so `clio desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(CLIO_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    CLIO_HOME,
  ...
  // `clio update` reaps stale `clio dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads CLIO_DESKTOP_CHILD_PID and excludes
  ...
  if (clioProcess && Number.isInteger(clioProcess.pid)) {
    desktopChildPids.push(clioProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.CLIO_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'clio update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'clio update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(clio, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Clio to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Clio.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Clio.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Clio to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.clio-update-new"; then
    rm -rf "$DST.clio-update-old" 2>/dev/null || true
    mv "$DST" "$DST.clio-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.clio-update-new" "$DST"
    rm -rf "$DST.clio-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `clio-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Clio to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `clio
  ...
  return isClioSourceRoot(ACTIVE_CLIO_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.CLIO_DESKTOP_WEB_DIST
...
function resolveClioCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Clio.app/Contents/...`
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'clio_cli.main', ...dashboardArgs],
    env: {
...
// createActiveBackend — build a backend pointing at ACTIVE_CLIO_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Clio at ${ACTIVE_CLIO_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'clio_cli.main', ...dashboardArgs],
    env: {
      PYTHONPATH: [ACTIVE_CLIO_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
    },
    root: ACTIVE_CLIO_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveClioBackend(dashboardArgs) {
  // 1. Explicit override -- CLIO_DESKTOP_CLIO_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.CLIO_DESKTOP_CLIO_ROOT && path.resolve(process.env.CLIO_DESKTOP_CLIO_ROOT)
  if (overrideRoot && isClioSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Clio source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `clio` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isClioSourceRoot.)
  if (!IS_PACKAGED && isClioSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Clio source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_CLIO_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\clio\clio-agent (Windows) or ~/.clio/clio-agent.
  ...
  //    to spawning clio. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `clio update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `clio` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    CLIO_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.CLIO_DESKTOP_IGNORE_EXISTING !== '1') {
    let clioCommand = null
    const clioOverride = process.env.CLIO_DESKTOP_CLIO

    if (clioOverride) {
      const resolvedOverride = findOnPath(clioOverride)
      if (resolvedOverride) {
        clioCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(clioOverride, { isWsl: IS_WSL })) {
        clioCommand = clioOverride
      } else {
        rememberLog(`Ignoring Windows Clio override under WSL: ${clioOverride}`)
      }
    } else {
      clioCommand = findOnPath('clio')
    }

    if (clioCommand) {
      if (looksLikeDesktopAppBinary(clioCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Clio CLI: ${clioCommand}`)
        clioCommand = null
      }
    }

    if (clioCommand) {
      // Smoke-test the candidate before trusting it. A `clio` shim
      ...
      const shellForProbe = isCommandScript(clioCommand)
      if (verifyClioCli(clioCommand, { shell: shellForProbe })) {
        return {
          label: `existing Clio CLI at ${clioCommand}`,
          command: clioCommand,
          args: dashboardArgs,
          bootstrap: false,
          env: {},
      ...
      rememberLog(
        `Ignoring existing Clio CLI at ${clioCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed clio_cli module via system Python.
  ...
  if (python) {
    ...
    // having clio_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\clio\clio-agent\venv.
    if (canImportClioCli(python)) {
      return {
        kind: 'python',
        label: `installed clio_cli module via ${python}`,
        command: python,
        args: ['-m', 'clio_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: clio_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveClioBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Clio Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveClioBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Clio install found; starting first-launch bootstrap')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      clioHome: CLIO_HOME,
      logRoot: path.join(CLIO_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Clio install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Clio bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(CLIO_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startClio() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // clio:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveClioBackend(backend.args))
  }
  ...
  if (!isClioSourceRoot(ACTIVE_CLIO_ROOT)) {
    throw new Error(
      `Clio install at ${ACTIVE_CLIO_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Clio' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\clio\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `clio` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Clio on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isClioSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Clio venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Clio at ${ACTIVE_CLIO_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Clio runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Clio backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Clio backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Clio backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Clio-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('clio:link-titles', { cache: false })
...
function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveClioCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clio:preview-file-changed', payload)
}
...
async function waitForClio(baseUrl, token) {
  ...
  throw new Error(`Clio backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clio:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clio:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Clio...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clio:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clio:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('clio:window-state-changed', state)
}
...
//
// Hosted Clio gateways gate the dashboard behind an OAuth provider (e.g.
// Omni Loop Labs) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``clio_session_at``),
...
//   * managed provider now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / clio #37247).
//     Both are set as HttpOnly cookies (``clio_session_at`` ~15 min,
//     ``clio_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:clio-remote-oauth'
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Clio backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root CLIO_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  return {
    ...
    // scope is never overridden by CLIO_DESKTOP_REMOTE_URL.
    envOverride: key ? false : Boolean(process.env.CLIO_DESKTOP_REMOTE_URL)
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (clio #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Clio gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Clio gateway is selected, but no session token is saved. ' +
...
//   2. env override (CLIO_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.CLIO_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.CLIO_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'CLIO_DESKTOP_REMOTE_URL is set but CLIO_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Clio backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.CLIO_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Clio
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Omni Loop Labs" instead of a generic
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startClio())
  ...
  // connect to Clio gateway". Mirror the renderer's connect here so the test
...
// startClio() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetClioConnection() nulls clioProcess.
  const dying = clioProcess && !clioProcess.killed ? clioProcess : null
  resetClioConnection()
...
// profile to startClio() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startClio() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForClio(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: clioLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  // --profile wins over the inherited CLIO_HOME env (see _apply_profile_override
  // step 3 in clio_cli/main.py), so the child re-homes to this profile.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', String(port)]
  const backend = await ensureRuntime(resolveClioBackend(dashboardArgs))
  const clioCwd = resolveClioCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting Clio backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Clio backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Clio backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(new Error(`Clio backend for profile "${profile}" exited before it became ready (${signal || code}).`))
    }
  })

  const baseUrl = `http://127.0.0.1:${port}`
  await Promise.race([waitForClio(baseUrl, token), startFailed])
  ready = true
...
async function startClio() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startClio() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Clio backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Clio backend at ${remote.baseUrl}`, 24)
      await waitForClio(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Clio backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.clio/active_profile file) and
    // resolves CLIO_HOME the same way `clio -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Clio runtime', 28)
    const backend = await ensureRuntime(resolveClioBackend(dashboardArgs))
    const clioCwd = resolveClioCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Clio backend via ${backend.label}`, 84)
    rememberLog(`Starting Clio backend via ${backend.label}`)

    clioProcess = spawn(backend.command, backend.args, {
      cwd: clioCwd,
      env: {
        ...process.env,
        // Explicitly pin CLIO_HOME for the child so Python's get_clio_home()
        // resolves to the SAME location our resolveClioHome() picked. Without
        // this pin, Python falls back to ~/.clio on every platform — fine on
        // mac/linux (where our default matches), but on Windows our default is
        // %LOCALAPPDATA%\clio, which differs from C:\Users\<u>\.clio.
        // Mismatch would split config / sessions / .env / logs across two
        // directories. install.ps1 sets CLIO_HOME via setx; the desktop
    ...
    clioProcess.stdout.on('data', rememberLog)
    clioProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    clioProcess.once('error', error => {
      rememberLog(`Clio backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Clio backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    clioProcess.once('exit', (code, signal) => {
      rememberLog(`Clio backend exited (${signal || code})`)
      clioProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Clio backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Clio backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentClioLog()}`
          )
        )
      }
    })
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Clio backend to become ready', 90)
    await Promise.race([waitForClio(baseUrl, token), backendStartFailed])
    backendReady = true
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Clio backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.on('close', () => {
    ...
    rememberLog('[window] main window close requested')
  })
  mainWindow.on('closed', () => {
    rememberLog('[window] main window closed')
    mainWindow = null
  })
  ...
  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    ...
    if (
      ...
      rememberLog('[renderer] unexpected clean exit; reloading renderer')
      setImmediate(() => {
        if (!mainWindow || mainWindow.isDestroyed()) return
        try {
          mainWindow.webContents.reload()
        } catch (err) {
          rememberLog(`[renderer] reload after clean exit failed: ${err?.message || err}`)
        }
      })
    }
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startClio().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('clio:connection', async (_event, profile) => ensureBackend(profile))
ipcMain.handle('clio:backend:touch', async (_event, profile) => {
...
ipcMain.handle('clio:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startClio() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('clio:bootstrap:cancel', async () => {
...
ipcMain.handle('clio:boot-progress:get', async () => bootProgressState)
ipcMain.handle('clio:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('clio:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('clio:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('clio:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('clio:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('clio:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('clio:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('clio:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('clio:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('clio:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new CLIO_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('clio:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('clio:requestMicrophoneAccess', async () => {
...
ipcMain.handle('clio:api', async (_event, request) => {
...
ipcMain.handle('clio:notify', (_event, payload) => {
  if (!Notification.isSupported()) return false
  new Notification({
    title: payload?.title || 'Clio',
    body: payload?.body || '',
...
ipcMain.handle('clio:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('clio:readFileText', async (_event, filePath) => {
...
ipcMain.handle('clio:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('clio:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('clio:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('clio:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('clio:saveClipboardImage', async () => {
...
ipcMain.handle('clio:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('clio:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('clio:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('clio:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('clio:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveClioCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('clio:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: path.join(app.getPath('home'), 'clio-projects')
}))

ipcMain.handle('clio:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('clio:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('clio:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('clio:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('clio:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: clioLog.slice(-200) }))
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Clio' TUI into
...
function terminalChannel(id, suffix) {
  return `clio:terminal:${id}:${suffix}`
}
...
ipcMain.handle('clio:fs:readDir', async (_event, dirPath) => {
...
ipcMain.handle('clio:fs:gitRoot', async (_event, startPath) => {
...
ipcMain.handle('clio:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Clio.')
  }
...
ipcMain.handle('clio:terminal:write', (_event, id, data) => {
...
ipcMain.handle('clio:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('clio:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('clio:updates:check', async () =>
...
ipcMain.handle('clio:updates:apply', async (_event, payload) =>
...
ipcMain.handle('clio:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('clio:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Clio version (the one `release.py` bumps in
// clio_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Clio version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveClioVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'clio_cli', '__init__.py')
...
ipcMain.handle('clio:version', async () => ({
  appVersion: resolveClioVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  clioRoot: resolveUpdateRoot()
}))
...
app.on('window-all-closed', () => {
  rememberLog('[app] window-all-closed')

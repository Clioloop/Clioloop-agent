 ...
 *     `X-Clio-Session-Token` header; WS uses `?token=`.
...
// clio_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - clio_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - clio_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / clio #37247). When the AT cookie
...
/**
 * True if any cookie in `cookies` is a clio session ACCESS-token cookie

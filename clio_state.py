#!/usr/bin/env python3
"""
SQLite State Store for Clio Agent.
...
# clio_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``clio_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see clio_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple clio processes (gateway + CLI sessions + worktree agents)
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `clio update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._clio_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._clio_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, managed provider) receive
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Clio surface and jump
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Clio, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Clio session.

        A Clio session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Clio session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``CLIO_DISABLE_FTS_TRIGRAM`` or not yet created), so

...
Usage (via cron with --no-agent):
    ...
    clio cron create clio-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$CLIO_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name clio-issues --repo Clioloop/Clioloop-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.clio/.env to avoid the 60 req/hr

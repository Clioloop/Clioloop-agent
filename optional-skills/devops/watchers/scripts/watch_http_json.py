...
Usage (via cron with --no-agent):
    ...
    clio cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CLIO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Clio-Watcher/1.0"})

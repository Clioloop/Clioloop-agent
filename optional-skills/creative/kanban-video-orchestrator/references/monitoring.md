...
clio kanban watch --tenant <project-slug>

# Snapshot of the board
clio kanban list --tenant <project-slug>
clio kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
clio kanban stats --tenant <project-slug>

# Visual dashboard (browser)
clio dashboard

# Inspect a specific task (includes comments + events)
clio kanban show <task-id>

# Follow a single task's event stream
clio kanban tail <task-id>
```

Verify available subcommands with `clio kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `clio kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `clio kanban block <id>` then `clio kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `clio kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `clio kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `clio profile list` (profile exists?), `clio status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(clio kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
clio kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
clio kanban block <task-id>
clio kanban archive <task-id>

# Diagnose what happened
clio kanban show <task-id>      # task body, comments, recent events
clio kanban tail <task-id>      # follow the live event stream
clio kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `clio kanban create` for the director
...
while true; do
    clear
    clio kanban list --tenant <slug>
    echo "---"
    clio kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `clio kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `clio profile list` and start

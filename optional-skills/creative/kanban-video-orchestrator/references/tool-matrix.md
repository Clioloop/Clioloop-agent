...
Maps each role archetype to the Clio skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public clio-agent
...
## Clio skills relevant to video production

### Visual / rendering skills (`clio-agent/skills/creative/`)
...
### Design / pre-production skills (`clio-agent/skills/creative/`)
...
### Audio / media skills (`clio-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`clio-agent/skills/devops/`)
...
These are **not** Clio skills but external CLIs / APIs that profiles invoke.
...
## Built-in Clio tools for media review

These are native Clio tools — not invoked via terminal but through their own
...
key is present in `~/.clio/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Clio profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.clio/.env` → environment variable.
...
When a new Clio-public video skill ships:

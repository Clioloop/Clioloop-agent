#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `clio kanban list` and
...
def clio_available() -> bool:
    return shutil.which("clio") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed clio CLI."""
    try:
        out = subprocess.run(
            ["clio", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `clio kanban list`
    out = subprocess.run(
        ["clio", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not clio_available():
        print("ERROR: 'clio' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)

"""Persistent slash-command worker — one ClioCLI per TUI session.
...
def _run(cli: ClioCLI, command: str) -> str:
...
def main():
    ...
    os.environ["CLIO_SESSION_KEY"] = args.session_key
    os.environ["CLIO_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ClioCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)

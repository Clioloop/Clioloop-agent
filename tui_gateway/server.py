...
from clio_constants import (
    get_clio_home,
    get_clio_home_override,
    reset_clio_home_override,
    set_clio_home_override,
)
...
load_clio_dotenv(
    clio_home=_clio_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.clio/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_clio_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("CLIO_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("CLIO_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("CLIO_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent ClioCLI subprocess for slash commands."""
...
# ``profile`` on those calls; we open that profile's db and bind its CLIO_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_clio_home).resolve():
        return None
...
def _fusion_event(sid: str, ev: Any) -> None:
    """Emit a structured Fusion progress event for rich clients.

    ``ev`` is a ``FusionEvent``. Rich clients (TUI/desktop) read ``phase``/
    ``detail``/``round`` to drive a per-round reviewer bubble; ``text`` is the
    human label and stays back-compatible with plain status rendering.
    """
    text = str(getattr(ev, "text", ev) or "").strip()
    if not text:
        return
    payload = {
        "kind": "fusion",
        ...
        "round": int(getattr(ev, "round", 0) or 0),
    }
    run_id = getattr(ev, "run_id", "") or ""
    if run_id:
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `clio` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # CLIO_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_clio_home_override(profile_home)
                try:
                    from clio_state import SessionDB
            ...
            if home_token is not None:
                reset_clio_home_override(home_token)
            with _sessions_lock:
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _clio_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_clio_home_override()
        home = override if isinstance(override, str) and override else _clio_home
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["CLIO_GATEWAY_SESSION"] = "1"
...
def resolve_skin() -> dict:
    try:
        from clio_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("CLIO_MODEL", "")
        or os.environ.get("CLIO_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("CLIO_TUI_PROVIDER", "").strip()
    ...
    try:
        from clio_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("CLIO_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("CLIO_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] CLIO_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from clio_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown CLIO_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in CLIO_TUI_TOOLSETS "
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid CLIO_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from clio_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from clio_cli.config import get_compatible_custom_providers, load_config
    ...
    os.environ["CLIO_MODEL"] = result.new_model
    os.environ["CLIO_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # CLIO_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["CLIO_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["CLIO_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from clio_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from clio_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from clio_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from clio_cli.config import load_config as _load_full_cfg
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("CLIO_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(sid: str, key: str, session_id: str | None = None, session_db=None):
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("CLIO_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("CLIO_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("CLIO_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("CLIO_IGNORE_RULES")),
        **_agent_cbs(sid),
    )
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind CLIO_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `CLIO_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_clio_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds CLIO_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Clio profile home
    # (~/.clio/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $CLIO_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_clio_home() / "spawn-trees"
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn CLIO_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_clio_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from clio_cli.config import load_config as _tui_load_config
            ...
            try:
                ...
                pass
            fusion_cfg = session.get("fusion_config") or getattr(agent, "_fusion_config", None)
            if (
                fusion_cfg is not None
                and getattr(fusion_cfg, "enabled", False)
                and getattr(fusion_cfg, "is_complete", lambda: False)()
                ...
                result = run_fusion_turn(
                    ...
                    run_message,
                    config=fusion_cfg,
                    progress=lambda ev: _fusion_event(sid, ev),
                    **run_kwargs,
                )
            else:
            ...
            if (
                ...
                try:
                    from clio_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: clio_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from clio_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _clio_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Clio Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Clio/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Clio Desktop. If it serves Clio/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "yolo":
        ...
        try:
            if session:
                ...
                current = is_truthy_value(os.environ.get("CLIO_YOLO_MODE"))
                if current:
                    os.environ.pop("CLIO_YOLO_MODE", None)
                    nv = "0"
                else:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from clio_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_clio_home), "display": display_clio_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from clio_cli.runtime_provider import resolve_runtime_provider
        from clio_cli.auth import has_usable_secret
        from clio_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Clio provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.clio/.env`` into the gateway process via
    ``clio_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `clio` is interactive — use `/clio chat -q …` or run `clio` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`clio setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`clio gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`clio sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`clio config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m clio_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "clio_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from clio_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from clio_cli.config import is_managed, save_env_value
        from clio_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `clio model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.clio/.env
...
@method("model.oauth_start")
def _(rid, params: dict) -> dict:
    """Start the Omni Loop Portal device-code login.

    Returns {user_code, verification_uri, verification_uri_complete,
    expires_in} immediately; a daemon thread polls the portal and persists
    credentials once the user approves in their browser. Track progress
    with model.oauth_status.
    """
    try:
        from clio_cli.config import is_managed
        from clio_cli.omni_portal import start_device_connect

        if is_managed():
            return _err(rid, 4006, "managed install — credentials are read-only")

        slug = (params.get("slug") or "managed").strip()
        if slug != "managed":
            return _err(rid, 4002, f"oauth connect is not supported for: {slug}")

        result = start_device_connect()

        # Best-effort: open the approval page in the user's browser (works
        # when the gateway runs on the same machine as the terminal).
        if not params.get("no_browser"):
            try:
                import webbrowser
...
@method("model.oauth_status")
def _(rid, params: dict) -> dict:
    """Poll the Omni Loop Portal device-login progress.

    Returns {status: idle|pending|connected|error, message?}. When status is
    "connected" the refreshed provider row is included so the picker can move
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from clio_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from clio_cli.config import remove_env_value
...
def _set_session_fusion_config(session: dict, config: Any) -> None:
    session["fusion_config"] = config
    agent = session.get("agent")
    if agent is not None:
...
def _fusion_status_text(config: Any | None) -> str:
    if config is None or not getattr(config, "is_complete", lambda: False)():
        return "🔮 Fusion is not configured. Run /fusion to pick your models."
    from agent.fusion_engine import label_model

    state = "ON" if getattr(config, "enabled", False) else "off"
    planners = "  +  ".join(label_model(m) for m in getattr(config, "advisors", []) or [])
    reviewers = "  +  ".join(label_model(m) for m in getattr(config, "reviewers", []) or [])
    return (
        f"🔮 Fusion is {state}.\n"
        f"• Planners ({len(getattr(config, 'advisors', []) or [])}): {planners}\n"
        f"• Reviewers ({len(getattr(config, 'reviewers', []) or [])}): {reviewers}\n"
        "• Main model: your current chat model (`/model` changes it)\n"
        f"• Fusion v{getattr(config, 'version', 2)} mode: "
        f"{str(getattr(config, 'mode', 'auto') or 'auto').lower()}"
    )


def _fusion_usage_hint() -> str:
    try:
    ...
    sample = ", ".join(f"`{m}`" for m in opens[:8]) if opens else "(none)"
    return (
        "🔮 Model Fusion (Max plan) — configure with:\n"
        "`/fusion advisors=m1,m2 reviewers=m3,m4`\n"
        "(1-5 planners, 1-5 reviewers; your current `/model` is the main worker)\n\n"
        f"Open models include: {sample}\n"
        "Planners plan with read_file/web_search, your main model does the "
        "full-tool work, reviewers critique it, then a judge/synthesizer pass "
        "fuses one final answer."
    )


def _handle_fusion_slash_for_session(sid: str, session: dict, command: str) -> str:
    ...
    current = session.get("fusion_config")

    if sub == "off":
        if current is not None:
            current.enabled = False
            _set_session_fusion_config(session, current)
        return "🔮 Fusion mode off — replies use your normal model again."

    if sub == "status":
        return _fusion_status_text(current)
    ...
    if session.get("running"):
        return "session busy — /interrupt the current turn before configuring Fusion"
    ...
    if tokens:
        ...
        if parsed is not None:
            _set_session_fusion_config(session, parsed)
            return activation_notice(parsed)
        return _fusion_usage_hint()

    # Desktop/TUI clients normally open their own picker after `fusion gate`.
    # If a client falls through with bare `/fusion`, return a useful hint
    # instead of the slash worker's empty output.
    return _fusion_usage_hint()
...
def _(rid, params: dict) -> dict:
    ...
    if _cmd_base == "fusion":
        try:
            return _ok(
                rid,
                {"output": _handle_fusion_slash_for_session(params.get("session_id", ""), session, cmd)},
            )
        except Exception as e:
            return _ok(rid, {"output": f"Fusion command error: {e}"})
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("CLIO_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("CLIO_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["CLIO_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from clio_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["CLIO_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from clio_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from clio_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from clio_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from clio_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        ...
        base_url = os.environ.get("CLIO_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_clio_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from clio_cli.skills_hub import do_install
        ...
        if action == "browse":
            from clio_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from clio_cli.skills_hub import inspect_skill

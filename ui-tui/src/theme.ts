...
// Mirrors @clio/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @clio/ink is bundled separately from packages/.
...
export const DARK_THEME: Theme = {
  color: {
    ...
    completionBg: '#18052f',
    completionCurrentBg: '#32145a',
    completionMetaBg: '#18052f',
    completionMetaCurrentBg: '#32145a',
...
// Terminal. Explicit CLIO_TUI_THEME / COLORFGBG signals above still win,
...
// `CLIO_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `CLIO_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `CLIO_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `CLIO_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Clio palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.CLIO_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.CLIO_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.CLIO_TUI_BACKGROUND ?? '')

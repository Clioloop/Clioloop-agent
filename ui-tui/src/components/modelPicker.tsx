...
type Stage = 'provider' | 'key' | 'model' | 'disconnect' | 'oauth'
...
export function ModelPicker({ allowPersistGlobal = true, gw, onCancel, onSelect, sessionId, t }: ModelPickerProps) {
  ...
  const [oauthInfo, setOauthInfo] = useState<{ url: string; userCode: string } | null>(null)
  ...
  const back = () => {
    ...
    if (stage === 'model' || stage === 'key' || stage === 'disconnect' || stage === 'oauth') {
      setStage('provider')
      setModelIdx(0)
      setKeyInput('')
      setKeyError('')
  ...
  // Poll the gateway while the Omni Loop Portal device login is pending.
  useEffect(() => {
    if (stage !== 'oauth' || !oauthInfo) {
      return
    }

    const timer = setInterval(() => {
      gw.request<{ message?: string; provider?: ModelOptionProvider; status?: string }>(
        'model.oauth_status',
        sessionId ? { session_id: sessionId } : {}
      )
        .then(raw => {
          const r = asRpcResult<{ message?: string; provider?: ModelOptionProvider; status?: string }>(raw)
          ...
          if (r?.status === 'connected') {
            ...
            setOauthError(r.message ?? 'login failed')
          }
        })
        .catch(() => {
  ...
  useInput((ch, key) => {
    // OAuth stage is display-only: overlay keys (Esc/q) own navigation.
    if (stage === 'oauth') {
      return
    }
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `clio model` to configure'
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Omni Loop Portal: one-click browser device login
          if (provider.auth_type === 'oauth_device_code') {
            setStage('oauth')
            setOauthInfo(null)
            setOauthError('')
            setFilter('')
            gw.request<{ user_code?: string; verification_uri_complete?: string }>('model.oauth_start', {
              slug: provider.slug,
              ...(sessionId ? { session_id: sessionId } : {})
            })
              .then(raw => {
                const r = asRpcResult<{ user_code?: string; verification_uri_complete?: string }>(raw)

                if (r?.user_code) {
                  setOauthInfo({ url: r.verification_uri_complete ?? '', userCode: r.user_code })
                } else {
                  setOauthError('failed to start the portal login')
                }
              })
              .catch((e: unknown) => {
          ...
          // Other auth types: no-op (warning shown tells them to run clio model)
          return
        }
    ...
    // filter. With Ctrl held, @clio/ink reports `ch` as the key name ('g'),
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          Paste your API key below (saved to ~/.clio/.env)
        </Text>
  ...
  // ── Omni Loop Portal device-login stage ───────────────────────────────
  if (stage === 'oauth' && provider) {
    return (
      <Box flexDirection="column" width={width}>
        ...
        {oauthError ? (
          <Text color={t.color.error ?? t.color.label} wrap="truncate-end">
            {oauthError}
          </Text>
        ) : oauthInfo ? (
          <>

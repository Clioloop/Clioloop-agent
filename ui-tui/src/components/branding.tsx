import { Box, Text, useStdout } from '@clio/ink'
import { useEffect, useState } from 'react'
import unicodeSpinners from 'unicode-animations'

import { artWidth, infinity, INFINITY_WIDTH, logo, LOGO_WIDTH, terminalArt } from '../banner.js'
...
const TAG_FULL = 'Omni Loop Labs · A digital companion for autonomous loops'
const TAG_MID = 'A digital companion for autonomous loops'
const TAG_TINY = 'Omni Loop Labs'
const HIDE_BELOW = 34
...
export function Banner({ maxWidth, t }: { maxWidth?: number; t: Theme }) {
  ...
  const terminalLines = terminalArt(t.color, cols - 4)

  if (terminalLines) {
    return (
      <Box flexDirection="column" marginBottom={1}>
        <ArtLines lines={terminalLines} />
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  const heroLines = infinity(t.color, t.bannerHero || undefined)
  const leftW = Math.min((artWidth(heroLines) || INFINITY_WIDTH) + 4, Math.floor(cols * 0.4))
      ...
      {wide && (
        <Box flexDirection="column" marginRight={2} width={leftW}>
          ...
          <Text color={t.color.accent}>
            ...
            <Text color={t.color.muted}> · Omni Loop Labs</Text>
          </Text>
      ...
      <Box flexDirection="column" width={w}>
        ...
        {typeof info.update_behind === 'number' && info.update_behind > 0 && (
          <Text bold color={t.color.warn}>
            ...
            <Text bold color={t.color.warn}>
              {info.update_command || 'clio update'}
            </Text>
            <Text bold={false} color={t.color.warn} dimColor>
              {' '}
              to update

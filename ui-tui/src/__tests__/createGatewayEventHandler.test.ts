...
describe('createGatewayEventHandler', () => {
  ...
  it('surfaces Clioloop completion summaries as a persistent system line', () => {
    const appended: Msg[] = []
    const ctx = buildCtx(appended)
    const onEvent = createGatewayEventHandler(ctx)

    onEvent({
      payload: { text: "💾 Clioloop completed: Skill 'clio-release' patched" },
      type: 'review.summary'
    } as any)
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.clio/config.yaml'
      },
  ...
  it('prefers raw text over Rich-rendered ANSI on message.complete (#16391)', () => {
    ...
    const rendered = '\u001b[33mClio here.\u001b[0m\n\n\u001b[2mLine two.\u001b[0m'
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `clio model` to select a provider, or run `clio setup` for first-time configuration.'
      },
      type: 'error'
    } as any)

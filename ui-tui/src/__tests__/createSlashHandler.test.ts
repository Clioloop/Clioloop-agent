...
describe('createSlashHandler', () => {
  ...
  it('routes /status to live session.status instead of slash worker', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ output: 'Clio TUI Status' }))
    ...
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
    await vi.waitFor(() => {
      expect(ctx.transcript.page).toHaveBeenCalledWith('Clio TUI Status', 'Status')
    })
  })
  ...
  it('falls through to command.dispatch for skill commands and sends the message', async () => {
      ...
      gateway: {
        gw: {
          ...
          request: vi.fn((method: string) => {
            ...
            if (method === 'command.dispatch') {
              return Promise.resolve({ type: 'skill', message: skillMessage, name: 'clio-agent-dev' })
            }
    ...
    const h = createSlashHandler(ctx)
    expect(h('/clio-agent-dev')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('⚡ loading skill: clio-agent-dev')
    })
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ file: '/tmp/clio_conversation_test.json' }))
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('conversation saved to: /tmp/clio_conversation_test.json')
    })
  })
  ...
  it('opens the Fusion picker only after the backend gate allows it', async () => {
    patchUiState({ sid: 'sid-abc' })
    const ctx = buildCtx({
      gateway: {
        ...buildGateway(),
        gw: { ...buildGateway().gw, request: vi.fn(() => Promise.resolve({ output: 'OK' })) }
      }
    })

    expect(createSlashHandler(ctx)('/fusion')).toBe(true)
    expect(ctx.gateway.gw.request).toHaveBeenCalledWith('slash.exec', {
      command: 'fusion gate',
      session_id: 'sid-abc'
    })
    await vi.waitFor(() => {
      expect(getOverlayState().fusionPicker).toBe(true)
    })
  })

  it('shows the Fusion gate denial instead of opening the picker', async () => {
    patchUiState({ sid: 'sid-abc' })
    const ctx = buildCtx({
      gateway: {
        ...buildGateway(),
        gw: { ...buildGateway().gw, request: vi.fn(() => Promise.resolve({ output: 'Fusion is only available on Max.' })) }
      }
    })

    expect(createSlashHandler(ctx)('/fusion')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('Fusion is only available on Max.')
    })
    expect(getOverlayState().fusionPicker).toBe(false)
  })

  it('routes explicit Fusion arguments through slash.exec', async () => {
    patchUiState({ sid: 'sid-abc' })
    const ctx = buildCtx({
      gateway: {
        ...buildGateway(),
        gw: { ...buildGateway().gw, request: vi.fn(() => Promise.resolve({ output: 'Fusion configured.' })) }
      }
    })

    expect(createSlashHandler(ctx)('/fusion advisors=a reviewers=b')).toBe(true)
    expect(ctx.gateway.gw.request).toHaveBeenCalledWith('slash.exec', {
      command: 'fusion advisors=a reviewers=b',
      session_id: 'sid-abc'
    })
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('Fusion configured.')
    })
  })

...
const logDir = join(process.env.CLIO_HOME?.trim() || join(homedir(), '.clio'), 'logs')
...
// a real ~/.clio (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('clio-tui: parent lifecycle log unavailable\n')
    }
  }
}

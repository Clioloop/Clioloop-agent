...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CLIO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Clio on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CLIO_TUI_TRUECOLOR=1.

import type { RunExternalProcess } from '@clio/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`clio ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching clio: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`clio ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })

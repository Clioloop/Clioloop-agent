import { forceRedraw, type MouseTrackingMode } from '@clio/ink'
...
export const coreCommands: SlashCommand[] = [
...

  {
    ...
    run: (_arg, ctx) => {
      ...
      // Exit code 42 signals the Python wrapper to exec `clio update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed — try CLIO_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Clio #${i + 1}`

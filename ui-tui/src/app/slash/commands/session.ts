...
export const sessionCommands: SlashCommand[] = [
  ...
  {
    help: 'configure Fusion planners/reviewers',
    name: 'fusion',
    run: (arg, ctx, cmd) => {
      if (!ctx.sid) {
      ...
      const trimmed = arg.trim()
      const render = (r: SlashExecResponse | null, fallback: string) => {
        const body = r?.output || fallback
        const text = r?.warning ? `warning: ${r.warning}\n${body}` : body
        const long = text.length > 180 || text.split('\n').filter(Boolean).length > 2

        long ? ctx.transcript.page(text, 'Fusion') : ctx.transcript.sys(text)
      }

      if (trimmed) {
        if (trimmed !== 'status' && ctx.session.guardBusySessionSwitch('configure Fusion')) {
          return
        }

        return ctx.gateway.gw
          .request<SlashExecResponse>('slash.exec', { command: cmd.slice(1), session_id: ctx.sid })
          .then(ctx.guarded<SlashExecResponse>(r => render(r, '/fusion: no output')))
          .catch(ctx.guardedErr)
      }

      if (ctx.session.guardBusySessionSwitch('configure Fusion')) {
        return
      }

      ctx.gateway.gw
        .request<SlashExecResponse>('slash.exec', { command: 'fusion gate', session_id: ctx.sid })
        .then(
          ctx.guarded<SlashExecResponse>(r => {
            const output = (r?.output ?? '').trim()
            if (output === 'OK') {
              patchOverlayState({ fusionPicker: true })
            } else {
              render(r, 'Fusion is not available for this account.')
            }
          })
        )
        .catch(ctx.guardedErr)

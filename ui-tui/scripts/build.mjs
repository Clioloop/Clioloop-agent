...
await build({
  ...
  // Skip the prebuilt @clio/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@clio/ink': resolve(root, 'packages/clio-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The clio_cli launcher

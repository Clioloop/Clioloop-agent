...
React + Ink terminal UI for Clio. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CLIO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.clio/.clio_history` or under `CLIO_HOME`.
...
```text
ui-tui/
  packages/clio-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      clio-ink.d.ts    type declarations for @clio/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ClioCLI subprocess for slash commands
```

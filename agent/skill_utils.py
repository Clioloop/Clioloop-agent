...
from clio_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``CLIO_KANBAN_TASK`` / ``CLIO_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("CLIO_KANBAN_TASK") or os.getenv("CLIO_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Clio Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``CLIO_PLATFORM`` or
            ``CLIO_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``clio`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.clio/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``clio`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against CLIO_HOME, not cwd
        if not p.is_absolute():
            p = (clio_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.clio/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    clio = metadata.get("clio") or {}
    if not isinstance(clio, dict):
        clio = {}
    return {
        "fallback_for_toolsets": clio.get("fallback_for_toolsets", []),
        "requires_toolsets": clio.get("requires_toolsets", []),
        "fallback_for_tools": clio.get("fallback_for_tools", []),
        "requires_tools": clio.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Clio metadata, VCS, virtualenv/dependency, and cache

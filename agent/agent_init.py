...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.clio/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from clio_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.clio/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from clio_logging import setup_logging, setup_verbose_logging
    setup_logging(clio_home=_ra()._clio_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that clio_logging.setup_logging() does not
        ...
        # handler level inside clio_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from clio_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from clio_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `clio model` to "
                        "select a provider, or run `clio setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff CLIO_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["CLIO_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.clio/sessions/ alongside gateway sessions
    clio_home = get_clio_home()
    agent.logs_dir = clio_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.clio/sessions/session_{sid}.json)
    ...
    try:
        from clio_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from clio_cli.config import load_config as _load_agent_config
    ...
    agent._stall_guards = bool((_agent_cfg.get("agent") or {}).get("stall_guards", True))
        ...
        try:
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                ...
                agent._memory_store = MemoryStore(
                    memory_char_limit=mem_config.get("memory_char_limit", 8800),
                    user_char_limit=mem_config.get("user_char_limit", 5500),
                )
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "clio"
                    except Exception:
                        pass
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from clio_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Clio Agent.  Choose a model with at least "

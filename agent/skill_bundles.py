...
Storage
-------
Bundles live in ``~/.clio/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CLIO_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``clio bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CLIO_HOME.

    Honors ``CLIO_BUNDLES_DIR`` for tests; falls back to
    ``<CLIO_HOME>/skill-bundles``.
    """
    override = os.environ.get("CLIO_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_clio_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `clio bundles` CLI subcommand.
# ---------------------------------------------------------------------------

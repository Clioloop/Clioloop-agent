...
Resolution order for text tasks (auto mode):
  ...
  3. managed provider (~/.clio/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Clio triggers summarization.  Higher values delay
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``CLIO_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``CLIO_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("CLIO_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("CLIO_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "ClioAgent",
}
...
# managed provider extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by managed provider.
#
# The tags are computed from agent.request_tags so the client= marker stays
# in lockstep with clio_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from agent.request_tags import request_tags as _request_tags


def _managed_extra_body() -> dict:
    """Return a fresh managed provider ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``clio_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _request_tags()}


# Backwards-compatible module attribute. Some callers (tests, third-party
# plugins) read ``MANAGED_EXTRA_BODY`` directly; keep it as a snapshot of the
# current tags. Callers that need the freshest value should call
# ``_managed_extra_body()`` or import ``request_tags`` directly.
MANAGED_EXTRA_BODY = _managed_extra_body()

# Set at resolve time — True if the auxiliary client points to managed provider
auxiliary_is_managed: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_MANAGED_MODEL = "google/gemini-3-flash-preview"
_MANAGED_DEFAULT_BASE_URL = "https:///v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_clio_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Clio Agent)",
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for managed).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    # runtime_base_url handles provider-specific logic (e.g. managed prefers inference_base_url).
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``clio_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_managed_auth() -> Optional[dict]:
    """Read and validate ~/.clio/auth.json for an active managed provider provider.

    Returns the provider state dict if managed provider is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("managed")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _MANAGED_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "managed":
            return None
        provider = data.get("providers", {}).get("managed", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read managed provider auth: %s", exc)
        return None


def _managed_api_key(provider: dict) -> str:
    """Extract a usable managed provider inference JWT from stored auth state."""
    from clio_cli.auth import _managed_invoke_jwt_is_usable
    ...
    for token_key, expiry_key in (
        ...
        if not isinstance(token, str) or not token.strip():
            continue
        if _managed_invoke_jwt_is_usable(
            token,
...
def _managed_base_url() -> str:
    """Resolve the managed provider inference base URL from env or default."""
    return os.getenv("CLIO_MANAGED_INFERENCE_BASE_URL", _MANAGED_DEFAULT_BASE_URL)


def _resolve_managed_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh managed provider runtime credentials when available.
    ...
    """
    try:
        from clio_cli.auth import resolve_managed_runtime_credentials

        creds = resolve_managed_runtime_credentials(
            timeout_seconds=float(os.getenv("CLIO_MANAGED_TIMEOUT_SECONDS", "15")),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary managed provider runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``clio auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``clio_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from clio_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Clio auth store.
    ...
    try:
        from clio_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from clio_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from clio_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from clio_cli.models import copilot_default_headers
...
def _try_managed(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting managed provider —
    # if another session already recorded a 429, skip managed provider entirely
    # to avoid piling more requests onto the tapped RPH bucket.
    try:
        from agent.portal_rate_guard import portal_rate_limit_remaining
        _remaining = portal_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping managed provider (rate-limited, resets in %.0fs)",
                _remaining,
            )
            _mark_provider_unhealthy("managed", ttl=_remaining)
            return None, None
    except Exception:
        pass

    managed = _read_managed_auth()
    runtime = _resolve_managed_runtime_api(force_refresh=False)
    if runtime is None and not managed:
        logger.warning(
            "Auxiliary managed provider client unavailable: no managed provider authentication found "
            "(run: clio auth)."
        )
        _mark_provider_unhealthy("managed", ttl=60)
        return None, None
    if runtime is None and managed:
        logger.debug(
            "Auxiliary managed provider: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_managed
    auxiliary_is_managed = True
    logger.debug("Auxiliary client: managed provider")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/managed/recommended-models endpoint is the authoritative source:
    # it distinguishes paid vs free tier recommendations, and get_managed_recommended_aux_model
    # auto-detects the caller's tier via check_managed_free_tier().  Fall back to
    # _MANAGED_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _MANAGED_MODEL
    try:
        from clio_cli.models import get_managed_recommended_aux_model
        recommended = get_managed_recommended_aux_model(vision=vision)
        if recommended:
            model = recommended
            logger.debug(
    ...
    if runtime is not None:
        ...
        api_key = _managed_api_key(managed or {})
        if not api_key:
            logger.warning(
                "Auxiliary managed provider client unavailable: no usable inference JWT found "
                "(run: clio auth add managed)."
            )
            _mark_provider_unhealthy("managed", ttl=60)
            return None, None
        base_url = str((managed or {}).get("inference_base_url") or _managed_base_url()).rstrip("/")
    return (
        OpenAI(
            api_key=api_key,
            base_url=base_url,
...
def _refresh_managed_recommended_model(
    *, vision: bool, stale_model: Optional[str]
) -> Optional[str]:
    """Re-fetch the managed provider's recommended model after a stale-model 404.
    ...
    the managed provider → OpenRouter catalog, every auxiliary call 404s with
    ...
    model name to retry with:
      ...
      * ``_MANAGED_MODEL`` (google/gemini-3-flash-preview), the known-good default,
    ...
    try:
        ...
        fresh = get_managed_recommended_aux_model(vision=vision, force_refresh=True)
    except Exception as exc:
        logger.debug(
            "managed provider recommended-model refresh failed (%s); using default %s",
            exc, _MANAGED_MODEL,
        )
    ...
    if _MANAGED_MODEL.strip().lower() != stale:
        return _MANAGED_MODEL
    return None
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `clio setup` or `clio model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    Mirrors the ``_try_anthropic`` / ``_try_managed`` shape but delegates to
    :func:`clio_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from clio_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second clio
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``clio config aux reset``)."""
...
def _managed_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh managed provider account API says paid access is allowed."""
    try:
        from clio_cli.portal_account import get_managed_portal_account_info

        account_info = get_managed_portal_account_info(force_fresh=True)
        return account_info.paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary managed-provider paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the managed provider → OpenRouter catalog. The managed provider
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "managed": "managed",
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/managed provider get their chosen chat model; users
      ...
      2. OpenRouter → managed provider → custom → Codex → API-key providers (fallback
         chain, only used when the main provider has no working client).
    """
    global auxiliary_is_managed, _stale_base_url_warned
    auxiliary_is_managed = False  # Reset — _try_managed() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
    ...
    #    scenario where a user switches providers via `clio model` but the
    #    old OPENAI_BASE_URL lingers in ~/.clio/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: clio model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.clio/.env",
    ...
    # on aggregators (OpenRouter, managed provider) who previously got routed to a
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from clio_cli.copilot_auth import copilot_request_headers
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from clio_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "managed", "openai-codex" (or "codex"),
    ...
    if provider == "managed":
        ...
        client, default = _try_managed(vision=_is_vision)
        if client is None:
            logger.warning("resolve_provider_client: managed requested "
                           "but managed provider not configured (run: clio auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: clio model)")
                return None, None
    ...
    # OpenRouter / managed provider bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: clio model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from clio_cli.runtime_provider import _get_named_custom_provider
        ...
        # entries that coincidentally match a canonical provider (e.g. ``managed``)
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: clio doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("clio_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from clio_cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "managed":
            return resolve_provider_client("managed", model, async_mode)
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(
    ...
    if provider == "managed":
        return _try_managed(vision=True)
    if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → managed provider → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. managed provider — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      zai → glm-5v-turbo). managed provider is the exception: it has a dedicated
        #      strict vision backend with tier-aware defaults, so it must not
        #      fall through to the user's text chat model here.
        #   2. OpenRouter  (vision-capable aggregator fallback)
        #   3. managed provider (vision-capable aggregator fallback)
        #   4. Stop
        main_provider = _read_main_provider()
        main_model = _read_main_model()
        if main_provider and main_provider not in {"auto", ""}:
            vision_model = _PROVIDER_VISION_MODELS.get(main_provider, main_model)
            if main_provider == "managed":
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes managed provider product tags when the auxiliary client is backed
    by managed provider. Returns empty dict otherwise.
    """
    return _managed_extra_body() if auxiliary_is_managed else {}
...
def auxiliary_max_tokens_param(value: int) -> dict:
    ...
    if (not or_key
            and _read_managed_auth() is None
...
def _refresh_managed_auxiliary_client(
    *,
    cache_provider: str,
    model: Optional[str],
    async_mode: bool,
    ...
    """Refresh managed provider runtime creds, rebuild the client, and replace the cache entry."""
    runtime = _resolve_managed_runtime_api(force_refresh=True)
    if runtime is None:
        return None, model
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`clio_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    if provider == "managed" or auxiliary_is_managed:
        merged_extra.setdefault("tags", []).extend(_request_tags())
    if merged_extra:
...
def call_llm(
    ...
    *,
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `clio model`."
                )
    ...
    try:
        ...
        # ── Stale-model self-heal (managed provider recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the managed provider → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to managed-provider-routed calls.
        _heal_is_managed = (
            resolved_provider == "managed"
            or base_url_host_matches(_base_info, "")
        )
        if _is_model_not_found_error(first_err) and _heal_is_managed:
            healed_model = _refresh_managed_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in managed provider catalog; "
        ...
        # ── managed provider auth refresh parity with main agent ──────────────────
        client_is_managed = (
            resolved_provider == "managed"
            or base_url_host_matches(_base_info, "")
        )
        if (
            ...
            and client_is_managed
            and _managed_portal_account_has_fresh_paid_access()
        ):
            refreshed_client, refreshed_model = _refresh_managed_auxiliary_client(
                cache_provider=resolved_provider or "managed",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed managed provider runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_managed:
            refreshed_client, refreshed_model = _refresh_managed_auxiliary_client(
                cache_provider=resolved_provider or "managed",
                model=final_model,
                async_mode=False,
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed managed provider runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        # the managed provider → OpenRouter catalog, 404'ing every auxiliary call. Force a
        ...
        if _is_model_not_found_error(first_err) and _heal_is_managed:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in managed provider catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed managed provider runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_managed:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed managed provider runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(

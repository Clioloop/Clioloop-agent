...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``CLIO_PLATFORM`` env var and
    ``CLIO_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("CLIO_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.clio/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.clio.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_clio_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.clio/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.clio/skills/`` and any ``skills.external_dirs`` so the

...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Clio starts and finds ``~/.openclaw/``.

    Points users at ``clio claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``clio claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Clio, run "
        "`clio claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `clio claw cleanup` (renames it to ~/.openclaw.pre-migration — "

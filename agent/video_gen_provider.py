...
as ``kind: backend``) or ``~/.clio/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _videos_cache_dir() -> Path:
    """Return ``$CLIO_HOME/cache/videos/``, creating parents as needed."""
    from clio_constants import get_clio_home

    path = get_clio_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CLIO_HOME/cache/videos/``.

...
as ``kind: backend``) or ``~/.clio/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$CLIO_HOME/cache/images/``, creating parents as needed."""
    from clio_constants import get_clio_home

    path = get_clio_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$CLIO_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$CLIO_HOME/cache/images/``.

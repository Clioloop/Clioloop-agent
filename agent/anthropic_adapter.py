"""Anthropic Messages API adapter for Clio Agent.

Translates between Clio's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Clio historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Clio)
    ...
    # 1. Clio-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Clio configs.
...
# ── Clio-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.clio/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_CLIO_OAUTH_FILE = get_clio_home() / ".anthropic_oauth.json"
...
def run_clio_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Clio-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Clio with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from clio_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_clio_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Clio-managed OAuth credentials from ~/.clio/.anthropic_oauth.json."""
    if _CLIO_OAUTH_FILE.exists():
        try:
            data = json.loads(_CLIO_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Clio OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See clio-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See clio-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See clio-agent#13848 (Kimi) and
    clio-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Clio Agent", "Claude Code")
                text = text.replace("Clio agent", "Claude Code")
                text = text.replace("clio-agent", "claude-code")
                text = text.replace("Omni Loop Labs", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Clio surfaces in its CLI. We

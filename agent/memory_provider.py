...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - clio_home (str): The active CLIO_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.clio``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "clio").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'clio memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], clio_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'clio memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``clio_home`` is the active CLIO_HOME directory path.

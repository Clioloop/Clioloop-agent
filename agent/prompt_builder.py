...
COMPUTER_USE_GUIDANCE = (
    ...
    "2. ALWAYS click by element index when the target has one: "
    "`action='click', element=14`. This is dramatically more reliable than "
    "pixel coordinates on every platform and every model, because the driver "
    "resolves the element itself (no coordinate/DPI guesswork). Prefer it for "
    "buttons, menus, links, fields, list items. Likewise drag by element: "
    "`action='drag', from_element=3, to_element=7`.\n"
    "3. Use raw `coordinate=[x,y]` ONLY when there is no element (e.g. dragging "
    "desktop icons, canvas/drawing surfaces, games). Pick the coordinate off "
    "the most recent capture's image; it is mapped to the real screen for you. "
    "Raw coordinates need a VISION model that can actually see the screenshot — "
    "a text-only model cannot place them and will misclick.\n"
    "4. For text input, `action='type', text='...'`. For key combos "

...
COMPUTER_USE_GUIDANCE = (
    ...
    "## Preferred workflow (default to the FAST path)\n"
    "1. Call `computer_use` with `action='capture'` and `mode='ax'` "
    "(the default) — the accessibility tree only, NO screenshot. You get a "
    "numbered list of every interactable element with role, label, and bounds. "
    "This is fast and cheap; a screenshot is NOT needed to navigate most apps "
    "(browsers, native apps, IDEs, settings). Prefer `mode='ax'` for nearly "
    "every step.\n"
    "2. ALWAYS click by element index: `action='click', element=14`. This is "
    "the most reliable AND the fastest path (no image, no coordinate/DPI "
    "guesswork — the driver resolves the element). Prefer it for buttons, "
    "menus, links, fields, list items. Likewise drag by element: "
    "`action='drag', from_element=3, to_element=7`.\n"
    "3. Only take a SCREENSHOT (`mode='som'` = screenshot+overlays, or "
    "`mode='vision'`) when you genuinely need to SEE pixels: the AX tree is "
    "missing/insufficient, a target has no element (canvas, drawing surfaces, "
    "games, desktop icons), or you must visually verify a result. Screenshots "
    "cost a full image to the model every call and are MUCH slower — use them "
    "deliberately, not by default.\n"
    "4. Use raw `coordinate=[x,y]` ONLY when there is no element (e.g. dragging "
    "desktop icons, canvas). Pick it off the most recent screenshot; it's "
    "mapped to the real screen for you. Raw coordinates require a vision model "
    "that can see the screenshot.\n"
    "5. For text input, `action='type', text='...'`. For key combos "
    "`action='key', keys='cmd+s'`. For scrolling `action='scroll', "
    "direction='down', amount=3`.\n"
    "6. To verify a state-changing action in one round-trip, pass "
    "`capture_after=true` — it returns a fast AX follow-up by default (no "
    "screenshot). Add `capture_after_mode='som'` only when you must verify "
    "the result VISUALLY.\n\n"

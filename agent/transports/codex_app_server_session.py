...
Owns one Codex thread per Clio session. Drives `turn/start`, consumes
...
# Clio' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_CLIO_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Clio/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Clio session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _CLIO_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("CLIO_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
        self._approval_callback = approval_callback
        self._on_event = on_event  # Display hook (clio-spinner spinner ticks etc.)
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            ...
            client_version=_get_clio_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Clio' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Clio' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own clio-tools callback we
            # auto-accept — the user already approved Clio' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Clio approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Clio returns 'once', 'session', 'always', or 'deny'.
...
def _get_clio_version() -> str:
    """Best-effort Clio version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("clio-agent")

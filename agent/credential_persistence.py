...
``auth.json``.  They intentionally have no dependency on ``clio_cli.auth`` so
...
# Sources Clio owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Clio-owned OAuth/device-code state)

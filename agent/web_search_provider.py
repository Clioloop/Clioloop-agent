...
``kind: backend``) or ``~/.clio/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class WebSearchProvider(abc.ABC):
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``clio tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``clio_cli/tools_config.py`` to inject this provider as a

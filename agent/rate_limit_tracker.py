...
the managed provider header format (also used by OpenRouter and OpenAI-compatible
...
def _provider_display_name(provider: Optional[str]) -> str:
    """Map a provider ID to a user-facing display name.

    The legacy ``"managed"`` provider ID is mapped to ``"managed provider"``
    since the original provider was removed in the rebrand and the
    display name should not leak the old vendor name.
    """
    if not provider:
        return "Provider"
    if provider.lower() in ("managed", "managed-provider", "managed_portal"):
...
def format_rate_limit_display(state: RateLimitState) -> str:
    ...
    provider_label = _provider_display_name(state.provider)

...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Clio tool use: "
    ...
    return (
        ...
        f"context, but Clio needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Clio config, set `model.ollama_num_ctx: 65536` "
...
def _print_managed_entitlement_guidance(agent, capability: str) -> bool:
    message = _managed_entitlement_message(capability)
    if not message:
        return False
...
def _is_managed_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        ...
        or base_url_host_matches(base, "inference.")
    )
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_managed_inference_route(provider, base_url):
        return _managed_entitlement_message(capability)
...
def _try_refresh_managed_paid_entitlement_credentials(agent) -> bool:
    """Refresh managed provider runtime credentials after a fresh paid-entitlement check."""
    try:
        ...
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_managed_client_credentials(
            force=True,
        )
    except Exception:
        return False
...
def run_conversation(
    ...
    # ``clio logs --session <id>`` can filter a single conversation.
    ...
    # are reused.  The system prompt is Clio's territory; plugins
    ...
    # Notify memory providers of real user turns so cadence tracking works.
    # Fusion draft/revision runs are internal scaffolding and should not advance
    # external memory cadence or dialectic refresh state.
    if agent._memory_manager and not internal_turn:
        try:
    ...
    # all run inside Codex). Default Clio path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Clio internals.
        #
        # Clio invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Clio tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── managed provider rate limit guard ──────────────────────
            # If another session already recorded that managed provider is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "managed":
                try:
                    from agent.portal_rate_guard import (
                        portal_rate_limit_remaining,
                        format_remaining as _fmt_portal_remaining,
                    )
                    _portal_remaining = portal_rate_limit_remaining()
                    if _portal_remaining is not None and _portal_remaining > 0:
                        _managed_msg = (
                            f"managed provider rate limit active — "
                            f"resets in {_fmt_portal_remaining(_portal_remaining)}."
                        )
            ...
            try:
                ...
                if env_var_enabled("CLIO_DUMP_REQUESTS"):
                ...
                # Clear managed provider rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting the managed provider.
                if agent.provider == "managed":
                    try:
                        from agent.portal_rate_guard import clear_portal_rate_limit
                        clear_portal_rate_limit()
                    except Exception:
                        pass
                ...
                if (
                    ...
                    and _is_managed_inference_route(
                        getattr(agent, "provider", "") or "",
                ...
                if (
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 managed-provider 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_managed_entitlement_guidance(agent, "Omni Loop Portal model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: clio auth add managed")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `clio doctor` for credential-chain diagnostics, or")
                    ...
                    from clio_constants import display_clio_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Clio-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: clio config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: clio config set ANTHROPIC_API_KEY \"\"")
                ...
                # When managed provider returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: managed provider multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Clio).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every managed provider model for minutes.  We use
                # ``is_genuine_portal_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_portal_rate_limit = False
                    try:
                        from agent.portal_rate_guard import (
                            is_genuine_portal_rate_limit,
                            record_portal_rate_limit,
                        )
                        ...
                        _genuine_portal_rate_limit = is_genuine_portal_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_portal_rate_limit:
                            record_portal_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                            ...
                            logger.info(
                                "managed provider 429 looks like upstream capacity "
                ...
                # which Clio' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Clio' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`clio",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "managed" and _print_managed_entitlement_guidance(
                            agent,
                            "managed provider model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "managed"} and status_code == 401:
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `clio auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `clio model`.", force=True)
                            else:  # managed
                                agent._vprint(f"{agent.log_prefix}   💡 managed provider OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: clio portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://", force=True)
                                # ``:free`` is OpenRouter slug syntax; managed provider will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         managed provider won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         managed provider catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: clio setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        clio fallback add   (interactive picker — same as `clio model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not a Clio/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `clio fallback add`."
                        )
                        return {
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("CLIO_KANBAN_TASK")
        if _kanban_task:
            try:
    ...
    if final_response and not interrupted and not internal_turn:
        try:

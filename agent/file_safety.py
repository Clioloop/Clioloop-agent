...
def _clio_home_path() -> Path:
    """Resolve the active CLIO_HOME (profile-aware) without circular imports."""
    try:
        from clio_constants import get_clio_home  # local import to avoid cycles
        return get_clio_home()
    except Exception:
        return Path(os.path.expanduser("~/.clio"))


def _clio_root_path() -> Path:
    """Resolve the Clio root dir (always the parent of any profile, never per-profile)."""
    try:
        from clio_constants import get_default_clio_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    clio_home = _clio_home_path()
    clio_root = _clio_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(clio_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(clio_root / ".anthropic_oauth.json"),
            ...
            # The Fusion thin client: its prompt-injection-defended client code
            # should not be rewritten by the agent's own tools. (The Fusion engine
            # itself runs server-side and is never present on user machines.)
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "fusion_engine.py"),
            "/etc/sudoers",
            "/etc/passwd",
            "/etc/shadow",
        ]
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved CLIO_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("CLIO_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Clio control-plane files: block both the ACTIVE profile's view
    # (clio_home) AND the global root view. Without the root pass, a
    ...
    clio_dirs = []
    for base in (_clio_home_path(), _clio_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in clio_dirs:
                clio_dirs.append(real)
        except Exception:
            continue

    for base_real in clio_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Clio path.

    Three categories are blocked:

      * Internal Clio cache files under ``CLIO_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under CLIO_HOME and the global Clio
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.clio/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active CLIO_HOME (profile-aware) AND the global
    # Clio root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (CLIO_HOME points at
    ...
    for hd in clio_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Clio cache file "
    ...
    for hd in clio_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Clio credential store "
    ...
    for hd in clio_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Clio MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Clio MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Clio profiles are separate CLIO_HOME dirs under
...
#
# Reference: May 2026 incident where a clio-security profile session
# edited skills under both ``~/.clio/profiles/clio-security/skills/``
# AND ``~/.clio/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under CLIO_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from CLIO_HOME.
    ...
    """
    try:
        home_real = _clio_home_path().resolve()
        root_real = _clio_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Clio scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Clio entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Clio profile {info['target_profile']!r}, but the "
...
#
#   <CLIO_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.clio/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.clio/…`` segment and warns
# regardless of which Clio profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.clio/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.clio`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.clio/…`` and returns the
    index where the inner Clio-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .clio / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".clio":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Clio state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.clio``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.clio`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Clio resolver to
    succeed, so it works correctly even when called from contexts where
    CLIO_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Clio process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real CLIO_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.clio/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.clio/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host CLIO_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Clio state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Clio "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real CLIO_HOME. Use the "

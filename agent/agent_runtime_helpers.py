...
def try_recover_primary_transport(
    ...
    Skipped for proxy/aggregator providers (OpenRouter, managed provider) which
    ...
    if provider_lower in {"managed", "managed_provider"}:
        return False
...
def extract_reasoning(agent, assistant_message) -> Optional[str]:
    ...
    # Check direct reasoning field.  SDK response objects occasionally expose
    # dynamic attributes (and mocks expose every attribute), so only accept the
    # string shape that providers are documented to return.
    ...
    if not reasoning_parts and isinstance(content, list):
        ...
        for block in content:
            if isinstance(block, dict) and block.get("type") == "thinking":
                raw_thinking = block.get("thinking") or block.get("text") or ""
                thinking_text = raw_thinking.strip() if isinstance(raw_thinking, str) else ""
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        if env_var_enabled("CLIO_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # The Omni Loop Portal proxies to OpenRouter behind the scenes — identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_clio_portal = provider_lower == "managed"
    ...
    if is_native_anthropic:
        return True, True
    if (is_openrouter or is_clio_portal) and is_claude:
        return True, False
    # managed provider Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    ...
    if is_clio_portal and "qwen" in model_lower:
        return True, False
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if not pre_tool_block_checked:
        try:
            from clio_cli.plugins import get_pre_tool_call_block_message
...
def trailing_continue_intent(text: str | None) -> bool:
    """Whether a short reply ends by announcing an immediate next action."""
    candidate = (text or "").strip()
    if not candidate or len(candidate) > _TRAILING_CONTINUE_INTENT_MAX_CHARS:
        return False
    return bool(_TRAILING_CONTINUE_INTENT_RE.search(candidate[-160:]))

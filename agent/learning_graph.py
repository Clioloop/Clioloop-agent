...
def _clio_meta(fm: dict[str, Any]) -> dict[str, Any]:
    """``metadata.clio`` as a dict, tolerant of the string-valued frontmatter
    that ``parse_frontmatter``'s malformed-YAML fallback produces."""
    meta = fm.get("metadata")
    clio = meta.get("clio") if isinstance(meta, dict) else None
    # Accept upstream metadata during migration without making it canonical.
    if not isinstance(clio, dict) and isinstance(meta, dict):
        clio = meta.get("hermes")
    return clio if isinstance(clio, dict) else {}


def _related(fm: dict[str, Any]) -> list[str]:
    raw = fm.get("related_skills") or _clio_meta(fm).get("related_skills")
...
def _category(fm: dict[str, Any], skill_md: Path) -> str:
    cat = fm.get("category") or _clio_meta(fm).get("category")
    if cat:
        return str(cat)
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        path = get_clio_home() / "skills" / ".usage.json"
        try:
...
def density_stats(nodes: dict[str, SkillNode], edges: list[tuple[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "edges_per_node": round(len(edges) / n, 3) if n else 0.0,
        "linked_nodes": len(linked),
        "isolated_pct": round(100 * (n - len(linked)) / n, 1) if n else 0.0,
        "categories": len(cats),
...
def _skill_roots() -> list[tuple[str, Path]]:
    ...
    home_skills = get_clio_home() / "skills"

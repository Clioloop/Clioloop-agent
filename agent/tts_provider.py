...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Clio with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.clio/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Clio
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``clio tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``clio setup`` for

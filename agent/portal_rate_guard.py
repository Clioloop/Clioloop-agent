"""Cross-session rate limit guard for managed inference providers.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether the managed provider is currently
rate-limited before making requests.  Prevents retry amplification when
the per-hour budget is tapped.

Each 429 from a managed provider can trigger up to 9 API calls per
conversation turn (3 SDK retries x 3 Clio retries), and every one of
those calls counts against RPH.  By recording the rate limit state on
first 429 and checking it before subsequent attempts, we eliminate the
...
_STATE_FILENAME = "portal_rate_limit.json"


def _state_path() -> str:
    """Return the path to the portal rate-limit state file."""
    try:
        from clio_constants import get_clio_home
        base = get_clio_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".clio")
...
def record_portal_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that the managed inference provider is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Portal rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Portal rate limit state: %s", exc)


def portal_rate_limit_remaining() -> Optional[float]:
    """Check if the managed provider is currently rate-limited.
...
def clear_portal_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful managed-provider request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Portal rate limit state: %s", exc)
...
def is_genuine_portal_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from a managed provider is a real account rate limit.

    A managed provider may multiplex multiple upstream providers (DeepSeek, Kimi,
    MiMo, Clio, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on the managed provider is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on the managed provider.

    Tripping the cross-session breaker on (b) blocks ALL managed-provider requests
    (and all models, since the managed provider is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  the managed provider emits

"""CLI presentation -- spinner, clio-spinner faces, tool preview formatting.
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Clio' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class ClioSpinner:
    """Animated spinner with clio-spinner faces for CLI feedback during tool execution.

    Renamed from KawaiiSpinner during the Clio rebrand (2026). The face
    strings have been replaced with minimal infinity/orbit glyphs to match
    the new brand mark; custom skins can still override them via
    ``spinner.waiting_faces`` and ``spinner.thinking_faces``.
    """
    ...
    CLIO_SPINNER_WAITING = [
        "∞", "◜", "◠", "◝", "◞", "◡", "◟", "∞",
    ]

    CLIO_SPINNER_THINKING = [
        "◜", "◠", "◝", "◞", "◡", "◟", "∞",
    ]

    THINKING_VERBS = [
        "weaving", "spiraling", "lacing", "looping", "fathoming",
        "deciphering", "decoding", "traversing", "composing", "resolving",
        "unfurling", "indexing", "shaping", "synthesizing",
    ]

    @classmethod
    def get_waiting_faces(cls) -> list:
        """Return waiting faces from the active skin, falling back to CLIO_SPINNER_WAITING."""
        try:
            skin = _get_skin()
            if skin:
            ...
            pass
        return cls.CLIO_SPINNER_WAITING

    @classmethod
    def get_thinking_faces(cls) -> list:
        """Return thinking faces from the active skin, falling back to CLIO_SPINNER_THINKING."""
        try:
            skin = _get_skin()
            if skin:
            ...
            pass
        return cls.CLIO_SPINNER_THINKING
    ...
    def _is_patch_stdout_proxy(self) -> bool:
        ...
        so ClioSpinner's \\r-based animation is redundant under StdoutProxy.
        """
        try:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("CLIO_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue

...
Plugins built on Clio Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Clio
...
:class:`~clio_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Clio' main config
...
handles every provider, fallback chain, and per-task override Clio
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`clio_cli.plugins.PluginContext`

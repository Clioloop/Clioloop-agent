...
upstream prefix cache warm.  See ``clio-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, managed subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_managed_subscription_prompt``,
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Clio never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CLIO_HOME persona while keeping
    ...
    # Pointer to the clio-agent skill + docs for user questions about Clio itself.
    stable_parts.append(CLIO_AGENT_HELP_GUIDANCE)
    ...
    # CLIO_KANBAN_TASK env var). Normal chat sessions never see
    ...
    managed_subscription_prompt = _r.build_managed_subscription_prompt(agent.valid_tool_names)
    if managed_subscription_prompt:
        stable_parts.append(managed_subscription_prompt)
    ...
    # Active-profile hint — names the Clio profile the agent is running
    # under so it doesn't conflate ~/.clio/skills/ (default profile) with
    # ~/.clio/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Clio profile: default. Other profiles (if any) live "
            "under ~/.clio/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Clio profile: {active_profile}. This session reads "
            f"and writes ~/.clio/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.clio/skills/, ~/.clio/plugins/, "
            f"~/.clio/cron/, ~/.clio/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Clio never rebuilds or reinjects parts of it

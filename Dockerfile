...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/clio/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when clio
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# clio process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main clio process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Clio WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via CLIO_UID at runtime
RUN useradd -u 10000 -m -d /opt/data clio
...
#
# ui-tui/packages/clio-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/clio-ink/ ui-tui/packages/clio-ink/
...
# lock on the @clio/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/clio/.venv at first
...
# ---------- Permissions ----------
# Make install dir world-readable so any CLIO_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the clio user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# clio_cli/main.py succeeds (see #18800). /opt/clio/web is build-time
# only (CLIO_WEB_DIST points at clio_cli/web_dist) and is intentionally
# not chowned here.
# /opt/clio/gateway is runtime-writable: Python may create __pycache__ and
# gateway state artifacts beneath the package after services drop privileges,
# especially when the clio UID is remapped at boot (#27221).
# The .venv MUST remain clio-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/clio && \
    chown -R clio:clio /opt/clio/.venv /opt/clio/ui-tui /opt/clio/gateway /opt/clio/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the clio user via
# `s6-setuidgid clio` in its run script. If CLIO_UID is unset, services
# run as the default clio user (UID 10000).

# ---------- Link clio-agent itself (editable) ----------
...
# container always returns nothing — meaning `clio dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the CLIO_GIT_SHA build-arg to
# /opt/clio/.clio_build_sha at build time, and have
# clio_cli/build_info.py read it at runtime.  Both `clio dump` and
...
ARG CLIO_GIT_SHA=
RUN if [ -n "${CLIO_GIT_SHA}" ]; then \
        printf '%s\n' "${CLIO_GIT_SHA}" > /opt/clio/.clio_build_sha && \
        chown clio:clio /opt/clio/.clio_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-clio + dashboard.
...
# `exec clio`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $CLIO_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/clio/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-clio-setup && \
    chmod +x /etc/cont-init.d/01-clio-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV CLIO_WEB_DIST=/opt/clio/clio_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/clio/ui-tui/dist/entry.js`) and skip the
...
ENV CLIO_TUI_DIR=/opt/clio/ui-tui
ENV CLIO_HOME=/opt/data

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> clio ...` they default to root, and any file the
# command writes under $CLIO_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/clio/bin/clio, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid clio`
# when invoked as root. Non-root callers (supervised processes,
# `--user clio`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/clio/.venv/bin/clio). See the shim source for
# the opt-out env var (CLIO_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/clio-exec-shim.sh /opt/clio/bin/clio

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> clio ...` and any
# subprocess that doesn't activate the venv first still find clio.
#
# /opt/clio/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/clio/bin:/opt/clio/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. clio
# subcommand vs. no-args), drops to the clio user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/clio/docker/main-wrapper.sh" ]
CMD [ ]

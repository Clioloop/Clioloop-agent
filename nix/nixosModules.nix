# nix/nixosModules.nix — NixOS module for clio-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: clio runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $CLIO_HOME/.env
# and read by clio at startup — no container recreation needed for env changes.
#
# Tool resolution: the clio wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.clio-agent;
    effectivePackage =
    ...
    clio-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-clio-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "clio-config-attrs";
      description = "Clio YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "clio-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "clio-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'CLIO_DOC_EOF'\n${value}\nCLIO_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "clio-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/clio";
    ...
    # clio user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "clio-container-entrypoint" ''
      set -eu

      CLIO_UID="''${CLIO_UID:?CLIO_UID must be set}"
      CLIO_GID="''${CLIO_GID:?CLIO_GID must be set}"

      # ── Group: ensure a group with GID=$CLIO_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$CLIO_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="clio"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$CLIO_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$CLIO_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$CLIO_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$CLIO_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="clio"
        TARGET_HOME="/home/clio"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$CLIO_UID" -g "$CLIO_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$CLIO_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$CLIO_UID:$CLIO_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure CLIO_HOME is owned by the target user.
      ...
      if [ -n "''${CLIO_HOME:-}" ] && [ -d "$CLIO_HOME" ]; then
        find "$CLIO_HOME" \! -user "$CLIO_UID" -exec chown "$CLIO_UID:$CLIO_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/clio-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/clio-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/clio ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/clio
        chmod 0440 /etc/sudoers.d/clio
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$CLIO_UID" --regid="$CLIO_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $CLIO_HOME/.env.
    ...
    # Default: /var/lib/clio/workspace → /data/workspace.
  ...
  in {
    options.services.clio-agent = with lib; {
      enable = mkEnableOption "Clio Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = clio-agent;
        description = "The clio-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/clio";
        description = "State directory. Contains .clio/ subdir (CLIO_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Clio config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $CLIO_HOME/.env at activation time.
          Clio reads this file on every startup via load_clio_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $CLIO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $CLIO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `clio gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the clio user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the clio plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Clio discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          clio_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as clio (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in clio-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the clio CLI to environment.systemPackages and export
          CLIO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.clio symlink to the service
            stateDir. These users are automatically added to the clio group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.clio-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the clio CLI to system PATH and export CLIO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.clio/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.CLIO_HOME = "${cfg.stateDir}/.clio";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.clio-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the clio user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.clio-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed clio
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure clio is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.clio        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clio/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clio/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clio/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clio/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clio/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."clio-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.clio
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.clio ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.clio ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.clio -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.clio/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.clio/$_subdir"
            chmod 2770 "${cfg.stateDir}/.clio/$_subdir"
            find "${cfg.stateDir}/.clio/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.clio/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.clio/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clio/config.yaml
            chmod 0640 ${cfg.stateDir}/.clio/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.clio/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clio/.managed
          chmod 0644 ${cfg.stateDir}/.clio/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.clio/.container-mode <<'CLIO_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    clio_bin=${containerDataDir}/current-package/bin/clio
    CLIO_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clio/.container-mode
            chmod 0644 ${cfg.stateDir}/.clio/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.clio/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.clio";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.clio" ]; then
                  rm -f "${symlinkPath}"
                  echo "clio-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.clio -> stateDir/.clio for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.clio";
                target = "${cfg.stateDir}/.clio";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "clio-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.clio/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.clio/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.clio/auth.json
              fi
            ''}
          ''}
          ...
          # Clio reads $CLIO_HOME/.env at startup via load_clio_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.clio/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'CLIO_NIX_ENV_EOF'
    ${envFileContent}
    CLIO_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.clio/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.clio/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clio/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.clio-agent = {
          description = "Clio Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $CLIO_HOME/.env by the activation script. load_clio_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/clio"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the clio group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.clio-agent = {
          description = "Clio Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              CLIO_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              CLIO_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env CLIO_UID="$CLIO_UID" \
                --env CLIO_GID="$CLIO_GID" \
                --env CLIO_HOME=${containerDataDir}/.clio \
                --env CLIO_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/clio gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}

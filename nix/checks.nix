...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      clio-agent = self'.packages.default;
      clioVenv = clio-agent.clioVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "clio-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${clioVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "clio-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "clio-build-package" { } ''
          echo "PASS: package built at ${clio-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "clio-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "clio-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${clio-agent}/bin/clio || (echo "FAIL: clio binary missing"; exit 1)
          test -x ${clio-agent}/bin/clio-agent || (echo "FAIL: clio-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${clio-agent}/bin/clio version 2>&1 | grep -qi "clio" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "clio-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in clio clio-agent clio-acp; do
            test -x ${clio-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "clio-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking clio --help ==="
          ${clio-agent}/bin/clio --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${clio-agent}/bin/clio --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "clio-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${clio-agent}/share/clio-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${clio-agent}/share/clio-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "CLIO_BUNDLED_SKILLS" ${clio-agent}/bin/clio || \
            (echo "FAIL: CLIO_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: CLIO_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "clio-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${clio-agent}/share/clio-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${clio-agent}/share/clio-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "CLIO_BUNDLED_PLUGINS" ${clio-agent}/bin/clio || \
            (echo "FAIL: CLIO_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: CLIO_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "clio-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${clio-agent}/share/clio-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${clio-agent}/share/clio-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${clio-agent}/share/clio-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "CLIO_BUNDLED_LOCALES" ${clio-agent}/bin/clio || \
            (echo "FAIL: CLIO_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: CLIO_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (CLIO_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && CLIO_BUNDLED_LOCALES=${clio-agent}/share/clio-agent/locales \
            ${clioVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with CLIO_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${clioVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${clioVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without CLIO_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "clio-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${clio-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${clio-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "CLIO_TUI_DIR" ${clio-agent}/bin/clio || \
            (echo "FAIL: CLIO_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: CLIO_TUI_DIR set in wrapper"
        ...
        # Verify CLIO_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        clio-node = pkgs.runCommand "clio-node-version" { } ''
          set -e
          echo "=== Checking CLIO_NODE in wrapper ==="
          grep -q "CLIO_NODE" ${clio-agent}/bin/clio || \
            (echo "FAIL: CLIO_NODE not set in wrapper"; exit 1)
          echo "PASS: CLIO_NODE present in wrapper"

          CLIO_NODE=$(sed -n "s/^export CLIO_NODE='\(.*\)'/\1/p" ${clio-agent}/bin/clio)
          test -x "$CLIO_NODE" || (echo "FAIL: CLIO_NODE=$CLIO_NODE not executable"; exit 1)
          echo "PASS: CLIO_NODE executable at $CLIO_NODE"

          NODE_MAJOR=$("$CLIO_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All CLIO_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify CLIO_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "clio-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(CLIO_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking CLIO_MANAGED guards ==="
          check_blocked "config set" ${clio-agent}/bin/clio config set model foo
          check_blocked "config edit" ${clio-agent}/bin/clio config edit
        ...
        extra-python-packages = let
          ...
          clioWithExtra = clio-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "clio-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${clioWithExtra}/bin/clio || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${clioWithExtra}/bin/clio || \
          ...
          if grep -q "PYTHONPATH" ${clio-agent}/bin/clio; then
        ...
        extra-dependency-groups = let
          clioWithGroups = clio-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "clio-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${clioWithGroups}"
          echo "venv: ${clioWithGroups.clioVenv}"
        ...
        messaging-variant = pkgs.runCommand "clio-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.clioVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "clio-config-roundtrip" {
          ...
          merge_and_load() {
            local clio_home="$1"
            export CLIO_HOME="$clio_home"
            ${configMergeScript} ${nixSettings} "$clio_home/config.yaml"
            ${clioVenv}/bin/python3 -c '
import json, sys

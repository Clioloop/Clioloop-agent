...
describe("telegram lib", () => {
  ...
  describe("parseCreatedBot", () => {
    ...
    it("extracts a nested managed_bot payload", async () => {
      const { parseCreatedBot } = await import("./telegram");
      const got = parseCreatedBot({
        managed_bot: { token: TOKEN, username: "@clio_xyz_bot" },
        from: { id: 4242 },
      });
      expect(got).toEqual({ token: TOKEN, username: "clio_xyz_bot", ownerUserId: 4242 });
    });

    it("extracts flat top-level fields", async () => {
      const { parseCreatedBot } = await import("./telegram");
      const got = parseCreatedBot({ bot_token: TOKEN, bot_username: "clio_flat_bot", owner_user_id: "77" });
      expect(got).toEqual({ token: TOKEN, username: "clio_flat_bot", ownerUserId: 77 });
    });

...
function layout(heading: string, bodyHtml: string, ctaLabel: string, ctaUrl: string): string {
  return `
  <div style="background:#050d0b;padding:40px 16px;font-family:-apple-system,Segoe UI,Roboto,sans-serif">
    <div style="max-width:480px;margin:0 auto;border:1px solid rgba(47,230,185,0.2);background:#081511;padding:36px">
      <div style="font-family:monospace;font-size:12px;letter-spacing:3px;text-transform:uppercase;color:#2fe6b9;margin-bottom:24px">
        &#8734; Omni Loop Portal
      </div>
      <h1 style="color:#f2efe6;font-size:22px;margin:0 0 14px">${heading}</h1>
      <div style="color:#93a69d;font-size:14.5px;line-height:1.7">${bodyHtml}</div>
      <a href="${ctaUrl}"
         style="display:inline-block;margin-top:26px;background:#2fe6b9;color:#03110d;font-family:monospace;font-size:13px;font-weight:600;letter-spacing:2px;text-transform:uppercase;text-decoration:none;padding:13px 26px">
        ${ctaLabel}
      </a>
      <p style="color:#5d7068;font-size:12px;margin-top:26px;line-height:1.6">
        Or paste this link into your browser:<br>
        <span style="font-family:monospace;color:#93a69d;word-break:break-all">${ctaUrl}</span>
      </p>
    </div>

"""
Session-scoped context variables for the Clio gateway.

Replaces the previous ``os.environ``-based session state
(``CLIO_SESSION_PLATFORM``, ``CLIO_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["CLIO_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("CLIO_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("CLIO_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("CLIO_SESSION_PLATFORM", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("CLIO_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("CLIO_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("CLIO_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("CLIO_SESSION_USER_ID", default=_UNSET)
...
_SESSION_MESSAGE_ID: ContextVar = ContextVar("CLIO_SESSION_MESSAGE_ID", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("CLIO_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("CLIO_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("CLIO_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "CLIO_SESSION_PLATFORM": _SESSION_PLATFORM,
    "CLIO_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "CLIO_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "CLIO_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "CLIO_SESSION_USER_ID": _SESSION_USER_ID,
    ...
    "CLIO_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``CLIO_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("CLIO_SESSION_ID")`` with an ``os.environ`` fallback,
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``CLIO_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("CLIO_SESSION_*", default)``.

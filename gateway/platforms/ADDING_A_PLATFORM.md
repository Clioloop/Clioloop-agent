...
There are two ways to add a platform to the Clio gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.clio/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Clio code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `clio gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `clio_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Clio core.
...
And add it to the `clio-gateway` composite:

```python
"clio-gateway": {
    "includes": [..., "clio-your-platform"]
}
```
...
## 12. Status Display (`clio_cli/status.py`)
...
## 13. Gateway Setup Wizard (`clio_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ clio_cli/ toolsets.py \
  --include="*.py" -l | sort -u

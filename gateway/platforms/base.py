...
# Audio file extensions Clio recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Clio' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Clio-created Telegram private-chat
...
from clio_constants import get_default_clio_root, get_clio_dir, get_clio_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.clio/.env manually."
)
...
# Default location: {CLIO_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_clio_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; ClioAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_clio_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_clio_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_clio_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_clio_dir("cache/screenshots", "browser_screenshots")
_CLIO_HOME = get_clio_home()
_CLIO_ROOT = get_default_clio_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "CLIO_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "CLIO_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "CLIO_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "CLIO_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _CLIO_HOME / "image_cache",
    _CLIO_HOME / "audio_cache",
    _CLIO_HOME / "video_cache",
    _CLIO_HOME / "document_cache",
    _CLIO_HOME / "browser_screenshots",
    ...
    _CLIO_HOME / "cache" / "images",
    _CLIO_HOME / "cache" / "audio",
    _CLIO_HOME / "cache" / "videos",
    _CLIO_HOME / "cache" / "documents",
    _CLIO_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Clio profile and shared Clio root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for clio_root in (_CLIO_HOME, _CLIO_ROOT):
        denied.append(clio_root / ".env")
        denied.append(clio_root / "auth.json")
        denied.append(clio_root / "credentials")
        denied.append(clio_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Clio
    secrets (``~/.clio/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``CLIO_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Clio-managed cache, under an operator-allowlisted root
    (``CLIO_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.clio/.env,
    # ~/.clio/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?clio\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+clio[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("CLIO_GATEWAY_BUSY_TEXT_MODE", "queue").strip().lower()
            or "queue"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "CLIO_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "CLIO_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    async def send_multiple_images(
        ...
        for image_url, alt_text in self.dedupe_image_refs(images):
            if human_delay > 0:
    ...
    @staticmethod
    def _delivery_path_key(path: str) -> str:
        """Stable identity key for local delivery paths and file:// URIs."""
    ...
    @staticmethod
    def dedupe_delivery_paths(file_paths) -> List[str]:
        """Deduplicate local delivery paths after validation, preserving order."""
        unique: List[str] = []
        seen: set[str] = set()
        for file_path in file_paths or []:
            key = BasePlatformAdapter._delivery_path_key(str(file_path))
            if key in seen:
                continue
            seen.add(key)
    ...
    @staticmethod
    def dedupe_media_and_local_delivery_paths(
        media_files,
        local_files,
    ) -> Tuple[List[Tuple[str, bool]], List[str]]:
        """Deduplicate MEDIA paths against bare local paths, preserving order.

        MEDIA directives win over bare-path auto-detection for the same file.
        """
        ...
        for media_path, is_voice in media_files or []:
            key = BasePlatformAdapter._delivery_path_key(str(media_path))
            if key in seen:
                continue
            seen.add(key)
            unique_media.append((media_path, bool(is_voice)))
        ...
        for file_path in local_files or []:
            ...
            if key in seen:
                continue
            seen.add(key)
            unique_local.append(file_path)

        return unique_media, unique_local

    @staticmethod
    def dedupe_image_refs(images) -> List[Tuple[str, str]]:
        """Deduplicate image URL/file refs before native batch delivery."""
        unique: List[Tuple[str, str]] = []
        seen: set[str] = set()
        for image_url, alt_text in images or []:
            url = str(image_url or "").strip()
            if url.startswith("file://"):
                key = "file://" + BasePlatformAdapter._delivery_path_key(url)
            else:
                key = url
            if key in seen:
                continue
            seen.add(key)
            unique.append((image_url, alt_text))
        return unique
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.clio/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from clio_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          CLIO_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          CLIO_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          CLIO_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("CLIO_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("CLIO_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("CLIO_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        # Skip it entirely for INTERNAL turns (e.g. the synthetic empty-text
        # event used to auto-resume a restart-interrupted session): the user
        # didn't send anything, so a "typing…" bubble they never triggered —
        # which lingers until the resume turn finishes — is confusing and was a
        # source of "the typing indicator never stops" after a gateway restart.
        _is_internal_turn = bool(getattr(event, "internal", False))
        typing_task = None
        if not _is_internal_turn:
        ...
        try:
            ...
            if response:
                ...
                media_files, local_files = self.dedupe_media_and_local_delivery_paths(
            ...
            # not at the top of this task.  _clio_run_generation is set on

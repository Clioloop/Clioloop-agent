...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("CLIO_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "CLIO_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "CLIO_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
        ...
        # Interactive /fusion 3-step picker state per chat
        self._fusion_picker_state: Dict[str, dict] = {}
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Clio-created private-chat topic lanes are marked with
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `clio update` or `--replace` handoffs)
    ...
    def _persist_dm_topic_thread_id(
        ...
        try:
            ...
            config_path = get_clio_home() / "config.yaml"
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Clio process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("CLIO_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("CLIO_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("CLIO_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("CLIO_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("CLIO_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("CLIO_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            try:
                ...
                from clio_cli.commands import telegram_menu_commands
    ...
    def reset_status_message(self, chat_id: str, status_key: str) -> None:
        """Forget the cached bubble for (chat_id, status_key).

        The next send_or_update_status for this key sends a fresh message
        instead of editing the previous one — used so each new fusion turn
        starts a new progress bubble rather than re-editing the last turn's.
        """
        self._status_message_ids.pop((str(chat_id), str(status_key)), None)
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``clio update --gateway``
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``clio model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from clio_cli.models import group_providers
        except Exception:
    ...
    def _build_model_keyboard(
        self, models: list, page: int, *, managed: bool = False, pricing: dict | None = None
    ) -> tuple:
        """Build paginated model buttons. Returns (keyboard, page_info_text).

        For the managed Omni Loop Portal provider each model is tagged
        ``(free)`` / ``(open)`` / ``(openrouter)`` and laid out one per row so
        the tag stays legible.
        """
        ...
        for i, model_id in enumerate(page_models):
            ...
            if tag_fn is not None:
                tag = tag_fn(model_id, pricing)
                label = f"{short} {tag}".strip()
                if len(label) > 40:
                    label = short[: max(4, 40 - len(tag) - 2)] + "… " + tag
            else:
                label = short if len(short) <= 38 else short[:35] + "..."
            buttons.append(
                InlineKeyboardButton(label, callback_data=f"mm:{abs_idx}")
            )

        if managed:
            rows = [[b] for b in buttons]  # one per row so the tag is legible
        else:
            rows = [buttons[i : i + 2] for i in range(0, len(buttons), 2)]
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            is_managed = provider_slug == "managed"
            pricing = None
            if is_managed:
                # Show the FULL live managed catalog (the passed list can be
                # capped), grouped open-weight first then OpenRouter — mirrors
                # the CLI picker — so users see every entitled model.
                try:
            ...
            state["model_pricing"] = pricing
            state["model_page"] = 0

            keyboard, page_info = self._build_model_keyboard(models, 0, managed=is_managed, pricing=pricing)

            pname = provider.get("name", provider_slug)
            total = len(models) if is_managed else provider.get("total_models", len(models))
            shown = len(models)
            ...
            is_managed = provider_slug == "managed"
            pricing = state.get("model_pricing")
            keyboard, page_info = self._build_model_keyboard(models, page, managed=is_managed, pricing=pricing)
            ...
            total = (
                len(models)
                if is_managed
                else (provider.get("total_models", len(models)) if provider else len(models))
            )
            shown = len(models)
            extra = f"\n_{total - shown} more available — type `/model <name>` directly_" if total > shown else ""
            ...
            group_id = data[4:]
            try:
                from clio_cli.models import PROVIDER_GROUPS
    ...
    # ---- /fusion: interactive 3-step model picker ----------------------

    _FUSION_PAGE_SIZE = 8

    @staticmethod
    def _fusion_tag(model_id: str) -> str:
        from agent.fusion_engine import model_open_tag
        return model_open_tag(model_id)

    # Maximum models per group (mirrors fusion_engine.FUSION_MAX_MODELS_PER_GROUP).
    _FUSION_MAX_PER_GROUP = 5

    def _build_count_keyboard(self, show_back: bool) -> "InlineKeyboardMarkup":
        """Buttons 1..N for choosing how many models in the current group."""
        row = [
            InlineKeyboardButton(str(n), callback_data=f"fc:{n}")
            for n in range(1, self._FUSION_MAX_PER_GROUP + 1)
        ]
        bottom: list = []
        if show_back:
            bottom.append(InlineKeyboardButton("◀ Back", callback_data="fb"))
        bottom.append(InlineKeyboardButton("✗ Cancel", callback_data="fx"))
        return InlineKeyboardMarkup([row, bottom])

    def _build_fusion_keyboard(self, models: list, page: int, show_back: bool) -> tuple:
        """Paginated managed-catalog buttons for one model-pick phase.

        Returns ``(InlineKeyboardMarkup, page_info)``. Each model is tagged
        ``(open)`` / ``(openrouter)``; one button per row so the tag is legible.
        """
        page_size = self._FUSION_PAGE_SIZE
        total = len(models)
        total_pages = max(1, (total + page_size - 1) // page_size)
        page = max(0, min(page, total_pages - 1))
        start = page * page_size
        ...
        rows: list = []
        for i, model_id in enumerate(models[start:end]):
            abs_idx = start + i
            tag = self._fusion_tag(model_id)
            short = model_id if "/" not in model_id else model_id.split("/", 1)[1]
            label = f"{short} {tag}"
            if len(label) > 40:
                label = short[: max(4, 40 - len(tag) - 2)] + "… " + tag
            rows.append([InlineKeyboardButton(label, callback_data=f"fm:{abs_idx}")])

        if total_pages > 1:
            nav: list = []
            if page > 0:
                nav.append(InlineKeyboardButton("◀ Prev", callback_data=f"fg:{page - 1}"))
            nav.append(InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="fx:noop"))
            if page < total_pages - 1:
                nav.append(InlineKeyboardButton("Next ▶", callback_data=f"fg:{page + 1}"))
            rows.append(nav)
        ...
        page_info = f"({start + 1}–{end} of {total})" if total_pages > 1 else f"({total} models)"
        return InlineKeyboardMarkup(rows), page_info
    ...
    def _fusion_picker_text(self, state: dict, page_info: str = "") -> str:
        phase = state.get("phase", "planner_count")
        advisors = state.get("advisors") or []
        reviewers = state.get("reviewers") or []
        if phase == "planner_count":
            title = "How many models should *plan* the task? (1–5)"
        elif phase == "planner_pick":
            title = f"Select *planner model {len(advisors) + 1} of {state.get('planner_count')}*"
        elif phase == "reviewer_count":
            title = "How many models should *review* the draft? (1–5)"
        elif phase == "reviewer_pick":
            title = f"Select *reviewer model {len(reviewers) + 1} of {state.get('reviewer_count')}*"
        else:
            title = "Fusion setup is complete."
        lines = ["🔮 *Model Fusion*", "", title]
        if advisors:
            lines.append("Planners: " + "  ·  ".join(f"`{p}`" for p in advisors))
        if reviewers:
            lines.append("Reviewers: " + "  ·  ".join(f"`{p}`" for p in reviewers))
        if page_info:
            lines.append(page_info)
        return "\n".join(lines)

    def _fusion_keyboard_for_state(self, state: dict) -> tuple:
        """Return ``(keyboard, page_info)`` for the current phase."""
        phase = state.get("phase", "planner_count")
        # Back is available everywhere except the very first phase.
        show_back = phase != "planner_count"
        if self._fusion_is_count_phase(phase):
            return self._build_count_keyboard(show_back), ""
        return self._build_fusion_keyboard(
            state.get("models") or [], state.get("page", 0), show_back
        )

    async def send_fusion_picker(
        self,
        chat_id: str,
        models: list,
        session_key: str,
        ...
        """Send the interactive Model Fusion picker.

        Flow: how-many-planners → pick planners → how-many-reviewers → pick
        reviewers. ``on_fusion_complete(chat_id, advisors: list, reviewers:
        list)`` is awaited once all picks are made and should return the
        confirmation text.
        """
        if not self._bot:
            return SendResult(success=False, error="Not connected")
        try:
            state = {
                ...
                "models": models,
                "session_key": session_key,
                "on_fusion_complete": on_fusion_complete,
                "phase": "planner_count",
                "planner_count": 0,
                "advisors": [],
                "reviewer_count": 0,
            ...
            keyboard, page_info = self._fusion_keyboard_for_state(state)
            text = self.format_message(self._fusion_picker_text(state, page_info))
            thread_id = metadata.get("thread_id") if metadata else None
            ...
            self._fusion_picker_state[str(chat_id)] = state
            return SendResult(success=True, message_id=str(msg.message_id))
        except Exception as e:
            logger.warning("[%s] send_fusion_picker failed: %s", self.name, e)
            return SendResult(success=False, error=str(e))

    async def _handle_fusion_picker_callback(
        self, query, data: str, chat_id: str
    ) -> None:
        """Handle fusion picker callbacks (fc:/fm:/fg:/fb/fx)."""
        state = self._fusion_picker_state.get(chat_id)
        if not state:
            await query.answer(text="Picker expired — use /fusion again.")
            return
        ...
        if data == "fx:noop":
            ...
            return
        if data == "fx":
            self._fusion_picker_state.pop(chat_id, None)
            with contextlib.suppress(Exception):
                await query.edit_message_text(text="Fusion setup cancelled.")
            await query.answer(text="Cancelled")
            return

        if data == "fb":
            self._fusion_back(state)
            await _render()
            await query.answer()
            return
        ...
        if data.startswith("fc:"):
            ...
            if not self._fusion_is_count_phase(phase):
                await query.answer()
                return
            try:
                n = int(data[3:])
                ...
                await query.answer()
                return
            n = max(1, min(self._FUSION_MAX_PER_GROUP, n))
            state["page"] = 0
            if phase == "planner_count":
            ...
            await _render()
            await query.answer(text=f"{n} model(s)")
            return
        ...
        if data.startswith("fm:"):
            ...
            if phase == "planner_pick":
                ...
                await _render()
                await query.answer(text=f"Planner: {model_id}")
                return

            if phase == "reviewer_pick":
                state["reviewers"].append(model_id)
                if len(state["reviewers"]) >= state["reviewer_count"]:
                    callback = state.get("on_fusion_complete")
                    advisors = list(state.get("advisors") or [])
                    reviewers = list(state.get("reviewers") or [])
                    self._fusion_picker_state.pop(chat_id, None)
                    confirmation = "🔮 Fusion configured."
                    if callback is not None:
                        try:
                            confirmation = await callback(chat_id, advisors, reviewers)
                        except Exception as exc:
                            logger.warning("on_fusion_complete failed: %s", exc)
                            confirmation = f"Fusion setup error: {exc}"
                    with contextlib.suppress(Exception):
                        await query.edit_message_text(
                            text=self.format_message(confirmation),
                            parse_mode=ParseMode.MARKDOWN_V2,
                        )
                    await query.answer(text="Fusion on")
                    return
                await _render()
                await query.answer(text=f"Reviewer: {model_id}")
                return

        await query.answer()

    @staticmethod
    def _fusion_back(state: dict) -> None:
        """Step the picker one move backwards through the phase machine."""
        phase = state.get("phase")
        state["page"] = 0
        if phase == "reviewer_pick":
            ...
            # Back into planner selection: re-open the last planner slot.
        ...
        # planner_count: first phase, nothing to go back to.
    ...
    async def _handle_callback_query(
        ...
        # --- Fusion picker callbacks (fc:/fm:/fg:/fb/fx) ---
        if data.startswith(("fc:", "fm:", "fg:", "fb", "fx")):
            chat_id = str(query.message.chat_id) if query.message else None
            if chat_id:
                await self._handle_fusion_picker_callback(query, data, chat_id)
            return
    ...
    # Scripts live in ~/.clio/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".clio" / "scripts" / "gmail-triage" / script_name
    ...
    async def send_multiple_images(
        ...
        if not images:
            return
        images = self.dedupe_image_refs(images)
        if not images:
            return
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@clio_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Clio bot profiles. A message like

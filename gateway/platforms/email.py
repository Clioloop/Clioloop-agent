"""
Email platform adapter for the Clio gateway.

Allows users to interact with Clio by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "clio-agent" "version" "{_clio_version}" '
            '"vendor" "Clioloop" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Clio Agent")
        ...
        msg_id = f"<clio-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"

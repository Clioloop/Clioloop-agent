"""Windows UTF-8 bootstrap for Clio entry points.
...
should be imported at the very top of every Clio entry point
(``clio``, ``clio-agent``, ``clio-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import clio_bootstrap``
# (or ``from clio_bootstrap import apply_windows_utf8_bootstrap``) at

...
import { ThemeContext, type ThemeContextValue } from "./context-state";
...
const STORAGE_KEY = "clio-dashboard-theme";
...
const THEME_NAME_ALIASES: Record<string, string> = {
  // Theme key kept stable across visual iterations of the blue light preset.
  "lens-5i": "managed-provider-blue",
};
 ...
 *  `_THEME_NAMED_ASSET_KEYS` in `clio_cli/web_server.py`. */
...
const CUSTOM_CSS_STYLE_ID = "clio-theme-custom-css";
...
function applyCustomCSS(css: string | undefined) {
  ...
  if (!el) {
    ...
    el.setAttribute("data-clio-theme-css", "true");
...
function injectFontStylesheet(url: string | undefined) {
  ...
  link.rel = "stylesheet";
  link.href = url;
  link.setAttribute("data-clio-theme-font", "true");
...
function applyTheme(theme: DashboardTheme) {
  ...
  // (e.g. managed provider Blue) doesn't leave its values behind when the user

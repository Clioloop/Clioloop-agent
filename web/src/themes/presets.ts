 ...
 * `_BUILTIN_DASHBOARD_THEMES` list in `clio_cli/web_server.py`.
 */
...
export const defaultTheme: DashboardTheme = {
  name: "default",
  label: "Clio Purple",
  description: "Classic dark violet — the canonical Clio look",
  palette: {
    background: { hex: "#1A0B2E", alpha: 1 },
    midground: { hex: "#C4B5FD", alpha: 1 },
    foreground: { hex: "#ffffff", alpha: 0 },
    warmGlow: "rgba(167, 139, 250, 0.35)",
    noiseOpacity: 1,
  },
...
/**
 * Clio Blue — the inverted "light mode" Clio look, ported from the
 * LENS_5I overlay preset in `@clioloop-agent/ui`.
 *
 ...
 * the orange midground reads as #0053FD managed-provider-blue on screen, against a
 * cream `#E8F2FD` canvas.
 *
  * Note on bg blend mode: the DS Lens uses `multiply` for LENS_5I because
  * clio-web's <body> is white; clio-agent's App root is `bg-black`,
 ...
 */
export const managedBlueTheme: DashboardTheme = {
  name: "managed-provider-blue",
  label: "Clio Blue",
  ...
  palette: {
    ...
    // Same warm-amber as clio-web's overlay glow; after the FG
  ...
  // bright managed-provider-blue canvas — the coral is the "red" users see for
  // Output values without these overrides. Source → on-screen:
  //   Input:  #ffe6cb → #001934 (dark navy)        ← unchanged
  //   Output: #ffac02 → #0053fd (vivid managed-provider-blue)  ← brand accent
  ...
  // managed-provider-blue so the "primary" series in token-flow charts reads as
  ...
  //   white → vivid managed-provider-blue → cream/light-blue
...
export const defaultLargeTheme: DashboardTheme = {
  name: "default-large",
  label: "Clio Purple (Large)",
  description: "Clio Purple with bigger fonts and roomier spacing",
...
export const BUILTIN_THEMES: Record<string, DashboardTheme> = {
  ...
  "managed-provider-blue": managedBlueTheme,

...
// (e.g. https://mission-control.tilos.com/clio/). The Python backend
// injects ``window.__CLIO_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__CLIO_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const CLIO_BASE_PATH = readBasePath();
const BASE = CLIO_BASE_PATH;
...
declare global {
  interface Window {
    __CLIO_SESSION_TOKEN__?: string;
    __CLIO_BASE_PATH__?: string;
    ...
    __CLIO_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Clio-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__CLIO_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``clio update``, ``clio gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__CLIO_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__CLIO_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("clio.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("clio.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__CLIO_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("clio.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__CLIO_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Clio dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``clio_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__CLIO_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Clio-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``clio_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__CLIO_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateClio: () =>
    fetchJSON<ActionResponse>("/api/clio/update", { method: "POST" }),
  checkClioUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/clio/update/check${force ? "?force=true" : ""}`,
    ),
  ...
  startPortalConnect: () =>
    fetchJSON<PortalConnectStart>("/api/portal/connect/start", {
      method: "POST",
    }),
  getPortalConnectStatus: () =>
    fetchJSON<PortalConnectStatus>("/api/portal/connect/status"),
  getPortalModels: () => fetchJSON<PortalModels>("/api/portal/models"),
 ...
 * managed provider contract V1 (the access token has no email/name claims —
...
export interface PortalConnectStart {
  status: string;
  user_code: string;
...
export interface PortalConnectStatus {
...
export interface StatusResponse {
  ...
  /*   * Phase 7: registered ``DashboardAuthProvider`` names (e.g. ``["managed"]``).

/**
 * ChatPage — embeds `clio --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (clio_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { CLIO_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${CLIO_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__CLIO_SESSION_TOKEN__ &&
    !window.__CLIO_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `clio dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__CLIO_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Clio TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[clio-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__CLIO_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Clio. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="clio-chat-xterm-host min-h-0 min-w-0 flex-1"
          />

...
import { Segmented } from "@clioloop-agent/ui/ui/components/segmented";
...
import {
  ...
  DialogHeader,
  DialogTitle,
} from "@clioloop-agent/ui/ui/components/dialog";
    ...
    async (id: string) => {
      try {
        const res = await fetch(api.exportSessionUrl(id), {
          credentials: "include",
          headers: {
            "X-Clio-Session-Token":
              (window as unknown as { __CLIO_SESSION_TOKEN__?: string })
                .__CLIO_SESSION_TOKEN__ ?? "",
          },
        });

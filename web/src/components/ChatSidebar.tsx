...
import { Card } from "@clioloop-agent/ui/ui/components/card";
...
export function ChatSidebar({ channel, className }: ChatSidebarProps) {
  ...
  // /api/events in clio_cli/web_server.py for the broadcast hop.
  //
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      ws = new WebSocket(
        `${proto}//${window.location.host}${CLIO_BASE_PATH}/api/events?${qs.toString()}`,
      );

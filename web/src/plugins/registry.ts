 ...
 *
 * Plugins call window.__CLIO_PLUGINS__.register(name, Component)
...
import { Card, CardHeader, CardTitle, CardContent } from "@clioloop-agent/ui/ui/components/card";
import { Input } from "@clioloop-agent/ui/ui/components/input";
import { Label } from "@clioloop-agent/ui/ui/components/label";
import { Separator } from "@clioloop-agent/ui/ui/components/separator";
import { Tabs, TabsList, TabsTrigger } from "@clioloop-agent/ui/ui/components/tabs";
 ...
 * Exposed at runtime as ``window.__CLIO_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__CLIO_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__CLIO_PLUGIN_SDK__ = {
    ...
    // window.__CLIO_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — managed provider DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,

/**
 * Clio Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__CLIO_PLUGIN_SDK__`` and
 * ``window.__CLIO_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__CLIO_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@clioloop-agent/ui``, ...). Deriving the type would
 ...
 *
 * Versioning: bump ``ClioPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@clio/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls UI package types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__CLIO_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__CLIO_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__CLIO_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface ClioPluginSDK {
  ...
  /**
   * Shared UI primitives (managed provider DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __CLIO_PLUGIN_SDK__?: ClioPluginSDK;
    __CLIO_PLUGINS__?: PluginRegistry;
  }
}

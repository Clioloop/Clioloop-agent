"""
Clio MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Clio-specific extra)

Usage:
    clio mcp serve
    clio mcp serve --verbose
...
logger = logging.getLogger("clio.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using CLIO_HOME."""
    try:
        from clio_constants import get_clio_home
        return get_clio_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("CLIO_HOME", Path.home() / ".clio")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from clio_constants import get_clio_home
        directory_file = get_clio_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("CLIO_HOME", Path.home() / ".clio")
...
class EventBridge:
    ...
    This is the Clio equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from clio_constants import get_clio_home
            db_file = get_clio_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("CLIO_HOME", Path.home() / ".clio")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Clio MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "clio",
        instructions=(
            "Clio Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Clio MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(

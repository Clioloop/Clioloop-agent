...
#
# Wired into the image as /etc/cont-init.d/01-clio-setup by the
...
# the container starts non-root. The baked image trees (/opt/data, /opt/clio/
# .venv, ui-tui, node_modules) stay owned by the clio build UID (10000), so an
...
# into $CLIO_HOME). See #34837 for the supervision-tree side of this.
#
# The supported way to match host-side ownership is to start as root (the image
# default) and pass CLIO_UID/CLIO_GID — or the PUID/PGID aliases — which the
...
# `id -u clio` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# clio UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u clio)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-clio UID).
...
and the baked image directories are owned by the clio user (UID $(id -u clio)),
...
# --- Bootstrap CLIO_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid clio mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom CLIO_HOME paths whose parents only
# root can create (e.g. `CLIO_HOME=/home/clio/.clio` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for CLIO_UID/CLIO_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  CLIO_UID/
# CLIO_GID still win when both are set.  See #15290, salvages #25872.
CLIO_UID="${CLIO_UID:-${PUID:-}}"
CLIO_GID="${CLIO_GID:-${PGID:-}}"

if [ -n "${CLIO_UID:-}" ] && validate_uid_gid "$CLIO_UID" && [ "$CLIO_UID" != "$(id -u clio)" ]; then
    echo "[stage2] Changing clio UID to $CLIO_UID"
    usermod -u "$CLIO_UID" clio
fi
if [ -n "${CLIO_GID:-}" ] && validate_uid_gid "$CLIO_GID" && [ "$CLIO_GID" != "$(id -g clio)" ]; then
    echo "[stage2] Changing clio GID to $CLIO_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$CLIO_GID" clio 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised clio user (UID 10000)
...
# clio process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes clio. Idempotent across container
...
#   - clio is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G clio 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] clio already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" clio 2>/dev/null; then
        echo "[stage2] Added clio to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group clio failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When CLIO_UID is remapped or the top-level $CLIO_HOME isn't owned by
# the runtime clio UID, restore ownership to clio — but ONLY for the
# directories clio actually writes to. The full $CLIO_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of clio-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_clio_uid=$(id -u clio)
needs_chown=false
if [ "$(stat -c %u "$CLIO_HOME" 2>/dev/null)" != "$actual_clio_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $CLIO_HOME (targeted) to clio ($actual_clio_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $CLIO_HOME: chown the directory itself (not its contents)
    # so clio can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown clio:clio "$CLIO_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $CLIO_HOME failed (rootless container?) — continuing"
    # Clio-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by clio (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$CLIO_HOME/$sub" ]; then
            chown -R clio:clio "$CLIO_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $CLIO_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi

# --- Fix ownership of build trees under $INSTALL_DIR ---
# Clio-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime clio UID no longer owns them — otherwise:
...
#     the source mtime is newer than dist/ or when CLIO_TUI_FORCE_BUILD
#     is set) and writes to ui-tui/dist/. Without this chown the new
#     clio UID can't write the build output (#28851).
...
# The set mirrors the build-time `chown -R clio:clio` line in the
# Dockerfile — keep them in sync if the Dockerfile chown set changes.
# These are under $INSTALL_DIR (not $CLIO_HOME), so the bind-mount
# concern doesn't apply — recursive is fine.
#
# This MUST be gated independently of the $CLIO_HOME ownership check
# above. `usermod -u <new> clio` re-chowns the clio home dir
# ($CLIO_HOME == /opt/data) to the new UID as a side effect, so after a
# CLIO_UID/PUID remap `stat $CLIO_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/clio
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $CLIO_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime clio UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_clio_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to clio ($actual_clio_uid)"
    chown -R clio:clio \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
# Always reset ownership of $CLIO_HOME/profiles to clio on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> clio …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as clio and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$CLIO_HOME/profiles" ]; then
    chown -R clio:clio "$CLIO_HOME/profiles" 2>/dev/null || true
fi

# Reset ownership of clio-owned top-level state files on every boot.
# The targeted data-volume chown above only covers clio-owned
# *subdirectories*; loose state files living directly under $CLIO_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> clio …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged clio runtime then hits
...
# sweep so host-owned files in a bind-mounted $CLIO_HOME are never
...
# entries of clio_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$CLIO_HOME/$f" ]; then
        chown clio:clio "$CLIO_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the clio runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$CLIO_HOME/config.yaml" ]; then
    chown clio:clio "$CLIO_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$CLIO_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as clio user ---
# Run as clio via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $CLIO_HOME values
...
as_clio mkdir -p \
    ...
    "$CLIO_HOME/home" \
    "$CLIO_HOME/pairing" \
    "$CLIO_HOME/platforms/pairing"

# --- Install-method stamp (read by detect_install_method() in clio status) ---
# Preserved from the tini-era entrypoint (PR #27843). Must be written as
# the clio user so ownership matches the file's documented owner.
# tee is invoked directly via s6-setuidgid (no `sh -c` wrapper) for the
# same shell-metacharacter safety described above.
printf 'docker\n' | as_clio tee "$CLIO_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$CLIO_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_clio cp "$INSTALL_DIR/$src" "$CLIO_HOME/$dest"
    fi
}
...
if [ -f "$CLIO_HOME/.env" ]; then
    chown clio:clio "$CLIO_HOME/.env" 2>/dev/null || true
    chmod 600 "$CLIO_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $CLIO_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `clio update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set CLIO_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$CLIO_HOME/config.yaml" ]; then
    s6-setuidgid clio "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$CLIO_HOME/auth.json" ] && [ -n "${CLIO_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$CLIO_AUTH_JSON_BOOTSTRAP" > "$CLIO_HOME/auth.json"
    chown clio:clio "$CLIO_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$CLIO_HOME/auth.json"
fi
...
# can set CLIO_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# CLIO_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$CLIO_HOME/gateway_state.json" ] && \
        [ "${CLIO_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$CLIO_HOME/gateway_state.json"
    chown clio:clio "$CLIO_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$CLIO_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_clio "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/clio/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Clio spawns for the
...
# main-wrapper.sh propagates it into the supervised ``clio`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-clio, dashboard, etc.).

...
// Matches the tier visuals from clioloop-account-service/src/app/manage-subscription/_components/TierCard.tsx
const TIERS = [
  {
    label: 'Scout',
    src: SCOUT_SRC,
...
const meta: Meta<typeof Poster> = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'A social-ready glitchy card built around the haptic-distortion image component. Defaults to the `vibe` variant — full-bleed distortion with minimal registration chrome — matching the overlay on the Clio Agent website. Switch to `variant="dispatch"` for the broadcast-card layout with copy + sidebar tags.\n\nPoster stories include all five tier images from the `manage-subscription` page so you can screen-record any character on-demand.'
      }
    },
    layout: 'centered'
  },
 ...
 * minimal chrome, just the subtle `Clio Agent` overlay at the bottom-right.
...
export const DispatchPricingTeaser: Story = {
  args: {
    ...
    scale: 1,
    seal: 'v0.9 · 2026',
    signature: 'OmniLoopLabs.com/Clio-Agent',
  ...
  render: ({ showLabels, ...args }) => (
    <div className="bg-background flex min-h-screen items-center justify-center p-8">
      <div className="flex items-stretch gap-4">
        <Poster
          {...args}
          channel={showLabels ? 'MANAGED • STUDIO' : undefined}
          eyebrow={showLabels ? 'TIER / 001' : undefined}
          headline={showLabels ? ['Clio', 'Studio.'] : ['']}
          seal={showLabels ? 'FREE · MIT' : undefined}
          signature={showLabels ? 'Clio.run / studio' : undefined}
          src={TIERS[0].src}
        ...
        <Poster
          {...args}
          channel={showLabels ? 'MANAGED • PRO' : undefined}
          eyebrow={showLabels ? 'TIER / 002' : undefined}
          headline={showLabels ? ['Clio', 'Pro.'] : ['']}
          seal={showLabels ? '$20/mo' : undefined}
          signature={showLabels ? 'Clio.run / pro' : undefined}
          src={TIERS[2].src}
          tags={
            showLabels
        ...
        <Poster
          {...args}
          channel={showLabels ? 'MANAGED • SOVEREIGN' : undefined}
          eyebrow={showLabels ? 'TIER / 003' : undefined}
          headline={showLabels ? ['Clio', 'Sovereign.'] : ['']}
          seal={showLabels ? 'talk to us' : undefined}
          signature={showLabels ? 'Clio.run / sovereign' : undefined}
          src={TIERS[3].src}
          tags={
            showLabels

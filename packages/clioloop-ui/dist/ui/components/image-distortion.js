...
export function ImageDistortion({ active = true, autoPlay, className, fallbackClassName, src, style, tint, tintStrength }) {
    ...
    if (tier === 0) {
        return (_jsx("img", { alt: "", className: cn('absolute inset-0 h-full w-full object-cover', fallbackClassName ?? className), fetchPriority: "high", src: src, style: { mixBlendMode: 'overlay', ...style } }));
    }

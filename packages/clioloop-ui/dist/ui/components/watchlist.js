...
export function Watchlist({ className, counter = false, items, scramble = false, ...props }) {
    return (_jsx("div", { className: cn('flex flex-col gap-3', className), ...props, children: items.map(({ label, right, url }, i) => {
            ...
            return (_jsxs("a", { className: cn('grid items-center gap-2.5 px-2.5 py-1.5', 'text-display leading-[1.4]', 'hover:bg-midground/10! hover:ring-2 hover:ring-current/20', 'transition-all duration-500 hover:duration-0', 'opacity-(--midground-alpha)'), href: url, rel: "noopener noreferrer", style: {

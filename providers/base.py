...
def _profile_user_agent() -> str:
    """Return a ``clio-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from clio_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"clio-cli/{_ver}"
    except Exception:
        return "clio-cli"
    ...
    def fetch_models(
        ...
        *,
        ...
        Resolution order for the endpoint URL:
          1. base_url + "/models", but only when the caller passed a URL
             different from this profile's default. That is an explicit
             custom/proxy endpoint and must control discovery too.
          2. self.models_url (explicit profile override — use when the models
        ...
        # clio-cli UA so the catalog endpoint is reachable.

...
# Python-driven tool session (e.g. Clio terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:Clioloop/Clioloop-agent.git"
REPO_URL_HTTPS="https://github.com/Clioloop/Clioloop-agent.git"
CLIO_HOME="${CLIO_HOME:-$HOME/.clio}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${CLIO_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$CLIO_INSTALL_DIR"
...
#   code at /usr/local/lib/clio-agent, command at /usr/local/bin/clio,
#   data still at /root/.clio (CLIO_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$CLIO_HOME/.no-bundled-skills so future"
            echo "                   'clio update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Clio.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.clio/clio-agent"
            echo "                   default (root, Linux): /usr/local/lib/clio-agent"
            echo "  --clio-home PATH  Data directory (default: ~/.clio, or \$CLIO_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Clio installs the code under"
            echo "  /usr/local/lib/clio-agent and links the command into"
            echo "  /usr/local/bin/clio (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$CLIO_HOME"
            echo "  (default /root/.clio).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$CLIO_HOME/clio-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs clio setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ∞ Clio Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Omni Loop Labs.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `clio update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Clio-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Clio Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install clio command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `clio` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $CLIO_HOME/clio-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $CLIO_HOME/clio-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/clio-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $CLIO_HOME/clio-agent — then preserve it)
#
# Always no-op when the user set --dir or $CLIO_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in CLIO_HOME.
    if is_termux; then
        INSTALL_DIR="$CLIO_HOME/clio-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$CLIO_HOME/clio-agent/.git" ]; then
            INSTALL_DIR="$CLIO_HOME/clio-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/clio-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/clio-agent"
        ...
        # /usr/local/bin/clio wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/clio"
        log_info "  Data:    $CLIO_HOME (unchanged)"
...
get_clio_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/clio" ]; then
        echo "$link_dir/clio"
    else
        echo "clio"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/Clioloop/Clioloop-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Clio owns its own uv at $CLIO_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (clio_cli/managed_uv.py) looks in the same
    # place, so install.sh and `clio update` stay in sync.
    local _managed_uv="$CLIO_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $CLIO_HOME/bin ..."
    mkdir -p "$CLIO_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/clio-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/clio-uv-installer.$$.sh")"
    ...
    # directly into $CLIO_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$CLIO_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Clio-
...
check_node() {
    ...
    if [ -x "$CLIO_HOME/node/bin/node" ] && node_satisfies_build "$("$CLIO_HOME/node/bin/node" --version)"; then
        export PATH="$CLIO_HOME/node/bin:$PATH"
        log_success "Node.js $("$CLIO_HOME/node/bin/node" --version) found (Clio-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Clio-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.clio/node/..."
    ...
    # Place into ~/.clio/node/ and symlink binaries into the same bin dir
    # the clio command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$CLIO_HOME/node"
    mkdir -p "$CLIO_HOME"
    mv "$extracted_dir" "$CLIO_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$CLIO_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$CLIO_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$CLIO_HOME/node/bin/npx"  "$node_link_dir/npx"

    export PATH="$CLIO_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$CLIO_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.clio/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Clio install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Clio Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="clio-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Clio behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up clio command..."

    if [ "$USE_VENV" = true ]; then
        CLIO_BIN="$INSTALL_DIR/venv/bin/clio"
    else
        CLIO_BIN="$(which clio 2>/dev/null || echo "")"
        if [ -z "$CLIO_BIN" ]; then
            log_warn "clio not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$CLIO_BIN" ]; then
        log_warn "clio entry point not found at $CLIO_BIN"
    ...
    # Create a user-facing shim for the clio command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $CLIO_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$CLIO_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/clio"
    cat > "$command_link_dir/clio" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CLIO_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/clio"
    log_success "Installed clio launcher → $command_link_display_dir/clio"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "clio command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v clio' \
                >/dev/null 2>&1; then
        ...
        log_info "clio not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Clio Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Clio Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Clio Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so clio works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.clio directory structure (config at top level, code in subdir)
    mkdir -p "$CLIO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.clio/.env (top level, easy to find)
    if [ ! -f "$CLIO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CLIO_HOME/.env"
            log_success "Created ~/.clio/.env from template"
        else
            touch "$CLIO_HOME/.env"
            log_success "Created ~/.clio/.env"
        fi
    else
        log_info "~/.clio/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$CLIO_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.clio/config.yaml (top level, easy to find)
    if [ ! -f "$CLIO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CLIO_HOME/config.yaml"
            log_success "Created ~/.clio/config.yaml from template"
        fi
    else
        log_info "~/.clio/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$CLIO_HOME/SOUL.md" ]; then
        cat > "$CLIO_HOME/SOUL.md" << 'SOUL_EOF'
# Clio Agent Persona
...
-->
SOUL_EOF
        log_success "Created ~/.clio/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.clio/"
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        # skills_sync.py and `clio update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'clio update'." \
            > "$CLIO_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $CLIO_HOME/.no-bundled-skills"
        log_info "  Future 'clio update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.clio/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.clio/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CLIO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$CLIO_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.clio/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Clio Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Clio will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (clio --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `clio update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'clio setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m clio_cli.main setup < /dev/tty
    else
        python -m clio_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CLIO_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Clio to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$CLIO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'clio whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                CLIO_CMD="$(get_clio_command_path)"
                $CLIO_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'clio whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'clio gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $CLIO_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $CLIO_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: clio gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: clio gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $CLIO_CMD gateway > "$CLIO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.clio/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: clio gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: clio gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $CLIO_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $CLIO_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $CLIO_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}clio${NC}              Start chatting"
    echo -e "   ${GREEN}clio setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}clio config${NC}       View/edit configuration"
    echo -e "   ${GREEN}clio config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}clio gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}clio update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'clio' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'clio' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'clio' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$CLIO_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$CLIO_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$CLIO_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$CLIO_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$CLIO_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Clio for pip install"
    ...
    CLIO_CMD="$(command -v clio 2>/dev/null || echo "")"
    if [ -n "$CLIO_CMD" ]; then
        log_info "Running clio setup..."
        "$CLIO_CMD" setup
    else
        log_warn "clio command not found on PATH"
        log_info "Try: python -m clio_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Clio-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Clio" ]; then
            app="$desktop_dir/release/linux-unpacked/Clio"
        elif [ -x "$desktop_dir/release/linux-unpacked/clio" ]; then
            app="$desktop_dir/release/linux-unpacked/clio"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Clio.app" \
            "$desktop_dir/release/mac/Clio.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Clio is damaged and can't be opened").
    ...
    # checkout stays clean for the next `clio update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Clio-managed Node
            # provisioned during prerequisites/node-deps (at $CLIO_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$CLIO_HOME/.install_method"
            ;;
        *)

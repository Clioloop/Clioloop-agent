#!/usr/bin/env python3
"""Live test harness for Clio Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any clio imports.
ORIGINAL_HOME = os.environ.get("CLIO_HOME")
ORIGINAL_AUTH = Path.home() / ".clio" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.clio/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.clio/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="clio_ts_live_"))
    clio_home = home_dir / ".clio"
    clio_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, clio_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".clio" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, clio_home / ".env")
        ...
        load_clio_dotenv(clio_home=str(Path.home() / ".clio"))
    ...
    (clio_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return clio_home
...
def reset_module_state():
    """Drop cached modules so the new CLIO_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "clio_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original CLIO_HOME
    if ORIGINAL_HOME is not None:
        os.environ["CLIO_HOME"] = ORIGINAL_HOME
    else:

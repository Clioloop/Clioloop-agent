# ============================================================================
# Clio Agent Installer for Windows
# ============================================================================
...
param(
    ...
    # When supplied, the HTTPS clone URL is rewritten to include the token:
    #   https://<token>@github.com/Clioloop/Clioloop-agent.git
    [string]$GitHubToken = "",
    [string]$ClioHome = $(if ($env:CLIO_HOME) { $env:CLIO_HOME } else { "$env:LOCALAPPDATA\clio" }),
    [string]$InstallDir = $(if ($env:CLIO_HOME) { "$env:CLIO_HOME\clio-agent" } else { "$env:LOCALAPPDATA\clio\clio-agent" }),
    ...
    # builds apps/desktop into a launchable Clio.exe.
    #
    # Why opt-in:
    #   * Clio-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Clio.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Clio.exe
    ...
    #     `clio desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:Clioloop/Clioloop-agent.git"
$RepoUrlHttps = "https://github.com/Clioloop/Clioloop-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Clio Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Omni Loop Labs.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ClioHome)) {
        New-Item -ItemType Directory -Force -Path $ClioHome | Out-Null
    }
    $envFile = Join-Path $ClioHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ClioHome "node"
...
function Install-Uv {
    # Clio owns its own uv at $ClioHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (clio_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `clio update` stay in sync.
    $managedUv = Join-Path $ClioHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ClioHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ClioHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ClioHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ClioHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and CLIO_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Clio uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\clio\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Clio needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Clio copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\clio\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``CLIO_GIT_BASH_PATH`` (User scope) so Clio can find it in a fresh
    ...
    # Download PortableGit into $ClioHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ClioHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Clio features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Clio needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``CLIO_GIT_BASH_PATH`` (User env scope) so Clio can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ClioHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ClioHome\git\usr\bin\bash.exe
    $candidates += "$ClioHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ClioHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("CLIO_GIT_BASH_PATH", $candidate, "User")
            $env:CLIO_GIT_BASH_PATH = $candidate
            Write-Info "Set CLIO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Clio may not find Git Bash."
    Write-Info "If needed, set CLIO_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    # Prefer a Clio-managed Node from a previous run over a too-old system one.
    $managedNode = "$ClioHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ClioHome\node;$env:Path"
        Write-Success "Node.js $version found (Clio-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Clio-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ClioHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ClioHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\clio-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ClioHome\node") { Remove-Item -Recurse -Force "$ClioHome\node" }
                Move-Item $extractedDir.FullName "$ClioHome\node"
                ...
                $version = & "$ClioHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ClioHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\clio-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    $stashName = "clio-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Clio behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running clio processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $httpsUrl = $RepoUrlHttps
                if ($GitHubToken) {
                    $httpsUrl = $httpsUrl -replace "^https://", "https://${GitHubToken}@"
                    Write-Info "Using authenticated HTTPS clone (token provided)"
                }
                git -c windows.appendAtomically=false clone --depth 1 --branch $Branch $httpsUrl $InstallDir
                if ($LASTEXITCODE -eq 0) { $cloneSuccess = $true }
            } catch { }
        }
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/Clioloop/Clioloop-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/Clioloop/Clioloop-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/Clioloop/Clioloop-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\clio-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\clio-agent-extract"

                $webRequestParams = @{ Uri = $zipUrl; OutFile = $zipPath; UseBasicParsing = $true }
                if ($GitHubToken) {
                    $webRequestParams.Headers = @{ Authorization = "token $GitHubToken" }
                }
    ...
    # next `clio update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `clio.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install clio-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and clio.exe broken with
    ...
    # here, not 30 seconds later when the user runs `clio`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `clio dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `clio dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `clio dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up clio command..."
    
    if ($NoVenv) {
        $clioBin = "$InstallDir"
    } else {
        $clioBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so clio is globally available
    # On Windows, the clio.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$clioBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$clioBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $clioBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set CLIO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\clio instead
    # of the Unix default ~/.clio
    $currentClioHome = [Environment]::GetEnvironmentVariable("CLIO_HOME", "User")
    if (-not $currentClioHome -or $currentClioHome -ne $ClioHome) {
        [Environment]::SetEnvironmentVariable("CLIO_HOME", $ClioHome, "User")
        Write-Success "Set CLIO_HOME=$ClioHome"
    }
    $env:CLIO_HOME = $ClioHome
    
    # Update current session
    $env:Path = "$clioBin;$env:Path"
    
    Write-Success "clio command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.clio-bootstrap-complete which tells the Clio
    ...
    $markerPath = Join-Path $InstallDir ".clio-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$ClioHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClioHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClioHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClioHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClioHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClioHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClioHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClioHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClioHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ClioHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.clio/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.clio/.env"
        }
    } else {
        Write-Info "~/.clio/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ClioHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.clio/config.yaml from template"
        }
    } else {
        Write-Info "~/.clio/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Clio's prompt-injection scanner
...
Edit this to customize how Clio communicates with you.
...
"@
        ...
        Write-Success "Created ~/.clio/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.clio/"
    
    # Seed bundled skills into ~/.clio/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.clio/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.clio/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ClioHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.clio/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Clio-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'clio setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\clio-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\clio-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\clio-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Clio` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Clio.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_clio_desktop command
    # resolves apps/desktop/release/win-unpacked/Clio.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Clio-managed
    ...
    # The --dir mode produces an unpacked Clio.exe in
    ...
    # are NOT trying to work around). The Clio icon + product name are
    # stamped onto Clio.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\clio-desktop-build-$(Get-Random).log"
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Clio` rename: app-builder's
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Clio.exe",
        "$desktopDir\release\win-arm64-unpacked\Clio.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Clio.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Clio icon + identity are stamped onto Clio.exe by the
    ...
    #    Clio.exe. We deliberately do NOT point them at `clio desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Clio.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Clio.lnk')
        )
        ...
        # with the Clio icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.clio/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'clio setup'."
        return
    }
    ...
    # Run clio setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m clio_cli.main setup
    } else {
        python -m clio_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $clioCmd = "$InstallDir\venv\Scripts\clio.exe"
    if (-not (Test-Path $clioCmd)) {
        $clioCmd = "clio"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ClioHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'clio whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: clio gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ClioHome\logs\gateway.log"
            Start-Process -FilePath $clioCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ClioHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: clio gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: clio gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ClioHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClioHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClioHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClioHome\clio-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   clio              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   clio setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   clio config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   clio config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   clio gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   clio update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Clio repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Clio-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Clio to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Clioloop/Clioloop-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow

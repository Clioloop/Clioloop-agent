...
VERSION_FILE = REPO_ROOT / "clio_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    # Omni Loop Labs (multiple emails)
    "clioloop@gmail.com": "clioloop",
    ...
    "annguyenClio@users.noreply.github.com": "annguyenClio",
    ...
    "annguyenClio@users.noreply.github.com": "annguyenClio",
    "285874597+annguyenClio@users.noreply.github.com": "annguyenClio",
    ...
    "127238744+clioloop@users.noreply.github.com": "clioloop",
    ...
    "274096618+clio-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "mind-dragon@omniloop.ai": "Mind-Dragon",
    ...
    "clio@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "Omni Loop Labs@noreply.github.com": "clioloop",
    ...
    "codex@openai.invalid": "clioloop",
    "screenmachine@gmail.com": "clioloop",
    ...
    "Omni Loop Labs@clio-agent": "clioloop",
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (CLIO_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (CLIO_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: clio-eval community link)
    ...
    "clioagent26@gmail.com": "clioagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+clioagent26@users.noreply.github.com": "clioagent26",
    ...
    "vigo@clio": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "ethie@omniloop": "ethernet8023",  # PR #29342 (TUI clipboard copy on linux/wayland)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Clio runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level CLIO_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Clio
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"clio-agent[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Clioloop/Clioloop-agent",
    ...
    lines.append(f"# Clio Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Clio Agent. See below for everything included in this initial release.")
        lines.append("")

    # Group commits by category
    categories = defaultdict(list)
    all_authors = set()
    clioloop_aliases = {"@clioloop"}

    for commit in commits:
        categories[commit["category"]].append(commit)
        author = commit["github_author"]
        if author not in clioloop_aliases:
            all_authors.add(author)
        for coauthor in commit.get("coauthors", []):
            if coauthor not in clioloop_aliases:
...
def main():
    parser = argparse.ArgumentParser(description="Clio Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Clio Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Clio Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Clio Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Clio Agent v{new_version} ({calver_date})' "

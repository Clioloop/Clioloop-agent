...
# Bootstrap Open WebUI against Clio Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.clio/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-clio.sh
...
#   CLIO_API_PORT=8642
#   CLIO_API_HOST=127.0.0.1
#   CLIO_API_MODEL_NAME='Clio Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Clio Agent WebUI}"
...
CLIO_ENV_FILE="${CLIO_ENV_FILE:-$HOME/.clio/.env}"
CLIO_API_PORT="${CLIO_API_PORT:-8642}"
CLIO_API_HOST="${CLIO_API_HOST:-127.0.0.1}"
CLIO_API_CONNECT_HOST="${CLIO_API_CONNECT_HOST:-127.0.0.1}"
CLIO_API_MODEL_NAME="${CLIO_API_MODEL_NAME:-Clio Agent}"
CLIO_API_BASE_URL="http://${CLIO_API_CONNECT_HOST}:${CLIO_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-clio.sh"
LOG_DIR="$HOME/.clio/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$CLIO_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$CLIO_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.clio.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.clio</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.clio"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.clio"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-clio.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Clio Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-clio.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.clio/logs/openwebui.log
StandardError=append:%h/.clio/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-clio.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$CLIO_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Clio API server is configured...'
  upsert_env API_SERVER_ENABLED true "$CLIO_ENV_FILE"
  upsert_env API_SERVER_HOST "$CLIO_API_HOST" "$CLIO_ENV_FILE"
  upsert_env API_SERVER_PORT "$CLIO_API_PORT" "$CLIO_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$CLIO_API_MODEL_NAME" "$CLIO_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$CLIO_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Clio gateway so API server settings take effect...'
  clio gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${CLIO_API_CONNECT_HOST}:${CLIO_API_PORT}/health" >/dev/null; then
    log 'Clio API server did not answer on the first check. Trying to start gateway in the background...'
    nohup clio gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${CLIO_API_CONNECT_HOST}:${CLIO_API_PORT}/health" >/dev/null
  ...
  log "Clio API endpoint: ${CLIO_API_BASE_URL}"

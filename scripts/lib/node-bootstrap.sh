...
#   2. ~/.clio/node/ from a prior Clio-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.clio/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$CLIO_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   CLIO_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   CLIO_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   CLIO_HOME               (default: $HOME/.clio)
# ============================================================================

CLIO_NODE_MIN_VERSION="${CLIO_NODE_MIN_VERSION:-20}"
CLIO_NODE_TARGET_MAJOR="${CLIO_NODE_TARGET_MAJOR:-22}"
CLIO_HOME="${CLIO_HOME:-$HOME/.clio}"
CLIO_NODE_AVAILABLE=false
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $CLIO_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$CLIO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$CLIO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $CLIO_NODE_TARGET_MAJOR..."
    proto install node "$CLIO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $CLIO_NODE_TARGET_MAJOR..."
    nvm install "$CLIO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$CLIO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${CLIO_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${CLIO_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${CLIO_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${CLIO_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${CLIO_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $CLIO_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $CLIO_HOME/node/..."
    ...
    mkdir -p "$_link_dir"
    ln -sf "$CLIO_HOME/node/bin/node" "$_link_dir/node"
    ln -sf "$CLIO_HOME/node/bin/npm"  "$_link_dir/npm"
...
ensure_node() {
    ...
    if [ -x "$CLIO_HOME/node/bin/node" ]; then
        export PATH="$CLIO_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Clio-managed)"
            CLIO_NODE_AVAILABLE=true
            return 0
        fi
    fi
    ...
    _nb_try_fnm   && { CLIO_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { CLIO_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { CLIO_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { CLIO_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { CLIO_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { CLIO_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $CLIO_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}

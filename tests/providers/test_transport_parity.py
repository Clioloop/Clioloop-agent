...
class TestManagedParity:
    """managed provider: product tags, reasoning, omit when disabled."""

    def test_tags(self, transport):
        from agent.request_tags import request_tags
        kw = transport.build_kwargs(
            model="clio-3-llama-3.1-405b",
            messages=_simple_messages(),
            tools=None,
    ...
    def test_reasoning_omitted_when_disabled(self, transport):
        """managed provider special case: reasoning omitted entirely when disabled."""
        kw = transport.build_kwargs(
            model="clio-3-llama-3.1-405b",
            messages=_simple_messages(),
            tools=None,

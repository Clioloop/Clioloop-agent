...
class TestRegistry:
    ...
    def test_alias_lookup(self):
        ...
        assert get_provider_profile("managed-provider") is None
...
class TestOpenRouterProfile:
    ...
    def test_reasoning_disabled_still_passes(self):
        """OpenRouter passes disabled reasoning through (unlike managed provider)."""
        p = get_provider_profile("openrouter")
        eb, _ = p.build_api_kwargs_extras(
...
class TestManagedProfile:
    def test_tags(self):
        ...
        p = get_provider_profile("managed")
        body = p.build_extra_body()
        assert body["tags"] == request_tags()

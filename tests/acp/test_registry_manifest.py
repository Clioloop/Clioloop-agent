"""Tests for ACP Registry metadata shipped with Clio."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "clio-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Clio Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/Clioloop/Clioloop-agent"
    assert data["website"].startswith("https://github.com/Clioloop/Clioloop-agent/")
    assert data["authors"] == ["Omni Loop Labs"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"clioloop-agent[acp]=={data['version']}"
    assert uvx["args"] == ["clio-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"clioloop-agent[acp]=={_pyproject_version()}"
    )

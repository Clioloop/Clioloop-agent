...
def test_show_session_status_prints_gateway_style_summary():
    ...
    assert "Clio CLI Status" in printed
    ...
    # model picker); the managed subscription renders as the portal subscription.
...
def test_show_session_status_brands_managed_provider_as_portal():
    """The managed subscription shows as the portal subscription, not the slug."""
    cli_obj = _make_cli()
    cli_obj.provider = "managed"
    cli_obj.model = "openai/gpt-oss-120b:free"

    with patch("cli.display_clio_home", return_value="~/.clio"):
        cli_obj._show_session_status()

    printed = "\n".join(str(call.args[0]) for call in cli_obj.console.print.call_args_list)
    assert "Model: openai/gpt-oss-120b:free (Omni Loop Portal Subscription)" in printed
    assert "(managed)" not in printed


def test_profile_command_reports_custom_root_profile(monkeypatch, tmp_path, capsys):
    """Profile detection works for custom-root deployments (not under ~/.clio)."""
    cli_obj = _make_cli()

...
class TestCLIPersonalityNone:
    ...
    def _make_cli(self, personalities=None):
        ...
        cli.system_prompt = "You are clio-spinner~"
        cli.agent = MagicMock()
...
class TestGatewayPersonalityNone:
    ...
    def _make_runner(self, personalities=None):
        ...
        runner._ephemeral_system_prompt = "You are clio-spinner~"
        runner.config = {
            "agent": {
    ...
    async def test_none_clears_ephemeral_prompt(self, tmp_path):
        ...
        config_data = {"agent": {"personalities": {"helpful": "You are helpful."}, "system_prompt": "clio-spinner"}}
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._clio_home", tmp_path), \
             patch("clio_constants.display_clio_home", return_value="~/.clio/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.clio/profiles/coder/config.yaml`"
...
class TestPersonalityDictFormat:
    ...
    def test_resolve_prompt_dict_no_tone_no_style(self):
        from cli import ClioCLI
        result = ClioCLI._resolve_personality_prompt({
    ...
    def test_resolve_prompt_string(self):
        from cli import ClioCLI
        result = ClioCLI._resolve_personality_prompt("You are helpful.")
        assert result == "You are helpful."

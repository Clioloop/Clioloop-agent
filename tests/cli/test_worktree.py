...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("clio/clio-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned clio/* and pr-* branches."""

    def test_prunes_orphaned_clio_branch(self, git_repo):
        """clio/clio-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "clio/clio-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "clio/clio-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "clio/clio-deadbeef" in result.stdout
        ...
        assert "clio/clio-deadbeef" in orphaned
        ...
        assert "clio/clio-deadbeef" not in result.stdout

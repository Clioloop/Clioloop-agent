...
from clio_cli import main as clio_main
...
def test_clio_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in ClioCLI.__init__")

    monkeypatch.setattr("clio_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("clio_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.ClioCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("clio_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.ClioCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "clio_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.ClioCLI(compact=True, max_turns=1)
...
@pytest.mark.skip(reason="Managed managed provider Tool Gateway / managed provider removed in Clio rebrand (phase 12)")
def test_model_flow_managed_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setattr(
        "clio_cli.portal_subscription.managed_tools_enabled",
        lambda *args, **kwargs: True,
    )
    config = {
        "model": {"provider": "managed", "default": "claude-opus-4-6"},
        "tts": {"provider": "elevenlabs"},
        "browser": {"cloud_provider": "browser-use"},
    }

    monkeypatch.setattr(
        "clio_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "managed-token"},
    )
    monkeypatch.setattr(
        "clio_cli.auth.resolve_managed_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "clio_cli.auth.fetch_managed_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("clio_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("clio_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("clio_cli.auth._update_config_for_provider", lambda provider, url: None)

    clio_main._model_flow_managed(config, current_model="claude-opus-4-6")
...
@pytest.mark.skip(reason="Managed managed provider Tool Gateway / managed provider removed in Clio rebrand (phase 12)")
def test_model_flow_managed_offers_tool_gateway_prompt_when_unconfigured(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: ManagedProviderAccountInfo(
            logged_in=True,
            source="account_api",
            fresh=True,
    ...
    monkeypatch.setattr("clio_cli.setup.prompt_checklist", _fake_checklist, raising=False)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.ClioCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.ClioCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "clio_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("clio_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("clio_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("clio_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("clio_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(clio_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    clio_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "clio_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("clio_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("clio_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("clio_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("clio_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "clio_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("clio_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    clio_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("clio_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "clio_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("clio_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    clio_main._model_flow_custom({"model": {"provider": "custom"}})
...
@pytest.mark.skip(reason="Managed managed provider Tool Gateway / managed provider removed in Clio rebrand (phase 12)")
def test_cmd_model_forwards_managed_login_tls_options(monkeypatch):
    monkeypatch.setattr(clio_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "clio_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "managed"}},
    )
    monkeypatch.setattr("clio_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("clio_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("clio_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("clio_cli.auth.resolve_provider", lambda requested, **kwargs: "managed")
    monkeypatch.setattr("clio_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(clio_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("clio_cli.auth._login_managed", _fake_login)

    clio_main.cmd_model(
        SimpleNamespace(
            portal_url="https://",
            inference_url="https://inference./v1",
            client_id="clio-local",
...
def test_auto_provider_name_localhost():
    from clio_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from clio_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "clio_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("clio_cli.config.save_config", _save)

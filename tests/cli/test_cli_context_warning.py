...
def _isolate(tmp_path, monkeypatch):
    """Isolate CLIO_HOME so tests don't touch real config."""
    home = tmp_path / ".clio"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ClioCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ClioCLI.__new__(ClioCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["clio-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Clio' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Clio' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Clio' minimum context length."""

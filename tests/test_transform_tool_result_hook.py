...
def _run_handle_function_call(
    ...
    *,
    ...
    if invoke_hook is not _UNSET:
        ...
        monkeypatch.setattr("clio_cli.plugins.invoke_hook", invoke_hook)
...
def test_result_unchanged_when_no_hook_registered(monkeypatch):
    ...
    monkeypatch.setenv("CLIO_HOME", "/tmp/clio_no_plugins")
...
def test_transform_tool_result_integration_with_real_plugin(monkeypatch, tmp_path):
    """End-to-end: load a real plugin from CLIO_HOME and verify it rewrites results."""
    import yaml

    clio_home = Path(os.environ["CLIO_HOME"])

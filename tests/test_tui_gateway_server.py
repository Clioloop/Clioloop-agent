...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["CLIO_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("CLIO_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("CLIO_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("CLIO_VOICE", "1")
    monkeypatch.delenv("CLIO_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("CLIO_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("CLIO_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "clio_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("CLIO_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("CLIO_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "clio_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    # whose tools live in clio-cli's universe (see toolsets.py).
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("CLIO_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "clio_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("CLIO_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("CLIO_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("CLIO_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("CLIO_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "clio_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown CLIO_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("CLIO_MODEL", raising=False)
    monkeypatch.setenv("CLIO_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("CLIO_MODEL", raising=False)
    monkeypatch.delenv("CLIO_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " managed/clio-test "}}
    )

    assert server._resolve_model() == "managed/clio-test"


def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("CLIO_MODEL", "managed/clio-test")
    monkeypatch.setenv("CLIO_TUI_PROVIDER", "managed")
    monkeypatch.delenv("CLIO_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("managed/clio-test", "managed")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("CLIO_MODEL", "managed/clio-test")
    monkeypatch.delenv("CLIO_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("CLIO_INFERENCE_PROVIDER", "managed")
    monkeypatch.setattr(
        "clio_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("managed/clio-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("CLIO_MODEL", "sonnet")
    monkeypatch.delenv("CLIO_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("CLIO_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "clio_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "clio_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "clio_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("CLIO_YOLO_MODE", "false")
    ...
    assert os.environ.get("CLIO_YOLO_MODE") == "1"
...
def test_config_set_statusbar_survives_non_dict_display(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_clio_home", tmp_path)
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("CLIO_EXEC_ASK", raising=False)
    monkeypatch.delenv("CLIO_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("CLIO_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["CLIO_GATEWAY_SESSION"] == "1"
    assert server.os.environ["CLIO_EXEC_ASK"] == "1"
    assert server.os.environ["CLIO_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("clio_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("clio_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("clio_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    """After an explicit provider switch, CLIO_INFERENCE_PROVIDER must
    ...
    server._sessions["sid"] = _session(agent=_Agent())
    monkeypatch.setenv("CLIO_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "clio_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    assert os.environ["CLIO_INFERENCE_PROVIDER"] == "anthropic"


def test_config_set_model_syncs_tui_provider_unconditionally(monkeypatch):
    """Regression for #16857: /model must set CLIO_TUI_PROVIDER even when
    ...
    # Both env vars must reflect the user's choice. CLIO_TUI_PROVIDER is
    # the canonical explicit-this-process carrier consumed by
    # _resolve_startup_runtime() on /new.
    assert os.environ["CLIO_TUI_PROVIDER"] == "custom:xuanji"
    assert os.environ["CLIO_INFERENCE_PROVIDER"] == "custom:xuanji"
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    agent = Agent()
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setenv("CLIO_TUI_PROVIDER", "openai-codex")
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("clio_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert resp["result"]["value"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["CLIO_TUI_PROVIDER"] == "anthropic"
        assert os.environ["CLIO_MODEL"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["CLIO_INFERENCE_MODEL"] == "anthropic/claude-sonnet-4.6"
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors ClioCLI._manual_compress's
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("clio_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "clio_cli.commands.SlashCommandCompleter",
...
def test_slash_exec_handles_fusion_gate_and_config(monkeypatch):
        ...
        gate = server.handle_request(
            {
                "id": "1",
                "method": "slash.exec",
                "params": {"session_id": "sid", "command": "fusion gate"},
            }
        )
        assert gate["result"]["output"] == "OK"

        configured = server.handle_request(
            {
                "id": "2",
                "method": "slash.exec",
                "params": {
                    "session_id": "sid",
                    "command": "fusion advisors=a,b reviewers=c",
                },
            }
        )
        assert "Fusion mode is on" in configured["result"]["output"]
        cfg = session["fusion_config"]
        assert cfg.advisors == ["a", "b"]
        assert cfg.reviewers == ["c"]
        assert cfg.enabled is True
        assert session["agent"]._fusion_config is cfg
    finally:
        server._sessions.pop("sid", None)


def test_prompt_submit_routes_active_fusion(monkeypatch):
    from agent import fusion_engine
    ...
    class _Agent:
        ...
        def run_conversation(self, *_a, **_kw):
            raise AssertionError("normal run_conversation should not be used")
    ...
    def fake_run_fusion_turn(agent, message, **kwargs):
        calls["agent"] = agent
    ...
    session = _session(agent=_Agent(), fusion_config=cfg)
    server._sessions["sid"] = session
    emitted = []
    monkeypatch.setattr(server, "_emit", lambda *args: emitted.append(args))
    ...
    monkeypatch.setattr(server.threading, "Thread", _ImmediateThread)
    monkeypatch.setattr(fusion_engine, "run_fusion_turn", fake_run_fusion_turn)
        ...
        resp = server.handle_request(
            {
                "id": "1",
                "method": "prompt.submit",
                "params": {"session_id": "sid", "text": "do it"},
            }
        )
        assert resp["result"]["status"] == "streaming"
        assert calls["agent"] is session["agent"]
        assert calls["message"] == "do it"
        assert calls["kwargs"]["config"] is cfg
        assert any(
            event
            == (
                "status.update",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("clio_state")
    ...
    monkeypatch.setitem(sys.modules, "clio_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_clio_home
    # fixture pins CLIO_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `clio model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `clio model` and the classic CLI /model picker.
    ...
    for providers like managed provider whose /models endpoint returns image/video
    ...
    with patch(
        "clio_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("clio_cli.models.provider_model_ids") as live_fetch:
            resp = server._methods["model.options"](99, {"session_id": ""})

    assert "result" in resp, resp
    providers = resp["result"]["providers"]
    managed = next((p for p in providers if p.get("slug") == "managed"), None)
    assert managed is not None
    assert managed["models"] == [
        "moonshotai/kimi-k2.5",
        "anthropic/claude-opus-4.7",
    ]
    assert managed["total_models"] == 30
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"clio_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "clio_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
                "clio_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "clio_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_clio_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.clio/.env
    ...
    with patch.dict(sys.modules, {"clio_cli.config": fake}):
...
def test_session_save_writes_under_clio_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Clio profile
    ...
    Regression: the gateway handler wrote ``clio_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Clio.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("clio_conversation_*.json"))
    ...
    assert payload["model"] == "clio-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Clio."

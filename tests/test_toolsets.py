...
class TestToolsetConsistency:
    ...
    def test_clio_platforms_share_core_tools(self):
        """All clio-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on clio-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["clio-cli", "clio-telegram", "clio-discord", "clio-whatsapp", "clio-slack", "clio-signal", "clio-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_clio_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("clio-whatsapp")

    def test_clio_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("clio-api-server")

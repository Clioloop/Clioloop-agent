"""Tests for clio_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Clio entry point
(clio, clio-agent, clio-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Clio entry point imports clio_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported clio_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("clio_bootstrap", None)
    import clio_bootstrap  # noqa: WPS433
    return clio_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Clio CLI
...
class TestEntryPointsImportBootstrap:
    """Every Clio entry point must import clio_bootstrap as its
    ...
    # Entry points that invoke Clio as a process.  Each one must
    # import clio_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "clio_cli/main.py",   # clio CLI (console_script)
        "run_agent.py",          # clio-agent (console_script)
        "acp_adapter/entry.py",  # clio-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import clio_bootstrap' and that
        ...
        half-finished ``clio update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``clio_bootstrap`` as a top-level module) leaves clio
        ...
        """
        # Resolve relative to the clio-agent repo root.  Tests live
        # at tests/test_clio_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # clio start even when clio_bootstrap hasn't been
        ...
        assert first_import_name == "clio_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'clio_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import clio_bootstrap' line to be the first import."
        )

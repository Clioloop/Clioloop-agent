"""Tests for the secret-source tracking in ``clio_cli.env_loader``.

These cover the small public surface that lets `clio model` / `clio setup`
...
from clio_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_clio_dotenv()`` is called at module-import time from several
    hot modules (cli.py, clio_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per CLIO_HOME per process.
    """
    ...
    # cli.py, clio_cli/main.py, run_agent.py, trajectory_compressor.py,

...
Override the image with ``CLIO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("CLIO_TEST_IMAGE", "clio-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``CLIO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("CLIO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged clio user
# ---------------------------------------------------------------------------
#
# Background: every Clio runtime path inside the container drops to UID
# 10000 (the ``clio`` user) via ``s6-setuidgid clio``. ``docker exec``
...
# fails (PermissionError swallowed) for clio, so a test that ran as root
...
# helpers below run every probe under ``-u clio`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: clio).

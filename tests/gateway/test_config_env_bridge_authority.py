...
def _run_gateway_import(clio_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            "CLIO_MAX_ITERATIONS",
            "CLIO_AGENT_TIMEOUT",
            "CLIO_AGENT_TIMEOUT_WARNING",
            "CLIO_GATEWAY_BUSY_INPUT_MODE",
            "CLIO_GATEWAY_BUSY_TEXT_MODE",
            "CLIO_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
...
@pytest.fixture
def clio_home(tmp_path: Path) -> Path:
    home = tmp_path / ".clio"
    home.mkdir()
    return home
...
def test_config_gateway_timeout_wins_over_stale_env(clio_home: Path) -> None:
    ...
    _write_config(clio_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
    })
    _write_env(clio_home, {
        "CLIO_AGENT_TIMEOUT": "60",
        "CLIO_AGENT_TIMEOUT_WARNING": "30",
    })

    env = _run_gateway_import(clio_home, initial_env={})

    assert env.get("CLIO_AGENT_TIMEOUT") == "1800"
    assert env.get("CLIO_AGENT_TIMEOUT_WARNING") == "900"


def test_config_display_busy_input_mode_wins_over_stale_env(clio_home: Path) -> None:
    _write_config(clio_home, display_cfg={"busy_input_mode": "interrupt"})
    _write_env(clio_home, {"CLIO_GATEWAY_BUSY_INPUT_MODE": "queue"})

    env = _run_gateway_import(clio_home, initial_env={})

    assert env.get("CLIO_GATEWAY_BUSY_INPUT_MODE") == "interrupt"


def test_config_display_busy_text_mode_wins_over_stale_env(clio_home: Path) -> None:
    _write_config(clio_home, display_cfg={"busy_text_mode": "queue"})
    _write_env(clio_home, {"CLIO_GATEWAY_BUSY_TEXT_MODE": "interrupt"})

    env = _run_gateway_import(clio_home, initial_env={})

    assert env.get("CLIO_GATEWAY_BUSY_TEXT_MODE") == "queue"


def test_config_timezone_wins_over_stale_env(clio_home: Path) -> None:
    _write_config(clio_home, timezone="America/Los_Angeles")
    _write_env(clio_home, {"CLIO_TIMEZONE": "UTC"})

    env = _run_gateway_import(clio_home, initial_env={})

    assert env.get("CLIO_TIMEZONE") == "America/Los_Angeles"


def test_env_value_survives_when_config_omits_key(clio_home: Path) -> None:
    ...
    """
    _write_config(clio_home, agent_cfg={})  # no max_turns
    _write_env(clio_home, {"CLIO_MAX_ITERATIONS": "123"})

    env = _run_gateway_import(clio_home, initial_env={})

    assert env.get("CLIO_MAX_ITERATIONS") == "123"

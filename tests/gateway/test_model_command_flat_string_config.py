...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "clio_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_clio_home() / config.yaml`` — point it here.
    monkeypatch.setattr("clio_constants.get_clio_home", lambda: clio_home)
    monkeypatch.setattr("clio_cli.config.get_clio_home", lambda: clio_home)
    return cfg_path

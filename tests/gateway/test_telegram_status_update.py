...
@pytest.mark.asyncio
async def test_reset_status_message_forces_fresh_send(adapter):
    """After reset, the next call for the same key sends fresh, not edits.

    This is what gives each new fusion turn its own progress bubble instead of
    re-editing the previous turn's (scrolled-up) bubble.
    """
    adapter.send.side_effect = [
        SendResult(success=True, message_id="100"),
        SendResult(success=True, message_id="200"),
    ...
    await adapter.send_or_update_status("chat-1", "fusion:s1", "step 1")
    assert adapter._status_message_ids[("chat-1", "fusion:s1")] == "100"

    adapter.reset_status_message("chat-1", "fusion:s1")
    assert ("chat-1", "fusion:s1") not in adapter._status_message_ids

    result = await adapter.send_or_update_status("chat-1", "fusion:s1", "step 1 (new turn)")

    # Fresh send (no edit), and the new id is cached for the new turn.
    assert result.message_id == "200"
    assert adapter.send.await_count == 2
    adapter.edit_message.assert_not_awaited()
    assert adapter._status_message_ids[("chat-1", "fusion:s1")] == "200"


@pytest.mark.asyncio
async def test_reset_status_message_unknown_key_is_noop(adapter):
    """Resetting a key that was never cached must not raise."""
    adapter.reset_status_message("chat-1", "never-seen")  # no exception

...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.clio/media/'
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("CLIO_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("CLIO_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("CLIO_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("CLIO_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("CLIO_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("CLIO_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("CLIO_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("CLIO_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.clio/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.delenv("CLIO_MEDIA_DELIVERY_STRICT", raising=False)
        monkeypatch.delenv("CLIO_MEDIA_ALLOW_DIRS", raising=False)
    ...
    def test_denylist_blocks_clio_credentials(self, tmp_path, monkeypatch):
        """~/.clio/.env and ~/.clio/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        clio_dir = fake_home / ".clio"
        clio_dir.mkdir(parents=True)
        env_file = clio_dir / ".env"
        env_file.write_text("OPENAI_API_KEY=sk-...")
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._CLIO_HOME",
            clio_dir,
        )

        assert BasePlatformAdapter.validate_media_delivery_path(str(env_file)) is None

    def test_denylist_blocks_clio_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        clio_dir = fake_home / ".clio"
        clio_dir.mkdir(parents=True)
        config_file = clio_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_clio_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Clio root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".clio" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        clio_root = fake_home / ".clio"
        config_file = clio_root / "config.yaml"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._CLIO_ROOT",
            clio_root,
        )
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting CLIO_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``CLIO_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("CLIO_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_clio_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.clio/.env`` stays blocked under the $HOME exception — it is a
        ...
        monkeypatch.setattr("gateway.platforms.base._CLIO_HOME", clio_dir)
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"CLIO_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CLIO_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"CLIO_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "CLIO_HUMAN_DELAY_MODE": "natural",
            "CLIO_HUMAN_DELAY_MIN_MS": "oops",
            "CLIO_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "CLIO_HUMAN_DELAY_MODE": "custom",
            "CLIO_HUMAN_DELAY_MIN_MS": "100",
            "CLIO_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"CLIO_MEDIA_DELIVERY_STRICT": "1",
                                     "CLIO_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_MEDIA_DELIVERY_STRICT", "0")
...
class TestMediaDeliveryDedupe:
    """Regression coverage for duplicate attachment extraction."""

    @staticmethod
    def _extract_delivery_paths(response: str):
        media_files, cleaned = BasePlatformAdapter.extract_media(response)
        media_files = BasePlatformAdapter.filter_media_delivery_paths(media_files)
        _, cleaned = BasePlatformAdapter.extract_images(cleaned)
        local_files, _ = BasePlatformAdapter.extract_local_files(cleaned)
        local_files = BasePlatformAdapter.filter_local_delivery_paths(local_files)
        return BasePlatformAdapter.dedupe_media_and_local_delivery_paths(
            media_files, local_files
        )

    def test_media_tag_and_markdown_local_image_send_once(self, tmp_path, monkeypatch):
        image = tmp_path / "cat.png"
        image.write_bytes(b"\x89PNG")
        monkeypatch.setenv("CLIO_MEDIA_ALLOW_DIRS", str(tmp_path))

        media_files, local_files = self._extract_delivery_paths(
            f"![cat]({image})\nMEDIA:{image}"
        )

        assert media_files == [(str(image.resolve()), False)]
        assert local_files == []

    def test_duplicate_media_tags_send_once(self, tmp_path, monkeypatch):
        image = tmp_path / "cat.png"
        image.write_bytes(b"\x89PNG")
        monkeypatch.setenv("CLIO_MEDIA_ALLOW_DIRS", str(tmp_path))
    ...
    def test_distinct_images_remain_distinct(self, tmp_path, monkeypatch):
        ...
        assert media_files == [(str(first.resolve()), False)]
        assert local_files == [str(second.resolve())]

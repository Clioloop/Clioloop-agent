...
- Path resolution (absolute, relative to CLIO_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp CLIO_HOME."""
    clio_home = tmp_path / ".clio"
    clio_home.mkdir()
    (clio_home / "cron").mkdir()
    (clio_home / "cron" / "output").mkdir()
    (clio_home / "scripts").mkdir()
    monkeypatch.setenv("CLIO_HOME", str(clio_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "CLIO_DIR", clio_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", clio_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", clio_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", clio_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.clio/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("CLIO_SESSION_PLATFORM") is None
        assert os.environ.get("CLIO_SESSION_CHAT_ID") is None
        assert os.environ.get("CLIO_SESSION_CHAT_NAME") is None

...
binary download are exercised manually by `clio secrets bitwarden
...
def clio_home(tmp_path, monkeypatch):
    """Point Clio at an isolated home directory."""
    home = tmp_path / ".clio"
    home.mkdir()
    monkeypatch.setenv("CLIO_HOME", str(home))
    # Some modules cache get_clio_home; clear if needed.
    import clio_constants
    if hasattr(clio_constants, "_CLIO_HOME_CACHE"):
        clio_constants._CLIO_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(clio_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(clio_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(clio_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from clio_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under clio_home/cache/."""
    home = tmp_path / ".clio"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")

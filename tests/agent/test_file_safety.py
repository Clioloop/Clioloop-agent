...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_clio_env(self):
        """Clio' own .env inside CLIO_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: clio internal .env is in ~/.clio/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.clio/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.clio/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Clio cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        clio_home = tmp_path / ".clio"
        cache = clio_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._clio_home_path", return_value=clio_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Clio cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        clio_home = tmp_path / ".clio"
        hub = clio_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_clio_home(self, tmp_path):
        """The env basename guard does not depend on CLIO_HOME resolution."""
        clio_home = tmp_path / ".clio"
        clio_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        clio_home = tmp_path / ".clio"
        cache = clio_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")

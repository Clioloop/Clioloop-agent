...
def backup_env(monkeypatch, tmp_path):
    """Isolate CLIO_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".clio"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_clio_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd clio_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under CLIO_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level CLIO_DIR picks up the tmp HOME."""
    import clio_constants
    importlib.reload(clio_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.clio/cron/jobs.json

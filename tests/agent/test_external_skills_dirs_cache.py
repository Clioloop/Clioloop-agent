...
@pytest.fixture
def clio_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.clio/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".clio"
    home.mkdir()
...
def test_returns_configured_external_dir(clio_home_with_config):
    _home, external, _cfg = clio_home_with_config
    result = get_external_skills_dirs()
    assert result == [external.resolve()]


def test_cache_reuses_result_without_reparsing(clio_home_with_config):
    """Subsequent calls hit the cache and skip YAML parsing entirely."""
    _home, _external, _cfg = clio_home_with_config
...
def test_cache_invalidates_on_mtime_change(clio_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = clio_home_with_config
...
def test_returned_list_is_a_copy(clio_home_with_config):
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different CLIO_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".clio"
    ...
    monkeypatch.setenv("CLIO_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("CLIO_HOME", str(home_b))

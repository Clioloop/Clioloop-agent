"""Tests for CLIO_HOME credential-file read blocking in file_safety.

Regression for https://github.com/Clioloop/Clioloop-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``CLIO_HOME`` itself,
...
for the credential stores while leaving arbitrary ``CLIO_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_clio_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "clio_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_clio_home_path", lambda: home)
    return home
...
def test_arbitrary_clio_home_file_not_blocked(fake_home):
    """Non-credential files inside CLIO_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Clio cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into CLIO_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "clio_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at CLIO_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == CLIO_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at CLIO_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in CLIO_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_clio_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Clio-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside CLIO_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside CLIO_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside CLIO_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, CLIO_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_clio_home_path", lambda: profile)

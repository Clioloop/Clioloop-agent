...
from agent.auxiliary_client import (
    ...
    _refresh_managed_recommended_model,
...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "CLIO_MANAGED_INFERENCE_BASE_URL",
...
class TestAuxiliaryMaxTokensParam:
    def test_uses_max_completion_tokens_for_github_copilot_custom_base(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime", return_value=("https://api.githubcopilot.com", "key", None)), \
             patch("agent.auxiliary_client._read_managed_auth", return_value=None):
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("clio_cli.auth._read_codex_tokens", return_value={
    ...
    def test_missing_returns_none(self, tmp_path, monkeypatch):
        clio_home = tmp_path / "clio"
        clio_home.mkdir(parents=True, exist_ok=True)
        (clio_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
        monkeypatch.setenv("CLIO_HOME", str(clio_home))
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``clio auth status`` already reports these as logged in; compression
        ...
        monkeypatch.setenv("CLIO_HOME", str(clio_home))
        monkeypatch.delenv("CLIO_XAI_BASE_URL", raising=False)
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_HOME", str(clio_home))
        monkeypatch.setenv("CLIO_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``clio model``, no per-task override is
    ...
    fallback chain (OpenRouter / managed provider / etc.), so aux tasks billed
    ...
    def test_empty_model_for_catalog_provider_uses_catalog_default(self):
        """anthropic / managed / openrouter / etc.: catalog default wins
        ...
        with (
            ...
            patch(
                "agent.auxiliary_client._read_managed_auth", return_value=None
            ),
        ):
...
class TestExpiredCodexFallback:
    ...
    def test_clio_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-clio-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("clio_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        with patch("agent.auxiliary_client._read_managed_auth", return_value=None), \
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime",
             ...
             patch("agent.auxiliary_client._read_managed_auth", return_value=None), \
             patch("agent.auxiliary_client._resolve_managed_runtime_api", return_value=None), \
...
class TestAuxiliaryPoolAwareness:
    def test_try_managed_uses_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=_Pool()),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("clio_cli.models.get_managed_recommended_aux_model", return_value=None),
        ):
            from agent.auxiliary_client import _try_managed

            client, model = _try_managed()
    ...
    def test_try_managed_uses_portal_recommendation_for_text(self):
        """When the Portal recommends a compaction model, _try_managed honors it."""
        fresh_base = "https:///v1"
        with (
            patch("agent.auxiliary_client._read_managed_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_managed_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("clio_cli.models.get_managed_recommended_aux_model", return_value="minimax/minimax-m2.7") as mock_rec,
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            from agent.auxiliary_client import _try_managed

            mock_openai.return_value = MagicMock()
            client, model = _try_managed(vision=False)

        assert client is not None
        assert model == "minimax/minimax-m2.7"
        assert mock_rec.call_args.kwargs["vision"] is False

    def test_try_managed_uses_portal_recommendation_for_vision(self):
        """Vision tasks should ask for the vision-specific recommendation."""
        fresh_base = "https:///v1"
        with (
            patch("agent.auxiliary_client._read_managed_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_managed_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("clio_cli.models.get_managed_recommended_aux_model", return_value="google/gemini-3-flash-preview") as mock_rec,
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_managed
            client, model = _try_managed(vision=True)

        assert client is not None
        assert model == "google/gemini-3-flash-preview"
        assert mock_rec.call_args.kwargs["vision"] is True

    def test_try_managed_falls_back_when_recommendation_lookup_raises(self):
        """If the Portal lookup throws, we must still return a usable model."""
        fresh_base = "https:///v1"
        with (
            patch("agent.auxiliary_client._read_managed_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_managed_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("clio_cli.models.get_managed_recommended_aux_model", side_effect=RuntimeError("portal down")),
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_managed
            client, model = _try_managed()
    ...
    def test_call_llm_retries_managed_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https:///v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale managed key")
        ...
        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("managed", "managed-model", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", return_value=(stale_client, "managed-model")),
            patch("agent.auxiliary_client.OpenAI", return_value=fresh_client),
            patch("agent.auxiliary_client._validate_llm_response", side_effect=lambda resp, _task: resp),
            patch("agent.auxiliary_client._resolve_managed_runtime_api", return_value=("fresh-agent-key", "https:///v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
    ...
    def test_call_llm_refreshes_managed_after_free_tier_block_when_account_paid(self):
        ...
        with (
            ...
            patch(
                ...
                return_value=ManagedProviderAccountInfo(
                    logged_in=True,
                    source="account_api",
                    fresh=True,
    ...
    @pytest.mark.asyncio
    async def test_async_call_llm_retries_managed_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https:///v1"
        stale_client.chat.completions.create = AsyncMock(side_effect=_Auth401("stale managed key"))

        fresh_async_client = MagicMock()
        fresh_async_client.base_url = "https:///v1"
        fresh_async_client.chat.completions.create = AsyncMock(return_value={"ok": True})

        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("managed", "managed-model", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", return_value=(stale_client, "managed-model")),
            patch("agent.auxiliary_client._to_async_client", return_value=(fresh_async_client, "managed-model")),
            patch("agent.auxiliary_client._validate_llm_response", side_effect=lambda resp, _task: resp),
            patch("agent.auxiliary_client._resolve_managed_runtime_api", return_value=("fresh-agent-key", "https:///v1")),
        ):
    ...
    @pytest.mark.asyncio
    async def test_async_call_llm_refreshes_managed_after_free_tier_block_when_account_paid(self):
        from clio_cli.portal_account import ManagedProviderAccountInfo
...
class TestIsModelNotFoundError:
    ...
    def test_managed_openrouter_catalog_404(self):
        """The exact incident error: a Portal-recommended model dropped from
        the managed provider → OpenRouter catalog."""
        exc = Exception(
...
class TestRefreshManagedRecommendedModel:
    """_refresh_managed_recommended_model picks a fresh model after a stale 404."""

    def test_returns_fresh_portal_recommendation(self, monkeypatch):
        monkeypatch.setattr(
            "clio_cli.models.get_managed_recommended_aux_model",
            lambda **kw: "stepfun/step-3.7-flash:free",
        )
        out = _refresh_managed_recommended_model(
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "clio_cli.models.get_managed_recommended_aux_model", _boom)
        out = _refresh_managed_recommended_model(
...
class TestIsRateLimitError:
    ...
    def test_429_with_resets_in_message(self):
        """managed-provider-style 429: 'resets in 3508s'."""
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        assert labels == ["openrouter", "managed", "local/custom", "api-key"]
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_managed", return_value=(mock_client, "managed-model")), \
             patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
        assert client is mock_client
        assert model == "managed-model"
        assert label == "managed"

    def test_returns_none_when_no_fallback(self):
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_managed", return_value=(None, None)), \
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/managed provider/custom/api-key all fail, payment-fallback returns None —
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from clio_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("clio_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
        "clio_cli.auth.is_provider_explicitly_configured",
        lambda pid: False,
    )
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("clio_cli.config.load_config", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/Clioloop/Clioloop-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/Clioloop/Clioloop-agent/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "ClioAgent"
...
class TestAuxUnhealthyCache:
    ...
    def test_resolve_auto_skips_unhealthy_step2(self):
        ...
        # Mark OpenRouter unhealthy → chain should skip it and pick managed.
        _mark_provider_unhealthy("openrouter")
        with patch("agent.auxiliary_client._read_main_provider", return_value=""), \
             ...
             patch("agent.auxiliary_client._try_managed", return_value=(managed_client, "managed-model")), \
             patch("agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
        assert client is managed_client
        assert model == "managed-model"
    ...
    def test_resolve_auto_skips_unhealthy_main_in_step1(self):
        ...
        with patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"), \
             ...
             patch("agent.auxiliary_client._try_openrouter") as or_try, \
             patch("agent.auxiliary_client._try_managed", return_value=(managed_client, "n-model")), \
             patch("agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
        # Step-1 was bypassed — resolve_provider_client never invoked
        step1.assert_not_called()
        # Step-2 also skipped openrouter and landed on managed
        or_try.assert_not_called()
        assert client is managed_client
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        # unhealthy. The chain should still find managed.
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        managed_client = MagicMock()
        managed_resp = MagicMock()
        managed_resp.choices = [MagicMock(message=MagicMock(content="ok"))]
        managed_client.chat.completions.create.return_value = managed_resp
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._resolve_task_provider_model",
                    return_value=("auto", "google/gemini-3-flash-preview", None, None, None)), \
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(managed_client, "n-model", "managed")), \

...
class TestClientCredResolution:
    def test_env_override(self, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_GEMINI_CLIENT_ID", "custom-id.apps.googleusercontent.com")
...
class TestProjectIdResolution:
    ...
    def test_priority_order(self, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_GEMINI_PROJECT_ID", "higher-priority")
...
class TestHeadlessDetection:
    ...
    def test_detects_clio_headless(self, monkeypatch):
        from agent.google_oauth import _is_headless

        monkeypatch.setenv("CLIO_HEADLESS", "1")
        assert _is_headless() is True
...
class TestProviderRegistration:
    ...
    def test_oauth_capable_set_preserves_existing(self):
        from clio_cli.auth_commands import _OAUTH_CAPABLE_PROVIDERS

        for required in ("anthropic", "managed", "openai-codex", "qwen-oauth", "google-gemini-cli"):
...
class TestAuthStatus:
    ...
    def test_logged_in_reports_email_and_project(self):
        ...
        save_credentials(GoogleCredentials(
            ...
            expires_ms=int((time.time() + 3600) * 1000),
            email="tek@omniloop.ai",
            project_id="tek-proj",
        ))

        s = get_auth_status("google-gemini-cli")
        assert s["logged_in"] is True
        assert s["email"] == "tek@omniloop.ai"

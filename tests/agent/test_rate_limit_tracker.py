...
# ── Sample headers from the managed provider inference API ──────────────────────────────
...
class TestFormatting:
    ...
    def test_format_display_with_data(self):
        state = parse_rate_limit_headers(MANAGED_HEADERS, provider="managed")
        result = format_rate_limit_display(state)
        assert "Omni Loop Portal Subscription" in result
...
class TestAgentIntegration:
    ...
    def test_capture_rate_limits_from_headers(self):
        ...
        state = parse_rate_limit_headers(MockResponse.headers, provider="managed")
        assert state is not None
        assert state.requests_min.limit == 800
        assert state.tokens_hour.limit == 336000000

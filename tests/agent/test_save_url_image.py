...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate CLIO_HOME under tmp_path."""
    monkeypatch.setenv("CLIO_HOME", str(tmp_path / ".clio"))
    (tmp_path / ".clio").mkdir()

    # Force the constants/image cache helpers to re-read CLIO_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("clio_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_clio_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under CLIO_HOME — gateway cleanup

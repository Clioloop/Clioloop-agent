...
class TestChatCompletionsBasic:
    ...
    def test_convert_messages_strips_internal_scaffolding_markers(self, transport):
        """Clio-internal ``_``-prefixed markers must never reach the wire.
...
@pytest.mark.skip(reason="managed provider / managed provider Tool Gateway removed in Clio rebrand (phase 12)")
class TestChatCompletionsBuildKwargs:
    ...
    def test_managed_omits_disabled_reasoning(self, transport):
        from providers import get_provider_profile
        profile = get_provider_profile("managed")
        msgs = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(
        ...
        # managed provider rejects enabled=false; reasoning omitted entirely
    ...
    def test_gemma_does_not_receive_thinking_config(self, transport):
        ...
        # when Clio has reasoning enabled, the field must be omitted for
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (managed provider, OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
...
class TestChatCompletionsGeminiNativeExtraBodyStrip:
    """Profile extra_body (e.g. managed provider tags) must not reach a native
    Gemini endpoint — Google's REST API rejects unknown fields with HTTP 400.
    """

    def _managed_profile(self):
        from providers import get_provider_profile
        return get_provider_profile("managed")
    ...
    def test_tags_stripped_when_endpoint_is_native_gemini(self, transport):
        kw = transport.build_kwargs(
            ...
            [{"role": "user", "content": "hi"}],
            None,
            provider_profile=self._managed_profile(),
            base_url="https://generativelanguage.googleapis.com/v1beta",
            session_id="s1",
            max_tokens=None,
        )
    ...
    def test_tags_preserved_on_managed_endpoint(self, transport):
        kw = transport.build_kwargs(
            "clio-3-405b",
            [{"role": "user", "content": "hi"}],
            None,

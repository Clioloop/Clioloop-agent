...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Clio passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("CLIO_HOME", "/users/alice/.clio/profiles/backend-worker")
        monkeypatch.setenv("CLIO_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "CLIO_KANBAN_DB",
            "/users/alice/.clio/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.clio/kanban/boards/smoke"]'
            in cmd
        )

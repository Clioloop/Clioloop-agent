...
""",
        ...
        from clio_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        clio_home = tmp_path / ".clio"
        clio_home.mkdir()
        (clio_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("CLIO_HOME", str(clio_home))
        from clio_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from clio_cli.plugins import PluginContext, PluginManifest, PluginManager

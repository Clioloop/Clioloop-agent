...
engine plugins (e.g. clio-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``clio plugins`` picker."""
    from clio_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "clio_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "clio_cli.plugins.get_plugin_context_engine",

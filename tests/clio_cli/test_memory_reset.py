"""Tests for the `clio memory reset` CLI command.
...
- Profile-scoped reset (uses CLIO_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake CLIO_HOME with memory files."""
    clio_home = tmp_path / ".clio"
    memories = clio_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("CLIO_HOME", str(clio_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nClio repo is at ~/.clio/clio-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
    (memories / "USER.md").write_text(
        "§\nUser is Omni Loop Labs\n§\nTimezone: US Pacific",
        encoding="utf-8",
    )
    return clio_home, memories
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `clio memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        clio_home = tmp_path / ".clio"
        (clio_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("CLIO_HOME", str(clio_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's CLIO_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_clio_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"

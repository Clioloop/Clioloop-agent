...
columns + Free/Pro badges and gate paid models on free managed-provider accounts, the
same way the `clio model` CLI picker does.
"""

import clio_cli.inventory as inv
...
def _patch_pricing(monkeypatch, *, free_tier, pricing, unavailable=None):
    ...
    monkeypatch.setattr(models_mod, "check_managed_free_tier", lambda *, force_fresh=False: free_tier)
    monkeypatch.setattr(
...
def test_apply_pricing_managed_free_tier_gates_paid_models(monkeypatch):
    """A free-tier managed provider account marks paid models unavailable and sets the flag."""
    _patch_pricing(
        monkeypatch,
        free_tier=True,
        pricing={
    ...
    rows = [{"slug": "managed", "models": ["free/model", "paid/model"]}]
    inv._apply_pricing(rows)
...
def test_apply_pricing_managed_paid_tier_no_gating(monkeypatch):
    """A paid managed provider account gates nothing."""
    _patch_pricing(
        monkeypatch,
        free_tier=False,
        pricing={"managed": {"x/model": {"prompt": "0.000001", "completion": "0.000002"}}},
    )
    rows = [{"slug": "managed", "models": ["x/model"]}]
    inv._apply_pricing(rows)

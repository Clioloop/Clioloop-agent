...
class TestCommandRegistry:
    ...
    def test_fusion_command_is_registered_with_status_controls(self):
        cmd = resolve_command("fusion")
        assert cmd is not None
        assert cmd.name == "fusion"
        assert cmd.category == "Configuration"
        assert cmd.args_hint == "[off|status]"
        assert cmd.subcommands == ("off", "status")
        assert "advisors" in cmd.description
        assert "main model" in cmd.description
...
class TestSlackNativeSlashes:
    ...
    def test_clio_catchall_is_first(self):
        """``/clio`` must be reserved as the first slot so the legacy
        ``/clio <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "clio"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /clio <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `clio slack manifest`)."""
...
from clio_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch clio_cli.plugins.get_plugin_commands() to a fixed dict."""
        from clio_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/clio metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",

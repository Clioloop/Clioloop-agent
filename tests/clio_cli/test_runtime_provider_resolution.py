...
from clio_cli import runtime_provider as rp
...
@pytest.mark.skip(reason="pre-existing: code at runtime_provider.py:1427 uses imported name, not rp.resolve_qwen_runtime_credentials; monkeypatch doesn't reach the call site")
...
@pytest.mark.skip(reason="pre-existing: same monkeypatch-targeting issue as test_resolve_runtime_provider_qwen_oauth")
...
@pytest.mark.skip(reason="'managed' canonical provider removed in Clio rebrand (phase 4); resolve_managed_runtime_credentials is gone")
def test_named_custom_provider_does_not_shadow_builtin_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        rp,
        "resolve_managed_runtime_credentials",
        lambda **kwargs: {
            "base_url": "https:///v1",
            "api_key": "managed-runtime-key",
            "source": "portal",
            "expires_at": None,
        },
    )

    resolved = rp.resolve_runtime_provider(requested="managed")

    assert resolved["provider"] == "managed"
    assert resolved["base_url"] == "https:///v1"
    assert resolved["api_key"] == "managed-runtime-key"
    assert resolved["requested_provider"] == "managed"
...
@pytest.mark.skip(reason="'managed' canonical provider removed in Clio rebrand (phase 4); companion test to the skip above")
def test_named_custom_provider_skipped_for_canonical_built_in(monkeypatch):
    """Companion to the test above: ``managed`` is a canonical provider name
    (``resolve_provider('managed') == 'managed'``), so a custom entry with that name
    ...
    entry = rp._get_named_custom_provider("managed")
...
def test_resolve_requested_provider_precedence(monkeypatch):
    ...
    monkeypatch.setattr(rp, "_get_model_config", lambda: {})
    assert rp.resolve_requested_provider() == "managed"
...
def test_auto_detected_managed_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks managed provider but credentials are revoked, fall through to OpenRouter."""
    from clio_cli.auth import AuthError
    ...
    # resolve_provider returns "managed" (stale active_provider in auth.json)
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "managed")
    ...
    # managed-provider credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_managed_runtime_credentials",
        lambda **kw: (_ for _ in ()).throw(
            AuthError("Refresh session has been revoked",
                      provider="managed", code="invalid_grant", relogin_required=True)
        ),
    )
...
def test_explicit_managed_auth_failure_still_raises(monkeypatch):
    """When user explicitly requests managed provider and auth fails, the error should propagate."""
    from clio_cli.auth import AuthError
    import pytest
    ...
    # With explicit "managed", should raise — don't silently switch providers
    with pytest.raises(AuthError, match="Refresh session has been revoked"):
        rp.resolve_runtime_provider(requested="managed")
...
class TestAzureFoundryResolution:
    ...
    def test_azure_foundry_missing_api_key_raises(self, monkeypatch):
        ...
        # `get_env_value` reads from ~/.clio/.env — mock it to return None
        # so the resolver can't find a key there either.
        import clio_cli.config as cfg_mod
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from clio_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from clio_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from clio_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
...
def test_minimax_oauth_runtime_uses_inference_base_url(monkeypatch):
    ...
    from clio_cli.auth import MINIMAX_OAUTH_CN_INFERENCE

"""Tests for the top-level `./clio` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./clio` should use `clio_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "clio"
    called = []

    fake_main_module = types.ModuleType("clio_cli.main")

    def fake_main():
        called.append("clio_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "clio_cli.main", fake_main_module)

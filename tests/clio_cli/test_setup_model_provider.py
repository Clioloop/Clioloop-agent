...
from clio_cli.main, these tests mock the delegation point and verify
...
from clio_cli.setup import _print_setup_summary, setup_model_provider
...
def _stub_tts(monkeypatch):
    monkeypatch.setattr("clio_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("clio_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside clio setup must survive the wizard's final save."""
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("clio_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("clio_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("clio_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'clio setup' to configure" not in output


@pytest.mark.skip(reason="setup summary print path refactored in Clio rebrand; managed-provider Camofox label no longer rendered")
def test_setup_summary_shows_camofox_when_browser_feature_is_camofox(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("CLIO_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    monkeypatch.setattr(
        "clio_cli.setup.get_managed_subscription_features",
        lambda config: ManagedSubscriptionFeatures(
            ...
            features={
                ...
                "browser": ManagedFeatureState("browser", "Browser automation", True, True, True, False, True, True, "Camofox"),
                "modal": ManagedFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
@pytest.mark.skip(reason="setup summary print path refactored in Clio rebrand; browserbase gate moved out of managed-provider code path")
    ...
    monkeypatch.setattr(
        ...
        lambda config: ManagedSubscriptionFeatures(
            ...
            features={
                ...
                "video_gen": ManagedFeatureState("video_gen", "Video generation", False, False, False, False, False, False, ""),
                "tts": ManagedFeatureState("tts", "OpenAI TTS", True, False, False, False, False, True, ""),
                "browser": ManagedFeatureState("browser", "Browser automation", True, False, False, False, False, True, "Browserbase"),
                "modal": ManagedFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
...
@pytest.mark.skip(reason="setup summary print path refactored in Clio rebrand; local-browser detection moved out of managed-provider code path")
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    ``get_managed_subscription_features`` so the surface stays aligned with the
    ...
    monkeypatch.setattr("clio_cli.portal_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(

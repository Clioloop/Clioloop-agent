"""Tests for the ``clio prompt-size`` diagnostic (issue #34667)."""

import json

import pytest

from clio_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(clio_home, memory_text="", user_text=""):
    mem_dir = clio_home / "memories"
...
def _seed_skill(clio_home, name, description):
    skill_dir = clio_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "CLIO_MANAGED_API_KEY",
                "ANTHROPIC_API_KEY"):

"""Tests for clio_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_clio_home):
    """Drop the example-dashboard fixture into the per-test CLIO_HOME
    ...
    ``CLIO_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, clio-achievements, model providers
    ...
    #      and start serving requests against a torn-down CLIO_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Clio vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Clio env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints CLIO_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("CLIO_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_clio_home):
        """Create a TestClient and isolate the state DB under the test CLIO_HOME."""
        try:
        ...
        monkeypatch.setattr(clio_state, "DEFAULT_DB_PATH", get_clio_home() / "state.db")
    ...
    def test_get_status(self):
        ...
        assert "clio_home" in data
        assert "active_sessions" in data

    def test_oauth_providers_surface_omni_metadata(self, monkeypatch):
        """The dashboard provider catalog must promote Omni with launch copy."""
        import clio_cli.web_server as web_server

        monkeypatch.setattr(
            web_server,
            "_resolve_provider_status",
            lambda provider_id, status_fn: {"logged_in": provider_id == "managed"},
        )
        ...
        assert providers[0]["id"] == "managed"
        assert providers[0]["name"] == "Omni Loop Portal Subscription"
        assert providers[0]["badge"] == "Recommended"
        assert providers[0]["description"] == (
            "Currently the only way to use Model Fusion · one login · no API keys · includes Tool Gateway"
        )

    def test_portal_status_brands_managed_features_as_subscription(self, monkeypatch):
        """Portal feature states should not surface raw managed-provider wording."""
        import clio_cli.auth as auth_mod
        import clio_cli.portal_subscription as portal_subscription
        import clio_cli.web_server as web_server

        features = portal_subscription.ManagedSubscriptionFeatures(
            features={
                "web": portal_subscription.ManagedFeatureState(
                    "web", "Web tools", True, True, True, True, "Omni Loop Portal Subscription"
                ),
                "browser": portal_subscription.ManagedFeatureState(
                    "browser", "Browser automation", True, True, False, False, "Local Browser"
                ),
                "image_gen": portal_subscription.ManagedFeatureState(
                    "image_gen", "Image generation", False, False, False, False, ""
                ),
                "video_gen": portal_subscription.ManagedFeatureState(
                    "video_gen", "Video generation", False, False, False, False, ""
                ),
                "tts": portal_subscription.ManagedFeatureState(
                    "tts", "Text-to-speech", False, False, False, False, ""
                ),
                "modal": portal_subscription.ManagedFeatureState(
                    "modal", "Modal execution", False, False, False, False, ""
                ),
            }
        )
        monkeypatch.setattr(web_server, "load_config", lambda: {"model": {"provider": "managed"}})
        monkeypatch.setattr(
            auth_mod,
            ...
            lambda: {
                ...
                "inference_base_url": "https://inference.example.test",
            },
        )
        monkeypatch.setattr(
            portal_subscription,
        ...
        resp = self.client.get("/api/portal")
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("clio_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from clio_cli.web_server import app
    ...
    def test_update_clio_returns_docker_guidance_without_spawning(self, monkeypatch):
        import clio_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn clio update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_clio_action", fail_spawn)
        web_server._ACTION_PROCS.pop("clio-update", None)
        web_server._ACTION_RESULTS.pop("clio-update", None)

        resp = self.client.post("/api/clio/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "clio-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull clioloop/clioloop-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/clio-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull clioloop/clioloop-agent:latest" in line for line in status_data["lines"])

    def test_update_clio_spawns_on_non_docker_install(self, monkeypatch):
        import clio_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_clio_action", fake_spawn)
        web_server._ACTION_PROCS.pop("clio-update", None)
        web_server._ACTION_RESULTS.pop("clio-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "clio-update"}
        assert calls == [(["update"], "clio-update")]
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from clio_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from clio_cli.web_server import (
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from clio_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_bad_token(self, tmp_path):
        ...
        from clio_cli.config import save_env_value
        from clio_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from clio_cli.config import save_env_value
        from clio_cli.web_server import _SESSION_TOKEN
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from clio_cli.config import load_config, load_env
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "clio_pair123_bot",
                "deep_link": "https://t.me/newbot/ClioSetupBot/clio_pair123_bot",
                "qr_payload": "https://t.me/newbot/ClioSetupBot/clio_pair123_bot",
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "clio_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/ClioSetupBot/clio_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/ClioSetupBot/clio_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "clio_pair_ready_bot",
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "clio_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/ClioSetupBot/clio_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/ClioSetupBot/clio_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "clio_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/ClioSetupBot/clio_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/ClioSetupBot/clio_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/clio_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
            assert "FastAPI" not in resp.text  # Should not serve the actual source

    def test_set_model_main_managed_applies_gateway_defaults(self, monkeypatch):
        """Switching the main provider to managed provider calls apply_managed_defaults
        (mirroring the CLI's post-model-selection Tool Gateway routing) and
        surfaces the routed tools in the response."""
        import clio_cli.portal_subscription as ns
        ...
        monkeypatch.setattr(ns, "apply_managed_defaults", fake_apply)

        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "managed", "model": "clio-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_set_model_main_non_managed_skips_gateway_defaults(self, monkeypatch):
        """Non-managed provider providers must NOT trigger Tool Gateway auto-routing."""
        import clio_cli.portal_subscription as ns

        def boom(*args, **kwargs):  # pragma: no cover - must not be called
            raise AssertionError("apply_managed_defaults called for non-managed provider")

        monkeypatch.setattr(ns, "apply_managed_defaults", boom)
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from clio_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from clio_cli.web_server import _parse_model_ids
    ...
    def test_recommended_default_managed_honors_free_tier(self, monkeypatch):
        """For a free-tier managed provider user, the recommended default must be a free
        model (mirroring `clio model`), not the first curated paid entry."""
        import clio_cli.models as models_mod

        monkeypatch.setattr(models_mod, "get_curated_managed_model_ids", lambda: ["paid/expensive", "free/cheap"])
        monkeypatch.setattr(
            models_mod, "get_pricing_for_provider",
            lambda provider: {"paid/expensive": {"input": "1"}, "free/cheap": {"input": "0"}},
        )
        monkeypatch.setattr(models_mod, "check_managed_free_tier", lambda *, force_fresh=False: True)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            models_mod, "partition_managed_models_by_tier",
            lambda ids, pricing, free_tier: (["free/cheap"], ["paid/expensive"]),
        )

        resp = self.client.get("/api/model/recommended-default?provider=managed")
        assert resp.status_code == 200
        data = resp.json()
        assert data["provider"] == "managed"
    ...
    def test_recommended_default_managed_paid_uses_curated_default(self, monkeypatch):
        """A paid managed provider user gets the first curated/paid-augmented model."""
        import clio_cli.models as models_mod

        monkeypatch.setattr(models_mod, "get_curated_managed_model_ids", lambda: ["top/model", "other/model"])
        monkeypatch.setattr(models_mod, "get_pricing_for_provider", lambda provider: {})
        monkeypatch.setattr(models_mod, "check_managed_free_tier", lambda *, force_fresh=False: False)
        monkeypatch.setattr(
    ...
    def test_recommended_default_handles_failure_gracefully(self, monkeypatch):
        ...
        monkeypatch.setattr(models_mod, "get_curated_managed_model_ids", boom)
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from clio_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from clio_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_clio_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from clio_constants import get_clio_home
        get_clio_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from clio_constants import get_clio_home
        import clio_cli.profiles as profiles_mod
        ...
        named = clio_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_clio_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_clio_for_default_profile(self):
        from clio_constants import get_clio_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "clio setup"
    ...
    def test_profiles_create_with_clone_from_default_copies_default_skills(self, monkeypatch):
        ...
        default_skill = get_clio_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = get_clio_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_clio_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_clio_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_clio_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_clio_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import clio_cli.skills_config as skills_config
        import clio_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import clio_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from clio_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from clio_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from clio_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.clio/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``clio_cli/web_server.py``).
    ...
    def _setup_test_client(self, monkeypatch, _isolate_clio_home):
        ...
        monkeypatch.setattr(
            clio_state, "DEFAULT_DB_PATH", get_clio_home() / "state.db"
        )
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in clio_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_clio_home):
        ...
        # Pin the SessionDB to the isolated CLIO_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_clio_home, _install_example_plugin):
        ...
        installs it into the per-test ``CLIO_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into CLIO_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, clio-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (clio-achievements is loaded by default).
        resp = self.client.get("/api/plugins/clio-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``clio --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["CLIO_TUI_INLINE"] == "1"
        assert env["CLIO_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"clio-ws-ok" in buf:
                    break
            assert b"clio-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from clio_cli.pty_bridge import PtyUnavailableError
        ...
        import clio_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        CLIO_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from clio_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import clio_cli.main as cli_main
    import clio_cli.web_server as ws
    ...
    gateway_url = env.get("CLIO_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_clio_home, _install_example_plugin):
        ...
        the per-test ``CLIO_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestGatewayRestartDetached:
    """The gateway restart must be a plain DETACHED spawn of
    ``<venv-python> -m clio_cli.main gateway restart`` (mirrors upstream
    hermes-agent) — never a visible cmd.exe window with a base interpreter,
    which couldn't import the venv's deps and left the bot dead."""

    def test_visible_window_helpers_removed(self):
        from clio_cli import web_server
        assert not hasattr(web_server, "_spawn_visible_gateway_restart_win")
        assert not hasattr(web_server, "_write_gateway_restart_cmd")
        from clio_cli import gateway_windows
        assert not hasattr(gateway_windows, "_resolve_console_python")

    def test_spawn_clio_action_runs_detached_module(self, monkeypatch, tmp_path):
        ...
        proc = web_server._spawn_clio_action(["gateway", "restart"], "gateway-restart")

        assert proc.pid == 4321
        cmd = captured["cmd"]
        assert "-m" in cmd and "clio_cli.main" in cmd
        assert cmd[-2:] == ["gateway", "restart"]
        # No visible-terminal shim.
        assert not any("cmd.exe" in str(c) or c == "/k" for c in cmd)

...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (clio_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from clio_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "clio" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_clio_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:clio_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:clio_pkce"
...
def test_auth_add_google_gemini_cli_sets_active_provider(tmp_path, monkeypatch):
    """clio auth add google-gemini-cli must set active_provider in auth.json.
    ...
    # pool entry from pool.add_entry() still present for clio auth list
...
def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """clio auth add qwen-oauth must set active_provider in auth.json.
    ...
    monkeypatch.setattr(
        "clio_cli.auth.resolve_qwen_runtime_credentials",
...
@pytest.mark.skip(reason="managed oauth provider removed in Clio rebrand (phase 4)")
def test_auth_add_managed_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("CLIO_HOME", str(tmp_path / "clio"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("managed@example.com")
    monkeypatch.setattr(
        "clio_cli.auth._managed_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["managed"]
    device_code_entries = [
    ...
    # `clio auth add managed` must also populate providers.managed so the
    # 401-recovery path (resolve_managed_runtime_credentials) can refresh an
    # invoke JWT when the token expires. If this mirror is missing, recovery
    # raises "Clio is not logged into managed provider" and the agent dies.
    singleton = payload["providers"]["managed"]
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clio_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
@pytest.mark.skip(reason="managed oauth provider removed in Clio rebrand (phase 4)")
def test_auth_add_managed_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`clio auth add managed --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_managed_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("CLIO_HOME", str(tmp_path / "clio"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("managed@example.com")
    ...
    pool_entry = payload["credential_pool"]["managed"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-managed"

    # … and survives in providers.managed so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["managed"]["label"] == "my-managed"
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clio_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """clio auth add xai-oauth must write providers singleton and set active_provider.
    ...
    monkeypatch.setattr(
        "clio_cli.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from clio_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from clio_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`clio logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (clio_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `clio logout` should target configured Codex if auth has no active provider."""
    clio_home = tmp_path / "clio"
    monkeypatch.setenv("CLIO_HOME", str(clio_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (clio_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((clio_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from clio_cli.auth import _reset_config_provider
    ...
    with patch("clio_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from clio_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "clio_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("clio_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("clio_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("clio_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "clio_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (clio_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from clio_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((clio_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from clio_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from clio_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `clio auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("CLIO_HOME", str(tmp_path / "clio"))
    clio_home = tmp_path / "clio"
    clio_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `clio auth remove`)
    (clio_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    ...
    payload = json.loads((clio_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((clio_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`clio auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.clio/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.clio/.env (not the shell), the
    ...
    (clio_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (clio_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `clio auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from clio_cli.auth import is_source_suppressed
    from clio_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `clio auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Clio reads from has a
...
def test_seed_from_singletons_respects_managed_suppression(tmp_path, monkeypatch):
    """managed device_code must not re-seed from auth.json when suppressed."""
    clio_home = tmp_path / "clio"
    clio_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("CLIO_HOME", str(clio_home))
    ...
    (clio_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"managed": {"access_token": "tok", "refresh_token": "r", "expires_at": 9999999999}},
        "suppressed_sources": {"managed": ["device_code"]},
    }))

    from agent.credential_pool import _seed_from_singletons
    entries = []
    changed, active = _seed_from_singletons("managed", entries)
    assert changed is False
    assert entries == []
    assert active == set()
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import clio_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_clio_pkce_suppression(tmp_path, monkeypatch):
    """anthropic clio_pkce must not re-seed from ~/.clio/.anthropic_oauth.json when suppressed."""
    clio_home = tmp_path / "clio"
    clio_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("CLIO_HOME", str(clio_home))

    import yaml
    (clio_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (clio_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["clio_pkce"]},
    }))

    # Stub the readers so only clio_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_clio_oauth_credentials", lambda: {
    ...
    # clio_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "clio_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (clio_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.clio/.anthropic_oauth.json",
        "auth.json providers.managed",
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "clio_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "clio_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `clio auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `clio auth add openai-codex`)

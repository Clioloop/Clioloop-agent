...
``clio_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``clio --help``, ``clio version``,
``clio logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``clio --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from clio_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["clio", "-w", "--tui"], None),
        (["clio", "version"], "version"),
        (["clio", "--tui", "chat"], "chat"),
        (["clio", "-w", "logs"], "logs"),
        (["clio", "chat", "hello world"], "chat"),
        (["clio", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["clio", "-m", "gpt5", "chat"], "chat"),
        (["clio", "--model", "gpt5", "chat", "hi"], "chat"),
        (["clio", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["clio", "-z", "hello world"], None),
        (["clio", "-z", "hello", "chat"], "chat"),
        (["clio", "--model=gpt5", "chat"], "chat"),     # inline form
        (["clio", "--", "chat"], "chat"),               # -- terminator
        (["clio", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["clio", "some-plugin-cmd"], "some-plugin-cmd"),
        (["clio", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["clio"],                          # bare → chat
        ["clio", "--help"],                # top-level help
        ["clio", "-h"],
        ["clio", "version"],               # known built-in
        ["clio", "logs"],
        ["clio", "gateway", "run"],
        ["clio", "--tui"],
        ["clio", "-w", "--tui"],
        ["clio", "chat", "hi"],
        ["clio", "help"],                  # accepted built-in-ish
        ["clio", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["clio", "meet", "join"],          # potential google_meet plugin
        ["clio", "honcho", "status"],      # potential memory plugin
        ["clio", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``clio help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"clio_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "

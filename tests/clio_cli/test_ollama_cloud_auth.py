...
class TestDirectAliases:
    ...
    def test_direct_alias_loaded_from_config(self, monkeypatch):
        ...
        from clio_cli.model_switch import _load_direct_aliases
    ...
    def test_direct_alias_resolved_before_catalog(self, monkeypatch):
        ...
        from clio_cli.model_switch import DirectAlias, resolve_alias
        import clio_cli.model_switch as ms
...
class TestModelSwitchPersistence:
    ...
    def test_model_switch_result_fields(self):
        ...
        from clio_cli.model_switch import ModelSwitchResult
...
class TestEnsureDirectAliases:
    ...
    def test_ensure_no_reload_when_populated(self, monkeypatch):
        ...
        import clio_cli.model_switch as ms
        from clio_cli.model_switch import DirectAlias
...
class TestSwitchModelDirectAliasOverride:
    ...
    def test_switch_model_uses_alias_base_url(self, monkeypatch):
        ...
        monkeypatch.setattr("clio_cli.models.validate_requested_model",
            lambda *a, **kw: {"accepted": True, "persist": True, "recognized": True, "message": None})
        monkeypatch.setattr("clio_cli.models.opencode_model_api_mode",
...
class TestManagedPortalModelRouting:
    """Managed picker selections must stay on the Portal router."""

    def test_bare_managed_catalog_model_does_not_hop_to_native_provider(self, monkeypatch):
        """A bare (open) model selected from managed must keep provider=managed.

        Without the managed catalog guard, detect_provider_for_model can see a
        same-named native/static model and silently switch away from the Portal,
        bypassing the Portal's Ollama/OpenRouter router and keys.
        """
        import clio_cli.model_switch as ms

        calls = {}

        monkeypatch.setattr(ms, "resolve_alias", lambda *_args, **_kwargs: None)
        monkeypatch.setattr(
            ms,
            "list_provider_models",
            lambda provider: ["deepseek-v4-pro"] if provider == "managed" else [],
        )
        monkeypatch.setattr(
            "clio_cli.models.detect_provider_for_model",
            lambda *_args, **_kwargs: ("deepseek", "deepseek-v4-pro"),
        )
        ...
        monkeypatch.setattr("clio_cli.runtime_provider.resolve_runtime_provider", _runtime)
        monkeypatch.setattr(
            "clio_cli.models.validate_requested_model",
            lambda *a, **kw: {
                "accepted": True,
                "persist": True,
                "recognized": True,
        ...
        result = ms.switch_model(
            "deepseek-v4-pro",
            current_provider="managed",
            current_model="openai/gpt-oss-120b:free",
        )

        assert result.success
        assert result.target_provider == "managed"
        assert result.new_model == "deepseek-v4-pro"
        assert result.base_url == "https://portal.example/v1"
        assert calls["runtime"]["requested"] == "managed"
        assert calls["runtime"]["target_model"] == "deepseek-v4-pro"

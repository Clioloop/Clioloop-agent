...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (CLIO_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via CLIO_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in CLIO_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from clio_cli.providers import CLIO_OVERLAYS
        assert "bedrock" in CLIO_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from clio_cli.providers import CLIO_OVERLAYS
        assert CLIO_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from clio_cli.providers import CLIO_OVERLAYS
        assert CLIO_OVERLAYS["bedrock"].auth_type == "aws_sdk"

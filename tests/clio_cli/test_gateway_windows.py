"""Tests for clio_cli.gateway_windows."""
...
import clio_cli.gateway_windows as gateway_windows
...
def test_exec_schtasks_decodes_with_replace_errors(monkeypatch):
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "Clio_Gateway"])
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda clio_home: "")
    monkeypatch.setattr("clio_cli.config.get_clio_home", lambda: str(clio_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "clio_cli.main"]
    assert cwd == str(clio_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_clio_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".clio"
        home.mkdir()
        monkeypatch.setattr("clio_cli.config.get_clio_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".clio"
        project = tmp_path / "checkout"
        monkeypatch.setattr("clio_cli.config.get_clio_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_clio_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    clio_home = tmp_path / "clio-home"
    clio_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(clio_home.resolve()))}" in content
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    ...
    startup_entry = tmp_path / "Startup" / "Clio_Gateway_alice.cmd"
    calls = []
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Clio_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Clio_Gateway_alice")
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated Clio gateway install prompt" in out
...
def test_install_assume_yes_elevates_without_uac_prompt(monkeypatch, tmp_path, capsys):
    """--non-interactive (assume_yes) skips the terminal UAC question and opens
    the native elevation dialog directly when not admin."""
    script_path = tmp_path / "Clio_Gateway_alice.cmd"
    calls = []
    ...
    # If any prompt fires, fail loudly — assume_yes must not ask anything.
    def _no_prompt(prompt, default=True):
        raise AssertionError(f"unexpected prompt: {prompt!r}")
    monkeypatch.setattr(setup, "prompt_yes_no", _no_prompt)

    gateway_windows.install(start_now=True, start_on_login=True, assume_yes=True)

    assert calls == [("elevate", False, True, True)]
    assert "Launched elevated Clio gateway install prompt" in capsys.readouterr().out
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "clio --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated Clio gateway uninstall prompt" in out
...
# resume_pending=True) never fires when `clio gateway stop` kills the
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("clio_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `clio gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
    ...
    monkeypatch.setattr("clio_cli.gateway._get_restart_drain_timeout", lambda: 1.0)

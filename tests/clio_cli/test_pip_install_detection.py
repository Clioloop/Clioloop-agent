...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("clio_cli.config.get_managed_system", return_value=None), \
         patch("clio_cli.config.get_clio_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When CLIO_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("clio_cli.config.get_managed_system", return_value="NixOS"), \
         patch("clio_cli.config.get_clio_home", return_value=tmp_path):
        from clio_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from clio_cli.config import recommended_update_command_for_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``clio update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("clio_cli.config.get_managed_system", return_value=None), \
         patch("clio_cli.config.get_clio_home", return_value=tmp_path), \
         patch("clio_constants.is_container", return_value=True):
        from clio_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from clio_cli import banner

    hh = tmp_path / ".clio"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("clio_cli.config.get_clio_home", return_value=hh), \
         patch("clio_constants.get_clio_home", return_value=hh):
        buf = io.StringIO()

...
from clio_cli import setup as setup_mod
from clio_cli.setup import setup_model_provider
...
def test_setup_syncs_managed_from_disk(tmp_path, monkeypatch):
    """managed-provider OAuth writes config to disk; wizard config dict must pick it up."""
    monkeypatch.setenv("CLIO_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    _stub_tts(monkeypatch)
...
def test_setup_gateway_in_container_shows_docker_guidance(monkeypatch, capsys):
    ...
    monkeypatch.setattr(clio_constants, "is_container", lambda: True)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("clio_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("clio_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "clio_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "clio_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_managed_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("clio_cli.setup.managed_tools_enabled", lambda: True)
    monkeypatch.setenv("CLIO_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("clio_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("clio_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "clio_cli.setup.get_managed_subscription_features",
        lambda config: type("Features", (), {"provider_auth_present": True})(),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    from clio_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("clio_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("clio_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "clio_cli.setup.get_managed_subscription_features",
        lambda config: type("Features", (), {"provider_auth_present": True})(),

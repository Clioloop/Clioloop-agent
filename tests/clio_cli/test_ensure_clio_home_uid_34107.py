"""Regression tests for #34107 — Docker UID/GID handling in ensure_clio_home.

When Clio runs in Docker with ``CLIO_UID=1000`` / ``CLIO_GID=911``,
the entrypoint chowns the top-level ``CLIO_HOME`` once at startup. But
...
The fix is a ``_chown_to_clio_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_clio_uid_gid
# ---------------------------------------------------------------------------


class TestResolveClioUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("CLIO_UID", "1000")
        monkeypatch.setenv("CLIO_GID", "911")
        from clio_cli.config import _resolve_clio_uid_gid
        uid, gid = _resolve_clio_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("CLIO_UID", raising=False)
        monkeypatch.delenv("CLIO_GID", raising=False)
        from clio_cli.config import _resolve_clio_uid_gid
        uid, gid = _resolve_clio_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("CLIO_UID", "not-a-number")
        monkeypatch.setenv("CLIO_GID", "911")
        from clio_cli.config import _resolve_clio_uid_gid
        uid, gid = _resolve_clio_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("CLIO_UID", "")
        monkeypatch.setenv("CLIO_GID", "")
        from clio_cli.config import _resolve_clio_uid_gid
        uid, gid = _resolve_clio_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToClioUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_clio_uid(d)  # must not raise

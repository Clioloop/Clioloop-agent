...
def test_loopback_protected_route_accepts_session_token(client_loopback):
    ...
    r = client_loopback.get(
        "/api/sessions",
        headers={"X-Clio-Session-Token": web_server._SESSION_TOKEN},
    )
...
def test_loopback_index_injects_session_token(client_loopback):
    ...
    assert "__CLIO_SESSION_TOKEN__" in r.text
...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    ("clio-agent-prod-abc.fly.dev", False, True),
])
def test_should_require_auth_truth_table(host, allow_public, expected):
    from clio_cli.web_server import should_require_auth
...
def test_start_server_public_without_insecure_records_auth_required(monkeypatch):
    ...
    """
    from clio_cli.dashboard_auth import clear_providers
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    web_server.app.state.auth_required = None
...
@pytest.mark.skip(reason="managed provider plugin moved to rebrand-archive in Clio rebrand; this test no longer applicable")
def test_start_server_surfaces_skip_reason_when_unconfigured(monkeypatch):
    """When the bundled managed provider plugin loaded but skipped registration (no
    ...
    'set CLIO_DASHBOARD_OAUTH_CLIENT_ID', not 'install a plugin'."""
    from clio_cli.dashboard_auth import clear_providers
    from plugins.dashboard_auth import managed as managed_plugin

    # Simulate the plugin running and skipping for "no client_id".
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    monkeypatch.delenv("CLIO_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
    monkeypatch.delenv("CLIO_DASHBOARD_PORTAL_URL", raising=False)
    from unittest.mock import MagicMock
    managed_plugin.register(MagicMock())  # populates LAST_SKIP_REASON
    assert "CLIO_DASHBOARD_OAUTH_CLIENT_ID" in managed_plugin.LAST_SKIP_REASON
    ...
    # than the generic "Install the default provider" boilerplate.
    msg = str(exc_info.value)
    assert "CLIO_DASHBOARD_OAUTH_CLIENT_ID" in msg
    assert "managed_provider:" in msg

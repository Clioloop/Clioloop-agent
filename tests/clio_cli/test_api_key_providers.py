...
from clio_cli.copilot_auth import _try_gh_cli_token
...
class TestProviderRegistry:
    ...
    def test_oauth_providers_unchanged(self):
        ...
        The managed provider returns as the Omni Loop Portal Subscription
        (device-code OAuth) alongside the xai-oauth + openai-codex external
        OAuth flows.
        """
        assert "managed" in PROVIDER_REGISTRY
        assert PROVIDER_REGISTRY["managed"].auth_type == "oauth_device_code"
        assert PROVIDER_REGISTRY["managed"].name == "Omni Loop Portal Subscription"
        assert PROVIDER_REGISTRY["managed"].portal_base_url.startswith("https://")
...
PROVIDER_ENV_VARS = (
    ...
    "CLIO_MANAGED_API_KEY", "GITHUB_TOKEN", "GH_TOKEN",
    "OPENAI_BASE_URL", "CLIO_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "CLIO_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("clio_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_non_api_key_provider(self):
        status = get_api_key_provider_status("managed")
        assert status["configured"] is False
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr("clio_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("clio_cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "clio_cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "clio_cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("clio_cli.copilot_auth.subprocess.run", _fake_run)
    ...
    def test_resolve_invalid_provider_raises(self):
        with pytest.raises(AuthError):
            resolve_api_key_provider_credentials("managed")
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("clio_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "clio_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "clio_cli.models.fetch_github_model_catalog",
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        ...
        clio_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: clio_home / ".env")
        monkeypatch.setattr(config_module, "get_clio_home", lambda: clio_home)
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when Clio is unconfigured."""
        from clio_cli import config as config_module
        ...
        monkeypatch.setattr(config_module, "get_env_path", lambda: clio_home / ".env")
        monkeypatch.setattr(config_module, "get_clio_home", lambda: clio_home)
        monkeypatch.setattr("clio_cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        clio_home = tmp_path / ".clio"
        clio_home.mkdir()
        config_file = clio_home / "config.yaml"
    ...
    def test_claude_code_creds_counted_when_clio_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Clio has been explicitly configured."""
        import yaml
        from clio_cli import config as config_module
        clio_home = tmp_path / ".clio"
        clio_home.mkdir()
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from clio_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from clio_cli.main import _PROVIDER_MODELS as main_models
        from clio_cli.models import _PROVIDER_MODELS as models_models
...
class TestNovitaProvider:
    ...
    def test_novita_pricing_cache(self, monkeypatch):
        ...
        from clio_cli import models as models_mod
...
class TestMinimaxOAuthProvider:
    ...
    def test_minimax_oauth_listed_in_canonical_providers(self):
        from clio_cli.models import CANONICAL_PROVIDERS

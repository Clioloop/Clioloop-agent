...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(auth_module, "_refresh_codex_auth_tokens", _fake_refresh)
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr(auth_module.httpx, "Client", _factory)
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    """
    CODEX_RATE_LIMITED_CODE = auth_module.CODEX_RATE_LIMITED_CODE
    format_auth_error = auth_module.format_auth_error
    is_rate_limited_auth_error = auth_module.is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(auth_module, "_save_codex_tokens", _fake_save)
    monkeypatch.setattr(auth_module, "_update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",

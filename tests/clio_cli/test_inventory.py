"""Behavior tests for clio_cli.inventory.
...
from clio_cli.inventory import (
    ConfigContext,
...
def test_load_picker_context_full_dict():
    ...
    with patch("clio_cli.config.load_config", return_value=cfg):
...
def test_build_models_payload_promotes_managed_first():
    """Desktop model pickers should receive Omni first even if substrate order differs."""
    rows = [
        {"slug": "openrouter", "name": "OpenRouter", "models": ["m1"],
         "total_models": 1, "is_current": True, "is_user_defined": False,
         "source": "built-in"},
        {"slug": "managed", "name": "Omni Loop Portal Subscription", "models": ["openai/gpt-oss-120b:free"],
    ...
    assert [row["slug"] for row in payload["providers"]] == ["managed", "openrouter", "anthropic"]
...
def test_build_models_payload_preserves_managed_curated_catalog():
    ...
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("clio_cli.models.provider_model_ids",
               return_value=["zeta", "alpha", "v/b", "v/a"]) as mock_pm:
        payload = build_models_payload(ctx)
    mock_pm.assert_not_called()
    managed = next(r for r in payload["providers"] if r["slug"] == "managed")
    assert managed["models"] == ["clio-4-405b"]
...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        assert (
            ...
            or row["warning"].startswith("run `clio model`")
        )

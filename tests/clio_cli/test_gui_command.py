"""Tests for ``clio gui`` desktop launcher wiring."""
...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Clio.app" / "Contents" / "MacOS" / "Clio"
    elif platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Clio.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "clio"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("clio_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("clio_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("clio_cli.main._desktop_build_needed", return_value=True), \
         patch("clio_cli.main._write_desktop_build_stamp"), \
         patch("clio_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("clio_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    root = _make_desktop_tree(tmp_path)
    clio_root = tmp_path / "custom-clio"
    ...
    with patch("clio_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("clio_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("clio_cli.main._desktop_build_needed", return_value=True), \
         patch("clio_cli.main._write_desktop_build_stamp"), \
         patch("clio_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("clio_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(
            fake_boot=True,
            ignore_existing=True,
            clio_root=str(clio_root),
            cwd=str(cwd),
        ))

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["CLIO_DESKTOP_BOOT_FAKE"] == "1"
    assert launch_env["CLIO_DESKTOP_IGNORE_EXISTING"] == "1"
    assert launch_env["CLIO_DESKTOP_CLIO_ROOT"] == str(clio_root)
    assert launch_env["CLIO_DESKTOP_CWD"] == str(cwd)
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("clio_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skip_build_launches_existing_packaged_app_without_npm(tmp_path, monkeypatch):
    ...
    with patch("clio_cli.main.shutil.which", return_value=None), \
         patch("clio_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("clio_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with patch("clio_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("clio_cli.main.subprocess.run", return_value=ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_rejects_symlink_sandbox(tmp_path, monkeypatch):
    ...
    with patch("clio_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("clio_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("clio_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("clio_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("clio_cli.main._desktop_build_needed", return_value=True), \
         patch("clio_cli.main._write_desktop_build_stamp"), \
         patch("clio_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_build_stamp_skips_build_when_up_to_date(tmp_path, monkeypatch):
    ...
    with patch("clio_cli.main._desktop_build_needed", return_value=False), \
         patch("clio_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("clio_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         patch("clio_cli.main._desktop_macos_relaunchable_fixup"), \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("clio_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("clio_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("clio_cli.main._desktop_build_needed", return_value=False), \
         patch("clio_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("clio_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("clio_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_compute_desktop_content_hash_stable(tmp_path, monkeypatch):
    ...
    (root / "package.json").write_text('{"name":"clio"}', encoding="utf-8")
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    with patch("clio_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("clio_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("clio_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("clio_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("clio_cli.main._write_desktop_build_stamp"), \
         patch("clio_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("clio_cli.main.subprocess.run", side_effect=[pack_fail, pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_does_not_retry_when_purge_finds_nothing(tmp_path, monkeypatch, capsys):
    ...
    with patch("clio_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("clio_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("clio_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("clio_cli.main._purge_electron_build_cache", return_value=[]) as mock_purge, \
         patch("clio_cli.main.subprocess.run", side_effect=[pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())

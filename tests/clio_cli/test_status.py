...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    ...
    assert "Start with:   clio gateway" in output
    assert "systemd (user)" not in output


@pytest.mark.skip(reason="managed provider / paid inference-key flow removed in Clio rebrand (phase 4)")
def test_show_status_reports_managed_auth_error(monkeypatch, capsys, tmp_path):
    from clio_cli import status as status_mod
    import clio_cli.auth as auth_mod
    import clio_cli.gateway as gateway_mod
    ...
    output = capsys.readouterr().out
    assert "managed provider   ✗ not logged in (run: clio portal)" in output
...
@pytest.mark.skip(reason="managed provider / paid inference-key flow removed in Clio rebrand (phase 4)")
def test_show_status_reports_managed_inference_key_without_portal_login(monkeypatch, capsys, tmp_path):
    from clio_cli import status as status_mod
    ...
    monkeypatch.setattr(status_mod, "managed_tools_enabled", lambda: False, raising=False)
    monkeypatch.setattr(auth_mod, "get_codex_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(auth_mod, "get_qwen_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda exclude_pids=None: [], raising=False)
    ...
    output = capsys.readouterr().out
    assert "managed provider   ✗ not logged in (managed provider inference key configured)" in output
    assert "Inference:  https://inference.example.com/v1" in output
    assert "managed provider inference credentials are configured" in output
...
class TestShowStatusXaiOAuth:
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        status_mod = _base_xai_mocks(monkeypatch, tmp_path)
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.clio/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.clio/auth.json" in out
    ...
    def test_not_logged_in_shows_login_command(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: clio auth add xai-oauth)" in out

...
def container_env(tmp_path, monkeypatch):
    """Set up a fake CLIO_HOME with .container-mode file."""
    clio_home = tmp_path / ".clio"
    clio_home.mkdir()
    monkeypatch.setenv("CLIO_HOME", str(clio_home))
    monkeypatch.delenv("CLIO_DEV", raising=False)

    container_mode = clio_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=clio-agent\n"
        "exec_user=clio\n"
        "clio_bin=/data/current-package/bin/clio\n"
    )
    return clio_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("clio_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "clio-agent"
    assert info["exec_user"] == "clio"
    assert info["clio_bin"] == "/data/current-package/bin/clio"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("clio_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_clio_dev(container_env, monkeypatch):
    """Returns None when CLIO_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("CLIO_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_clio_dev_zero(container_env, monkeypatch):
    """CLIO_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("CLIO_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        clio_home = Path(tmpdir) / ".clio"
        clio_home.mkdir()
        (clio_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("clio_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_clio_home": lambda: clio_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CLIO_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=clio-custom\n"
        "exec_user=myuser\n"
        "clio_bin=/opt/clio/bin/clio\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "clio-custom"
    assert info["exec_user"] == "myuser"
    assert info["clio_bin"] == "/opt/clio/bin/clio"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from clio_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "clio"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "clio-agent" in cmd
    assert "/data/current-package/bin/clio" in cmd
    assert "chat" in cmd

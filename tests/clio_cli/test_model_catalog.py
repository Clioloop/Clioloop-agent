"""Tests for clio_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate CLIO_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".clio"
    home.mkdir()
    ...
    import importlib
    from clio_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "managed": {
                "metadata": {"display_name": "managed provider"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from clio_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/Clioloop/Clioloop-agent"
...
class TestCuratedAccessors:
    ...
    def test_managed_returns_ids(self, isolated_home):
        from clio_cli import model_catalog
        with patch.object(
            model_catalog, "_fetch_manifest", return_value=_valid_manifest()
        ):
            result = model_catalog.get_curated_managed_models()
    ...
    def test_managed_returns_none_when_catalog_empty(self, isolated_home):
        from clio_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_managed_models() is None
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in clio_cli.models."""

    def test_curated_managed_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from clio_cli import model_catalog
        from clio_cli.models import get_curated_managed_model_ids, _PROVIDER_MODELS

        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            result = get_curated_managed_model_ids()

        assert result == list(_PROVIDER_MODELS["managed"])

    def test_curated_managed_ids_prefers_manifest(self, isolated_home):
        from clio_cli import model_catalog
        from clio_cli.models import get_curated_managed_model_ids
    ...
    def test_picker_managed_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        # ``CLIO_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` CLIO_HOME directly instead.
        import importlib
        from clio_cli import model_catalog
        from clio_cli.models import get_curated_managed_model_ids
        importlib.reload(model_catalog)
        try:
            from clio_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["CLIO_HOME"])
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"managed": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            with patch.object(
                ...
                "clio_cli.models.union_with_portal_free_recommendations",
        ...
        managed_row = next((r for r in picker if r["slug"] == "managed"), None)
        assert managed_row is not None, "managed row must appear when authed"
        assert managed_row["models"] == expected
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["managed"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['managed'] / OPENROUTER_MODELS. "

...
Covers:
 - Normal path (tokens already in Clio auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Clio
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `clio auth openai-codex`. The old
...
@pytest.fixture()
def clio_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Clio auth store (no Codex CLI needed)."""
    ...
    for var in [
        ...
        "CLIO_MANAGED_API_KEY", "DEEPSEEK_API_KEY",
    ]:
...
def test_normal_path_still_works(clio_auth_only_env):
    """openai-codex appears when tokens are already in Clio auth store."""
...
def test_codex_picker_uses_live_codex_catalog(clio_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Clio
    auth store."""
    clio_home = tmp_path / ".clio"
    clio_home.mkdir()
    ...
    (clio_home / "auth.json").write_text(
...
def test_no_codex_when_no_credentials(tmp_path, monkeypatch):
    ...
    for var in [
        "OPENROUTER_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY",
        "CLIO_MANAGED_API_KEY", "DEEPSEEK_API_KEY", "COPILOT_GITHUB_TOKEN",

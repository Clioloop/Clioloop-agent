"""Tests for the Clio plugin system (clio_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<clio_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<clio_home>/plugins/``; we derive
    ``<clio_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <CLIO_HOME>/config.yaml.
        # Config is always read from CLIO_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        clio_home_str = os.environ.get("CLIO_HOME")
        if clio_home_str:
            clio_home = Path(clio_home_str)
        else:
            clio_home = base.parent
        clio_home.mkdir(parents=True, exist_ok=True)
        cfg_path = clio_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.clio/plugins/ are discovered."""
        plugins_dir = tmp_path / "clio_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("CLIO_HOME", str(tmp_path / "clio_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".clio" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under clio_plugins.<name>."""
        plugins_dir = tmp_path / "clio_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("CLIO_HOME", str(tmp_path / "clio_test"))

        # Clean up any prior namespace module
        sys.modules.pop("clio_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "clio_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="clio_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import clio_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("clio_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "clio_test" / "plugins"
        # Set CLIO_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("clio_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("clio_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("clio_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("clio_cli.plugins.PluginContext.dispatch_tool.__module__", "clio_cli.plugins"):
...
class TestPluginDebugLogging:
    """CLIO_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("CLIO_PLUGINS_DEBUG", raising=False)
        from clio_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With CLIO_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("CLIO_PLUGINS_DEBUG", "1")
        from clio_cli import plugins as plugins_mod

"""Tests for `clio memory setup [provider]` routing.
...
``clio memory setup honcho``) without the interactive picker — which matters
because the per-provider ``clio <provider>`` subcommand is only registered
...
from clio_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "clio memory setup" in out

...
import pytest
from clio_cli import model_switch
...
def test_openrouter_falls_back_to_base_models_on_fetch_failure(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "list_authenticated_providers",
                        lambda **kw: list(base))
    monkeypatch.setattr("clio_cli.models.fetch_openrouter_models", _raise)
...
def test_managed_provider_promoted_first(monkeypatch):
    """Omni is the recommended provider, so picker rows always start there."""
    base = [
        _make_provider("anthropic", models=["claude-sonnet-4-6"]),
        _make_provider("managed", name="Omni Loop Portal Subscription", models=["openai/gpt-oss-120b:free"]),
        _make_provider("openai-codex", models=["gpt-5.3-codex"]),
    ]
    ...
    assert [p["slug"] for p in result] == ["managed", "anthropic", "openai-codex"]

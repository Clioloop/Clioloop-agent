...
def _client():
    ...
    # Keep the state DB under the isolated CLIO_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_clio_home):
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_clio_home):
        ...
        (get_clio_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "clio_version"):
...
class TestUpdateCheckEndpoint:
    """``GET /api/clio/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/clio/update`` runs ``clio update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/clio/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/clio/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        ...
        monkeypatch.setattr("clio_cli.dump.run_dump", lambda a: None)

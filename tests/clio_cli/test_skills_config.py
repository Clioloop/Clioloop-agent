"""Tests for clio_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("clio_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from clio_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("clio_cli.config.load_config")
    ...
    @patch("clio_cli.config.load_config")
    @patch.dict("os.environ", {"CLIO_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """CLIO_SESSION_PLATFORM should be used when CLIO_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_clio_platform_takes_precedence(self, tmp_path, monkeypatch):
        """CLIO_PLATFORM should win over CLIO_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_HOME", str(tmp_path))
        monkeypatch.setenv("CLIO_PLATFORM", "telegram")
        monkeypatch.setenv("CLIO_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from clio_cli.skills_config import _get_categories
        skills = [

...
Covers:
- Defense 1: gateway stop/restart refuse when _CLIO_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "clio gateway stop",
        "clio gateway start",
        "clio  gateway  restart",         # double spaces
        "Clio Gateway Restart".lower(),  # case handled
        "CLIO GATEWAY RESTART",           # uppercase
    ])
    def test_clio_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.clio.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.clio.gateway.plist",
        "launchctl stop ai.clio.gateway",
        "systemctl restart clio-gateway",
        "systemctl stop clio-gateway.service",
        "systemctl start clio-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill clio gateway process",
        "pkill -f clio.*gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_clio_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade clio then run clio gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.clio.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify clio gateway stop/restart refuse when _CLIO_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_CLIO_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_CLIO_GATEWAY", raising=False)

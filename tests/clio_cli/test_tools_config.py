...
def test_visible_providers_include_managed_subscription_when_logged_in(monkeypatch):
    ...
    assert any(p["name"].startswith("Omni Loop Portal Subscription") for p in providers)
...
def test_visible_providers_show_managed_subscription_when_paid_access_is_false(monkeypatch):
    ...
    shown; ``ensure_managed_provider_access`` blocks activation if still unpaid.
    """
    config = {"model": {"provider": "managed"}}
...
def test_fresh_install_browser_default_is_free_local_not_paid_managed():
    ...
    backend, never the paid Omni Loop Portal Subscription gateway.
...
def test_tts_gateway_selects_omni_loop_row_not_openai(monkeypatch):
    ...
    'Omni Loop Portal Subscription' row is the active selection — not the bare 'OpenAI TTS'

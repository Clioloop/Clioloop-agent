"""Tests for clio_cli.tools_config platform tool persistence."""
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `clio tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "clio_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "clio_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `clio tools`,
    ...
    # User explicitly opted into spotify but not x_search via `clio tools`.
    config = {"platform_toolsets": {"cli": ["clio-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[clio-cli, spotify]`` (composite + configurable) must keep the full
    ``clio-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``clio-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when clio-cli is listed"
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``clio-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["clio-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _CLIO_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `clio tools`
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("clio_cli.tools_config.save_config"):
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/Clioloop/Clioloop-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (clio-cli, clio-telegram, etc.) must NOT
    ...
    ``clio tools``, but clio-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "clio-cli",  # <-- the culprit
    ...
    # clio-cli must NOT survive — it's a platform default, not an MCP server
    assert "clio-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_clio_telegram():
    """Same bug for Telegram — clio-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "clio-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "clio-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "clio-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_managed_subscription_when_logged_in(monkeypatch):
    config = {"model": {"provider": "managed"}}
    ...
    # The managed provider row is listed (not necessarily first — "Local Browser"
    # sorts first so a fresh-install Enter lands on the free local backend).
    assert any(p["name"].startswith("managed provider Subscription") for p in providers)
    # "Local Browser" must be the index-0 default so pressing Enter never
    # walks a user into a paid managed provider login.
    assert providers[0]["name"] == "Local Browser"


def test_visible_providers_show_managed_subscription_when_logged_out(monkeypatch):
    """managed-provider Tool Gateway rows are always listed, even logged out.
...
def test_visible_providers_show_managed_subscription_when_paid_access_is_false(monkeypatch):
    ...
    shown; ``ensure_managed_portal_access`` blocks activation if still unpaid.
    """
    config = {"model": {"provider": "managed"}}
...
def test_visible_providers_force_fresh_shows_managed_subscription_after_upgrade(monkeypatch):
    calls = []
    ...
    def fake_subscription_features(config, *, force_fresh=False):
        ...
        return SimpleNamespace(
            ...
            account_info=ManagedProviderAccountInfo(
                logged_in=True,
    ...
    # The managed provider row reappears after the entitlement upgrade. It is no
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.tools_config._run_post_setup", lambda key: None)
    _configure_provider(local_provider, config)

    assert config["browser"]["cloud_provider"] == "local"


@pytest.mark.skip(reason="managed provider Subscription / paid gateway default-fallback path removed in Clio rebrand (phase 4)")
def test_fresh_install_browser_default_is_free_local_not_paid_managed():
    """On a fresh install the browser picker must default to the free local
    backend, never the paid managed provider Subscription gateway.

    Regression: the managed provider row used to sort first, so the menu cursor defaulted
    to index 0 (managed provider) and pressing Enter walked users straight into a managed provider
    Portal login for a paid offering (Javier's bug, June 2026).
    """
    from clio_cli.tools_config import _detect_active_provider_index
...
@pytest.mark.skip(reason="managed provider Subscription / paid gateway default-fallback path removed in Clio rebrand (phase 4)")
def test_fresh_install_tts_default_is_free_edge_not_paid_managed():
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "clio_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("clio_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "clio_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("clio_cli.tools_config.save_config", lambda config: None)
...
@pytest.mark.skip(reason="apply_managed_defaults stubbed out in Clio rebrand (phase 4); auto-config managed provider path no longer runs")
def test_first_install_managed_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "clio_cli.tools_config._prompt_toolset_checklist",
    ...
    # apply_managed_defaults per platform; a second iteration sees values
    # set by the first as "explicit" and skips them.
    monkeypatch.setattr(
        "clio_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "clio_cli.portal_subscription.get_managed_portal_account_info",
    ...
    configured = []
    monkeypatch.setattr(
        "clio_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from clio_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """clio-gateway includes list should cover all messaging platforms."""
        from clio_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["clio-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from clio_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from clio_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/Clioloop/Clioloop-agent/issues/6901
    """
    config = {
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `clio tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's CLIO_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"CLIO_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"CLIO_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from clio_cli.tools_config import IMAGEGEN_BACKENDS
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from clio_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("clio_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("clio_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("clio_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`clio tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["clio-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "clio-_test_platform"},
    }

    with mock_patch("clio_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'clio-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `clio tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from clio_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `clio tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from clio_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `clio tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from clio_cli.tools_config import _get_effective_configurable_toolsets
...
@pytest.mark.skip(reason="Managed Tool Gateway / managed subscription removed in Clio rebrand (phase 12)")
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "clio_cli.portal_subscription.ensure_managed_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("clio_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("clio_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("clio_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("clio_cli.tools_config.save_env_value", lambda k, v: None)
...
# ---------------------------------------------------------------------------
# Inline managed provider login gate on managed-provider selection
# ---------------------------------------------------------------------------


@pytest.mark.skip(reason="Managed Tool Gateway / managed subscription removed in Clio rebrand (phase 12)")
def test_configure_managed_provider_blocks_when_not_entitled(monkeypatch):
    """Selecting a managed-provider backend without paid access writes no config."""
    monkeypatch.setattr(
        "clio_cli.portal_subscription.ensure_managed_portal_access",
        lambda **kwargs: False,
    )
    provider = {
        "name": "managed provider Subscription (Firecrawl)",
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    """A self-hosted provider must never trigger the managed provider login gate."""
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from clio_cli.tools_config import apply_provider_selection
...
#    reported as added/removed by `clio tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `clio tools` printed ``- kanban``

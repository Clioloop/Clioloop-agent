"""Tests for clio_cli.security_advisories.
...
import clio_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect CLIO_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".clio"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full clio_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "clio_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "clio_cli.config.save_config",

"""Tests for the ``clio hooks`` CLI subcommand."""
...
from clio_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("CLIO_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("CLIO_ACCEPT_HOOKS", raising=False)
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `clio hooks doctor` used to run every

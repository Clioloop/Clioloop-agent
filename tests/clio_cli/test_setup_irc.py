"""Tests for IRC gateway configuration via `clio setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Clio install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Clio install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#clio")
            monkeypatch.setenv("IRC_NICKNAME", "clio-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `clio setup gateway` experience with IRC present."""

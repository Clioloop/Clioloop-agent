"""Tests for clio_cli configuration management."""
...
class TestGetClioHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            home = get_clio_home()
            assert home == Path.home() / ".clio"

    def test_env_override(self):
        with patch.dict(os.environ, {"CLIO_HOME": "/custom/path"}):
            home = get_clio_home()
            assert home == Path("/custom/path")


class TestEnsureClioHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"CLIO_HOME": str(tmp_path)}):
            ensure_clio_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``clio logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from clio_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"CLIO_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="clio_cli.config"):
                config = load_config()
            ...
            # stands out at clio startup before logging is configured)
            captured = capsys.readouterr()
            assert "clio config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"CLIO_HOME": str(tmp_path)}):
            ...
            assert "clio config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"CLIO_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"CLIO_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"CLIO_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"CLIO_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"CLIO_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from clio_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """CLIO_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `clio setup`) lets a
        ...
        via config.yaml; CLIO_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from clio_cli.config import OPTIONAL_ENV_VARS
        assert "CLIO_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``CLIO_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Clio
    process on next startup via the dotenv → ``os.environ`` chain in
    ``clio_cli/env_loader.py``.
    ...
    def _clio_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "CLIO_SPOTIFY_CLIENT_ID",
            "CLIO_QWEN_BASE_URL",
            "CLIO_MAX_ITERATIONS",
        ],
    )
    def test_clio_integration_keys_still_writable(self, allowed_key):
        """``CLIO_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``CLIO_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``CLIO_*``) keep working. The denylist

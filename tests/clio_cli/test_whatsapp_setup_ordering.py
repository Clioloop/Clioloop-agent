...
Before the fix, ``clio whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``clio gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CLIO_HOME", str(clio))
...
def _env_value(clio_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``clio whatsapp`` with an existing paired session and

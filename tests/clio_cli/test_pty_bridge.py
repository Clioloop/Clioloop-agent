"""Unit tests for clio_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf clio-ok"])
        try:
            output = _read_until(bridge, b"clio-ok")
            assert b"clio-ok" in output
        finally:
            bridge.close()
...
class TestClampDimension:
    def test_clamps_above_max(self):
        from clio_cli.pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp_dimension

        assert _clamp_dimension(131072, _MAX_COLS) == _MAX_COLS
        assert _clamp_dimension(131072, _MAX_ROWS) == _MAX_ROWS

    def test_floors_at_one(self):
        from clio_cli.pty_bridge import _MAX_COLS, _clamp_dimension
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$CLIO_PTY_TEST\""],
            env={**os.environ, "CLIO_PTY_TEST": "pty-env-works"},
        )
        try:

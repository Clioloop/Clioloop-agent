...
class TestDeveloperRoleSwap:
    ...
    def test_developer_role_via_managed_portal(self, monkeypatch):
        agent = _make_agent(
            monkeypatch,
            "managed",
            base_url="https:///v1",
...
class TestBuildApiKwargsManagedPortal:
    ...
    def test_includes_managed_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == request_tags()
...
# ── Chat completions response handling (OpenRouter/managed provider) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_managed_when_no_openrouter(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_managed_auth", return_value=managed_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("clio_cli.models.get_managed_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == "google/gemini-3-flash-preview"

    def test_custom_endpoint_when_no_managed(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/managed provider keys are available.

...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    clio_state = ModuleType("clio_state")
    clio_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "clio_state", clio_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Clio"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Clio"

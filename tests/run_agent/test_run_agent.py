...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._clio_home / "logs" / "errors.log").resolve()
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host CLIO_REDACT_SECRETS state.
...
class TestBuildSystemPrompt:
    ...
    def test_user_profile_only_guidance_when_notes_store_disabled(
    ...
    def test_no_builtin_memory_guidance_when_both_stores_disabled(
        self, agent_with_memory_tool
    ):
        from agent.prompt_builder import MEMORY_GUIDANCE, USER_PROFILE_GUIDANCE
    ...
    def test_includes_managed_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_managed_subscription_prompt", lambda tool_names: "MANAGED SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "MANAGED SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_managed_route(self, agent):
        agent.provider = "managed"
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("clio_cli.plugins.invoke_hook", _capture_hook)
        monkeypatch.setattr("clio_cli.plugins.has_hook", lambda name: True)
...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(self, agent, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr(
            "clio_cli.plugins.get_pre_tool_call_block_message",
            lambda *args, **kwargs: None,
        )
        monkeypatch.setattr(
            "clio_cli.plugins.invoke_hook",
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Clio tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "clio_cli.plugins.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("clio_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("clio_cli.plugins.has_hook", lambda name: False)
        monkeypatch.setattr("clio_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("clio_cli.plugins.has_hook", _has_hook)
        ...
        with (
            patch("clio_cli.plugins.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_managed_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "managed"
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_managed_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("clio_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("clio_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when CLIO_KANBAN_TASK
...
class TestManagedCredentialRefresh:
    """Verify managed-provider credential refresh rebuilds the runtime client."""

    def test_try_refresh_managed_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "clio_cli.auth.resolve_managed_runtime_credentials", _fake_resolve
        )

        agent.client = _ExistingClient()
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_managed_client_credentials(force=True)

        assert ok is True
        assert closed["value"] is True
        assert captured["force_refresh"] is True
        assert rebuilt["kwargs"]["api_key"] == "new-managed-key"
        assert (
...
class TestGpt5ApiModeRouting:
    ...
    def test_managed_gpt5_stays_on_chat_completions(self, agent):
        """managed provider serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "managed"
        agent.base_url = "https:///v1"
        agent.api_mode = "chat_completions"
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Clio Agent" in agent._cached_system_prompt

...
AlexKucera reported on Discord (2026-04-16) that after ``clio update`` pulled
...
Opt-in — not part of default CI:
    CLIO_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.clio/.env).
"""
...
# Load ~/.clio/.env so live runs pick up OPENROUTER_API_KEY without
...
LIVE = os.environ.get("CLIO_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set CLIO_LIVE_TESTS=1"),

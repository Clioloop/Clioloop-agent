...
def test_background_review_summary_uses_clioloop_completed_prefix(monkeypatch):
    """The CLI/gateway emission must use the public Clioloop label.

    Users who miss the line in their terminal should see a clear branded
    completion notification before the skill/memory details.
    """
    import json
    ...
    # Exactly one summary should have been emitted, and it must use
    # the public Clioloop completion prefix.
    assert len(captured_prints) == 1, captured_prints
    printed = captured_prints[0]
    assert "Clioloop completed" in printed, printed
    assert "Memory updated" in printed, printed

    # Gateway path gets the same prefix.
    assert len(captured_bg_callback) == 1
    assert captured_bg_callback[0].startswith("💾 Clioloop completed:"), (

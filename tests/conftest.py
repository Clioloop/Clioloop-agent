"""Shared fixtures for the clio-agent test suite.
...
2. **Isolated CLIO_HOME.** CLIO_HOME points to a per-test tempdir so
   code reading ``~/.clio/*`` via ``get_clio_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".clio"`` instead of the canonical
   ``get_clio_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No CLIO_SESSION_* inheritance** — the agent's current gateway
...
# CLIO_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_CLIO_BEHAVIORAL_VARS = frozenset({
    "CLIO_YOLO_MODE",
    "CLIO_INTERACTIVE",
    "CLIO_QUIET",
    "CLIO_TOOL_PROGRESS",
    ...
    "CLIO_SESSION_CHAT_ID",
    "CLIO_SESSION_CHAT_NAME",
    "CLIO_SESSION_THREAD_ID",
    "CLIO_SESSION_SOURCE",
    "CLIO_SESSION_KEY",
    "CLIO_GATEWAY_SESSION",
    "CLIO_CRON_SESSION",
    "_CLIO_GATEWAY",
    "CLIO_PLATFORM",
    "CLIO_MODEL",
    "CLIO_INFERENCE_MODEL",
    "CLIO_INFERENCE_PROVIDER",
    "CLIO_TUI_PROVIDER",
    "CLIO_MANAGED",
    "CLIO_DEV",
    "CLIO_CONTAINER",
    ...
    "CLIO_TIMEZONE",
    "CLIO_REDACT_SECRETS",
    "CLIO_BACKGROUND_NOTIFICATIONS",
    "CLIO_EXEC_ASK",
    "CLIO_HOME_MODE",
    "CLIO_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.clio/kanban.db instead of the per-test CLIO_HOME.
    "CLIO_KANBAN_DB",
    "CLIO_KANBAN_BOARD",
    "CLIO_KANBAN_HOME",
    "CLIO_KANBAN_WORKSPACES_ROOT",
    "CLIO_KANBAN_LOGS_ROOT",
    "CLIO_KANBAN_TASK",
    "CLIO_KANBAN_WORKSPACE",
    "CLIO_KANBAN_RUN_ID",
    "CLIO_KANBAN_CLAIM_LOCK",
    "CLIO_KANBAN_DISPATCH_IN_GATEWAY",
    "CLIO_TENANT",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth managed plugin auto-registers itself on plugin discovery,
    ...
    "CLIO_DASHBOARD_OAUTH_CLIENT_ID",
    "CLIO_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and CLIO_HOME to per-test tempdirs so code that
    reads ``~/.clio/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral CLIO_* vars that could change test semantics.
    for name in _CLIO_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect CLIO_HOME to a per-test tempdir. Code that reads
    #    ``~/.clio/*`` via ``get_clio_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.clio/*`` via
    #    ``Path.home() / ".clio"`` instead of ``get_clio_home()``
    #    is a bug to fix at the callsite.
    fake_clio_home = tmp_path / "clio_test"
    fake_clio_home.mkdir()
    (fake_clio_home / "sessions").mkdir()
    (fake_clio_home / "cron").mkdir()
    (fake_clio_home / "memories").mkdir()
    (fake_clio_home / "skills").mkdir()
    monkeypatch.setenv("CLIO_HOME", str(fake_clio_home))
    ...
    #    ~/.clio/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import clio_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_clio_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal clio config dict suitable for unit tests."""
    return {
...
def _ensure_current_event_loop(request):
    ...
    Python 3.11+ no longer guarantees a current loop for plain synchromanaged tests.
...
# environment and finds the developer's live ``clio-gateway`` process
...
# correlated with a ``tests/clio_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> clio-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart clio-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting clio/python patterns are also blocked.
    """
    ...
    def _matches_clio_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _CLIO_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_clio_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch clio-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m clio_cli.main".
                if (
                    "clio" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live clio-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting clio/python could hit the live gateway. "

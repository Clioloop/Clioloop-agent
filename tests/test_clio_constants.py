"""Tests for clio_constants module."""
...
class TestGetDefaultClioRoot:
    """Tests for get_default_clio_root() — Docker/custom deployment awareness."""

    def test_no_clio_home_returns_native(self, tmp_path, monkeypatch):
        """When CLIO_HOME is not set, returns ~/.clio."""
        monkeypatch.delenv("CLIO_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_clio_root() == tmp_path / ".clio"

    def test_clio_home_is_native(self, tmp_path, monkeypatch):
        """When CLIO_HOME = ~/.clio, returns ~/.clio."""
        native = tmp_path / ".clio"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLIO_HOME", str(native))
        assert get_default_clio_root() == native

    def test_clio_home_is_profile(self, tmp_path, monkeypatch):
        """When CLIO_HOME is a profile under ~/.clio, returns ~/.clio."""
        native = tmp_path / ".clio"
    ...
    def test_clio_home_is_docker(self, tmp_path, monkeypatch):
        """When CLIO_HOME points outside ~/.clio (Docker), returns CLIO_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLIO_HOME", str(docker_home))
        assert get_default_clio_root() == docker_home

    def test_clio_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any CLIO_HOME outside ~/.clio is treated as the root."""
        custom = tmp_path / "my-clio-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLIO_HOME", str(custom))
        assert get_default_clio_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (CLIO_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLIO_HOME", str(profile))
        assert get_default_clio_root() == docker_root

    def test_no_clio_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\clio, not ~/.clio."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("CLIO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(clio_constants.sys, "platform", "win32")

        assert get_default_clio_root() == local_appdata / "clio"

    def test_no_clio_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/clio without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("CLIO_HOME", raising=False)
        ...
        assert get_default_clio_root() == home / "AppData" / "Local" / "clio"


class TestGetClioHome:
    """Tests for get_clio_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When CLIO_HOME is unset on Windows, use %LOCALAPPDATA%\\clio."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("CLIO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(clio_constants.sys, "platform", "win32")
        monkeypatch.setattr(clio_constants, "_profile_fallback_warned", False)

        assert get_clio_home() == local_appdata / "clio"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(clio_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(clio_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".clio"

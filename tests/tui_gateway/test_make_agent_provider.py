...
def test_make_agent_ignores_display_personality_without_system_prompt():
    ...
    fake_cfg = {
        "agent": {
            "system_prompt": "",
            "personalities": {"clio-spinner": "sparkle system prompt"},
        },
        "display": {"personality": "clio-spinner"},
        "model": {"default": "glm-5"},
    }
...
def test_make_agent_honors_tui_launch_env_flags():
    ...
    with (
        patch.dict(
            os.environ,
            {
                "CLIO_TUI_MAX_TURNS": "7",
                "CLIO_TUI_CHECKPOINTS": "1",
                "CLIO_TUI_PASS_SESSION_ID": "1",
                "CLIO_IGNORE_RULES": "1",
            },
        ),
        patch("tui_gateway.server._load_cfg", return_value=fake_cfg),
...
def test_make_agent_tolerates_null_config_sections():
    """Bare `agent:` / `display:` keys in ~/.clio/config.yaml parse as

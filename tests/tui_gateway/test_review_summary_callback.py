...
def server():
    with patch.dict(
        "sys.modules",
        {
            "clio_constants": MagicMock(
                get_clio_home=MagicMock(return_value="/tmp/clio_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    assert callable(cb), (
        ...
        "agent so the Clioloop completion summary is visible in the TUI."
    )
    ...
    cb("💾 Clioloop completed: Skill 'clio-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Clioloop completed: Skill 'clio-release' patched"
    }

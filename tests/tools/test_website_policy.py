...
def test_check_website_access_uses_dynamic_clio_home(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new CLIO_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old CLIO_HOME set by the autouse _isolate_clio_home fixture.
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing CLIO_HOME to tmp_path

...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad CLIO_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: CLIO_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("CLIO_GATEWAY_SESSION", "1")
    monkeypatch.delenv("CLIO_INTERACTIVE", raising=False)
    monkeypatch.delenv("CLIO_CRON_SESSION", raising=False)
    ...
    with A._lock:
        old_session_approved = set(A._session_approved.get(session_key, set()))
        old_permanent_approved = set(A._permanent_approved)
    ...
    try:
        ...
        with A._lock:
            if old_session_approved:
                A._session_approved[session_key] = old_session_approved
            else:
                A._session_approved.pop(session_key, None)
            A._permanent_approved.clear()
            A._permanent_approved.update(old_permanent_approved)
            A._session_yolo.discard(session_key)
            A._gateway_queues.pop(session_key, None)
            A._gateway_notify_cbs.pop(session_key, None)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("CLIO_CRON_SESSION", "1")
    monkeypatch.delenv("CLIO_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_clio_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "CLIO_HOME": "/h", "CLIO_PROFILE": "p",
        "CLIO_CONFIG": "/c.yaml", "CLIO_ENV": "/e",
        # other CLIO_* → dropped (broad prefix removed)
        "CLIO_BASE_URL": "https://x", "CLIO_INTERACTIVE": "1",
        "CLIO_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("CLIO_HOME", "CLIO_PROFILE", "CLIO_CONFIG", "CLIO_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "CLIO_BASE_URL", "CLIO_INTERACTIVE", "CLIO_KANBAN_DB",
...
def test_env_scrub_logs_dropped_clio_vars(caplog):
    """Dropping a non-allowlisted, non-secret CLIO_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset CLIO_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "CLIO_HOME": "/h",          # allowlisted → kept, not logged
        "CLIO_BASE_URL": "https://x",   # dropped → logged
        "CLIO_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "CLIO_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "CLIO_HOME" in out and "PATH" in out
    assert "CLIO_BASE_URL" not in out and "CLIO_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "CLIO_BASE_URL" in msgs and "CLIO_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "CLIO_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped CLIO_* vars."""
    import logging

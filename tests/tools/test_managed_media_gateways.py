...
def _enable_managed_tools(monkeypatch):
    """Patch the source modules so managed_tools_enabled() returns True
...
def test_managed_fal_submit_uses_gateway_origin_and_managed_token(monkeypatch):
    captured = {}
...
def test_video_gen_managed_fal_submit_uses_gateway(monkeypatch):
    ...
    assert captured["submit_via"] == "managed_client"
    assert captured["client_key"] == "managed-video-token"

...
def fake_clio(tmp_path, monkeypatch):
    """Build a two-profile Clio layout and point CLIO_HOME at
    the clio-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setenv("CLIO_HOME", str(sec_home))

    import clio_constants
    monkeypatch.setattr(clio_constants, "get_default_clio_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_clio_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_clio):
        from tools.file_tools import write_file_tool
        target = fake_clio["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_clio):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_clio["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "clio-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_clio):
    ...
    def test_non_clio_path_unaffected(self, fake_clio, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_clio):
    ...
    def test_cross_profile_patch_bypass(self, fake_clio):
        from tools.file_tools import patch_tool
        target = fake_clio["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_clio):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_clio, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_clio["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up CLIO_HOME (set in
        ...
        assert "not found in active profile 'clio-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_clio, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_clio["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_clio["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.clio/profiles/<name>/."""
        # Don't set CLIO_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_clio_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_clio_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_clio):
        """When active profile is 'clio-security', the prompt warns
        ...
        assert "Active Clio profile" in src
        assert "cross_profile=True" in src
        assert "~/.clio/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Clio profile: default" in src
        assert "Active Clio profile: {active_profile}" in src

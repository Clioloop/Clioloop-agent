"""Regression test for the ``ClioMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
@pytest.mark.asyncio
async def test_clio_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    assert _CLIO_PROVIDER_CLS is not None, "SDK OAuth types must be available"
...
@pytest.mark.asyncio
async def test_clio_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):

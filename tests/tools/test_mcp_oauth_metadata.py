...
Covers:
- :class:`ClioTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.ClioMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _CLIO_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# ClioTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = ClioTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CLIO_HOME", str(tmp_path))
        storage = ClioTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (ClioMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: ClioTokenStorage, **context_attrs):
    ...
    """
    if _CLIO_PROVIDER_CLS is None:
        pytest.skip("MCP SDK auth not available")
    provider = _CLIO_PROVIDER_CLS.__new__(_CLIO_PROVIDER_CLS)
    provider._clio_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_HOME", str(tmp_path))
        storage = ClioTokenStorage("mgr-srv")
        ...
        with patch.object(
            _CLIO_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_initialize_skips_restore_when_in_memory_present(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_HOME", str(tmp_path))
        storage = ClioTokenStorage("mgr-srv2")
    ...
    def test_persist_metadata_if_changed_writes_on_first_discover(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_HOME", str(tmp_path))
        storage = ClioTokenStorage("persist-srv")
        assert storage.load_oauth_metadata() is None
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_HOME", str(tmp_path))
        storage = ClioTokenStorage("noop-srv")
        ...
        with patch.object(
            ClioTokenStorage, "save_oauth_metadata"
        ) as save_spy:
            provider._persist_oauth_metadata_if_changed()
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLIO_HOME", str(tmp_path))
        storage = ClioTokenStorage("flow-srv")
        ...
        with patch.object(
            _CLIO_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",

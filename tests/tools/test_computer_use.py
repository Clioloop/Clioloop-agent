...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"CLIO_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Clio-managed
...
class TestClickButtonPassthrough:
    """Surface 5 (the cua-driver integration) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (the cua-driver integration): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (the cua-driver integration): instead of hardcoding
    ...
    the MCP subcommand doesn't require a Clio patch.
    ...
    def test_future_renamed_subcommand_is_honored(self):
        ...
        instead of `mcp` keeps working without a Clio patch."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so CLIO_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (the cua-driver integration): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (the cua-driver integration): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (the cua-driver integration): trycua/cua#1961 added
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): Clio never declared a cua-driver
    ...
    def test_session_id_format(self):
        from tools.computer_use.cua_backend import CuaDriverBackend
        backend = CuaDriverBackend()
        # clio-{12 hex chars} — short enough to surface in logs
        # without being a privacy hazard, unique enough for concurrent runs.
        assert backend._session_id.startswith("clio-")
        assert len(backend._session_id) == len("clio-") + 12

    def test_session_id_unique_per_backend(self):
        from tools.computer_use.cua_backend import CuaDriverBackend
        a = CuaDriverBackend()._session_id
        b = CuaDriverBackend()._session_id
        assert a != b, "each Clio run should mint its own session id"

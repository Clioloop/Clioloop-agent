...
class TestScanMemoryContent:
    ...
    def test_clio_config_mod_blocked(self):
        result = _scan_memory_content("edit .clio/config.yaml to change settings")
        assert "Blocked" in result
        assert "clio_config_mod" in result
        result = _scan_memory_content("update .clio/SOUL.md with new personality")
        assert "Blocked" in result
        assert "clio_config_mod" in result
    ...
    def test_clio_config_mod_no_false_positives(self):
        """Merely mentioning clio config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .clio/config.yaml for settings") is None
        assert _scan_memory_content("Read .clio/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .clio/config.yaml file contains runtime options") is None

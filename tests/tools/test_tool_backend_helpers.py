...
Tests cover:
- managed_tools_enabled() subscription-based gate
...
# managed_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedToolsEnabled:
    """Subscription-based gate: True for paid managed provider subscribers."""
    ...
    def test_disabled_when_not_logged_in(self, monkeypatch):
        ...
        assert managed_tools_enabled() is False
    ...
    def test_enabled_for_paid_subscriber(self, monkeypatch):
        ...
        assert managed_tools_enabled() is True
    ...
    def test_force_fresh_is_forwarded(self, monkeypatch):
        ...
        assert managed_tools_enabled(force_fresh=True) is True
        assert calls == [True]
...
class TestManagedToolGatewayUnavailableMessage:
    """Actionable guidance strings for unavailable gateway capabilities."""

    def test_logged_out_points_at_portal_setup(self, monkeypatch):
        monkeypatch.setattr(
            "clio_cli.portal_account.get_managed_portal_account_info",
            lambda force_fresh=False: ManagedProviderAccountInfo(
                logged_in=False, source="none", fresh=False,
            ),
        )
        message = managed_tool_gateway_unavailable_message("image generation")
        assert "image generation" in message
        assert "clio setup --portal" in message

    def test_logged_in_unentitled_points_at_pricing(self, monkeypatch):
        monkeypatch.setattr(
            "clio_cli.portal_account.get_managed_portal_account_info",
            lambda force_fresh=False: ManagedProviderAccountInfo(
                logged_in=True,
                ...
                fresh=True,
                paid_service_access=False,
                raw={"portal_url": "https://portal.example.test"},
            ),
        )
        message = managed_tool_gateway_unavailable_message("image generation")
        assert "image generation" in message
        assert "https://portal.example.test/pricing" in message

    def test_failure_falls_back_to_generic_hint(self, monkeypatch):
        monkeypatch.setattr(
            "clio_cli.portal_account.get_managed_portal_account_info",
            _raise_import,
        )
        message = managed_tool_gateway_unavailable_message()
        assert "clio setup --portal" in message
...
class TestResolveModalBackendState:
    ...
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, managed_enabled=False):
        ...
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_tools_enabled",
            lambda: managed_enabled,
        )
    ...
    def test_auto_managed_ready_but_managed_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, managed_enabled=False)
        assert result["selected_backend"] == "direct"

    def test_auto_nothing_when_only_managed_and_managed_disabled(self, monkeypatch):
    ...
    def test_managed_blocked_when_managed_disabled(self, monkeypatch):

...
The managed Omni Loop Portal Subscription gateway is a self-hosted Supertonic server that
speaks the OpenAI ``/v1/audio/speech`` dialect, so the config provider stays
``"openai"``. The tool result/log must surface ``"Omni Loop Portal Subscription"`` when the
...
def test_openai_via_managed_gateway_shows_portal_brand(monkeypatch):
    ...
    assert tts_tool._provider_display("openai") == "Omni Loop Portal Subscription"

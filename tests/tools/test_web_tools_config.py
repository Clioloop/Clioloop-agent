...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.web_tools.managed_tools_enabled", return_value=True),
            patch("tools.managed_tool_gateway.managed_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("tools.web_tools.Firecrawl"):
            with patch("tools.web_tools._read_managed_access_token", return_value=None):
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "tools.example.com"}):
            with patch("tools.web_tools._read_managed_access_token", return_value="managed-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    from tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="managed-token",
                        api_url="https://firecrawl-gateway.tools.example.com",
                    )
    ...
    def test_tool_gateway_scheme_can_switch_derived_gateway_origin_to_http(self):
        ...
        with patch.dict(os.environ, {
            "TOOL_GATEWAY_DOMAIN": "tools.example.com",
            ...
            with patch("tools.web_tools._read_managed_access_token", return_value="managed-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="managed-token",
                        api_url="http://firecrawl-gateway.tools.example.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_default_gateway_domain_targets_managed_production_origin(self):
        """Default gateway origin is the Omni Loop Portal's built-in tool gateway."""
        import tools.managed_tool_gateway as _mtg

        env = {"TOOL_GATEWAY_DOMAIN": "", "FIRECRAWL_GATEWAY_URL": "",
               "CLIO_PORTAL_BASE_URL": "", "MANAGED_PORTAL_BASE_URL": ""}
        with patch.dict(os.environ, env):
            with patch.object(_mtg, "_read_managed_provider_state",
                              return_value={"portal_base_url": "https://portal.example.com"}):
                with patch("tools.web_tools._read_managed_access_token", return_value="managed-token"):
                    with patch("tools.web_tools.Firecrawl") as mock_fc:
                        from tools.web_tools import _get_firecrawl_client
                        _get_firecrawl_client()
                        mock_fc.assert_called_once_with(
                            api_key="managed-token",
                            api_url="https://portal.example.com/api/gateway/firecrawl",
                        )

    def test_managed_auth_token_respects_clio_home_override(self, tmp_path):
        """Auth lookup should read from CLIO_HOME/auth.json, not ~/.clio/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".clio").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "CLIO_HOME": str(clio_home),
        }, clear=False):
            import tools.web_tools
            importlib.reload(tools.web_tools)
            assert tools.web_tools._read_managed_access_token() == "managed-token"
...
class TestBackendSelection:
    ...
    ``clio tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_returns_true(self):
        with patch("tools.web_tools._peek_managed_access_token", return_value="managed-token"):
            from tools.web_tools import check_web_api_key
            assert check_web_api_key() is True
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "clio_cli.auth.resolve_managed_access_token",
            _record_refresh,
        )

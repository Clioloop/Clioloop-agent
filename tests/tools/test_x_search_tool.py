...
def test_x_search_posts_responses_request(monkeypatch):
    ...
    assert captured["headers"]["User-Agent"] == f"Clio-Agent/{__version__}"
...
def _no_xai_env(monkeypatch):
    ...
    for var in ("XAI_API_KEY", "XAI_BASE_URL", "CLIO_XAI_BASE_URL"):
...
def test_x_search_prefers_oauth_when_both_available(monkeypatch):
    """Both credentials present: OAuth wins (matches Omni Loop Labs's billing preference).
...
def test_x_search_returns_tool_error_when_no_credentials(monkeypatch):
    ...
    assert "clio auth add xai-oauth" in result
...
def test_x_search_honors_config_model_and_timeout(monkeypatch, tmp_path):
    ...
    # Patch the in-module config loader so tests don't touch ~/.clio/config.yaml.
    monkeypatch.setattr(

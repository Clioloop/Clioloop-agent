...
def _clean_state():
    ...
    saved = {}
    for k in ("CLIO_INTERACTIVE", "CLIO_GATEWAY_SESSION", "CLIO_EXEC_ASK", "CLIO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["CLIO_INTERACTIVE"] = "1"
        session_key = os.getenv("CLIO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No CLIO_INTERACTIVE or CLIO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True

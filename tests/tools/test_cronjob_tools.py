...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `clio-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.clio/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.clio/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because clio uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("CLIO_INTERACTIVE", raising=False)
        monkeypatch.delenv("CLIO_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("CLIO_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("CLIO_INTERACTIVE", false_like_value)
        monkeypatch.delenv("CLIO_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("CLIO_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["CLIO_INTERACTIVE", "CLIO_GATEWAY_SESSION", "CLIO_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("CLIO_INTERACTIVE", "CLIO_GATEWAY_SESSION", "CLIO_EXEC_ASK"):

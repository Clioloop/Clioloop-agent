...
def _restore_tool_modules():
    original_clio_home = os.environ.get("CLIO_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_clio_home is None:
            os.environ.pop("CLIO_HOME", None)
        else:
            os.environ["CLIO_HOME"] = original_clio_home
        _reset_modules(("tools", "clio_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    clio_cli = types.ModuleType("clio_cli")
    clio_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["clio_cli"] = clio_cli
    clio_home = tmp_path / "clio-home"
    os.environ["CLIO_HOME"] = str(clio_home)
    sys.modules["clio_cli.config"] = types.SimpleNamespace(
        get_clio_home=lambda: clio_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="clio-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": clio_home / "modal_snapshots.json",

...
class TestWriteDenyExactPaths:
    ...
    def test_clio_env(self):
        # ``.env`` under the active CLIO_HOME (profile-aware, not just
        # ``~/.clio``) must be write-denied. The hermetic test conftest
        # points CLIO_HOME at a tempdir — resolve via get_clio_home()
        # to match the denylist.
        from clio_constants import get_clio_home
        path = str(get_clio_home() / ".env")
        assert _is_write_denied(path) is True

    def test_clio_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.clio/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "clio_root"
        ...
        # Sanity check: CLIO_HOME does point to the profile dir, not the root.
        from clio_constants import get_clio_home, get_default_clio_root
        assert get_clio_home() == profile_home
        assert get_default_clio_root() == root
...
class TestWriteAllowed:
    ...
    def test_clio_config_not_env(self):
        path = os.path.join(str(Path.home()), ".clio", "config.yaml")

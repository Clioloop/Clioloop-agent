...
class TestIsWriteDenied:
    ...
    def test_clio_control_files_oauth_and_mcp_tokens_denied(self, path):
        """Clio control files, PKCE creds, mcp-tokens, and pairing entries must be write-denied."""
        from clio_constants import get_clio_home
        clio_home = get_clio_home()
        full_path = str(clio_home / path)
    ...
    def test_clio_control_files_and_oauth_traversal_denied(self, path):
        """Path traversal attempts to protected Clio files must be blocked."""
        from clio_constants import get_clio_home
        clio_home = get_clio_home()
        full_path = str(clio_home / path)
        assert _is_write_denied(full_path) is True
    ...
    def test_control_files_and_oauth_protected_in_profile_mode(self, tmp_path, monkeypatch, name):
        ...
        global ~/.clio/{auth.json,config.yaml,webhook_subscriptions.json,
        .anthropic_oauth.json} writable — the same gap PR #15981 fixed
        for .env.
        """
        # Simulate a profile-mode CLIO_HOME layout:
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.clio/pairing/{platform}-approved.json as the
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__CLIO_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__CLIO_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert "CLIO_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__CLIO_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__CLIO_FENCE_a9f7b3__\n"
        )
...
class _DeletedTestGitBaselineCheck:
    ...
    AttributeError on origin/main. Deleted wholesale per Omni Loop Labs's

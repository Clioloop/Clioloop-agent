...
When CLIO_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the clio user's home directory ($CLIO_HOME == /opt/data).
Runtime-writable trees under ``/opt/clio`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/clio/gateway`` is one such

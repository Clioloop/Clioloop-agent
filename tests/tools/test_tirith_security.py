...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._clio_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(clio_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(clio_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_sync_resolve_persists_failure(self, mock_which, mock_install,
                                            mock_disk_check, mock_mark):
        """Synchromanaged _resolve_tirith_path persists failure to disk."""
        from tools.tirith_security import _resolve_tirith_path
        _tirith_mod._resolved_path = None
    ...
    def test_install_failed_recovers_from_clio_bin(self):
        """After _INSTALL_FAILED, manual install in CLIO_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        clio_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(clio_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(clio_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._clio_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == clio_bin
            assert _tirith_mod._resolved_path == clio_bin
            mock_clear.assert_called_once()
...
class TestClioHomeIsolation:
    def test_clio_bin_dir_respects_clio_home(self):
        """_clio_bin_dir must use CLIO_HOME, not hardcoded ~/.clio."""
        from tools.tirith_security import _clio_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"CLIO_HOME": tmpdir}):
            result = _clio_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_clio_home(self):
        """_failure_marker_path must use CLIO_HOME, not hardcoded ~/.clio."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"CLIO_HOME": "/custom/clio"}):
            result = _failure_marker_path()
        assert result == "/custom/clio/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets CLIO_HOME; verify it's active."""
        clio_home = os.getenv("CLIO_HOME")
        assert clio_home is not None, "CLIO_HOME should be set by conftest"
        assert "clio_test" in clio_home, "Should point to test temp dir"

    def test_get_clio_home_fallback(self):
        """Without CLIO_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_clio_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove CLIO_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".clio")
            result = _get_clio_home()

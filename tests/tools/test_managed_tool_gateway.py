...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        ...
        clear=False,
    ), patch.object(managed_tool_gateway, "managed_tools_enabled", return_value=True):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "managed-token",
        )

    assert result is not None
    assert result.gateway_origin == "https://firecrawl-gateway.tools.example.com"
    assert result.managed_user_token == "managed-token"
    assert result.managed_mode is True


def test_resolve_managed_tool_gateway_defaults_to_portal_gateway_path():
    """Without overrides the origin is the portal's built-in tool gateway."""
    env = {"TOOL_GATEWAY_DOMAIN": "", "FIRECRAWL_GATEWAY_URL": "",
           "CLIO_PORTAL_BASE_URL": "", "MANAGED_PORTAL_BASE_URL": ""}
    with patch.dict(os.environ, env, clear=False), patch.object(
        managed_tool_gateway, "managed_tools_enabled", return_value=True
    ), patch.object(
        managed_tool_gateway, "_read_managed_provider_state",
    ...
    assert result is not None
    assert result.gateway_origin == "https://portal.example.com/api/gateway/firecrawl"
...
def test_resolve_managed_tool_gateway_is_inactive_without_managed_token():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "",
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": ""}, clear=False), \
         patch.object(managed_tool_gateway, "managed_tools_enabled", return_value=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "managed-token",
        )
...
def test_read_managed_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    assert managed_tool_gateway.read_managed_access_token() == "fresh-token"

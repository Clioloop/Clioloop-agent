...
CLIO_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
CLIO_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as
/etc/cont-init.d/01-clio-setup, which runs lexicographically BEFORE
...
This mirrors the existing CLIO_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def test_seed_block_present_and_guarded(stage2_text: str) -> None:
    ...
    assert '[ ! -f "$CLIO_HOME/gateway_state.json" ]' in block, (
        "seed must be guarded by [ ! -f ] so persisted state wins on restart"
    )
    assert "CLIO_GATEWAY_BOOTSTRAP_STATE" in block
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $CLIO_HOME.

    ``env_value`` is the CLIO_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            f'export CLIO_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'
            if env_value is not None
            else "unset CLIO_GATEWAY_BOOTSTRAP_STATE\n"
        )
        script = (
            "set -e\n"
            f'CLIO_HOME="{home}"\n'
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when CLIO_GATEWAY_BOOTSTRAP_STATE is unset"

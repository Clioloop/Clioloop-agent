...
def test_clio_provider_subclass_exists():
    """ClioMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _CLIO_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _CLIO_PROVIDER_CLS is not None
    assert issubclass(_CLIO_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_clio_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ClioMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _CLIO_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("CLIO_HOME", str(tmp_path))
    ...
    assert _CLIO_PROVIDER_CLS is not None
    assert isinstance(provider, _CLIO_PROVIDER_CLS)
    assert provider._clio_server_name == "srv"

...
class TestErrorResponseShapes:
    ...
    def test_firecrawl_config_error_points_paid_users_to_managed_subscription(self, monkeypatch):
        from plugins.web.firecrawl import provider as firecrawl_provider

        monkeypatch.setattr(
            "tools.web_tools.managed_tools_enabled",
            lambda: True,
            raising=False,
        )
        ...
        assert "With your Omni Loop Portal Subscription you can also use the Tool Gateway" in message
        assert "select Omni Loop Portal Subscription as the web provider" in message
    ...
    def test_firecrawl_config_error_uses_entitlement_message_when_not_paid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "tools.web_tools.managed_tool_gateway_unavailable_message",

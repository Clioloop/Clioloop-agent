...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "clio_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_board_query_param_default_overrides_current_board_pointer(client):
    ...
    through to whichever board ``clio kanban boards switch`` last pinned.
    """
...
def test_add_comment(client):
    ...
    r = client.post(
        ...
        json={"body": "how's progress?", "author": "Omni Loop Labs"},
    )
    assert r.status_code == 200
    ...
    assert comments[0]["author"] == "Omni Loop Labs"
...
def test_board_auto_initializes_missing_db(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("CLIO_HOME", str(home))
    monkeypatch.delenv("CLIO_KANBAN_BOARD", raising=False)
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "clio_cli.web_server", stub)
    monkeypatch.setattr(clio_cli, "web_server", stub, raising=False)
...
def test_dashboard_writes_send_json_content_type():
    """Dashboard POST/PATCH bodies must be parsed as JSON objects by FastAPI."""
    repo_root = Path(__file__).resolve().parents[2]
    bundle = (
        repo_root / "plugins" / "kanban" / "dashboard" / "dist" / "index.js"
    ).read_text()

    assert "function jsonRequest(method, payload)" in bundle
    assert '"Content-Type": "application/json"' in bundle
    assert 'jsonRequest("POST", { title })' in bundle
    assert 'jsonRequest("PATCH", patch)' in bundle


def test_dashboard_card_actions_include_archive_not_direct_running():
    """Board cards should expose safe removal and avoid invalid direct claims."""
    repo_root = Path(__file__).resolve().parents[2]
    bundle = (
        repo_root / "plugins" / "kanban" / "dashboard" / "dist" / "index.js"
    ).read_text()

    assert 'async function archiveTask(task)' in bundle
    assert 'moveTask(task, "archived")' in bundle
    assert '"Archive"' in bundle
    assert '["triage", "todo", "ready", "done"]' in bundle
    assert '["triage", "todo", "ready", "running", "blocked", "review", "done"]' not in bundle


def test_dashboard_operator_console_controls_exist():
    """Kanban dashboard must expose the operator endpoints, not only cards."""
    repo_root = Path(__file__).resolve().parents[2]
    bundle = (
        repo_root / "plugins" / "kanban" / "dashboard" / "dist" / "index.js"
    ).read_text()

    assert "Nudge dispatcher" in bundle
    assert "Dry run" in bundle
    assert '`${API}/profiles`' in bundle
    assert '`${API}/orchestration`' in bundle
    assert '`${API}/workers/active`' in bundle
    assert '`${API}/tasks/${encodeURIComponent(task.id)}/comments`' in bundle
    assert '`${API}/tasks/${encodeURIComponent(task.id)}/reclaim`' in bundle
    assert '`${API}/tasks/${encodeURIComponent(task.id)}/reassign`' in bundle
    assert '`${API}/tasks/${encodeURIComponent(task.id)}/log?tail=120000`' in bundle
    assert "Run history" in bundle
    assert "Comments" in bundle
    assert "Save orchestration" in bundle


def test_dashboard_profile_names_reads_initialized_assignees():
    """Regression: profileNames must not touch assignees before declaration."""
    repo_root = Path(__file__).resolve().parents[2]
    bundle = (
        repo_root / "plugins" / "kanban" / "dashboard" / "dist" / "index.js"
    ).read_text()

    assert bundle.index("const assignees =") < bundle.index("const profileNames = useMemo")
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "clio_cli.kanban._check_dispatcher_presence",
        lambda: (False, "No gateway is running — start `clio gateway start`."),
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
# tests/clio_cli/test_kanban_db.py. The OUTER fallback here is not, which
...
def test_board_endpoint_survives_task_age_exception(client, monkeypatch):
    ...
    monkeypatch.setattr("clio_cli.kanban_db.task_age", _boom)
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "clio_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_board_surfaces_warnings_field_for_hallucinated_completions(client):
    ...
    raw event kind — see clio_cli.kanban_diagnostics for the rule
    that produces it.
    """
    conn = kb.connect()
    try:
...
def test_dashboard_bulk_actions_include_reclaim_first():
    ...
    assert "clio-kanban-bulk-reclaim-first" in dist
    assert '"→ todo"' in dist
...
def test_dashboard_failed_card_highlight_class_exists():
    ...
    assert "clio-kanban-card--failed" in js
    assert "clio-kanban-card--failed" in css

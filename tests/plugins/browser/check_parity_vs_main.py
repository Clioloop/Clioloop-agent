...
Run from the PR worktree:
    ...
    cd ~/.clio/clio-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.clio/clio-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.clio/clio-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a clio-agent checkout"
)
...
# Isolated CLIO_HOME for the config write.

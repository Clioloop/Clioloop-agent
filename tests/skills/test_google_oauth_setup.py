...
class TestClioConstantsFallback:
    """Tests for _clio_home.py fallback when clio_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_clio_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _clio_home.py with clio_constants blocked."""
        monkeypatch.setitem(sys.modules, "clio_constants", None)
        spec = importlib.util.spec_from_file_location("_clio_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_clio_home_env_var(self, monkeypatch, tmp_path):
        """When clio_constants is missing, CLIO_HOME comes from env var."""
        monkeypatch.setenv("CLIO_HOME", str(tmp_path / "custom-clio"))
        module = self._load_helper(monkeypatch)
        assert module.get_clio_home() == tmp_path / "custom-clio"

    def test_fallback_defaults_to_dot_clio(self, monkeypatch):
        """When clio_constants is missing and CLIO_HOME unset, default to ~/.clio."""
        monkeypatch.delenv("CLIO_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_clio_home() == Path.home() / ".clio"

    def test_fallback_ignores_empty_clio_home(self, monkeypatch):
        """Empty/whitespace CLIO_HOME is treated as unset."""
        monkeypatch.setenv("CLIO_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_clio_home() == Path.home() / ".clio"

    def test_fallback_display_clio_home_shortens_path(self, monkeypatch):
        """Fallback display_clio_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("CLIO_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_clio_home() == "~/.clio"

    def test_fallback_display_clio_home_profile_path(self, monkeypatch):
        """Fallback display_clio_home() handles profile paths under ~/."""
        monkeypatch.setenv("CLIO_HOME", str(Path.home() / ".clio/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_clio_home() == "~/.clio/profiles/coder"

    def test_fallback_display_clio_home_custom_path(self, monkeypatch):
        """Fallback display_clio_home() returns full path for non-home locations."""
        monkeypatch.setenv("CLIO_HOME", "/opt/clio-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_clio_home() == "/opt/clio-custom"

    def test_delegates_to_clio_constants_when_available(self):
        """When clio_constants IS importable, _clio_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_clio_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import clio_constants
        assert module.get_clio_home is clio_constants.get_clio_home
        assert module.display_clio_home is clio_constants.display_clio_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Clio Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "clio-agent[google]" in out

...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Clioloop/Clioloop-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    clio_meta = (fm.get("metadata") or {}).get("clio") or {}
    tags = clio_meta.get("tags") or []
    related = clio_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `clio skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Clio Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Clio ships with a large built-in skill library copied into `~/.clio/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Clio also syncs bundled skills on `clio update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.clio/skills/` tree, it is still shipped with Clio; restore it with `clio skills reset <name> --restore`.",
        "",

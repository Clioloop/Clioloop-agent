#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Clio docs site.
...
Both publish at:
  https://clio-agent./docs/llms.txt
  https://clio-agent./docs/llms-full.txt
...
SITE_BASE = "https://clio-agent./docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Clio as a Python Library", None),
        ("guides/use-mcp-with-clio", "Use MCP with Clio", None),
        ("guides/use-voice-mode-with-clio", "Use Voice Mode with Clio", None),
        ("guides/use-soul-with-clio", "Use SOUL.md with Clio", None),
        ("guides/build-a-clio-plugin", "Build a Clio Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Clio"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Clio Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Omni Loop Labs. A terminal-native "
        ...
        "backends. Works with managed provider, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/Clioloop/"
        "clio-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/Clioloop/Clioloop-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Clio Agent — Full Documentation\n",
        (
            "This file is the entire Clio Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Clio, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://clio-agent./docs\n",
        "Short index: https://clio-agent./docs/llms.txt\n",
        "\n---\n\n",
    ]

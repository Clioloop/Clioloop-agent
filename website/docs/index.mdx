...
slug: /
sidebar_position: 0
title: "Clio Agent Documentation"
description: "The self-improving AI agent built by Omni Loop Labs. A built-in learning loop that creates skills from experience, improves them during use, and remembers across sessions."
...
The self-improving AI agent built by [Omni Loop Labs](https://). The only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
After installing, run `clio setup --portal` — one OAuth covers a model plus all four Tool Gateway tools (web search, image generation, TTS, browser). See [managed provider](/integrations/managed-provider).
:::
...
| 🔌 **[MCP Integration](/user-guide/features/mcp)**                      | Connect to MCP servers, filter their tools, and extend Clio safely  |
| 🧭 **[Use MCP with Clio](/guides/use-mcp-with-clio)**               | Practical MCP setup patterns, examples, and tutorials                 |
| 🎙️ **[Voice Mode](/user-guide/features/voice-mode)**                    | Real-time voice interaction in CLI, Telegram, Discord, and Discord VC |
| 🗣️ **[Use Voice Mode with Clio](/guides/use-voice-mode-with-clio)** | Hands-on setup and usage patterns for Clio voice workflows          |
| 🎭 **[Personality & SOUL.md](/user-guide/features/personality)**        | Define Clio' default voice with a global SOUL.md                    |
...
| 💡 **[Tips & Best Practices](/guides/tips)**                            | Quick wins to get the most out of Clio                              |
...
- **Built by model trainers** — Created by [Omni Loop Labs](https://), the lab behind Clio, Nomos, and Psyche. Works with [managed provider](https://), [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint
...
- **Full web control** — Search, extract, browse, vision, image generation, TTS — one subscription via [managed provider](/integrations/managed-provider) bundles all of them
- **MCP support** — Connect to any MCP server for extended tool capabilities
- **Research-ready** — Batch processing, trajectory export, RL training with Atropos. Built by [Omni Loop Labs](https://) — the lab behind Clio, Nomos, and Psyche models

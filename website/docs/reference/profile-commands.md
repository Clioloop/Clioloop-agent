...
This page covers all commands related to [Clio profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `clio profile` without a subcommand shows help.
...
```bash
clio profile use <name>
```

Sets `<name>` as the active profile. All subsequent `clio` commands (without `-p`) will use this profile.
...
clio profile use default
```

## `clio profile create`

```bash
clio profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `clio profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `clio update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.clio`), use `clio skills opt-out` / `clio skills opt-in`. |
...
clio profile create mybot

# Clone config only from current profile
clio profile create work --clone

# Clone everything from current profile
clio profile create backup --clone-all

# Clone config from a specific profile
clio profile create work2 --clone --clone-from work
```

## `clio profile describe`

```bash
clio profile describe [<name>] [options]
```
...
clio profile describe researcher

# Set it explicitly
clio profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
clio profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
clio profile describe --all --auto
```

## `clio profile delete`

```bash
clio profile delete <name> [options]
```
...
```bash
clio profile delete mybot
clio profile delete mybot --yes
```
...
```bash
clio profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Clio home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ clio profile show work
Profile: work
Path:    ~/.clio/profiles/work
...
```bash
clio profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Clio installation.
...
```bash
clio profile alias work
# Creates/updates ~/.local/bin/work

clio profile alias work --name mywork
# Creates ~/.local/bin/mywork

clio profile alias work --remove
...
```bash
clio profile rename <old-name> <new-name>
```
...
```bash
clio profile rename mybot assistant
# ~/.clio/profiles/mybot → ~/.clio/profiles/assistant
...
```bash
clio profile export <name> [options]
```
...
```bash
clio profile export work
# Creates work.tar.gz in the current directory

clio profile export work -o ./work-2026-03-29.tar.gz
```

## `clio profile import`

```bash
clio profile import <archive> [options]
```
...
```bash
clio profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

clio profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`clio profile export` / `import` are still the right commands for
...
### `clio profile install`

```bash
clio profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `clio -p telemetry`). |
...
clio profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
clio profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
clio profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
clio profile install ./telemetry/
```

### `clio profile update`

```bash
clio profile update <name> [--force-config] [--yes]
```
...
```bash
clio profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Clio version, author, env var requirements, the source URL/path, and
...
`clio profile list` also shows the distribution name and version in a
`Distribution` column, and `clio profile show <name>` / `delete <name>`
...
clio profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
clio profile install https://github.com/your-org/internal-assistant.git
```
...
`clio_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Clio version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Clio can clone from.
3. Tell recipients to run `clio profile install <your-repo-url>`.
...
```bash
clio -p <name> <command> [options]
clio --profile <name> <command> [options]
```

Global flag to run any Clio command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
clio -p work chat -q "Check the server status"
clio --profile dev gateway start
clio -p personal skills list
clio -p work config edit
```
...
- `clio profile <TAB>` — subcommands (list, use, create, etc.)
- `clio profile use <TAB>` — profile names
- `clio -p <TAB>` — profile names

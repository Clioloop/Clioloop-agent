...
description: "Run Clio Agent directly on an Android phone with Termux"
---

# Clio on Android with Termux

This is the tested path for running Clio Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Clio from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Clio now ships a Termux-aware installer path:
...
- links `clio` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `clio` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/clio" "$PREFIX/bin/clio"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `clio` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.clio/.env`.
...
### `clio doctor` says ripgrep or Node is missing

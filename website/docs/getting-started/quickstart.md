...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Clio Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Clio setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Clio Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Clio Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Clio for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Clio working on my machine | `clio setup` | Run a real chat and verify it responds |
| I already know my provider | `clio model` | Save the config, then start chatting |
| I want a bot or always-on setup | `clio gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `clio model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `clio model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Clio cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Clio Agent
...
The single most important setup step. Use `clio model` to walk through the choice interactively:

```bash
clio model
```

:::tip Easiest path: managed provider
...
|----------|-----------|---------------|
| **managed provider** | Subscription-based, zero-config | OAuth login via `clio model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `clio model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `clio model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `clio_cli/models.py` may change between releases) | `clio model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `clio model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `clio model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `clio model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `clio model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `clio model` (requires `copilot` CLI + `copilot login`) |
...
Clio Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `clio model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Clio separates secrets from normal config:

- **Secrets and tokens** → `~/.clio/.env`
- **Non-secret settings** → `~/.clio/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
clio config set model anthropic/claude-opus-4.6
clio config set terminal.backend docker
clio config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
clio            # classic CLI
clio --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Clio ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `clio` vs `clio --tui`.
:::
...
```bash
clio --continue    # Resume the most recent session
clio -c            # Short form
```
...
```bash
clio gateway setup    # Interactive platform configuration
```
...
- `clio tools` — tune tool access per platform
- `clio skills` — browse and install reusable workflows
...
```bash
clio config set terminal.backend docker    # Docker isolation
clio config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Clio install directory (the curl installer placed it at
# ~/.clio/clio-agent on Linux/macOS or %LOCALAPPDATA%\clio\clio-agent on Windows):
cd ~/.clio/clio-agent
...
Skills are on-demand instruction documents that teach Clio how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Clio ships with a catalog of bundled skills already installed in `~/.clio/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
clio skills browse                      # list everything available
clio skills search kubernetes           # find skills by keyword
clio skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `clio skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Clio ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.clio/clio-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Clio opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `clio model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `clio gateway setup` and check `clio gateway status` |
| `clio --continue` can't find old session | Switched profiles or session never saved | Check `clio sessions list` and confirm you're in the right profile |
...
| `clio doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `clio model`
3. `clio setup`
4. `clio sessions list`
5. `clio --continue`
6. `clio gateway status`
...
| `clio tools` | Configure which tools are enabled per platform |
| `clio setup` | Full setup wizard (configures everything at once) |
| `clio doctor` | Diagnose issues |
| `clio update` | Update to latest version |
| `clio gateway` | Start the messaging gateway |
| `clio --continue` | Resume last session |

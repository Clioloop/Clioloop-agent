...
This page covers setting up inference providers for Clio Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Clio.

## Inference Providers

You need at least one way to connect to an LLM. Use `clio model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **managed provider** | `clio model` (OAuth, subscription-based) |
| **OpenAI Codex** | `clio model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `clio model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `clio model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `clio model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.clio/.env` |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.clio/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.clio/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.clio/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.clio/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.clio/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.clio/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.clio/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.clio/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.clio/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `clio model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.clio/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.clio/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.clio/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.clio/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.clio/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.clio/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.clio/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.clio/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.clio/.env` (provider: `gemini`) |
| **Google Gemini (OAuth)** | `clio model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.clio/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `clio model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `clio model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.clio/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `clio model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `clio model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `clio model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.clio/.env` (provider: `stepfun`) |
| **LM Studio** | `clio model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `clio model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
[managed provider](https://) is Omni Loop Labs's unified subscription gateway and **the recommended way to run Clio Agent**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) plus [managed provider Chat](https://chat.) — billed against your managed-provider subscription instead of separate per-provider accounts.

```bash
clio setup --portal     # fresh install — OAuth + provider + gateway in one command
clio model              # existing install — pick "managed provider" from the list
clio portal info        # inspect login + routing at any time
```

Don't have a subscription yet? Get one at [/manage-subscription](https:///manage-subscription).

**For full details:** see the dedicated [managed provider integration page](/integrations/managed-provider) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Clio Agent with managed provider guide](/guides/run-clio-with-managed-provider).

**Client identification.** Every Portal request from Clio Agent carries a `client=clio-client-v<version>` tag (e.g. `client=clio-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish Clio traffic from other clients. No config required; the tag updates automatically when you `clio update`.

**JWT auth (automatic).** Clio prefers scoped `inference:invoke` JWTs for Portal requests with the legacy opaque session-key path as a fallback. No configuration is required — credentials are managed by the OAuth flow and rotate transparently. Revoked refresh tokens are quarantined to avoid replay loops.


:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Clio stores the resulting credentials in its own auth store under `~/.clio/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Clio marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `clio auth add codex-oauth` (or `clio model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using managed provider, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Clio routes these tasks to your **main chat model** — the same model you picked in `clio model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip managed provider Tool Gateway
Paid managed provider subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `clio setup --portal` logs you in, sets managed provider as your provider, and turns the gateway on in one command. Existing users can enable it from `clio model` or per-tool from `clio tools`. Inspect routing at any time with `clio portal info`.
:::

### Two Commands for Model Management

Clio has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`clio model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Clio chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `clio model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `clio model`, complete the provider setup, then start a new session.
...
When you authenticate via `clio model` → Anthropic OAuth (or via `clio auth add anthropic --type oauth`), Clio routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Clio — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
clio chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `clio model`
# Clio will use Claude Code's credential store directly when available
clio model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
clio chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
clio chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `clio model`, Clio prefers Claude Code's own credential store over copying the token into `~/.clio/.env`. That keeps refreshable Claude credentials refreshable.
...
Clio supports GitHub Copilot as a first-class provider with two modes:
...
```bash
clio chat --provider copilot --model gpt-5.4
```
...
If no token is found, `clio model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `clio model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `clio model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Clio
Clio sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Clio now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Clio therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
clio chat --provider copilot-acp --model copilot-acp
...
| `CLIO_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `CLIO_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# NovitaAI Model API
clio chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.clio/.env

# z.ai / ZhipuAI GLM
clio chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.clio/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
clio chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.clio/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
clio chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.clio/.env

# MiniMax (global endpoint)
clio chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.clio/.env

# MiniMax (China endpoint)
clio chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.clio/.env

# Qwen Cloud / DashScope (Qwen models)
clio chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.clio/.env

# Xiaomi MiMo
clio chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.clio/.env

# Tencent TokenHub (Hy3 Preview)
clio chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.clio/.env

# Arcee AI (Trinity models)
clio chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.clio/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
clio chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.clio/.env
```
...
When using the Z.AI / GLM provider, Clio automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.clio/.env` and pick xAI in `clio model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `clio model`, or run `clio auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Clio runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), Clio automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `clio tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `clio doctor` and `clio chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `clio migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).

```bash
clio migrate xai          # preview replacements
clio migrate xai --apply  # rewrite ~/.clio/config.yaml in place
```
...
# Short alias
clio chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `clio model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Clio auto-discovers the available models.
...
clio chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 clio chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `clio model`, sign in through the browser, and Clio persists the refresh token.

```bash
clio model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.clio/auth.json

clio chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `CLIO_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Clio exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
clio chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `clio model`, sign in through the browser, and Clio persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
clio chat   # uses api.minimax.io/anthropic endpoint
```
...
clio chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.clio/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 clio chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
Clio automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
clio chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.clio/.env
```
...
```bash
# StepFun
clio chat --provider stepfun --model step-3-mini
# Requires: STEPFUN_API_KEY in ~/.clio/.env
```
...
```bash
# Use any available model
clio chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.clio/.env

# Short alias
clio chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
# → done — Clio auto-provisions your free tier on first request
```

Clio ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.clio/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `CLIO_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of Clio works unchanged
...
| Workspace / Standard / Enterprise account | Set `CLIO_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Clio detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Clio shows
...
```bash
CLIO_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
CLIO_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
Clio Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Clio at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`LLM_MODEL` in `.env` is **removed** — `config.yaml` is the single source of truth for model and endpoint configuration. `OPENAI_BASE_URL` is still honored, but **only** for the `openai-api` provider (it overrides the OpenAI endpoint for direct API-key access). For other providers and custom endpoints, use `clio model` or set `model.base_url` in `config.yaml` directly. If you have stale entries in your `.env`, they are automatically cleared on the next `clio setup` or config migration.
:::
...
:::warning clio model vs /model
**`clio model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Clio chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `clio model`, set up the new provider, then start a new session.
:::
...
When switching providers, Clio persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
Clio Agent requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Clio.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Clio) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `clio` (Qwen 2.5, Clio 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
**Context length (`-c`):** Recent builds default to `0` which reads the model's training context from the GGUF metadata. For models with 128k+ training context, this can OOM trying to allocate the full KV cache. Set `-c` explicitly to at least 64,000 tokens for Clio. If using parallel slots (`-np`), the total context is divided among slots — with `-c 64000 -np 4`, each slot only gets 16k, which is below Clio' minimum per active session.

Then configure Clio to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Clio won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Clio 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
Clio will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Clio) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Clio Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Clio config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Clio config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Clio.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Clio inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser clio` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Clio's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Clio thinks the context is
...
Clio auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Clio uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Clio with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in Clio.
...
Then configure Clio with `clio model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Clio with `clio model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `clio model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Clio uses this to decide when to compress history and to validate API requests.
...
Clio uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
7. **managed provider** — suffix-matches managed provider model IDs against OpenRouter metadata
...
`clio model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Clio uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `clio model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and Clio will merge it into each chat-completions request for that endpoint:
...
The `clio model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.

**Native vision for custom-provider models.** If your custom endpoint serves a vision-capable model that isn't in models.dev, set `model.supports_vision: true` so Clio routes attached images natively (as `image_url` parts) instead of pre-processing them through `vision_analyze`. Single knob — no need to also set `agent.image_input_mode: native`.
...
You can also select named custom providers from the interactive `clio model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.clio/config.yaml` and the matching API key goes in `~/.clio/.env`.
...
Together's `/v1/models` endpoint works, so `clio model` can auto-discover available models.
...
:::tip Troubleshooting
- `clio doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `clio model` will persist the model with a warning rather than hard-reject — see #15136.
...
|----------|-------------|
| **Just want it to work** | OpenRouter (default) or managed provider |
...
:::tip
You can switch between providers at any time with `clio model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Clio at your instance (no API key needed):
   ```bash
   clio config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.clio/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.clio/config.yaml`:
...
Configure a chain of backup providers Clio tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
Supported providers: `openrouter`, `managed`, `novita`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `gemini`, `google-gemini-cli`, `qwen-oauth`, `huggingface`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `minimax-oauth`, `deepseek`, `nvidia`, `xai`, `xai-oauth`, `ollama-cloud`, `bedrock`, `azure-foundry`, `opencode-zen`, `opencode-go`, `kilocode`, `xiaomi`, `arcee`, `gmi`, `stepfun`, `lmstudio`, `alibaba`, `alibaba-coding-plan`, `tencent-tokenhub`, `custom`.

:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `clio fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::

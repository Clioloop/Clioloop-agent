...
When Clio Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.clio/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.clio/config.yaml` (switchable per session
via `/personality`).
...
If you use Clio' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.clio/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Clio Agent's deliverable

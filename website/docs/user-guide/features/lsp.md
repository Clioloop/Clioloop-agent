...
Clio runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Clio
...
1. Clio captures a baseline of current diagnostics for the file.
...
…). Clio auto-detects the binary on PATH or in
`<CLIO_HOME>/lsp/bin/`.
...
`node_modules` tree — Clio installs both packages together when you
run `clio lsp install typescript` or auto-install fires on first
use.

## CLI

```
clio lsp status          # service state + per-server install status
clio lsp list            # registry, optionally --installed-only
clio lsp install <id>    # eagerly install one server
clio lsp install-all     # try every server with a known recipe
clio lsp restart         # tear down running clients
clio lsp which <id>      # print resolved binary path
```

`clio lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CLIO_HOME>/lsp/bin
...
When `install_strategy: auto`, Clio installs binaries into
`<CLIO_HOME>/lsp/bin/`. NPM packages land in
`<CLIO_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Clio-owned and is
...
Servers are kept alive for the life of the Clio process. There's
...
**`clio lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CLIO_HOME>/lsp/bin/`. Run
`clio lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `clio lsp status`**
...
When `clio lsp status` shows a `Backend warnings` section, install
...
Check `~/.clio/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `clio lsp restart` to clear the set;

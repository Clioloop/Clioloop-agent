...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Clio Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::tip
Traffic routed through [managed provider](/integrations/managed-provider) still respects per-model routing and priority configs — and Portal subscribers get 10% off token-billed providers.
:::

## Configuration

Add a `provider_routing` section to your `~/.clio/config.yaml`:
...
- **CLI mode** — configured in `~/.clio/config.yaml`, loaded at startup

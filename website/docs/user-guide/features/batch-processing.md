...
Batch processing lets you run the Clio agent across hundreds or thousands of prompts in parallel, generating structured trajectory data. This is primarily used for **training data generation** — producing ShareGPT-format trajectories with tool usage statistics that can be used for fine-tuning or evaluation.
...
Batch runs spin up many concurrent agent sessions, each making model calls and tool calls. A [managed provider](/user-guide/features/tool-gateway) subscription bundles model access plus web search, image gen, TTS, and cloud browsers under one bill — useful when you want stable cost-per-trajectory without juggling rate limits across five vendor accounts. Set up with `clio setup --portal`, then point `--model` at a managed provider model.
:::

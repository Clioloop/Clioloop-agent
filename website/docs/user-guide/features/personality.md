...
description: "Customize Clio Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Clio Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CLIO_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Clio is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Clio now seeds a default `SOUL.md` automatically in:

```text
~/.clio/SOUL.md
```

More precisely, it uses the current instance's `CLIO_HOME`, so if you run Clio with a custom home directory, it will use:
...
- Clio creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Clio loads `SOUL.md` only from `CLIO_HOME`
- Clio does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Clio falls back to a built-in default identity
...
If Clio loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CLIO_HOME`, the personality belongs to the Clio instance itself.

That also makes it easier to teach users:
- "Edit `~/.clio/SOUL.md` to change Clio' default personality."
...
- how Clio should handle uncertainty, disagreement, or ambiguity
...
## What Clio injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Clio falls back to a built-in default identity ("You are Clio Agent, an intelligent AI assistant created by Omni Loop Labs..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Clio ships with built-in personalities you can switch to with `/personality`.
...
| **clio-spinner** | Cute expressions, sparkles, and enthusiasm ★ |
| **catgirl** | Neko-chan with cat-like expressions, nya~ |
| **pirate** | Captain Clio, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Clio its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.clio/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.clio/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-clio)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Clio speaks
- `display.skin` and `/skin` affect how Clio looks in the terminal

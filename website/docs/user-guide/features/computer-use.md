...
Clio Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `clio computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `clio tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
- **`clio computer-use install --upgrade`** — manual force-refresh.
...
`clio computer-use status` shows the installed version next to the
binary path.
...
Clio applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.clio/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Clio applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `CLIO_CUA_DRIVER_VERSION`
...
```
CLIO_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
CLIO_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
CLIO_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`clio computer-use install` to fetch the cua-driver binary, or run
`clio tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/Clioloop/Clioloop-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)

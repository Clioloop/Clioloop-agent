...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Clio's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Clio instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
clio memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `clio` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Clio syncs them back at session init.
...
The `clio honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `clio memory setup honcho` (or run `clio memory setup` and pick it from the list); the `clio honcho` subcommand then appears on the next invocation.

```bash
clio memory setup honcho    # Configure Honcho directly (works before activation)
clio honcho status          # Connection status, config, and key settings
clio honcho setup           # Redirects to `clio memory setup` (post-activation alias)
clio honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
clio honcho peer            # Show or update peer names + dialectic reasoning level
clio honcho mode            # Show or set recall mode (hybrid/context/tools)
clio honcho tokens          # Show or set token budget for context and dialectic
clio honcho identity        # Seed or show the AI peer's Honcho identity
clio honcho sync            # Sync Honcho config to all existing profiles
clio honcho peers           # Show peer identities across all profiles
clio honcho sessions        # List known Honcho session mappings
clio honcho map             # Map current directory to a Honcho session name
clio honcho enable          # Enable Honcho for the active profile
clio honcho disable         # Disable Honcho for the active profile
clio honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `clio honcho`

If you previously used the standalone `clio honcho setup`:
...
No re-login or re-setup needed. Run `clio memory setup` and select "honcho" — the wizard detects your existing config.

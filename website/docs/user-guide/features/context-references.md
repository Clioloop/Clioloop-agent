...
Type `@` followed by a reference to inject content directly into your message. Clio expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Clio env: `$CLIO_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CLIO_HOME/skills/.hub/`

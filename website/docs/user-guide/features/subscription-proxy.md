...
description: "Use your managed provider subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Clio-managed provider subscription as their
...
| Use case | "Use Clio as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the managed provider OAuth flow. Clio stores
the refresh token in `~/.clio/auth.json` — the same place all Clio
...
```
Starting Clio proxy for managed provider
...
Model:      Clio-4-70B    # or Clio-4.3-36B, Clio-4-405B
```
...
```bash
clio proxy providers
```

Currently shipped: `managed` (managed provider) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `clio_cli/proxy/adapters/`.
...
```
Clio proxy upstream adapters

  [managed    ] managed provider — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `clio portal`. If you see
...
The proxy only forwards paths the upstream actually serves. For managed provider
Portal:
...
INFERENCE_TEXT_MODEL=Clio-4-70B
```
...
```bash
clio proxy start --host 0.0.0.0 --port 8645
```
...
`clio_cli/proxy/adapters/<provider>.py` and registering it in

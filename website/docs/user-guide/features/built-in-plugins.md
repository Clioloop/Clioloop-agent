...
description: "Plugins shipped with Clio Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Clio ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.clio/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Clio Plugin](/guides/build-a-clio-plugin) to write your own.
...
2. **User** — `~/.clio/plugins/<name>/`
3. **Project** — `./.clio/plugins/<name>/` (requires `CLIO_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `clio_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `clio memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `clio plugins list` and the interactive `clio plugins` UI), but none load until you explicitly enable them:

```bash
clio plugins enable disk-cleanup
```

Or via `~/.clio/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Clio. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
clio plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `clio plugins enable <name>`.
...
| `clio-achievements` | dashboard tab | Steam-style collectible badges generated from your real Clio session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `clio memory` and `clio plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CLIO_HOME` or `/tmp/clio-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under CLIO_HOME | always | Never |
...
**State** — everything lives at `$CLIO_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `CLIO_HOME` or `/tmp/clio-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `clio plugins enable disk-cleanup` (or check the box in `clio plugins`).

**Disabling again:** `clio plugins disable disk-cleanup`.
...
**Enabling:** `clio plugins enable security-guidance` (or check the box in `clio plugins`).

**Disabling again:** `clio plugins disable security-guidance`.
...
Traces Clio turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Clio' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `clio logs`.
...
```bash
clio tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Clio and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
clio plugins enable observability/langfuse
```

Then put the credentials in `~/.clio/.env`:

```bash
CLIO_LANGFUSE_PUBLIC_KEY=pk-lf-...
CLIO_LANGFUSE_SECRET_KEY=sk-lf-...
CLIO_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Clio turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `CLIO_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Clio session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `clio chat` session lives under one Langfuse session.

**Verify:**

```bash
clio plugins list                 # observability/langfuse should show "enabled"
clio chat -q "hello"              # check the Langfuse UI for a "Clio turn" trace
```
...
| `CLIO_LANGFUSE_RELEASE` | — | Release/version tag |
| `CLIO_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `CLIO_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `CLIO_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Clio-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Clio-prefixed wins when both are set.
...
**Disabling:** `clio plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.clio/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
clio plugins enable google_meet
...
**Disabling:** `clio plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.clio/cache/google_meet/` until you remove them.

### clio-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Clio session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Clio feature changes.

**How it works:**

- Scans your entire `~/.clio/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$CLIO_HOME/plugins/clio-achievements/state.json`
...
| Secret | Hidden until Clio detects the first related signal in your history |

**API** — routes mount under `/api/plugins/clio-achievements/`:
...
**State files** — live under `$CLIO_HOME/plugins/clio-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Clio updates. |
...
**Enabling:** Nothing to enable — `clio-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `clio dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/clio-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.clio/plugins/clio-achievements/` that ships no dashboard. The plugin's state files under `$CLIO_HOME/plugins/clio-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Clio plugin — see [Build a Clio Plugin](/guides/build-a-clio-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.clio/plugins/<name>/`
- Manifest source is reported as `bundled` in `clio plugins list`

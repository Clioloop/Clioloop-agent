...
The web dashboard is a browser-based UI for managing your Clio Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses managed provider OAuth; if you also want the dashboard to talk to a real backend, `clio setup --portal` wires up the model and tool gateway too. See [managed provider](/integrations/clio-portal).
:::
...
```bash
# Custom port
clio dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
clio dashboard --host 0.0.0.0

# Start without opening browser
clio dashboard --no-open
```

## Prerequisites

The default `clio-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'clio-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install clio-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `clio dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `clio dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `clio --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'clio-agent[web,pty]'`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.clio/.env
ExecStart=/path/to/venv/bin/python -m clio_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.clio/.env` containing:

```bash
CLIO_DASHBOARD_BASIC_AUTH_USERNAME=admin
CLIO_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
CLIO_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `clio config set` and the gateway read from.
:::
...
Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.clio/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `clio skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `clio mcp` reads from.
...
**Catalog:** browse managed provider-approved MCP servers (the bundled `optional-mcps/`
...
`clio mcp catalog` / `clio mcp install` use.
...
Connect Clio to any messaging platform from the browser — full parity with
`clio setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.clio/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Clio versions, CPU core count + utilization, memory, disk usage of the Clio home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Clio version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `clio update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **managed provider** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `clio portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `clio curator`.
...
![System admin page — host stats and managed provider status](/img/dashboard/admin-system-top.png)
...
This re-reads `~/.clio/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
| `GET /api/mcp/catalog` | Browse managed provider-approved MCP catalog |
...
| `GET /api/clio/update/check` | Report update availability (commits behind, install method) without applying. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | managed provider auth + Tool Gateway routing (read-only) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Clio Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
- **[OAuth (managed provider)](#default-provider-clioloop)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote Clio Desktop connection](#connecting-clio-desktop-to-a-remote-backend). Every login is verified against your managed-provider account, so this is the provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). No managed provider involved; suitable for public-internet exposure when fronted by a conformant OIDC server.
...
|-------|-----------|----------|
| `clio dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `clio dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no managed provider plugin, no custom plugin), `clio dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

### Default provider: Omni Loop Labs

The bundled `plugins/dashboard_auth/clio` plugin is **always installed** and auto-loaded. It auto-registers a `DashboardAuthProvider` named `clio` when a client ID is configured.

Because every login is verified against managed provider and protected by your managed-provider account, **managed provider provider is the one suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use managed provider provider you need an OAuth client ID (shape `agent:{id}`). There are two ways to get one:

- **CLI — `clio dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing managed provider login (run `clio setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `CLIO_DASHBOARD_OAUTH_CLIENT_ID` into `~/.clio/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).
...
- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https:///local-dashboards) in the managed provider to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `CLIO_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `CLIO_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `clio dashboard register` |

Per the Clio Agent convention (`~/.clio/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • clio: CLIO_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    dashboard.oauth.client_id in config.yaml is empty). The managed provider
    provisions this env var (shape 'agent:{instance_id}') when it
    deploys a Clio Agent instance — set it to your provisioned
...
#### Worked example: Omni Loop Labs

From a logged-in Clio install to a managed provider-gated dashboard in three steps.

**1. Log in and register the dashboard.** `clio dashboard register` uses your existing managed provider login to provision an OAuth client and writes `CLIO_DASHBOARD_OAUTH_CLIENT_ID` into `~/.clio/.env` for you:

```bash
clio setup            # if you're not already logged into managed provider
clio dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes CLIO_DASHBOARD_OAUTH_CLIENT_ID to ~/.clio/.env
```

**2. Run the dashboard on a reachable address.** A non-loopback bind without `--insecure` engages the OAuth gate, and the `client_id` just written activates the `clio` provider:

```bash
clio dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with Omni Loop Labs** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: clio`). For an internet-facing host, register with `--redirect-uri https://clio.example.com/auth/callback` and set `CLIO_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
The username/password provider is intended for self-hosted / on-prem / homelab dashboards on a **trusted network**, or reachable only over a **VPN**. It protects a single shared credential with no external identity provider, MFA, or per-user accounts behind it, so it is **not suitable for exposing a dashboard directly to the public internet**. For an internet-facing dashboard, use the [Omni Loop Labs provider](#default-provider-clioloop) (or your own [self-hosted OIDC](#self-hosted-oidc-provider) / [custom OAuth](#custom-providers) provider) instead.
:::

#### Configuration

Like managed provider provider, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback/`--insecure` operators are unaffected.
...
|---------|-----------|-------|
| `CLIO_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `CLIO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `CLIO_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `CLIO_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `CLIO_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.clio/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.clio/.env <<EOF
CLIO_DASHBOARD_BASIC_AUTH_USERNAME=admin
CLIO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
CLIO_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.clio/.env
```
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [Omni Loop Labs](#default-provider-clioloop) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no managed provider involved. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like managed provider provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback / `--insecure` dashboards.
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/clio/   # required
      client_id: clio-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `CLIO_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `CLIO_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `CLIO_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-clio.json` — it defines a `clio` realm, a **public PKCE client** (`clio-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "clio-dashboard",
      "name": "Clio Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-clio.json:/opt/keycloak/data/import/realm-clio.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/clio/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/clio`). The admin console is at
...
```bash
export CLIO_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/clio"
export CLIO_DASHBOARD_OIDC_CLIENT_ID="clio-dashboard"
export CLIO_DASHBOARD_PUBLIC_URL="http://localhost:9119"
clio dashboard --host 0.0.0.0 --port 9119 --no-open
```

`CLIO_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → clio-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `CLIO_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/clio"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `CLIO_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `CLIO_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements the [managed provider OAuth contract v1](https://github.com/Clioloop/clio-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with Omni Loop Labs" button → `/auth/login?provider=clio`.
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `clio_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `clio_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `clio_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `clio_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
The sidebar widget shows `Logged in as <user_id…> via clio` with a logout icon. Clicking it POSTs `/auth/logout`, which clears all dashboard-auth cookies and redirects back to `/login`.

### Audit log

Every login start, success, failure, and session-verify failure is written as a JSON line to `$CLIO_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.

### Custom providers

To plug a non-managed-provider OAuth provider (e.g. Google, GitHub, custom OIDC), create a plugin that registers a `DashboardAuthProvider`:

```python
# ~/.clio/plugins/dashboard-auth-myidp/__init__.py
from clio_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# Quick env-var path.
CLIO_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  clio dashboard --host 0.0.0.0
...
Clio Desktop can drive a Clio backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Clio Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (managed provider)** (register it with [`clio dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Omni Loop Labs*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Default provider: Omni Loop Labs](#default-provider-clioloop).

### On the backend (the remote machine)

```bash
# 1. Set the dashboard login credentials in ~/.clio/.env (secrets file, 0600).
cat >> ~/.clio/.env <<'EOF'
CLIO_DASHBOARD_BASIC_AUTH_USERNAME=admin
CLIO_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/clio` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `CLIO_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `CLIO_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `CLIO_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `CLIO_DASHBOARD_BASIC_AUTH_USERNAME` / `CLIO_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `CLIO_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `clio_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `clio update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `clio dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Clio Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Clio Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |

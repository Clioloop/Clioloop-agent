...
Clio Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
clio memory setup      # interactive picker + configuration
clio memory status     # check what's active
clio memory off        # disable external provider
```

You can also select the active memory provider via `clio plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.clio/config.yaml`:
...
When a memory provider is active, Clio automatically:
...
**Setup Wizard:**
```bash
clio memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `clio honcho setup` command still works (it now redirects to `clio memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$CLIO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$CLIO_HOME/honcho.json` > `~/.clio/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/Clioloop/Clioloop-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/clio).
...
:::tip Migrating from `clio honcho`
If you previously used `clio honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Clio profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Clio profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Clio profile. Host key `clio` → default; `clio.<profile>` for others. |
...
```bash
clio profile create coder --clone
```

`--clone` creates a `clio.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Clio profile, creates host blocks for any profile without one, inherits settings from the default `clio` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/Clioloop/Clioloop-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/clio).
...
# Then configure Clio
clio memory setup    # select "openviking"
# Or manually:
clio config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.clio/.env
```
...
**Setup:**
```bash
clio memory setup    # select "mem0"
# Or manually:
clio config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.clio/.env
```

**Config:** `$CLIO_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `clio-user` | User identifier |
| `agent_id` | `clio` | Agent identifier |
...
**Setup:**
```bash
clio memory setup    # select "hindsight"
# Or manually:
clio config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.clio/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p clio ui start`

**Config:** `$CLIO_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `clio` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Clio Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/Clioloop/Clioloop-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
clio memory setup    # select "holographic"
# Or manually:
clio config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.clio-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$CLIO_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
clio memory setup    # select "retaindb"
# Or manually:
clio config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.clio/.env
```
...
# Then configure Clio
clio memory setup    # select "byterover"
# Or manually:
clio config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$CLIO_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
clio memory setup    # select "supermemory"
# Or manually:
clio config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.clio/.env
```

**Config:** `$CLIO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `clio` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `clio-{identity}` → `clio-coder`) to isolate memories per Clio profile
...
| **Requires** | `pip install clio-memori` + `clio-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install clio-memori
clio-memori install
clio config set memory.provider memori
clio memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `clio-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$CLIO_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$CLIO_HOME/` so each profile has its own credentials

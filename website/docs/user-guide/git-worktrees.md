...
title: "Git Worktrees"
description: "Run multiple Clio agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Clio Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Clio so each session has a clean, isolated working directory.

## Why Use Worktrees with Clio?

Clio treats the **current working directory** as the project root:

- CLI: the directory where you run `clio` or `clio chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.clio/config.yaml`
...
git worktree add ../repo-feature feature/clio-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/clio-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Clio there:

```bash
cd ../repo-feature

# Start Clio in the worktree
clio
```
...
git worktree add ../repo-experiment-a feature/clio-a
git worktree add ../repo-experiment-b feature/clio-b
```
...
- Works on its own branch (`feature/clio-a` vs `feature/clio-b`).
...
- Clio checkpoint data under `~/.clio/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Clio experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/clio-checkpoints-docs`, `feature/clio-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Clio from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `clio -w` (Automatic Worktree Mode)

Clio has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `clio/clio-<hash>`).
...
```bash
clio -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `clio -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Clio session its own clean checkout.

...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Clio Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Clio Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `clio setup --portal` — and you're ready to `clio chat`. See [managed provider](/integrations/managed-provider).
:::

:::tip
Clio also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `clio --tui` — see the [TUI](tui.md) guide.
:::
...
clio chat --model "anthropic/claude-sonnet-4"

# With a specific provider
clio chat --provider managed        # Use managed provider
clio chat --provider openrouter  # Force OpenRouter
...
clio chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
clio --continue             # Resume the most recent CLI session (-c)
clio --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
clio chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
clio -w                         # Interactive mode in worktree
clio -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Clio CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Clio CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `CLIO_YOLO_MODE` is on (either `clio --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`clio -c` or `clio --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Clio to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Clio supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Clio replies |
...
```bash
clio -s clio-agent-dev,github-auth
clio chat -s github-pr-workflow -s github-auth
```

Clio loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.clio/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
```
/personality pirate
/personality clio-spinner
/personality concise
```

Built-in personalities include: `helpful`, `concise`, `technical`, `creative`, `teacher`, `clio-spinner`, `catgirl`, `pirate`, `shakespeare`, `surfer`, `noir`, `uwu`, `philosopher`, `hype`.

You can also define custom personalities in `~/.clio/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  clio-spinner: "You are a clio-spinner assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Clio..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Clio recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Clio — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Clio is working, Clio prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Clio to the background — just like any terminal process. The shell prints a confirmation:

```
Clio Agent has been suspended. Run `fg` to bring Clio Agent back.
```
...
Resume this session with:
  clio --resume 20260225_143052_a1b2c3
...
```bash
clio --continue                          # Resume the most recent CLI session
clio -c                                  # Short form
clio -c "my project"                     # Resume a named session (latest in lineage)
clio --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
clio --resume "refactoring auth"         # Resume by title
clio -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `clio sessions rename <id> <title>` from the command line. Use `clio sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Clio's SQLite state database under `~/.clio/state.db`. The database keeps:
...
Clio immediately confirms the task and gives you back the prompt:
...
- Enables clio-spinner-style animated feedback

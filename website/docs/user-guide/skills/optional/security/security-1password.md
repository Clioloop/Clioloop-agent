...
| | |
|---|---|
| Source | Optional — install with `clio skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Clio Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Clio terminal calls (desktop app flow only)
...
### Service Account (recommended for Clio)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.clio/.env` (the skill will prompt for this on first load).
...
## Clio Execution Pattern (desktop app flow)

Clio terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/clio-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/clio-op.sock"

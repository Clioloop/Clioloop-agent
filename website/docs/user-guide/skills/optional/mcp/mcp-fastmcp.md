...
| | |
|---|---|
| Source | Optional — install with `clio skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Clio or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Clio. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.clio/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.clio/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Clio integration, either:

- configure the server in `~/.clio/config.yaml` using the `native-mcp` skill, or
...
### Clio cannot see the deployed server

The server-building part may be correct while the Clio config is not. Load the `native-mcp` skill and configure the server in `~/.clio/config.yaml`, then restart Clio.

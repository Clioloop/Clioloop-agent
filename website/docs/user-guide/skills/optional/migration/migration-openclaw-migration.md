---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Clio Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Clio Agent"
---
...
Migrate a user's OpenClaw customization footprint into Clio Agent. Imports Clio-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `clio skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Clio Agent (Omni Loop Labs) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Clio`, `Memory`, `Persona`, `Import` |
| Related skills | [`clio-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-clio-agent) |
...
# OpenClaw -> Clio Migration

Use this skill when a user wants to move their OpenClaw setup into Clio Agent with minimal manual cleanup.
...
```bash
clio claw migrate              # Full interactive migration
...
clio claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `clio setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_clio.py` to:

- import `SOUL.md` into the Clio home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Clio memory entries
- merge OpenClaw command approval patterns into Clio `command_allowlist`
- migrate Clio-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Clio working-directory configuration
- copy OpenClaw skills into `~/.clio/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Clio workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.clio/tts/`
- archive non-secret docs that do not have a direct Clio destination
...
- `scripts/openclaw_to_clio.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.clio/skills/migration/openclaw-migration/scripts/openclaw_to_clio.py`

Do not guess a shorter path like `~/.clio/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.clio/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Clio-compatible secrets, currently:
...
Clio CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Clio. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Clio workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Clio skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.clio/skills/migration/openclaw-migration/scripts/openclaw_to_clio.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.clio/skills/migration/openclaw-migration/scripts/openclaw_to_clio.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.clio/skills/migration/openclaw-migration/scripts/openclaw_to_clio.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.clio/skills/migration/openclaw-migration/scripts/openclaw_to_clio.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.clio/skills/migration/openclaw-migration/scripts/openclaw_to_clio.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.clio/skills/migration/openclaw-migration/scripts/openclaw_to_clio.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Clio unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Clio targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Clio destination. Unsupported auth blobs must still be reported as skipped.
...
- Clio persona state imported
- Clio memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.clio/skills/openclaw-imports/`

...
| | |
|---|---|
| Source | Optional — install with `clio skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Clio Agent |
| License | MIT |
...
import upstream classes into Clio itself.
...
```bash
mkdir -p ~/.clio/cache/darwinian-evolver && cd ~/.clio/cache/darwinian-evolver
...
```bash
cd ~/.clio/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.clio/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.clio/skills/research/darwinian-evolver
DE_DIR=~/.clio/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Clio core.
   Custom driver scripts under `~/.clio/skills/...` are user-side and fine.

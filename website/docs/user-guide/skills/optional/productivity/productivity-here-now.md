...
| | |
|---|---|
| Source | Optional — install with `clio skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${CLIO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CLIO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${CLIO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client clio
```
...
```bash
PUBLISH="${CLIO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client clio
```
...
```bash
DRIVE="${CLIO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `clio`)    |

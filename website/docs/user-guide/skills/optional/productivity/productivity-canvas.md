...
| | |
|---|---|
| Source | Optional — install with `clio skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Clio Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.clio/.env`:
...
```bash
CANVAS="python $CLIO_HOME/skills/productivity/canvas/scripts/canvas_api.py"

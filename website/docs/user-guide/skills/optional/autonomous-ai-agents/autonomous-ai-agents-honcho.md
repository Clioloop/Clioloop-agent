...
description: "Configure and use Honcho memory with Clio -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Clio -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `clio skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Clio

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Clio profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/clio#running-honcho-locally-with-clio

### Verify

```bash
clio honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Clio profile's AI peer
...
Honcho models conversations as interactions between **peers**. Clio creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Clio instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Clio run |
| `global` | Single session across all directories |

Manual override: `clio honcho map my-project-name`
...
Each Clio profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block clio.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `clio.coder` host block in `honcho.json`
...
```bash
clio honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Clio when Honcho memory is active.
...
Config file: `$CLIO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `clio honcho setup`. Ensure `memory.provider: honcho` is in `~/.clio/config.yaml`.

### Memory not persisting across sessions
Check `clio honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `clio profile create <name> --clone`. For existing profiles: `clio honcho sync`.
...
| Command | Description |
|---------|-------------|
| `clio honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `clio honcho status` | Show resolved config, connection test, peer info for active profile |
| `clio honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `clio honcho disable` | Disable Honcho for the active profile |
| `clio honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `clio honcho peers` | Show peer identities across all profiles |
| `clio honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `clio honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `clio honcho sessions` | List known directory-to-session-name mappings |
| `clio honcho map <name>` | Map current working directory to a Honcho session name |
| `clio honcho identity` | Seed AI peer identity or show both peer representations |
| `clio honcho sync` | Create host blocks for all Clio profiles that don't have one yet |
| `clio honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Clio + Honcho |
| `clio memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `clio memory status` | Show active memory provider and config |
| `clio memory off` | Disable external memory provider |

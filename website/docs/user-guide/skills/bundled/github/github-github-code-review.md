...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.clio/.env ] && grep -q "^GITHUB_TOKEN=" ~/.clio/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.clio/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Clio Agent\",
    \"comments\": [
...
```bash
source "${CLIO_HOME:-$HOME/.clio}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Clio Agent. Code looks clean — good test coverage, no security concerns."

...
- Give it a name like "clio-agent"
- Select scopes:
...
- Give it a title like "clio-agent-&lt;machine-name>"
...
elif [ -f ~/.clio/.env ] && grep -q "^GITHUB_TOKEN=" ~/.clio/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.clio/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"

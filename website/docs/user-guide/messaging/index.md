...
description: "Chat with Clio from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Clio from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Clio](/guides/use-voice-mode-with-clio).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [managed provider](/integrations/managed-provider) subscription bundles all of them.
:::
...
```bash
clio gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
clio gateway              # Run in foreground
clio gateway setup        # Configure messaging platforms interactively
clio gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo clio gateway install --system   # Linux only: install a boot-time system service
clio gateway start        # Start the default service
clio gateway stop         # Stop the default service
clio gateway status       # Check default service status
clio gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Clio Agent to the latest version |
...
Configure per-platform overrides in `~/.clio/gateway.json`:
...
clio pairing approve telegram XKGH5N7P

# Other pairing commands:
clio pairing list          # View pending + approved users
clio pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Clio appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.clio/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.clio/config.yaml`:
...
```bash
CLIO_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
clio gateway install               # Install as user service
clio gateway start                 # Start the service
clio gateway stop                  # Stop the service
clio gateway status                # Check status
journalctl --user -u clio-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo clio gateway install --system
sudo clio gateway start --system
sudo clio gateway status --system
journalctl -u clio-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Clio will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Clio installations on the same machine (with different `CLIO_HOME` directories), each gets its own systemd service name. The default `~/.clio` uses `clio-gateway`; other installations use `clio-gateway-<hash>`. The `clio gateway` commands automatically target the correct service for your current `CLIO_HOME`.
:::

### macOS (launchd)

```bash
clio gateway install               # Install as launchd agent
clio gateway start                 # Start the service
clio gateway stop                  # Stop the service
clio gateway status                # Check status
tail -f ~/.clio/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.clio.gateway.plist`. It includes three environment variables:
...
- **CLIO_HOME** — scopes the gateway to your Clio installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `clio gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `CLIO_HOME` directory gets its own launchd label. The default `~/.clio` uses `ai.clio.gateway`; other installations use `ai.clio.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `clio-cli` | Full access |
| Telegram | `clio-telegram` | Full tools including terminal |
| Discord | `clio-discord` | Full tools including terminal |
| WhatsApp | `clio-whatsapp` | Full tools including terminal |
| Slack | `clio-slack` | Full tools including terminal |
| Google Chat | `clio-google_chat` | Full tools including terminal |
| Signal | `clio-signal` | Full tools including terminal |
| SMS | `clio-sms` | Full tools including terminal |
| Email | `clio-email` | Full tools including terminal |
| Home Assistant | `clio-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `clio-mattermost` | Full tools including terminal |
| Matrix | `clio-matrix` | Full tools including terminal |
| DingTalk | `clio-dingtalk` | Full tools including terminal |
| Feishu/Lark | `clio-feishu` | Full tools including terminal |
| WeCom | `clio-wecom` | Full tools including terminal |
| WeCom Callback | `clio-wecom-callback` | Full tools including terminal |
| Weixin | `clio-weixin` | Full tools including terminal |
| BlueBubbles | `clio-bluebubbles` | Full tools including terminal |
| QQBot | `clio-qqbot` | Full tools including terminal |
| Yuanbao | `clio-yuanbao` | Full tools including terminal |
| Microsoft Teams | `clio-teams` | Full tools including terminal |
| API Server | `clio-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `clio-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.clio/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.

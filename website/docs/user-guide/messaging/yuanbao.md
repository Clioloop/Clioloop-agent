...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Clio Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Clio to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.clio/.env`:
...
All standard Clio commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.clio/logs/gateway.log`
...
4. Consider enabling verbose logging: `CLIO_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Clio automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
CLIO_LOG_LEVEL=debug clio gateway
```
...
```bash
clio chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```

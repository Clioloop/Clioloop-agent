...
sidebar_position: 12
title: "Google Chat"
description: "Set up Clio Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Clio Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Clio
...
> Run `clio gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Clio can read (e.g.,
`~/.clio/google-chat-sa.json`, `chmod 600`).
...
- Topic ID: `clio-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `clio-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a clio restart)
...
- Principal: `clio-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Clio calls
...
- **App name**: whatever you want users to see ("Clio" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/clio-chat-events`.
...
`ADDED_TO_SPACE` event that Clio uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Clio

Add the Google Chat section to `~/.clio/.env`:

```bash
# Required
GOOGLE_CHAT_PROJECT_ID=my-chat-bot-123
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/clio-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.clio/google-chat-sa.json
...
Install the dependencies the Google Chat adapter needs (no Clio extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Clio is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Clio detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Clio session.
...
3. Download the JSON. Move it onto the host that runs Clio.
4. Register the client with Clio (run under the profile you want it scoped to):
...
clio -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Clio home (e.g.
`~/.clio/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.clio/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.clio/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Clio isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `clio gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Clio will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.clio/google_chat_user_tokens/<sanitized_email>.json` (filesystem

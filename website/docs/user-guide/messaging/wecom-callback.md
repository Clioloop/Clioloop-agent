...
Connect Clio to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Clio supports two WeCom integration modes:
...
> Run `clio gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Clio decrypts the message, queues it for the agent
...
(Use `clio gateway start` only after `clio gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Clio and the token the
...
in `~/.clio/.env` values around `=` will also break signature checks. After
fixing, restart `clio gateway run`.
...
Check `clio gateway run` logs for the bound host/port. If the adapter bound to

...
If you only run one Clio agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Clio agents that should all
...
(`ai.clio.gateway-<name>.plist`) or systemd user service
(`clio-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
clio profile create coder
clio profile create personal-bot
clio profile create research
...
`~/.local/bin/clio-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: clio-gateways {start|stop|restart|status|list}"
}
...
```bash
clio-gateways start      # start every configured profile
clio-gateways stop       # stop every configured profile
clio-gateways restart    # restart all
clio-gateways status     # status across all
clio-gateways list       # delegates to `clio gateway list`
```

:::tip
The `default` profile is targeted with `clio gateway <action>` (no `-p`),
not `clio -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `clio -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.clio.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/clio-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.clio.gateway.plist` /
`clio-gateway.service`.
...
```bash
# Default profile
tail -f ~/.clio/logs/gateway.log
tail -f ~/.clio/logs/gateway.error.log

# Named profile
tail -f ~/.clio/profiles/<name>/logs/gateway.log
tail -f ~/.clio/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.clio/logs/gateway.log ~/.clio/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
clio logs --tail              # follow default profile
clio -p coder logs --tail     # follow one profile
clio logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
clio profile list             # profiles + model + gateway state
clio-gateways status          # full status across every profile
launchctl list | grep clio    # macOS — PIDs and labels
systemctl --user list-units 'clio-gateway-*'   # Linux — units
```
...
The default profile uses `~/.clio/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
clio config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.clio/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=clio --why="gateways running" \
  sleep infinity &
...
`clio-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.clio/.env ~/.clio/profiles/*/.env
```

## Updating the code

`clio update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `clio gateway start` after a previous `clio gateway stop`. The
...
```bash
ps -ef | grep "clio_cli.*-p <profile>"
cat ~/.clio/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.clio.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.clio.gateway-<profile>.plist

# Linux
systemctl --user restart clio-gateway-<profile>.service
```

### Health check

```bash
clio doctor                  # default profile
clio -p <profile> doctor     # one profile
```

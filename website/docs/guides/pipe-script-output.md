...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `clio send`."
---

# Pipe Script Output to Messaging Platforms

`clio send` is a small, scriptable CLI that pushes a message to any
messaging platform Clio is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | clio send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `clio
...
clio send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
clio send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
clio send --list

# Filter by platform
clio send --list telegram
```
...
Any platform Clio ships adapters for works as a target:
...
`clio send` resolves the message body in this order:

1. **Positional argument** — `clio send --to telegram "hi"`
2. **`--file PATH`** — `clio send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | clio send --to telegram`

When stdin is a TTY (no pipe), Clio does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  clio send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `clio send` reuses your Clio config, the same script works on
any host where Clio is installed — no need to export bot tokens into
...
`clio send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  clio send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | clio send \
...
```bash
./train.py --epochs 200 && \
  clio send --to telegram "training done" || \
  clio send --to telegram "training failed (exit $?)"
```
...
clio send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(clio send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `clio send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `clio send` calls
the platform's REST endpoint directly using credentials from
`~/.clio/.env` and `~/.clio/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `clio gateway start` and retry.
...
The listing is built from `~/.clio/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`clio gateway
...
| Approach | Multi-platform | Reuses Clio creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `clio send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`clio send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `clio send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `clio send` shares with cron delivery.

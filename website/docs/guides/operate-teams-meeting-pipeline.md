...
```bash
clio teams-pipeline token-health
clio teams-pipeline token-health --force-refresh
```
...
```bash
clio teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
clio teams-pipeline maintain-subscriptions
clio teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Clio cron (recommended if you already run the Clio gateway)

Clio ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.clio/scripts/`. First create the script:

```bash
mkdir -p ~/.clio/scripts
cat > ~/.clio/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec clio teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.clio/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
clio cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/clio-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Clio Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=clio
EnvironmentFile=/etc/clio/env
ExecStart=/usr/local/bin/clio teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/clio-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Clio Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now clio-teams-pipeline-maintain.timer
systemctl list-timers clio-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/clio teams-pipeline maintain-subscriptions >> /var/log/clio/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.clio/.env` at the top of a wrapper script that crontab calls.
...
```bash
clio teams-pipeline subscriptions   # should show expirationDateTime advanced
clio teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
clio teams-pipeline list
clio teams-pipeline list --status failed
clio teams-pipeline show <job-id>
```

### Replay a stored job

```bash
clio teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
clio teams-pipeline fetch --meeting-id <meeting-id>
clio teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `clio teams-pipeline maintain-subscriptions --dry-run`
- inspect `clio teams-pipeline list --status failed`
...
- run `clio teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `clio teams-pipeline run`
...
- [ ] `clio teams-pipeline validate` returns an OK snapshot
- [ ] `clio teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Clio cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.

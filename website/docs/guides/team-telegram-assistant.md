...
This tutorial walks you through setting up a Telegram bot powered by Clio Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Clio Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.clio/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Clio itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Clio Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_clio_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Clio Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u clio-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `clio gateway install` to update the plist.
:::
...
```bash
clio gateway stop && clio gateway start
```
...
clio pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.clio/.env`:
...
Control how much detail the bot shows when using tools. In `~/.clio/config.yaml`:
...
Customize how the bot communicates by editing `~/.clio/SOUL.md`:

For a full guide, see [Use SOUL.md with Clio](/guides/use-soul-with-clio).
...
```bash
# From the CLI
clio cron list          # View all scheduled jobs
clio cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u clio-gateway` (Linux) or `~/.clio/logs/gateway.log` (macOS) |
| Cron job output | `~/.clio/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.clio/cron/jobs.json` |
| Pairing data | `~/.clio/pairing/` |
| Session history | `~/.clio/sessions/` |

...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Clio Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Clio instantly when PRs are opened or updated.
:::
...
- **Clio Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  clio gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.clio/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Clio can access GitHub. Start a chat:
...
```
Run: gh pr list --repo Clioloop/Clioloop-agent --state open --limit 3
```
...
Still in the chat, ask Clio to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo Clioloop/Clioloop-agent
```
...
A skill gives Clio consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.clio/skills/code-review
```

Create `~/.clio/skills/code-review/SKILL.md`:
...
Verify it loaded — start `clio` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Clio your team's standards:
...
2. Teach Clio your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
clio gateway status    # Is the gateway running?
clio cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-clio-plugin)** — wrap the review logic into a shareable plugin

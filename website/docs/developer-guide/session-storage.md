...
Clio Agent uses a SQLite database (`~/.clio/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `clio_state.py`


## Architecture Overview

```
~/.clio/state.db (SQLite, WAL mode)
...
Multiple clio processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.clio/state.db
...
Default path: `~/.clio/state.db`

This is derived from `clio_constants.get_clio_home()` which resolves to
`~/.clio/` by default, or the value of `CLIO_HOME` environment variable.

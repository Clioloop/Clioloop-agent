...
description: "How Clio stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `clio_cli/cron.py` | CLI `clio cron` subcommands |
...
Jobs are stored in `~/.clio/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `clio cron` commands are run or during active CLI sessions.
...
```python
# ~/.clio/scripts/check_competitors.py
...
2. **Environment variable** — `CLIO_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.clio/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `clio cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `clio cron` CLI provides direct job management:

```bash
clio cron list                    # Show all jobs
clio cron create                  # Interactive job creation (alias: add)
clio cron edit <job_id>           # Edit job configuration
clio cron pause <job_id>          # Pause a running job
clio cron resume <job_id>         # Resume a paused job
clio cron run <job_id>            # Trigger immediate execution
clio cron remove <job_id>         # Delete a job
```

...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Clio Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Clio Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Clio core? Start with [Build a Clio Plugin](../guides/build-a-clio-plugin.md)
- Building a new built-in core tool for Clio itself? Start with [Adding Tools](./adding-tools.md)
...
```bash
mkdir -p ~/.clio/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.clio/config.yaml
touch ~/.clio/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.clio/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/clio" ~/.local/bin/clio
...
- **Profile-safe paths**: Never hardcode `~/.clio` — use `get_clio_home()` from `clio_constants` for code paths and `display_clio_home()` for user-facing messages. See [AGENTS.md](https://github.com/Clioloop/Clioloop-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

Clio officially supports **Linux, macOS, WSL2, and native Windows (via PowerShell install)**.  Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands.  A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. If you're doing Windows-heavy dev, run the Windows-footgun lint (`scripts/check-windows-footguns.py`) before pushing.
...
Clio has terminal access. Security matters.
...
2. **Test manually**: Run `clio` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/Clioloop/Clioloop-agent/issues)
- Include: OS, Python version, Clio version (`clio version`), full error traceback
...
- **Discord**: [discord.gg/Clioloop](https://discord.gg/Clioloop)
- **GitHub Discussions**: For design proposals and architecture discussions
- **Skills Hub**: Upload specialized skills and share with the community

## License

By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/Clioloop/Clioloop-agent/blob/main/LICENSE).

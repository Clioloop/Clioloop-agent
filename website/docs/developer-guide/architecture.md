...
sidebar_position: 1
title: "Architecture"
description: "Clio Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Clio Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ clio_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ClioCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── clio_state.py           # SQLite session/state database with FTS5
├── clio_constants.py       # CLIO_HOME, profile-aware paths
...
│
├── clio_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `clio` subcommands (large file)
...
│   ├── skills_config.py      # clio skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # clio tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # clio gateway start/stop
│
...
```text
User input → ClioCLI.process_input()
...
Three discovery sources: `~/.clio/plugins/` (user), `.clio/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `clio plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-clio-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Clio as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`clio -p <name>`) gets its own CLIO_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |

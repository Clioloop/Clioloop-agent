...
The ACP adapter wraps Clio' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
clio acp / clio-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.clio/.env
  -> configure stderr logging
  -> construct ClioACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'clio-agent[acp]==<version>' clio-acp`, pointed at the `clio-agent` PyPI release.
...
- `allow_once` -> Clio `once`
- `allow_always` -> Clio `always`
- reject options -> Clio `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Clio tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["clio-acp"])
...
Instead it reuses Clio' runtime resolver:

- `acp_adapter/auth.py`
- `clio_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Clio provider/credentials. It also always advertises a terminal setup auth method (`clio-setup`, args `--setup`) so first-run registry clients can open Clio' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.clio/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `clio-acp` toolset definition
- `clio_cli/main.py` — `clio acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `clio-acp` script

...
Clio observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Clio' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Clio catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Clio only builds

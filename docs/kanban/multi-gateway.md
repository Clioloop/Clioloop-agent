...
Clio supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.clio/config.yaml`:
...
Or set the env var: `CLIO_KANBAN_DISPATCH_IN_GATEWAY=false`

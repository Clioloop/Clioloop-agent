...
When running Clio inside Docker, the default `network_mode: host` gives the
...
The Clio [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   CLIO_UID=$(id -u) CLIO_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=clio,clio-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://clio-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Clio trust model and vulnerability reporting

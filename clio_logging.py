"""Centralized logging setup for Clio Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.clio/logs/`` (profile-aware via ``get_clio_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``clio_cli.web_server``,
    ``clio_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from clio_constants import get_config_path, get_clio_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_clio_session_injector", False):
    ...
    _session_record_factory._clio_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``clio logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "clio_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Clio logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    clio_home
        Override for the Clio home directory.  Falls back to
        ``get_clio_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_clio_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._clio_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the clio group. However, both ``_open()`` (initial creation)

"""
Microsoft Teams platform adapter for Clio Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Clio"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        clio_action = data.get("clio_action", "")
        session_key = data.get("session_key", "")

        if not clio_action or not session_key:
        ...
        choice = choice_map.get(clio_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="clio_approve",
                    data={**btn_data_base, "clio_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="clio_approve",
                    data={**btn_data_base, "clio_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="clio_approve",
                    data={**btn_data_base, "clio_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="clio_approve",
                    data={**btn_data_base, "clio_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Clio\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.clio/.env")

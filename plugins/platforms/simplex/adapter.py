"""SimpleX Chat platform adapter (Clio plugin).
...
This adapter ships as a Clio platform plugin under
``plugins/platforms/simplex/``. The Clio plugin loader scans the
...
discoverable and `clio setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Clio — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"clio-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``clio setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.clio/.env`` via ``clio_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("clio_cli.config not available; set SIMPLEX_* vars manually in ~/.clio/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `clio gateway status` without

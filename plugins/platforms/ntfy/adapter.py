"""ntfy platform adapter (Clio plugin).
...
``plugins/platforms/ntfy/``. The Clio plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "clio-in"              # subscribe topic (incoming)
          publish_topic: "clio-out"     # optional — defaults to topic
...
_ECHO_TAG = "clio-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``clio cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Clio dependency",

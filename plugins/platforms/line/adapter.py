"""
LINE Messaging API platform adapter for Clio Agent.
...
to Clio Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CLIO_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            clio_home = Path.home().joinpath(".clio").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``clio status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``clio status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``clio setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.clio/.env`` via ``clio_cli.config``.
    """
    print()
    ...
    try:
        from clio_cli.config import get_env_var, set_env_var
    except ImportError:
        print("clio_cli.config not available; set LINE_* vars manually in ~/.clio/.env")
        return

...
# ``model_tools`` import time, so ``clio status``, ``clio chat``, etc.
...
# ``clio_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Clio is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a clio MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CLIO_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _clio_home = _get_clio_home()
        except (ModuleNotFoundError, ImportError):
            _clio_home = _Path.home() / ".clio"
        self._thread_count_store = _ThreadCountStore(
            _clio_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Clio is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("CLIO_HOME", str(_Path.home() / ".clio"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CLIO_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a clio MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Clio is thinking…' marker message.
        ...
        "Clio is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Clio is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Clio is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Clio is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Clio is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``clio setup``.

    The setup wizard at ``clio_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.clio/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. clio-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.clio/.env")
    print_info("Restart the gateway: clio gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``clio cron`` running as a
    separate process from ``clio gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Clio plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'clio-agent[google_chat]'",
        ...
        # the "Clio is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Clio is thinking…' marker message "

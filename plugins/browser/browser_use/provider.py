...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed provider tool
gateway (which Clio uses to bill Browser Use sessions to a . 
...
Auth env vars (one of)::
    ...
    # OR a managed provider gateway entry (configured via 'clio setup')
"""
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed provider tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed provider gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the managed provider auth stack which can be
        ...
        managed = resolve_managed_tool_gateway(
            "browser-use",
            token_reader=None if refresh_token else peek_managed_access_token,
        )
        if managed is None:
            return None
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Clio only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"clio_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (

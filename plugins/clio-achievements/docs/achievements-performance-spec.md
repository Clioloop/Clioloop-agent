# Clio Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: clio-achievements plugin
...
### `GET /api/plugins/clio-achievements/achievements`
...
### `POST /api/plugins/clio-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/clio-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/clio-achievements/overview`
...
- Persist plugin data under:
  - `~/.clio/plugins/clio-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.clio/plugins/clio-achievements/scan_checkpoint.json`
...
- `~/.clio/plugins/clio-achievements/`

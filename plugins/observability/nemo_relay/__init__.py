"""nemo_relay — optional Clio plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atof_filename: str = "clio-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "clio-atif-{session_id}.json"
    atif_subagent_export_mode: str = "embedded"
    atif_agent_name: str = "Clio Agent"
...
class _Runtime:
    ...
    def _configure_atof(self) -> None:
        ...
        self.atof_exporter.register("clio.nemo_relay.atof")
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if self.settings.atif_enabled:
            state.atif_exporter = self.nemo_relay.AtifExporter(
                ...
                extra={"source": "clio-agent", "plugin": "observability/nemo_relay"},
            )
            state.atif_subscriber_name = f"clio.nemo_relay.atif.{session_id}"
    ...
    def mark_subagent_stop(self, kwargs: dict[str, Any]) -> None:
        ...
        self.mark("clio.subagent.stop", kwargs)
...
def on_session_end(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: (runtime.mark("clio.session.end", kwargs), runtime.export_atif(runtime.ensure_session(kwargs))))
...
def on_pre_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("clio.turn.start", kwargs))


def on_post_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("clio.turn.end", kwargs))
...
def on_post_api_request(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("clio.api.response.unmatched", kwargs)
            return
...
def on_api_request_error(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        span = state.llm_spans.pop(_api_key(kwargs), None)
        if span is None:
            runtime.mark("clio.api.error", kwargs)
            return
        runtime.nemo_relay.llm.call_end(
            span,
...
def on_post_tool_call(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("clio.tool.response.unmatched", kwargs)
            return
...
def on_pre_approval_request(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("clio.approval.request", kwargs))


def on_post_approval_response(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("clio.approval.response", kwargs))
...
def _load_settings() -> _Settings:
    return _Settings(
        plugins_toml_path=_env("CLIO_NEMO_RELAY_PLUGINS_TOML"),
        atof_enabled=_env_bool("CLIO_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("CLIO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("CLIO_NEMO_RELAY_ATOF_FILENAME") or "clio-atof.jsonl",
        atof_mode=_env("CLIO_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("CLIO_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("CLIO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("CLIO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "clio-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("CLIO_NEMO_RELAY_ATIF_AGENT_NAME") or "Clio Agent",
        atif_agent_version=_env("CLIO_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("CLIO_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
    )
...
def _atif_subagent_export_mode() -> str:
    mode = _env("CLIO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()

...
Optional Clio observability plugin that maps Clio observer hooks to
...
not replace Clio Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Clio emit NeMo Relay lifecycle events for
the work Clio already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Clio Agent can:

- Preserve Clio execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
clio plugins enable observability/nemo_relay
```

The `CLIO_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `CLIO_HOME` that the
agent run will use:

```bash
env CLIO_HOME=/tmp/clio-nemo-relay-test \
  clio plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`CLIO_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`CLIO_HOME` is the Clio profile/config home used by both
`clio plugins enable ...` and the later `clio chat ...` run. If unset,
Clio uses the user's default home, usually `~/.clio`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export CLIO_HOME=/tmp/clio-nemo-relay-test
clio plugins enable observability/nemo_relay
clio chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `clio` command you run is built from the
...
uv run clio plugins enable observability/nemo_relay
uv run clio chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/clio_agent-*.whl
...
The plugin can configure exporters directly from `CLIO_NEMO_RELAY_*`
...
```bash
export CLIO_NEMO_RELAY_ATOF_ENABLED=1
export CLIO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export CLIO_NEMO_RELAY_ATIF_ENABLED=1
export CLIO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `CLIO_NEMO_RELAY_ATOF_FILENAME`
- `CLIO_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `CLIO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `CLIO_NEMO_RELAY_ATIF_AGENT_NAME`
- `CLIO_NEMO_RELAY_ATIF_AGENT_VERSION`
- `CLIO_NEMO_RELAY_ATIF_MODEL_NAME`
- `CLIO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Clio at it:

```bash
export CLIO_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `CLIO_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`CLIO_NEMO_RELAY_ATOF_*` fallback setup is skipped.
...
export CLIO_HOME=/tmp/clio-nemo-relay-docs/clio-home
mkdir -p "$CLIO_HOME"

cat > "$CLIO_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Clio session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export CLIO_NEMO_RELAY_ATOF_ENABLED=1
export CLIO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/clio-nemo-relay-docs/subagent/atof
export CLIO_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export CLIO_NEMO_RELAY_ATOF_MODE=overwrite
export CLIO_NEMO_RELAY_ATIF_ENABLED=1
export CLIO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/clio-nemo-relay-docs/subagent/atif
export CLIO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export CLIO_NEMO_RELAY_ATIF_AGENT_NAME='Clio Agent E2E'
export CLIO_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export CLIO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"clio.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Clio Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Clio dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/clio-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/clio-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/clio-nemo-relay-docs/workdir/beta.txt
cd /tmp/clio-nemo-relay-docs/workdir

export CLIO_NEMO_RELAY_ATOF_ENABLED=1
export CLIO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/clio-nemo-relay-docs/parallel/atof
export CLIO_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export CLIO_NEMO_RELAY_ATOF_MODE=overwrite
export CLIO_NEMO_RELAY_ATIF_ENABLED=1
export CLIO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/clio-nemo-relay-docs/parallel/atif
export CLIO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export CLIO_NEMO_RELAY_ATIF_AGENT_NAME='Clio Agent E2E'
export CLIO_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Clio sessions map to `agent` scopes.
- Clio API request hooks map to `llm` scope start/end events.
- Clio tool hooks map to `tool` scope start/end events.

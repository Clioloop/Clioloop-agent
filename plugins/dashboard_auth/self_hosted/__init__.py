...
A standards-compliant OpenID Connect Relying Party for the ``clio dashboard``
OAuth gate. Unlike the bundled ``managed`` provider (which encodes managed provider's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-managed-refresh-token`` header, an ``oauth_contract_version`` claim),
...
:class:`~clio_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``clio_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~clio_cli.dashboard_auth.Session`.
...
only choice that is universally correct across self-hosted IDPs. (The ``managed``
provider verifies its *access* token because managed provider mints a custom JWT
...
same precedence convention as the ``managed`` plugin)::
    ...
    CLIO_DASHBOARD_OIDC_ISSUER
    CLIO_DASHBOARD_OIDC_CLIENT_ID
    CLIO_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the managed
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the managed plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "clio_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Clio does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        operator-error case with a clear message. Mirrors the managed provider.
        """
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``CLIO_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "CLIO_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "CLIO_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("CLIO_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(CLIO_DASHBOARD_OIDC_ISSUER + CLIO_DASHBOARD_OIDC_CLIENT_ID) "

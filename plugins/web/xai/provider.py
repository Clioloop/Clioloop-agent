...
``{title, url, description, position}`` rows every other Clio web
provider produces.
...
prefers Clio-managed xAI Grok OAuth (via ``clio auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.clio/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``clio tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `clio auth` to sign in with "

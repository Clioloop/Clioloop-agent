"""disk_cleanup — ephemeral file cleanup for Clio Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under CLIO_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CLIO_HOME and /tmp/clio-*
Never touches: ~/.clio/logs/ or any system directory.
"""
...
try:
    ...
    def get_clio_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CLIO_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".clio").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CLIO_HOME/logs/``."""
    return get_clio_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CLIO_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CLIO_HOME or ``/tmp/clio-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    clio_home = get_clio_home()
    try:
        path.resolve().relative_to(clio_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/clio-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("clio-"):
        return True
    return False
...
# Paths under $CLIO_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so CLIO_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        clio_home = get_clio_home()
        for parent in ("cron", "cronjobs"):
            base = clio_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CLIO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CLIO_HOME (but leave CLIO_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "clio-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(clio_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == clio_home:
                continue
            try:
                rel_parts = dirpath.relative_to(clio_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    clio_home = get_clio_home()
    try:
        rel = path.resolve().relative_to(clio_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "clio-agent",
        }:
            return None
        ...
        # Path isn't under CLIO_HOME (e.g. /tmp/clio-*) — fall through.
        pass

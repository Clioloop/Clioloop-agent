...
Auto-tracks and cleans up ephemeral files created during Clio Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CLIO_HOME` and `/tmp/clio-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CLIO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CLIO_HOME` or `/tmp/clio-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CLIO_HOME/disk-cleanup/` is itself excluded
- `$CLIO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

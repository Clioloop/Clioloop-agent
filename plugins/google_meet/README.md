...
Let the clio agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where clio runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `clio meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `clio meet` CLI |
...
| `cli.py` | `clio meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CLIO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `clio meet node {run,list,approve,remove,status,ping}` |
...
clio meet install                                      # pip + Chromium
clio meet setup                                        # preflight
clio meet auth                                         # optional
clio meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
clio meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.clio/.env
clio meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
clio meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
clio meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.clio/.env
clio meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, clio will **not** switch your system audio input automatically — the
...
clio plugins enable google_meet
clio meet node run --display-name my-mac --host 0.0.0.0 --port 18789

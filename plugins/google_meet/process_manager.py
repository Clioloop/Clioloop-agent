...
session-scoped state file under ``$CLIO_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $CLIO_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_clio_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this clio install, leave it first —
    ...
    env = os.environ.copy()
    env["CLIO_MEET_URL"] = url
    env["CLIO_MEET_OUT_DIR"] = str(out)
    env["CLIO_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["CLIO_MEET_HEADED"] = "1"
    if auth_state:
        env["CLIO_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["CLIO_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if CLIO_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["CLIO_MEET_MODE"] = mode
    if realtime_model:
        env["CLIO_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["CLIO_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["CLIO_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["CLIO_MEET_REALTIME_KEY"] = realtime_api_key

...
```bash
clio plugins enable security-guidance
# or edit ~/.clio/config.yaml manually:
plugins:
  enabled:
    - security-guidance
```
...
* **No LLM diff review.** Anthropic's layer 2 spawns an auxiliary LLM call
  on every agent turn that touched files. On clio that would route
...
* **No project-local rules file.** Anthropic's `.claude/claude-security-guidance.md`
  ...
  add an analogous `.clio/security-guidance.md` once layer 2 lands.
...
* `__init__.py`, `plugin.yaml`, `README.md`, and tests are original work by
  Clioloop, MIT-licensed alongside the rest of clio-agent.

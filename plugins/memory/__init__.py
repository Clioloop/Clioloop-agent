...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with clio-agent)
2. User-installed providers: ``$CLIO_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_clio_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_clio_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_clio_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$CLIO_HOME/plugins/`` or None if unavailable."""
    try:
        from clio_constants import get_clio_home
        d = get_clio_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$CLIO_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _clio_user_memory.<name>.cli, usually

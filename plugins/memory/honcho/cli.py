...
Handles: clio honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "clio" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Clio profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'clio honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'clio honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `clio update` -- no output, no exceptions.
    """
    try:
        from clio_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Clio profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $CLIO_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_clio_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        clio_host.get("pinUserPeer"),
        clio_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in clio_host:
        aliases_src = clio_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in clio_host:
        prefix_src = clio_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(clio_host: dict) -> None:
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Clio persistent cross-session memory.")
    ...
    clio_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        current_host_key = clio_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            clio_host["apiKey"] = new_local_key
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'clio honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = clio_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        clio_host["peerName"] = new_peer

    current_ai = clio_host.get("aiPeer") or cfg.get("aiPeer", "clio")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        clio_host["aiPeer"] = new_ai

    current_workspace = clio_host.get("workspace") or cfg.get("workspace", "clio")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        clio_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, clio_host)
    ...
    if current_shape == "single" and new_shape == "multi":
        peer_target = clio_host.get("peerName") or current_peer or "user"
        print(
    ...
    if new_shape == "single":
        _scrub_identity_mapping(clio_host)
        clio_host["pinPeerName"] = True
        print(f"  pinPeerName=true → all gateway users route to '{clio_host.get('peerName', '?')}'.")
        ...
        if prior_aliases:
            clio_host["userPeerAliases"] = prior_aliases
        ...
        if _new_prefix and not (prefix_from_root and _new_prefix == current_prefix):
            clio_host["runtimePeerPrefix"] = _new_prefix
        print("  Multi-user mode: each runtime ID → own peer. Use 'clio honcho status' to inspect.")
        ...
        if existing_aliases:
            clio_host["userPeerAliases"] = existing_aliases
        _prefix_default = current_prefix or ""
        _new_prefix = _prompt(
    ...
    # --- 4. Observation mode ---
    current_obs = clio_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        clio_host["observationMode"] = new_obs
    else:
        clio_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(clio_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        clio_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        clio_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = clio_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        clio_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = clio_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        clio_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                clio_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(clio_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            clio_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        clio_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        clio_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        clio_host["dialecticReasoningLevel"] = new_reasoning
    else:
        clio_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = clio_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        clio_host["sessionStrategy"] = new_strat

    clio_host["enabled"] = True
    clio_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        clio_config = load_config()
        clio_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(clio_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: clio config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    clio honcho status     -- show full config")
    print("    clio honcho mode       -- change recall/observation mode")
    print("    clio honcho tokens     -- tune context and dialectic budgets")
    print("    clio honcho peer       -- update peer names")
    print("    clio honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Clio profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: clio honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'clio honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: clio honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        clio = hosts.get(_host_key(), {})
        user = clio.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = clio.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = clio.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = clio.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Clio' identity in Honcho. Seed with 'clio honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: clio honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: clio honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = clio.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = clio.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = clio.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Clio asks Honcho's AI peer a question")
        ...
        print("\n  Set with: clio honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'clio honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'clio honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    clio honcho identity --show        — show both peer representations")
        print("    clio honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Clio + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Clio\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Clio persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  clio honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'clio honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'clio honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  clio honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'clio'")
        print("  with Honcho configured and no prior session history.")
        print("  (Clio calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    clio honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'clio honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Clio, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'clio honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    clio honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  clio honcho identity <path/to/SOUL.md>")
    ...
    print("    Clio:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Clio:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Clio:   Honcho observes every message and updates representations")
    ...
    print("    Clio:   per-session by default — each run gets its own session")
    print("              Map a custom name:  clio honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. clio honcho setup              — configure API key (required)")
        print("  2. clio honcho migrate            — re-run this walkthrough")
    else:
        print("  1. clio honcho status             — verify Honcho connection")
        print("  2. clio                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. clio honcho identity --show    — verify AI peer representation")
        print("  4. clio honcho tokens             — tune context and dialectic budgets")
        print("  5. clio honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'clio memory setup'...\n")
        from clio_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``clio honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``clio honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to clio memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Clio Honcho",
    )

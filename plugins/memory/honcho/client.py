...
Resolution order for config file:
  1. $CLIO_HOME/honcho.json  (instance-local, enables isolated Clio instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Clio profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Clio profile.

    Resolution order:
      1. CLIO_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``clio.<profile>``
      3. Fallback: ``"clio"`` (default profile)
    """
    explicit = os.environ.get("CLIO_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $CLIO_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.clio/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_clio_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_clio_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Clio system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.clio
    ...
    def from_global_config(
        ...
        Resolution: $CLIO_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Clio profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.clio block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Clio session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Clio session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Clio session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'clio honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run clio honcho setup" hint they used to.
    try:
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `clio honcho setup` to configure)."
        )
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            clio_cfg = load_config()
            honcho_cfg = clio_cfg.get("honcho", {})

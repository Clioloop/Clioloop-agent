...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/clio>
...
```bash
clio memory setup honcho   # configure Honcho directly (works on a fresh install)
clio memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
clio config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.clio/.env
```

> `clio honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `clio memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CLIO_HOME/honcho.json` | Profile-local (isolated Clio instances) |
| 2 | `~/.clio/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Clio profile: `clio` (default) or `clio_<profile>`.
...
**Deployment shapes** (`clio memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Clio to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Clio session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`clio-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`clio`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-clio-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening clio in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Clio session ID (timestamp + hex). Every `clio` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Clio profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`clio`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Clio profile: `clio` (default) or `clio_<profile>` (e.g. `clio -p coder` -> host key `clio_coder`). Older `clio.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `CLIO_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `clio memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `clio honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `clio memory setup`) |
| `clio honcho status` | Show resolved config for active profile |
| `clio honcho enable` / `disable` | Toggle Honcho for active profile |
| `clio honcho mode <mode>` | Change recall or observation mode |
| `clio honcho peer --user <name>` | Update user peer name |
| `clio honcho peer --ai <name>` | Update AI peer name |
| `clio honcho tokens --context <N>` | Set context token budget |
| `clio honcho tokens --dialectic <N>` | Set dialectic max chars |
| `clio honcho map <name>` | Map current directory to a session name |
| `clio honcho sync` | Create host blocks for all Clio profiles |

...
Config file: `$CLIO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `clio` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `clio-{identity}` → `clio-coder`). |
...
All Supermemory API calls send `x-sm-source: clio`, and document writes stamp
`metadata.sm_source: clio`. This is a **functional routing key, not telemetry**:
it groups Clio-written memories into a dedicated "Clio" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Clio profile:

```json
{
  "container_tag": "clio-{identity}"
}
```

For a profile named `coder`, this resolves to `clio-coder`. The default profile resolves to `clio-default`. Without `{identity}`, all profiles share the same container.

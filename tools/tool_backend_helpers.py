...
def _portal_account_info(*, force_fresh: bool = False):
    """Fetch Omni Loop Portal account info via the module attribute.
...
def managed_tool_gateway_unavailable_message(
    capability: str = "the Omni Loop Portal Tool Gateway",
    *,
    ...
    if info is None or not getattr(info, "logged_in", False):
        return (
            f"{capability} requires an Omni Loop Portal login. "
    ...
    return f"{capability} is not included in your Omni Loop Portal plan.{upgrade}"

...
def managed_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Stub: managed provider Tool Gateway removed in Clio rebrand.

    Always returns False — managed provider tools are no longer available.
    Kept as a symbol so callers in status.py / tools_config.py compile.
    """
    return False


def managed_tool_gateway_unavailable_message(
    capability: str = "the managed provider Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Stub: managed provider Tool Gateway removed in Clio rebrand."""
    return (
        f"{capability} is unavailable. managed provider Tool Gateway support has been "
        "removed in the Clio rebrand. Use the built-in `browser` toolset "
        "or configure your own image-gen / TTS provider."
    )

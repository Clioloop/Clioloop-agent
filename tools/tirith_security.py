...
it is automatically downloaded from GitHub releases to $CLIO_HOME/bin/tirith.
...
def _get_clio_home() -> str:
    """Return the Clio home directory, respecting CLIO_HOME env var."""
    return str(get_clio_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_clio_home(), ".tirith-install-failed")
...
def _clio_bin_dir() -> str:
    """Return $CLIO_HOME/bin, creating it if needed."""
    d = os.path.join(_get_clio_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CLIO_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_clio_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CLIO_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CLIO_HOME/bin/tirith
    ...
    clio_bin = os.path.join(_clio_bin_dir(), "tirith")
    if os.path.isfile(clio_bin) and os.access(clio_bin, os.X_OK):
        _resolved_path = clio_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return clio_bin

...
One ``CDPSupervisor`` runs per Clio ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "clio-dialog-bridge.invalid"
...
(() => {
  if (window.__clioDialogBridgeInstalled) return;
  window.__clioDialogBridgeInstalled = true;
  const ENDPOINT = "http://clio-dialog-bridge.invalid/";

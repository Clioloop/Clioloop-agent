"""
Lazy dependency installer for opt-in Clio Agent backends.

Many Clio features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``clio-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``clio tools`` or the manual pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    "tool.qrcode": ("qrcode==7.4.2",),

    # ─── Terminal backends ─────────────────────────────────────────────────
    "terminal.modal": ("modal==1.3.4",),
    "terminal.daytona": ("daytona==0.155.0",),
    "terminal.vercel": ("vercel==0.10.0",),
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("CLIO_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``clio update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``clio_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``clio update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``clio update``. Never raises; lazy-install failures

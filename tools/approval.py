...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("CLIO_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("CLIO_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("CLIO_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("CLIO_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set CLIO_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind CLIO_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds CLIO_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("CLIO_CRON_SESSION"):
        return False
    if env_var_enabled("CLIO_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $CLIO_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_CLIO_ENV_PATH = (
    r'(?:~\/\.clio/|'
    r'(?:\$home|\$\{home\})/\.clio/|'
    r'(?:\$clio_home|\$\{clio_home\})/)'
    r'\.env\b'
)
# ~/.clio/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _CLIO_ENV_PATH; matches the CLIO_HOME override form as
# well as ~/.clio/.
_CLIO_CONFIG_PATH = (
    r'(?:~\/\.clio/|'
    r'(?:\$home|\$\{home\})/\.clio/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bclio\s+gateway\s+(stop|restart)\b', "stop/restart clio gateway (kills running agents)"),
    (r'\bclio\s+update\b', "clio update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `clio gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart clio-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart clio-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(clio|gateway|cli\.py)\b', "kill clio/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill clio` but not
    # `kill -9 $(pgrep -f clio)` because the substitution is opaque
    ...
    # In-place edit of a Clio-managed security file (~/.clio/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_CLIO_CONFIG_PATH}|{_CLIO_ENV_PATH})', "in-place edit of Clio config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_CLIO_CONFIG_PATH}|{_CLIO_ENV_PATH})', "in-place edit of Clio config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_CLIO_CONFIG_PATH}|{_CLIO_ENV_PATH})', "in-place edit of Clio config/env (perl/ruby)"),
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("CLIO_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set CLIO_INTERACTIVE or CLIO_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("CLIO_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("CLIO_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("CLIO_EXEC_ASK")

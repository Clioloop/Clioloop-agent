...
This module provides the glue:
    - ``ClioTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Clio Agent"
"""
...
from clio_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__clio_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses CLIO_HOME so each profile gets its own OAuth tokens.
    Layout: ``CLIO_HOME/mcp-tokens/``
    """
    try:
        from clio_constants import get_clio_home
        base = Path(get_clio_home())
    except ImportError:
        base = Path(os.environ.get("CLIO_HOME", str(Path.home() / ".clio")))
...
# ---------------------------------------------------------------------------
# ClioTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ClioTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        CLIO_HOME/mcp-tokens/<server_name>.json         -- tokens
        CLIO_HOME/mcp-tokens/<server_name>.client.json   -- client info
        CLIO_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Clio records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Clio.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://github.com/Clioloop/Clioloop-agent/tree/main/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `clio mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = ClioTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Clio Agent")
    scope = cfg.get("scope")

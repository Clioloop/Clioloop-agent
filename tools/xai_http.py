...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``clio tools`` repaint, tool-registration scans,
    ...
    2. ``~/.clio/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_clio_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.clio/.env`` first, then ``os.environ``.

    Wraps :func:`clio_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from clio_cli.config import get_env_value as _clio_get_env_value

        value = _clio_get_env_value(name)
...
def clio_xai_user_agent() -> str:
    """Return a stable Clio-specific User-Agent for xAI HTTP calls."""
    try:
        from clio_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Clio-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Clio-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``clio_cli.config.get_env_value`` so keys
    stored in ``~/.clio/.env`` (the standard Clio location) are honored —

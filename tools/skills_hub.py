#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Clio Skills Hub.
...
Used by clio_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            clio_meta = metadata.get("clio", {})
            if isinstance(clio_meta, dict):
                tags = clio_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Omni Loop Labs) but not activated
...
# ---------------------------------------------------------------------------
# Clio centralized index source
# ---------------------------------------------------------------------------

CLIO_INDEX_URL = "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/api/skills-index.json"
CLIO_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "clio-index.json"
CLIO_INDEX_TTL = 6 * 3600  # 6 hours


def _load_clio_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for CLIO_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if CLIO_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - CLIO_INDEX_CACHE_FILE.stat().st_mtime
            if age < CLIO_INDEX_TTL:
                return json.loads(CLIO_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(CLIO_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Clio index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Clio index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        CLIO_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        CLIO_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ClioIndexSource(SkillSource):
    """Skill source backed by the centralized Clio Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "clio-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "clio-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ClioIndexSource(auth=auth), # Centralized index (search + resolved install paths)

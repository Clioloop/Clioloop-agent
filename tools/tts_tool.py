...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.clio/config.yaml``. Clio
...
Configuration is loaded from ~/.clio/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``clio_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from clio_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import clio_xai_user_agent
...
def _import_supertonic():
    ...
    Supertonic (Supertone) is a fast local ONNX TTS engine — `pip install
    supertonic` ships CPU wheels; the supertonic-3 model (~66M params)
    auto-downloads on first use and synthesizes faster than realtime on CPU.
    """
...
DEFAULT_SUPERTONIC_MODEL = "supertonic-3"
...
def _get_default_output_dir() -> str:
    ...
    return str(get_clio_dir("cache/audio", "audio_cache"))
...
PROVIDER_MAX_TEXT_LENGTH: Dict[str, int] = {
    ...
    "supertonic": 5000,   # local ONNX model, chunks internally
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.clio/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.clio/config.yaml.
    ...
    try:
        ...
        logger.debug("clio_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
def _provider_display(provider: str) -> str:
    ...
    The managed Omni Loop Portal Subscription gateway is a self-hosted Supertonic server
    ...
    if provider == "openai" and prefers_gateway("tts"):
        return "Omni Loop Portal Subscription"
    return provider
...
# custom voice-cloning scripts, etc.) into Clio without any Python code
...
#           output_format: wav
#
# Clio writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__CLIO_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `clio model` or set XAI_API_KEY.")
    ...
    # output_format when Clio actually needs a non-default format/override.
...
def _check_supertonic_available() -> bool:
    """Check if the supertonic engine is importable (installed locally)."""
    try:
        import importlib.util
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Clio caches Piper voice models.

    Resolves to ``~/.clio/cache/piper-voices/`` under the active
    CLIO_HOME so voice downloads follow profile boundaries.
    """
    from clio_constants import get_clio_dir
    root = Path(get_clio_dir("cache/piper-voices", "piper_voices_cache"))
...
_supertonic_engine_cache: Dict[str, Any] = {}
_supertonic_style_cache: Dict[tuple, Any] = {}


def _generate_supertonic(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    """Generate speech with Supertonic (local ONNX, CPU, no API key).

    Config (tts.supertonic.*): model (supertonic|supertonic-2|supertonic-3),
    voice (style name, e.g. F1/M1), speed, lang.
    """
    TTS = _import_supertonic()
    st_config = tts_config.get("supertonic", {})
    model_name = st_config.get("model", DEFAULT_SUPERTONIC_MODEL)
    voice = st_config.get("voice", DEFAULT_SUPERTONIC_VOICE)
    speed = float(st_config.get("speed", 1.05))
    lang = st_config.get("lang") or None

    engine = _supertonic_engine_cache.get(model_name)
    if engine is None:
        logger.info("[Supertonic] Loading model: %s", model_name)
        engine = TTS(model=model_name)
        _supertonic_engine_cache[model_name] = engine
        logger.info("[Supertonic] Model ready")

    style_key = (model_name, voice)
    style = _supertonic_style_cache.get(style_key)
    if style is None:
        style = engine.get_voice_style(voice)
        _supertonic_style_cache[style_key] = style

    wav, _sr = engine.synthesize(text, style, speed=speed, lang=lang)
    ...
    # Convert to the requested container when ffmpeg is available.
    if wav_path != output_path:
        ffmpeg = shutil.which("ffmpeg")
        if ffmpeg:
            conv_cmd = [ffmpeg, "-i", wav_path, "-y", "-loglevel", "error", output_path]
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.clio/config.yaml (tts: section).
    ...
    platform = get_session_env("CLIO_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'clio-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run clio setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'clio setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Supertonic provider selected but 'supertonic' package not installed. "
                             "Run 'clio setup tts' and choose Supertonic, or install manually: "
                             "pip install supertonic soundfile"
                }, ensure_ascii=False)
            logger.info("Generating speech with Supertonic (local ONNX)...")
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'clio tools' and select Piper under TTS, or install manually: "
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    if managed_gateway is None:
        ...
        if managed_tools_enabled() or prefers_gateway("tts"):
            message += (
                ". "
                + managed_tool_gateway_unavailable_message(
    ...
    return managed_gateway.managed_user_token, urljoin(
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_clio_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }

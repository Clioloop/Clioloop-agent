...
from tools.environments.local import _CLIO_PROVIDER_ENV_BLOCKLIST
...
def _load_clio_env_vars() -> dict[str, str]:
    """Load ~/.clio/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Clio profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale clio-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=clio-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Clio process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=clio-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a clio process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Clio
    process leaves its container behind permanently because there's no
    subsequent Clio process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=clio-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=clio-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``CLIO_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("CLIO_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using CLIO_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'clio'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``clio-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.clio/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. clio-agent:latest) already use /init as PID 1
        ...
        container_name = f"clio-{uuid.uuid4().hex[:8]}"
        # Labels make clio-created containers identifiable to:
        #   * the orphan reaper (`clio-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`clio-task-id`, `clio-profile`)
        #   * operators running `docker ps --filter label=clio-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Clio profile is captured at
        ...
        label_args = [
            "--label", "clio-agent=1",
            "--label", f"clio-task-id={task_label}",
            "--label", f"clio-profile={profile_name}",
        ]
        ...
        self._labels = {
            "clio-agent": "1",
            "clio-task-id": task_label,
            "clio-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Clio process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Clio secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _CLIO_PROVIDER_ENV_BLOCKLIST)
        clio_env = _load_clio_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = clio_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("clio-task-id", "")
        profile_label = self._labels.get("clio-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"clio-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``clio-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=clio-agent=1",
                    "--filter", f"label=clio-task-id={task_label}",
                    "--filter", f"label=clio-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Clio process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Clio exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Clio process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Clio startup. That covers the
        ...
        # outlives Clio processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"clio-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``clio /quit``

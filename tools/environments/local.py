...
# Clio-internal env vars that should NOT leak into terminal subprocesses.
_CLIO_PROVIDER_ENV_FORCE_PREFIX = "_CLIO_FORCE_"

# Clio-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Clio inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from clio_cli.config import OPTIONAL_ENV_VARS
...
def _inject_context_clio_home(env: dict) -> None:
    """Bridge the context-local Clio home override into subprocess env."""
    try:
        from clio_constants import get_clio_home_override

        value = get_clio_home_override()
        if value:
            env["CLIO_HOME"] = value
    except Exception:
        pass
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Clio-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (extra_env or {}).items():
        ...
        elif key not in _CLIO_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_clio_home(sanitized)
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("CLIO_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\clio\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\clio\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _clio_portable_git = os.path.join(_local_appdata, "clio", "git") if _local_appdata else ""
    if _clio_portable_git:
        for candidate in (
            os.path.join(_clio_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_clio_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Clio Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set CLIO_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        ...
        if k.startswith(_CLIO_PROVIDER_ENV_FORCE_PREFIX):
            ...
            run_env[real_key] = v
        elif k not in _CLIO_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # Clio is driving via _find_bash (Git Bash), and Git Bash itself
    ...
    _inject_context_clio_home(run_env)

    # Per-profile HOME isolation: redirect system tool configs (git, ssh, gh,
    # npm …) into {CLIO_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Clio trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``CLIO_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under CLIO_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_clio_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "clio_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._clio_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_clio_pgid", None)
                    if pgid is None:
                        raise

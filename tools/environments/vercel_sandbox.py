...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Clio's
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``CLIO_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_clio_home() / _SNAPSHOT_STORE_NAME

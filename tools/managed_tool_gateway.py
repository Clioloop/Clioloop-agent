"""Generic managed-tool gateway helpers for managed-provider-hosted vendor passthroughs."""
...
_CLIO_MANAGED_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120
...
class ManagedToolGatewayConfig:
    vendor: str
    gateway_origin: str
    managed_user_token: str
    managed_mode: bool


def auth_json_path():
    """Return the Clio auth store path, respecting CLIO_HOME overrides."""
    return get_clio_home() / "auth.json"


def _read_managed_provider_state() -> Optional[dict]:
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        managed_provider = providers.get("managed", {})
...
def peek_managed_access_token() -> Optional[str]:
    """Cheap probe for a managed provider gateway token without triggering refresh.

    Availability scans (`clio tools`, banner/status paint, provider
    ...
    that call :func:`read_managed_access_token`.
    """
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()

    managed_provider = _read_managed_provider_state() or {}
    access_token = managed_provider.get("access_token")
    if isinstance(access_token, str) and access_token.strip():
        return access_token.strip()
    return None


def read_managed_access_token() -> Optional[str]:
    """Read a managed provider Subscriber OAuth access token from auth store or env override."""
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()
    managed_provider = _read_managed_provider_state() or {}
    cached_token = peek_managed_access_token()

    if cached_token and not _access_token_is_expiring(
        managed_provider.get("expires_at"),
        _CLIO_MANAGED_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token
    ...
    try:
        ...
        refreshed_token = resolve_managed_access_token(
            refresh_skew_seconds=_CLIO_MANAGED_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
        if isinstance(refreshed_token, str) and refreshed_token.strip():
            return refreshed_token.strip()
    except Exception as exc:
        logger.debug("managed provider access token refresh failed: %s", exc)
...
def build_vendor_gateway_url(vendor: str) -> str:
    ...
    Precedence: explicit ``<VENDOR>_GATEWAY_URL`` env override, then a
    ``TOOL_GATEWAY_DOMAIN`` subdomain scheme, then the Omni Loop Portal's
    built-in gateway at ``{portal_base_url}/api/gateway/{vendor}`` (derived
    from the auth store, falling back to the default portal URL).
    """
    ...
    if not portal_base_url:
        managed_state = _read_managed_provider_state() or {}
        portal_base_url = str(managed_state.get("portal_base_url") or "").strip()
    if not portal_base_url:
        try:
            ...
            return ""
    return f"{portal_base_url.rstrip('/')}/api/gateway/{vendor}"
...
def resolve_managed_tool_gateway(
    ...
    resolved_token_reader = token_reader or read_managed_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    managed_user_token = resolved_token_reader()
    if not gateway_origin or not managed_user_token:
        return None

    return ManagedToolGatewayConfig(
        vendor=vendor,
        gateway_origin=gateway_origin,
        managed_user_token=managed_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable managed provider token are present.

    Defaults to :func:`peek_managed_access_token` so read-only availability scans
    ...
    still defaults to the refresh-aware :func:`read_managed_access_token`).
    """
    return resolve_managed_tool_gateway(
        vendor,
        gateway_builder=gateway_builder,
        token_reader=token_reader or peek_managed_access_token,
    ) is not None

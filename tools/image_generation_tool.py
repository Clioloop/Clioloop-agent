...
selectable via ``clio tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on the Omni Loop Portal Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps managed provider
    ...
    # existing FAL key / managed provider subscription rather than register
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (Omni Loop Portal Subscription)
# ---------------------------------------------------------------------------
...
def _get_managed_fal_client(managed_gateway):
    ...
    with _managed_fal_client_lock:
        ...
        _managed_fal_client = _ManagedFalSyncClient(
            fal_client,
            key=managed_gateway.managed_user_token,
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"Omni Loop Portal Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the managed provider's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `clio tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_tools_enabled():
        lines.append(
            "  - the Omni Loop Portal Subscription image gateway is unreachable or not included in your plan"
        )
    else:
        lines.append("  - Omni Loop Portal Subscription image generation is not connected")
        gateway_message = managed_tool_gateway_unavailable_message(
            "Omni Loop Portal Subscription image generation",
        )
        if gateway_message:
    ...
    if managed_tools_enabled():
        lines.append(
            "  2. Verify your Omni Loop Portal Subscription plan includes image generation"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `clio tools` "
        "→ Image Generation (run `clio plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)

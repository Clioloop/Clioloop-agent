...
* ``XAI_API_KEY`` is set in ``~/.clio/.env`` or the process environment
  (paid xAI API key), OR
* The user is signed in via xAI Grok OAuth — SuperGrok subscription —
  i.e. ``clio auth add xai-oauth`` has been run and the stored refresh
  token still works.
...
reworked to honor both auth modes per Omni Loop Labs's design.
"""
...
def _resolve_xai_bearer() -> Tuple[str, str, str]:
    ...
    if not api_key:
        raise RuntimeError(
            "No xAI credentials available. Run `clio auth add xai-oauth` "
...
def check_x_search_requirements() -> bool:
    ...
    :func:`clio_cli.auth.resolve_xai_oauth_runtime_credentials` which

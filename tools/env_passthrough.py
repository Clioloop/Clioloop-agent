...
def _is_clio_provider_credential(name: str) -> bool:
    """True if ``name`` is a Clio-managed provider credential (API key,
    token, or similar) per ``_CLIO_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Clio API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _CLIO_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Clio-managed provider credentials (from
    ``_CLIO_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Clio-managed
    ...
    Non-Clio third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_clio_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Clio provider "
                "credential %r (blocked by _CLIO_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Clio-managed provider credentials must not be passed
                ...
                if _is_clio_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Clio "
                        "provider credential %r from config.yaml (blocked "
                        "by _CLIO_PROVIDER_ENV_BLOCKLIST). Operator "

...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/clio-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/clio-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/clio-results"
HEREDOC_MARKER = "CLIO_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/clio-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"CLIO_PERSIST_{uuid.uuid4().hex[:8]}"
...
def extract_persisted_path(content: str) -> str | None:
    """Extract the spill path from a ``<persisted-output>`` replacement."""

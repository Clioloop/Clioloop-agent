...
Backend is selected during ``clio tools`` setup (web.backend in config.yaml).
When available, Clio can route Firecrawl calls through a managed-provider-hosted tool-gateway
for managed-provider subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for managed-provider subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    peek_managed_access_token as _peek_managed_access_token,
    read_managed_access_token as _read_managed_access_token,
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.clio/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``clio tools``).
    ...
    # tool gateway is configured for managed provider subscribers.
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `clio tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the managed provider portal on every CLI startup (invoked at
...
def _is_managed_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is managed provider."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "" or host.endswith(".")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_managed_auxiliary_client(client):
        from agent.auxiliary_client import get_auxiliary_extra_body
        from agent.request_tags import request_tags
        extra_body = get_auxiliary_extra_body() or {"tags": request_tags()}
...
if __name__ == "__main__":
    ...
    managed_available = check_auxiliary_model()
    ...
    if not managed_available:
        ...
        print("Set OPENROUTER_API_KEY, configure managed provider, or set OPENAI_BASE_URL + OPENAI_API_KEY")

"""
`clio computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Clio side — the only
...
def _cua_child_env() -> Dict[str, str]:
    """cua-driver child env with the Clio telemetry policy applied.
...
def run_doctor(
    ...
    *,
    ...
    Honors `CLIO_CUA_DRIVER_CMD` via the same `_cua_driver_cmd()` resolver
    ...
    if driver_cmd is None:
        try:
            from clio_cli.tools_config import _cua_driver_cmd
            driver_cmd = _cua_driver_cmd()
        except Exception:
            driver_cmd = os.environ.get("CLIO_CUA_DRIVER_CMD") or "cua-driver"

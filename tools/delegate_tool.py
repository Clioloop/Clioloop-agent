...
# Must match clio_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (clio-* prefixed), and scenario toolsets.
#
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``clio-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on managed provider).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. clio-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g. managed provider).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.clio/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, managed, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'clio auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (clio_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Clio subagents)."
                ),
            },
            "acp_args": {
                "type": "array",

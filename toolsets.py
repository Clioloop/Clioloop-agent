...
_CLIO_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (CLIO_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `clio tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "computer_use": {
        "description": (
            "Background desktop control via cua-driver — screenshots, "
            "mouse, keyboard, scroll, drag. Does NOT steal the user's cursor "
            "or keyboard focus. Works on macOS, Windows, and Linux with "
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (CLIO_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Clio toolsets (CLI + messaging platforms)
    #
    ...
    "clio-cli": {
        ...
        "tools": _CLIO_CORE_TOOLS,
        "includes": []
    },

    "clio-cron": {
        # Mirrors clio-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `clio tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as clio-cli; gated by `clio tools`",
        "tools": _CLIO_CORE_TOOLS,
        "includes": []
    },
    ...
    "clio-email": {
        "description": "Email bot toolset - interact with Clio via email (IMAP/SMTP)",
        "tools": _CLIO_CORE_TOOLS,
        "includes": []
    },
    ...
    "clio-sms": {
        "description": "SMS bot toolset - interact with Clio via SMS (Twilio)",
        "tools": _CLIO_CORE_TOOLS,
        "includes": []
    },
    ...
    "clio-webhook": {
        ...
        "tools": _CLIO_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "clio-gateway": {
        ...
        "tools": [],
        "includes": ["clio-telegram", "clio-discord", "clio-whatsapp", "clio-slack", "clio-signal", "clio-bluebubbles", "clio-homeassistant", "clio-email", "clio-sms", "clio-mattermost", "clio-matrix", "clio-dingtalk", "clio-feishu", "clio-wecom", "clio-wecom-callback", "clio-weixin", "clio-qqbot", "clio-webhook", "clio-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (clio-<name>).
        # Gives them _CLIO_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("clio-"):
            platform_name = name[len("clio-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_CLIO_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(

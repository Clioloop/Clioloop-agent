...
def upsert(conn, record: Mapping[str, Any]) -> None:
    ...
    conn.execute(
        ...
        (
            ...
            now,
            _json(record.get("event") if "event" in record else record.get("event_json")),
            _json(record.get("result") if "result" in record else record.get("result_json")),
            _json(record.get("task") if "task" in record else record.get("task_json")),

...
# 5. Symlinks the 'clio' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}∞ Clio Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/clio-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink clio into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up clio command..."

CLIO_BIN="$SCRIPT_DIR/venv/bin/clio"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$CLIO_BIN" "$COMMAND_LINK_DIR/clio"
echo -e "${GREEN}✓${NC} Symlinked clio → $COMMAND_LINK_DISPLAY_DIR/clio"
...
# ============================================================================
# Seed bundled skills into ~/.clio/skills/
# ============================================================================

CLIO_SKILLS_DIR="${CLIO_HOME:-$HOME/.clio}/skills"
mkdir -p "$CLIO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.clio/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CLIO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  clio status        # Check configuration"
if is_termux; then
    echo "  clio gateway       # Run gateway in foreground"
else
    echo "  clio gateway install # Install gateway service (messaging + cron)"
fi
echo "  clio cron list     # View scheduled jobs"
echo "  clio doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m clio_cli.main setup
fi

...
2. **Clio overlays** — transport type, auth patterns, aggregator flags,
...
# -- Clio overlay ----------------------------------------------------------
# Clio-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ClioOverlay:
    """Clio-specific provider metadata layered on top of models.dev."""
...
CLIO_OVERLAYS: Dict[str, ClioOverlay] = {
    "openrouter": ClioOverlay(
    ...
    "openai-codex": ClioOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": ClioOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": ClioOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ClioOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="CLIO_QWEN_BASE_URL",
    ),
    "google-gemini-cli": ClioOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": ClioOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ClioOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ClioOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ClioOverlay(
    ...
    "minimax-oauth": ClioOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ClioOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ClioOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": ClioOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": ClioOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ClioOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "clio", "user-config"
...
_LABEL_OVERRIDES: Dict[str, str] = {
    "managed": "Omni Loop Portal Subscription",
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Clio overlays (for providers not in models.dev: managed, openai-codex, etc.)
      2. models.dev catalog + Clio overlay
    ...
    overlay = CLIO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + clio extra
    ...
    if overlay is not None:
        # Clio-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in CLIO_OVERLAYS
    if provider == "bedrock":

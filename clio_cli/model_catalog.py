...
The Clio docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
managed provider). This module fetches, validates, and caches that manifest,
...
DEFAULT_CATALOG_URL = (
    "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/api/model-catalog.json"
)
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/Clioloop/Clioloop-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_clio_home() / "cache" / "model_catalog.json"
...
def get_curated_managed_models() -> list[str] | None:
    """Return managed provider's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("managed")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``clio model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None

"""MCP picker — interactive `clio mcp picker` (also the default `clio mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``clio mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `clio plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from clio_cli.cli_output import prompt_yes_no
from clio_cli.curses_ui import curses_single_select
from clio_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Clio session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from clio_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `clio mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: clio mcp install <name>    Picker: clio mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Clio is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Clio — run `clio update` "
...
def show_catalog() -> None:
    """`clio mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`clio mcp picker` (and default `clio mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`clio mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `clio mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:

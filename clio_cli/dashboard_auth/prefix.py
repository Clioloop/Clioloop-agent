...
prefix (e.g. ``mission-control.tilos.com/clio/*`` -> dashboard on
:10119), injecting ``X-Forwarded-Prefix: /clio`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``CLIO_DASHBOARD_PUBLIC_URL`` /
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/clio"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# CLIO_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``CLIO_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("CLIO_DASHBOARD_PUBLIC_URL", "")

...
from clio_cli.dashboard_auth.base import (
    InvalidCodeError,
...
from clio_cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``CLIO_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /clio`` (Mission Control deploys) — we
         prepend the prefix to the path FastAPI's ``url_for`` produces
         (it doesn't natively honor this header — it isn't part of the
    ...
    from clio_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``clio_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("clio_session_pkce", "")
...
async def auth_callback(
    ...
    # Homanagedr the ``next=`` value the gate's _unauth_response set in the
...
def _validate_post_login_target(raw: str) -> str:
    ...
    # callback URL must not be honored: a successful redirect to an
...
async def api_auth_ws_ticket(request: Request):
    ...
    from clio_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket

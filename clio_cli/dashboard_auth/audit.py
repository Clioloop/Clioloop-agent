...
from ``clio_constants`` or other clio_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$CLIO_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``clio_constants.get_clio_home`` semantics: env var wins,
    else ``~/.clio``. A local copy avoids an import cycle with the
    middleware which lives below ``clio_cli``.
    """
    home = os.environ.get("CLIO_HOME") or str(Path.home() / ".clio")

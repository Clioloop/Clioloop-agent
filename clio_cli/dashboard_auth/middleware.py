...
from clio_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from clio_cli.dashboard_auth.cookies import read_session_cookies
from clio_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _path_is_public(path: str) -> bool:
    ...
    * :data:`PUBLIC_API_PATHS` — the shared ``/api/*`` allowlist that
      the legacy ``_SESSION_TOKEN`` middleware also honors. Matched
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /clio``, the
    ``login_url`` is prefixed (``/clio/login?next=...``) so the
    ...
    """
    from clio_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["clio.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``clio_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a self-hosted-OIDC session hits the `managed` provider first, which
        # tries to reach managed provider's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        if refreshed is not None:
            ...
            from clio_cli.dashboard_auth.cookies import (
                detect_https,
                set_session_cookies,
            )
            from clio_cli.dashboard_auth.prefix import prefix_from_request
        ...
        from clio_cli.dashboard_auth.cookies import clear_session_cookies
        from clio_cli.dashboard_auth.prefix import prefix_from_request

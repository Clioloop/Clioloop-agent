...
Three cookies in play:
  - clio_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - clio_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         managed provider issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / clio
  ...
  - clio_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
request URL scheme, which honors ``X-Forwarded-Proto`` upstream of
...
setter wrote ``__Secure-clio_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "clio_session_at"
SESSION_RT_COOKIE = "clio_session_rt"
PKCE_COOKIE = "clio_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /clio`` we want ``Path=/clio`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. managed provider
    issues a 24h rotating refresh token (clio #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/clio``)
...
def detect_https(request: Request) -> bool:
    ...
    (which start_server enables when the gate is active), this honors

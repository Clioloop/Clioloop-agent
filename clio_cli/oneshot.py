...
configured for "cli" in `clio tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (CLIO_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `clio chat`:
...
def _build_preloaded_skills_prompt(skills: object = None) -> str | None:
    """Load requested skills using the normal CLI partial-success contract."""
    ...
    if missing_skills:
        ...
        if loaded_skills:
            logging.warning(
                ...
                "List available skills with `clio skills list`.",
                missing_display,
                ", ".join(loaded_skills),
            )
        else:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"clio -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "clio -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"clio -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "clio -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "clio -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to CLIO_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("CLIO_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "clio -z: --provider requires --model (or CLIO_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        if isinstance(failure, (KeyboardInterrupt, SystemExit)):
            raise failure
        real_stderr.write(f"clio -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1

    if not (response or "").strip():
        real_stderr.write("clio -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``clio -z`` / oneshot mode.

    Oneshot bypasses ``ClioCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when clio is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from clio_cli.config import load_config
    from clio_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("CLIO_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from clio_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("CLIO_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # honor the same merge semantics as interactive CLI and gateway sessions.
    ...
    agent = AIAgent(
        ...
        #                CLIO_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via CLIO_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via CLIO_YOLO_MODE=1

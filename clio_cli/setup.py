...
_DOCS_BASE = "https://github.com/Clioloop/Clioloop-agent/tree/main/docs"
...
# display_clio_home imported lazily at call sites (stale-module safety during clio update)
...
from clio_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from clio_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure Clio using environment variables or config commands:")
    print_info("  clio config set model.provider custom")
    print_info("  clio config set model.base_url http://localhost:8080/v1")
    print_info("  clio config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'clio setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from clio_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'clio setup')")


def _print_setup_summary(config: dict, clio_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'clio setup' to configure"))
    ...
    if subscription_features.web.managed_by_provider:
        tool_status.append(("Web Search & Extract (Omni Loop Portal Subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_provider:
        tool_status.append(("Browser Automation (Omni Loop Portal Subscription)", True, None))
    ...
    # Image generation — FAL (direct or via the managed provider), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_provider:
        tool_status.append(("Image Generation (Omni Loop Portal Subscription)", True, None))
    ...
    if subscription_features.video_gen.managed_by_provider:
        tool_status.append(("Video Generation (Omni Loop Portal Subscription)", True, None))
    else:
        try:
    ...
    if subscription_features.tts.managed_by_provider:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'clio setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'clio setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_provider:
        tool_status.append(("Modal Execution (Omni Loop Portal Subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'clio setup terminal'"))
    elif managed_tools_enabled() and subscription_features.provider_auth_present:
        tool_status.append(("Modal Execution (optional via Omni Loop Portal Subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via clio auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'clio setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {clio_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('clio setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('clio setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('clio setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('clio setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('clio setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('clio config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('clio config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('clio config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('clio', Colors.GREEN)}              Start chatting")
    print(f"   {color('clio gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('clio doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `clio tools` and `clio setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``clio model``)
    ...
    provider added to ``clio model`` is automatically available here.
    ...
    # Delegate to the shared clio model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from clio_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: clio model")
    ...
    # on demand via `clio auth add`, `clio setup` vision, and
    # `clio setup tts`. This keeps both quick and full setup thin.

    # Tool Gateway prompt is already shown by _model_flow_managed() above.
    save_config(config)
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``clio model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from clio_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_tools_enabled() and subscription_features.provider_auth_present:
        choices.append("Omni Loop Portal Subscription (premium TTS, billed to your subscription)")
    ...
    selected_via_managed = selected == "managed-openai"
    if selected == "managed-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed provider gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.clio/.env."
            )
    ...
    if selected == "neutts":
    ...
    elif selected == "openai" and not selected_via_managed:
        ...
        # via the Clio auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via clio setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
        ...
        if already_installed:
            ...
            print()
            print_info("Supertonic is a studio-quality local TTS (ONNX, CPU-only, no API key).")
            print_info("The supertonic-3 model (~66M params) downloads on first use.")
            print()
            if prompt_yes_no("Install Supertonic now?", True):
                result = subprocess.run(
                    [sys.executable, "-m", "pip", "install", "-U", "supertonic", "soundfile", "--quiet"],
                    capture_output=True, text=True, timeout=300,
                )
                if result.returncode == 0:
                    print_success("Supertonic installed")
                else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'clio setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Clio runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `clio setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `clio setup terminal`.
        ...
        managed_modal_available = bool(
            ...
            and
            get_managed_subscription_features(config).provider_auth_present
        ...
        if managed_modal_available:
            modal_choices = [
                "Use my Omni Loop Portal Subscription",
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed provider gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `clio setup agent` later to customize.")
...
def _setup_telegram_auto_result():
    ...
    try:
        from clio_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in clio_home:
            profile_name = clio_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Clio delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `clio update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under CLIO_HOME, and print
    ...
    """
    try:
        from clio_cli.slack_cli import _build_full_manifest
        from clio_constants import get_clio_home

        manifest = _build_full_manifest(
            bot_name="Clio",
            bot_description="Your Clio agent on Slack",
        )
        target = Path(get_clio_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `clio slack manifest --write` anytime to refresh after "
            "Clio adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Clio delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Clio to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from clio_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  clio config edit")
    print_info("   Open config in your editor:  clio config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from clio_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Clio from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'clio setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     clio config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_clio_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo clio gateway install --system")
                print_info("  Or run in foreground:  clio gateway")
        else:
            from clio_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   clio gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   clio gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `clio setup tools` and `clio tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from clio_cli.config
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_clio migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_clio", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Clio.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Clio to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Clio at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Clio at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Clio at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Clio at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Clio equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Clio config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Clio.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Clio's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(clio_home: Path) -> bool:
    ...
    print_info("Clio can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: clio claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: clio claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Clio configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Clio config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Clio (use clio claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Omni Loop Portal setup — browser login, then pick a model.

    Wired into ``clio setup --portal``. Runs the portal device-code flow
    directly: approve in the browser, choose a model, done.
    """
    from clio_cli.config import load_config

    print()
    print(color("┌───────────────────────────────────────────────────┐", Colors.MAGENTA))
    print(color("│  ∞  Omni Loop Portal — one-click setup            │", Colors.MAGENTA))
    print(color("└───────────────────────────────────────────────────┘", Colors.MAGENTA))
    print()
    print_info("  One login, 300+ models. Approve in the browser, pick a model, go.")
    print()

    try:
        from clio_cli.main import _model_flow_managed

        _model_flow_managed(config)
    except (KeyboardInterrupt, EOFError, SystemExit):
        print()
        print_info("  Setup cancelled.")
        print_info("  You can retry later with `clio model`.")
        return
    except Exception as exc:
        logger.debug("select_provider_and_model error during `clio setup --portal`: %s", exc)
        print()
        print_error(f"  Provider setup encountered an error: {exc}")
        print_info("  You can retry later with `clio model`.")
        return

    # Re-sync the in-memory config from disk.
    try:
        _refreshed = load_config()
    ...
    print_info("  Run `clio` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      clio setup           — full or quick (auto-detected)
      clio setup model     — just model/provider
      clio setup tts       — just text-to-speech
      clio setup terminal  — just terminal backend
      clio setup gateway   — just messaging platforms
      clio setup tools     — just tool configuration
      clio setup agent     — just agent settings
    """
    from clio_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, clio_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Clio configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'clio setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(clio_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Clio?",
            [
                "Quick Setup — one-click Omni Loop Portal or your own provider (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, clio_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {clio_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'clio config edit'")
    ...
    # Tune later with `clio setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, clio_home)


def _run_first_time_quick_setup(config: dict, clio_home, is_existing: bool):
    """Streamlined first-time setup: provider picker, model, terminal & messaging.

    Routes straight to the provider picker — runs the same provider selection
    flow as ``clio model`` (pick a provider, enter credentials, choose a model),
    then configures the terminal backend and (optionally) a messaging platform.
    Applies sensible defaults for everything else (agent settings, tools); the
    user can customize later via ``clio setup <section>`` or switch providers
    with ``clio model``.
    """
    from clio_cli.config import load_config

    # Step 1: Provider + model selection.
    # Delegates to the shared `clio model` flow (provider picker → credential
    # prompt → model picker → config persistence). First prompt is the
    # provider list, not any specific provider.
    print()
    print_header("Inference Provider")
    print_info("Choose how to connect to your main chat model.")
    print()
    try:
        portal_choice = prompt_choice(
            "How do you want to connect?",
            [
                "Omni Loop Portal — one login, 300+ models (recommended)",
                "Bring your own provider (OpenAI, Anthropic, OpenRouter, local, …)",
            ],
            0,
        )
        if portal_choice == 0:
            # One-click OAuth setup: browser device approval → model picker.
            from clio_cli.main import _model_flow_managed
            _model_flow_managed(config)
        else:
            from clio_cli.main import select_provider_and_model
    ...
    # Re-sync the wizard's config dict from disk — the shared provider flow
    # writes via its own load/save cycle, and the wizard's later save_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'clio setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    clio setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  clio setup gateway")
    print()

    _print_setup_summary(config, clio_home)


def _run_quick_setup(config: dict, clio_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'clio setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Clio to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'clio setup gateway'.")

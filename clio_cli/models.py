...
Add, remove, or reorder entries here — both `clio setup` and
`clio` provider-selection will pick up the change automatically.
"""
...
from clio_cli import __version__ as _CLIO_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_CLIO_USER_AGENT = f"clio-cli/{_CLIO_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `clio model`
    flow without maintaining a separate static list.
    """
    from clio_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $CLIO_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $CLIO_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Clio refreshes the cache from
...
# ---------------------------------------------------------------------------
# managed provider free-model helper
# ---------------------------------------------------------------------------
# The managed provider models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# managed provider account tier detection
# ---------------------------------------------------------------------------
def is_managed_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_managed_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split managed provider models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/managed/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["managed"]`` list happens to contain or whether the
    ...
    older Clio that doesn't ship that model in its hardcoded curated
    ...
    * ``pricing`` gets a synthetic ``{"prompt": "0", "completion": "0"}``
    ...
    """
    try:
        payload = fetch_managed_recommended_models(
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/managed/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["managed"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Clio that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_managed_models_by_tier` keeps free models selectable;
...
def check_managed_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current managed provider user is on a free (unpaid) tier.
    ...
    try:
        ...
        account_info = get_managed_provider_account_info(force_fresh=force_fresh)
...
# ---------------------------------------------------------------------------
# managed provider recommended models
#
...
MANAGED_RECOMMENDED_MODELS_PATH = "/api/managed/recommended-models"
_MANAGED_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_managed_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def fetch_managed_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the managed provider's curated recommended-models payload.

    Hits ``<portal>/api/managed/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_MANAGED_RECOMMENDED_CACHE_TTL`` seconds; pass ``force_refresh=True`` to
    bypass the cache.
    ...
    now = time.monotonic()
    cached = _managed_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _MANAGED_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{MANAGED_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    _managed_recommended_cache[base] = (data, now)
    return data


def _resolve_managed_portal_url() -> str:
    ...
    try:
        ...
        return str(DEFAULT_MANAGED_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://"
...
def get_managed_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_managed_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_managed_portal_url()
    payload = fetch_managed_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_managed_free_tier()
        except Exception:
...
# this list:  clio model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `clio model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `clio model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry(
        "managed",
        "Omni Loop Portal Subscription",
        "Omni Loop Portal Subscription (Currently the only way to use Model Fusion · one login · no API keys · includes Tool Gateway)",
    ),
...
#
# Some vendors expose several Clio provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `clio model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``clio model``, the
...
# sensible default, but managed provider is a per-token *metered aggregator* whose
...
# non-interactive fallback when a profile sets ``provider: managed`` with no model
...
# path. The *interactive* default (GUI onboarding / ``clio model``) uses the
...
def get_default_model_for_provider(provider: str, *, free_tier: bool = False) -> str:
    ...
    model was ever selected (e.g. ``clio auth add openai-codex`` without
    ``clio model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``clio model`` picker offers first. For metered aggregators
...
def get_curated_managed_model_ids() -> list[str]:
    """Return the curated managed provider model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["managed"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from clio_cli.model_catalog import get_curated_managed_models
        remote = get_curated_managed_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("managed", []))
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, managed provider).
    """
...
_DEFAULT_MANAGED_INFERENCE_BASE = "https://"


def _resolve_managed_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for managed provider pricing.

    The managed-provider inference ``/v1/models`` endpoint exposes pricing without
    ...
    try:
        ...
        pass
    return ("", _DEFAULT_MANAGED_INFERENCE_BASE)


def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, managed, novita)."""
    ...
    if normalized == "managed":
        api_key, base_url = _resolve_managed_pricing_credentials()
        if base_url:
            # managed provider base_url typically looks like https:///v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``clio model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from clio_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, managed provider, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"managed", "openrouter", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from clio_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model managed` or `/model anthropic`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Clio' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``clio_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Clio should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``clio auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.clio/.env``.
    ...
    """
    try:
        from clio_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from clio_cli.copilot_auth import (
...
#   - "managed": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, managed provider),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Clio release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Clio release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "managed":
        # Try live managed provider /models endpoint
        try:
            from clio_cli.auth import fetch_managed_models, resolve_managed_runtime_credentials
            creds = resolve_managed_runtime_credentials()
            if creds:
                live = fetch_managed_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["managed"] snapshot — so newly
        # added Portal models still surface without a Clio release.
        manifest_ids = get_curated_managed_model_ids()
        if manifest_ids:
            return manifest_ids
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``clio model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $CLIO_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_clio_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, managed portal), the
    relevant tokens live in ``$CLIO_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _CLIO_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Clio' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _CLIO_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from clio_constants import get_clio_home
    return get_clio_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Clio will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Clio cannot verify the model name."

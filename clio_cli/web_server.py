...
Usage:
    python -m clio_cli.main web          # Start on http://127.0.0.1:10119
    python -m clio_cli.main web --port 8080
"""
...
try:
    ...
    # running `clio dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["CLIO_WEB_DIST"]) if "CLIO_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
# CLIO_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("CLIO_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Clio-Session-Token"
...
#
# This list is defined in ``clio_cli.dashboard_auth.public_paths`` so the
# OAuth gate middleware can honor the same allowlist — keeping the two
...
# ---------------------------------------------------------------------------
from clio_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from clio_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Clio (no terminal prompt), "
...
async def get_status():
    ...
    # and which providers are registered so ``clio status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Omni Loop Labs" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from clio_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the clio home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_clio_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_clio_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — managed provider auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from clio_cli.auth import get_managed_auth_status

        auth = get_managed_auth_status() or {}
    except Exception:
        auth = {}

    features = []
    try:
        from clio_cli.portal_subscription import get_managed_subscription_features

        feats = get_managed_subscription_features(cfg)
        if feats is not None:
            for feat in feats.items():
                if getattr(feat, "managed_by_provider", False):
                    state = "via Omni Loop Portal Subscription"
    ...
    try:
        from clio_cli.auth import DEFAULT_MANAGED_PORTAL_URL as _default_portal
    except Exception:
        _default_portal = ""
    portal_url = (
        str(auth.get("portal_base_url") or "").strip()
        or os.getenv("CLIO_PORTAL_BASE_URL", "").strip()
        or _default_portal
    ).rstrip("/")
    return {
        ...
        "subscription_url": f"{portal_url}/pricing" if portal_url else "",
        "features": features,
    }


# ---------------------------------------------------------------------------
# Omni Loop Portal connect — browser-driven device-code login from the
# dashboard.  POST /api/portal/connect/start returns the verification URL +
# user code; clio_cli.omni_portal polls the portal token endpoint on a
# daemon thread and persists the OAuth credentials on approval.
# GET /api/portal/connect/status reports progress; GET /api/portal/models
# lists the account's available models.
# ---------------------------------------------------------------------------


@app.post("/api/portal/connect/start")
async def portal_connect_start():
    from clio_cli.omni_portal import resolve_portal_base_url, start_device_connect

    try:
        return start_device_connect()
    except Exception as exc:
        try:
            portal_url = resolve_portal_base_url()
        except Exception:
            portal_url = "the Omni Loop Portal Subscription"
        return JSONResponse(
            {"error": f"Could not reach {portal_url}: {exc}"},
            status_code=502,
        )


@app.get("/api/portal/connect/status")
async def portal_connect_status():
    from clio_cli.omni_portal import get_connect_status
...
async def portal_models():
    try:
        ...
        if not status.get("logged_in"):
            return {"logged_in": False, "models": []}
        token = resolve_managed_access_token()
        inference_url = str(status.get("inference_base_url") or "").rstrip("/")
        models = fetch_managed_models(inference_base_url=inference_url, api_key=token)
        return {"logged_in": True, "models": models}
    except Exception as exc:
        _log.debug("portal models failed: %s", exc)
...
async def run_prompt_size():
    try:
        proc = _spawn_clio_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_clio_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_clio_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from clio_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.clio/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_clio_home() / "logs"
...
def _spawn_clio_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``clio <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``clio_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "clio_cli.main", *subcommand]
...
async def update_clio():
    """Kick off ``clio update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("clio-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "clio-update",
    ...
    try:
        proc = _spawn_clio_action(["update"], "clio-update")
    except Exception as exc:
        _log.exception("Failed to spawn clio update")
...
@app.get("/api/clio/update/check")
async def check_clio_update(force: bool = False):
    """Report whether a Clio update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/clio/update`` actually runs ``clio update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Clio version string
    ...
    try:
        ...
        if force:
            try:
                (get_clio_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.clio/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Clio surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Clio supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in clio_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `clio model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. For
    managed provider this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for managed provider and None otherwise. `model` may be
    ...
    if slug == "managed":
        try:
            from clio_cli.models import (
                get_curated_managed_model_ids,
                get_pricing_for_provider,
                check_managed_free_tier,
                partition_managed_models_by_tier,
                union_with_portal_free_recommendations,
                union_with_portal_paid_recommendations,
            )
            from clio_cli.auth import get_provider_auth_state

            model_ids = get_curated_managed_model_ids()
            pricing = get_pricing_for_provider("managed") or {}
            ...
            if free_tier:
                ...
                model_ids, _unavailable = partition_managed_models_by_tier(
            ...
            # Always prefer the promotional DeepSeek V4 Pro model as the
            # recommended default, regardless of tier. This is the product
            # default and also prevents setup from landing on Opus.
            from clio_cli.models import _MANAGED_FREE_SILENT_DEFAULT

            free_model = _MANAGED_FREE_SILENT_DEFAULT
            ...
            return {"provider": "managed", "model": "", "free_tier": None}

    # Non-managed provider: first curated model for the provider, matching prior behaviour.
    try:
        from clio_cli.inventory import build_models_payload, load_picker_context
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.clio/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            # When switching the main provider to managed provider, mirror the CLI's
            # post-model-selection behaviour (clio_cli/main.py
            # prompt_enable_tool_gateway / tools_config apply_managed_defaults):
            # auto-route any *unconfigured* tools through the managed provider Tool Gateway.
            ...
            if provider.strip().lower() == "managed":
                try:
                    ...
                    # Portal lookup hiccups / non-subscriber / non-managed gating
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Clio from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Clio to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Clio from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Clio to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Clio in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Clio through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Clio via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Clio through an IMAP/SMTP mailbox.",
        "docs_url": "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Clio to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Clio inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Clio through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Clio to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Clio to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Clio as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/messaging/webhooks/",
...
# Default managed-bot onboarding API base. Reuse the CLI's canonical constant so
# the desktop and CLI never drift (both append ``/v1/telegram/...`` and honor the
# TELEGRAM_ONBOARDING_URL override). The previous placeholder ("https://setup.clio-agent.")
# was an unreachable host, so the desktop QR setup silently failed.
try:
    from clio_cli.telegram_managed_bot import DEFAULT_API_URL as _TELEGRAM_ONBOARDING_DEFAULT_URL
except Exception:  # pragma: no cover - defensive fallback if the import path changes
    _TELEGRAM_ONBOARDING_DEFAULT_URL = "https://portal.clioloop.com/api"
...
# Anthropic, device-code for managed-provider/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``clio auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Clio resolves Anthropic creds in this order at runtime:
    1. ``~/.clio/.anthropic_oauth.json`` — Clio-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_clio_oauth_credentials,
            read_claude_code_credentials,
            _CLIO_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_clio_oauth_credentials = None  # type: ignore
        _CLIO_OAUTH_FILE = None  # type: ignore

    clio_creds = None
    if read_clio_oauth_credentials:
        try:
            clio_creds = read_clio_oauth_credentials()
        except Exception:
            clio_creds = None
    if clio_creds and clio_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "clio_pkce",
            "source_label": f"Clio PKCE ({_CLIO_OAUTH_FILE})",
            "token_preview": _truncate_token(clio_creds.get("accessToken")),
            "expires_at": clio_creds.get("expiresAt"),
            "has_refresh_token": bool(clio_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Clio even
    when they also have a separate Clio-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "badge": "Recommended",
        "description": "Currently the only way to use Model Fusion · one login · no API keys · includes Tool Gateway",
        "flow": "device_code",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "clio auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "clio auth add qwen-oauth",
    ...
    },
    {
        ...
        # as managed provider'ss device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "clio auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "clio auth add xai-oauth",
        "docs_url": "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "clio auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from clio_cli import auth as hauth
        if provider_id == "managed":
            raw = hauth.get_managed_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "managed_portal",
                "source_label": raw.get("portal_base_url") or "Omni Loop Portal Subscription",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("clio_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Clio-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _CLIO_OAUTH_FILE
            if _CLIO_OAUTH_FILE.exists():
                _CLIO_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from clio_cli.auth import clear_provider_auth
...
#          → persists to ~/.clio/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (managed provider, OpenAI Codex):
#     1. POST /api/providers/oauth/{managed|openai-codex}/start
...
# Guarded so clio web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Clio file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``clio auth add anthropic``.
    """
    from agent.anthropic_adapter import _CLIO_OAUTH_FILE
    payload = {
    ...
    _CLIO_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _CLIO_OAUTH_FILE.with_name(
        f"{_CLIO_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _CLIO_OAUTH_FILE)
        try:
            _CLIO_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    """Initiate a device-code flow (managed provider, OpenAI Codex, or MiniMax).
    ...
    if provider_id == "managed":
        ...
        portal_base_url = (
            os.getenv("CLIO_PORTAL_BASE_URL")
        ...
        def _do_managed_device_request():
            with httpx.Client(
        ...
        device_data, effective_scope = await asyncio.get_running_loop().run_in_executor(
            None, _do_managed_device_request
        )
        sid, sess = _new_oauth_session("managed", "device_code")
        ...
        threading.Thread(
            target=_managed_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to managed provider'ss device-code
...
# the tokens exactly like `clio auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `clio auth add xai-oauth`'s credential-pool insert.
...
def _managed_poller(session_id: str) -> None:
    """Background poller that drives a managed provider device-code flow to completion."""
    from clio_cli.auth import (
        _poll_for_token,
        refresh_managed_oauth_from_state,
    )
    from datetime import datetime, timezone
    import httpx
    with _oauth_sessions_lock:
    ...
    try:
        ...
        # Same post-processing as _managed_device_code_login (validate/refresh JWT)
        ...
        full_state = refresh_managed_oauth_from_state(
            auth_state,
            timeout_seconds=15.0,
            force_refresh=False,
        )
        from clio_cli.auth import persist_managed_credentials
        persist_managed_credentials(full_state)
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: managed login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("managed device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_managed_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by managed provider. On success, builds the same
    auth_state dict that ``_minimax_oauth_login`` (the CLI flow) builds
    and persists via ``_minimax_save_auth_state`` — so the dashboard
    path leaves the system in the same state as
    ``clio auth add minimax-oauth``.
    """
    from clio_cli.auth import (
        _minimax_poll_token,
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from clio_cli.auth import _save_codex_tokens
...
async def poll_oauth_session(provider_id: str, session_id: str):
    ...
    Shared by the device-code flows (managed provider, OpenAI Codex, MiniMax) and the
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `clio sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `clio sessions prune`)."""
...
async def get_logs(
    ...
    from clio_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_clio_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from clio_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, CLIO_HOME)."""
    from clio_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process CLIO_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (clio_cli.mcp_config), so
# servers managed here show up under `clio mcp list` and vice versa.  Secrets
...
async def list_mcp_servers():
    from clio_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    from clio_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str):
    from clio_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from clio_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    """Browse the managed provider-approved MCP catalog (the optional-mcps/ manifests).
    ...
    `clio mcp catalog` / `clio mcp install` read.
    """
    try:
        from clio_cli import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall):
    ...
    if entry.install is not None:
        try:
            proc = _spawn_clio_action(["mcp", "install", name], "mcp-install")
        except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (clio_cli.webhook); the webhook
...
# spawn the real `clio gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_clio_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_clio_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from clio_cli.auth import read_credential_pool
...
# `clio memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `clio memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_clio_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_clio_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_clio_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    try:
        proc = _spawn_clio_action(["import", archive], "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from clio_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from clio_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <clio_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_clio_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_clio_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def install_skill_hub(body: SkillInstallRequest):
    ...
    try:
        proc = _spawn_clio_action(["skills", "install", identifier], "skills-install")
    except Exception as exc:
...
async def uninstall_skill_hub(body: SkillUninstallRequest):
    ...
    try:
        proc = _spawn_clio_action(["skills", "uninstall", name, "--yes"], "skills-uninstall")
    except Exception as exc:
...
async def update_skills_hub():
    try:
        proc = _spawn_clio_action(["skills", "update"], "skills-update")
    except Exception as exc:
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_clio_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "clio setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local CLIO_HOME override so the write lands in the target
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``clio profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from CLIO_HOME).
    """
    from clio_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``clio profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the CLIO_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``clio profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from clio_cli import profile_describer
...
async def get_skills():
    ...
    from clio_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from clio_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from clio_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``clio tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``clio tools`` picker shows
    ...
    if cat:
        for prov in _visible_providers(cat, config, force_fresh=True):
            ...
            providers.append({
                ...
                "requires_managed_auth": bool(prov.get("requires_managed_auth")),
                "is_active": is_active,
            })
    return {
        "name": name,
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``clio tools``
...
#
# The endpoint spawns the same ``clio --tui`` binary the CLI uses, behind
...
try:
    from clio_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from clio_cli.dashboard_auth.audit import AuditEvent, audit_log
        from clio_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``clio --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``CLIO_TUI_RESUME`` env var —
    matching what ``clio_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``CLIO_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``CLIO_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from clio_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("CLIO_TUI_DISABLE_MOUSE", "1")
    env.setdefault("CLIO_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["CLIO_TUI_RESUME"] = resume

    if sidecar_url:
        env["CLIO_TUI_SIDECAR_URL"] = sidecar_url

    if gateway_ws_url := _build_gateway_ws_url():
        env["CLIO_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from clio_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Clio inside WSL2 to use the dashboard's /chat "
...
# CLIO_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`clio_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from clio_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/clio/*`` -> local Caddy -> :10119), the
    proxy injects ``X-Forwarded-Prefix: /clio`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__CLIO_BASE_PATH__`` honor that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/clio``)
        ...
        ``__CLIO_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__CLIO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__CLIO_BASE_PATH__="{prefix}";'
                f"window.__CLIO_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__CLIO_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__CLIO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__CLIO_BASE_PATH__="{prefix}";'
                f"window.__CLIO_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/clio`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Clio backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Clio Teal",         "description": "Classic dark teal — the canonical Clio look"},
    {"name": "default-large", "label": "Clio Teal (Large)", "description": "Clio Teal with bigger fonts and roomier spacing"},
    {"name": "managed-provider-blue",     "label": "managed-provider Blue",           "description": "Light mode — vivid managed-provider-blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.clio/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.clio/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_clio_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.clio/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as clio_cli.plugins):
    1. User plugins:    ~/.clio/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.clio/plugins/  (only if CLIO_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from clio_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_clio_home() / "plugins", "user"),
    ...
    # ``clio_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("CLIO_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".clio" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from clio_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_clio_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from clio_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from clio_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from clio_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from clio_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.clio/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"clio_dashboard_plugin_{plugin['name']}"
...
from clio_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from clio_cli.dashboard_auth import list_providers
        if not list_providers():
            ...
            try:
                from plugins.dashboard_auth import managed as _managed_plugin

                if _managed_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • managed: {_managed_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            raise SystemExit(
                ...
                f"(was the dashboard_auth/managed plugin removed?).\n"
    ...
    print(f"  Clio Web UI → http://{host}:{port}")

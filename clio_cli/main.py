...
Usage:
    ...
    clio chat                # Interactive chat
    clio gateway             # Run gateway in foreground
    clio gateway start       # Start gateway as service
    clio gateway stop        # Stop gateway service
    clio gateway status      # Show gateway status
    clio gateway install     # Install gateway service
    clio gateway uninstall   # Uninstall gateway service
    clio setup               # Interactive setup wizard
    clio logout              # Clear stored authentication
    clio status              # Show status of all components
    clio cron                # Manage cron jobs
    clio cron list           # List cron jobs
    clio cron status         # Check if cron scheduler is running
    clio doctor              # Check configuration and dependencies
    clio honcho setup                    # Configure Honcho AI memory integration
    clio honcho status                   # Show Honcho config and connection status
    clio honcho sessions                 # List directory → session name mappings
    clio honcho map <name>               # Map current directory to a session name
    clio honcho peer                     # Show peer names and dialectic settings
    clio honcho peer --user NAME         # Set user peer name
    clio honcho peer --ai NAME           # Set AI peer name
    clio honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    clio honcho mode                     # Show current memory mode
    clio honcho mode [hybrid|honcho|local]  # Set memory mode
    clio honcho tokens                   # Show token budget settings
    clio honcho tokens --context N       # Set session.context() token cap
    clio honcho tokens --dialectic N     # Set dialectic result char cap
    clio honcho identity                 # Show AI peer identity representation
    clio honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    ...
    clio claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: clio_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``clio_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``clio update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``clio_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, clio crashes on import and the user can't run
# ``clio update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import clio_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'clio' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``clio tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``clio.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("clio")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"clio", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"clio")
        # Windows: the .exe name is already ``clio.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* clio_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("CLIO_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".clio", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``CLIO_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("CLIO_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``CLIO_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("CLIO_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`clio --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``clio --version`` before config/logging imports on Termux."""
    if os.environ.get("CLIO_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to CLIO_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (clio tools, clio setup, clio model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'clio {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any clio module import.
#
# Many modules cache CLIO_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("CLIO_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.clio/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set CLIO_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors clio_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If CLIO_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.clio/profiles/coder or
    # /opt/data/profiles/coder).  If CLIO_HOME points to the clio root
    # instead (e.g. systemd hardcodes CLIO_HOME=/root/.clio), we must
    # still read active_profile — the user may have switched profiles via
    # `clio profile use` and the gateway should honor that choice.
    # See issue #22502.
    clio_home_env = os.environ.get("CLIO_HOME", "")
    if profile_name is None and clio_home_env:
        if Path(clio_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the clio root
    if profile_name is None:
        try:
            from clio_constants import get_default_clio_root

            active_path = get_default_clio_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set CLIO_HOME
    if profile_name is not None:
        try:
            from clio_cli.profiles import resolve_profile_env

            clio_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent clio from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["CLIO_HOME"] = clio_home
...
# Load .env from ~/.clio/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from clio_cli.config import get_clio_home
from clio_cli.env_loader import load_clio_dotenv

load_clio_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → CLIO_REDACT_SECRETS env
# var BEFORE clio_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_clio_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "CLIO_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["CLIO_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `clio` subcommands
...
try:
    from clio_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from clio_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if clio_constants not importable yet
...
from clio_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_clio_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("CLIO_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("CLIO_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from clio_cli.config import get_env_path, get_clio_home, load_config
    from clio_cli.auth import get_auth_status

    # Determine whether Clio itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from clio_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_clio_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check the auth store's active provider (covers Omni Loop Portal OAuth logins)
    auth_file = get_clio_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Clio has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Clio to use their tokens.
    if _has_clio_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, clio_bin
        cli_args: the original CLI arguments (everything after 'clio')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    clio_bin = container_info["clio_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo clio {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo clio {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  clio --tui --resume {target}")
    if title:
        print(f'  clio --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @clio/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@clio" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/clio-ink/package.json",
    "packages/clio-ink/index.js",
    "packages/clio-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``CLIO_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("CLIO_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``CLIO_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("CLIO_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    clio_home = os.environ.get("CLIO_HOME") or str(Path.home() / ".clio")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "CLIO_HOME": clio_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(clio_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(clio_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if clio_cli_dir is None:
        clio_cli_dir = Path(__file__).parent
    bundled = clio_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (CLIO_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("CLIO_NODE")
    ...
    ext_dir = os.environ.get("CLIO_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with CLIO_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset CLIO_TUI_DIR (e.g. `unset CLIO_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("CLIO_QUIET"):
    ...
    if tui_dev:
        # Keep the local @clio/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @clio/ink resolves through
        # packages/clio-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "clio-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from clio_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="clio-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["CLIO_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["CLIO_PYTHON_SRC_ROOT"] = os.environ.get(
        "CLIO_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("CLIO_PYTHON", sys.executable)
    env.setdefault("CLIO_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["CLIO_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["CLIO_MODEL"] = model
        env["CLIO_INFERENCE_MODEL"] = model
    if provider:
        env["CLIO_TUI_PROVIDER"] = provider
        env["CLIO_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["CLIO_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["CLIO_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["CLIO_TUI_SKILLS"] = value
    if query:
        env["CLIO_TUI_QUERY"] = query
    if image:
        env["CLIO_TUI_IMAGE"] = image
    if checkpoints:
        env["CLIO_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["CLIO_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["CLIO_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["CLIO_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["CLIO_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["CLIO_ACCEPT_HOOKS"] = "1"
    ...
    # CLIO_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `clio --tui` try to
    ...
    # still set CLIO_TUI_RESUME themselves.
    env.pop("CLIO_TUI_RESUME", None)
    if resume_session_id:
        env["CLIO_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `clio update` so
    ...
    if code == 42:
        from clio_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``CLIO_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``clio kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``clio kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("CLIO_KANBAN_BOARD"):
        return
    try:
        from clio_cli.kanban_db import get_current_board

        os.environ["CLIO_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.clio/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Clio — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Clio still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``CLIO_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("CLIO_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'clio sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from clio_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from clio_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'clio doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Clio isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  clio setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'clio setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from clio_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["CLIO_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.clio/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["CLIO_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["CLIO_IGNORE_RULES"] = "1"
...
def cmd_gateway(args):
    ...
    from clio_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from clio_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from clio_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Clio?")
        print()
    ...
    # has no creds.json.  Every subsequent `clio gateway` then paid a 30s
    ...
    session_dir = get_clio_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: clio gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `clio gateway` skips it cleanly instead of paying a 30s
        ...
        print("⚠ Pairing may not have completed. Run 'clio whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from clio_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from clio_cli.config import stamp_install_method
    from clio_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("∞ Clio post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from clio_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``clio model``) and the setup wizard
    ...
    from clio_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("CLIO_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from clio_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'clio model' for "
                "available providers, or run 'clio doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in clio_cli/models.py). A multi-member group shows one
    ...
    if selected_provider == "managed":
        _model_flow_managed(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.clio/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.clio/.env if the active provider is not 'custom'.
    ...
    """
    from clio_cli.config import get_env_value, save_env_value, load_config
...
#
# Clio uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `clio model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `clio model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`clio_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from clio_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Clio only falls back to a lightweight backend (OpenRouter)")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``clio model`` flow, then route aux tasks to them here.
    """
    from clio_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from clio_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from clio_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from clio_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from clio_constants import OPENROUTER_BASE_URL
    from clio_cli.auth import (
        ProviderConfig,
    ...
    # in-flow (K/R/C) instead of having to edit ~/.clio/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `clio setup` from scratch. OpenRouter
    ...
    from clio_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_managed(config, current_model="", args=None):
    """Omni Loop Portal: device-code login (or reuse), then pick a model.

    Delegates to ``_login_managed`` in clio_cli.auth, which handles the full
    flow — browser device approval, token persistence, curated model picker,
    and atomic config update.
    """
    from clio_cli.auth import (
        PROVIDER_REGISTRY,
        _login_managed,
    ...
    try:
        status = get_managed_auth_status() or {}
    except Exception:
        status = {}

    if status.get("logged_in"):
        print("  Omni Loop Portal credentials: ✓")
        print()
        print("    1. Use existing login (pick a model)")
        print("    2. Reauthenticate (new browser login)")
        print("    3. Cancel")
        print()
        try:
        ...
        if choice == "3":
            return
        reuse_existing = choice != "2"
    else:
        print("  Connecting to Omni Loop Portal — your browser will open to approve this device.")
        print()

    try:
        mock_args = argparse.Namespace(reuse_existing=reuse_existing)
        _login_managed(mock_args, PROVIDER_REGISTRY["managed"])
    except SystemExit:
        ...
        print(f"Omni Loop Portal Subscription login failed: {exc}")
        return

    # Light up the subscription's bundled tools (web search, image gen, TTS,
    # cloud browser) for entitled plans — never overrides explicit choices.
    try:
        from clio_cli.config import save_config
        from clio_cli.portal_subscription import apply_managed_defaults

        enabled = apply_managed_defaults(config, force_fresh=True)
        if enabled:
            save_config(config)
            labels = ", ".join(sorted(enabled))
            print(f"  ✓ Subscription tools enabled via the portal gateway: {labels}")
    except Exception:
        logger.debug("apply_managed_defaults after portal login failed", exc_info=True)
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from clio_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `clio auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from clio_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from clio_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``clio model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``clio model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``clio auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from clio_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from clio_cli.auth import _save_model_choice, deactivate_provider
    from clio_cli.config import get_env_value, load_config, save_config
    from clio_cli.secret_prompt import masked_secret_prompt
    ...
    from clio_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `clio model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from clio_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Clio URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from clio_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from clio_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from clio_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Clio will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Clio will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        from clio_cli.curses_ui import curses_radiolist
...
# downstream call sites read `clio_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `clio` invocation — including
# fast paths like `clio --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from clio_cli.config import save_env_value, load_config, save_config
    from clio_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from clio_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from clio_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from clio_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Clio turns to `copilot --acp`.")
    print("  Clio currently starts its own ACP subprocess for each request.")
    print("  Clio uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set CLIO_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``clio setup`` / ``clio model``.
    ...
    recover from a malformed paste without editing ``~/.clio/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from clio_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from clio_cli.config import save_env_value
    from clio_cli.secret_prompt import masked_secret_prompt
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `clio setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from clio_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so clio knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Clio typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Clio, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, managed provider."
                )
                print()
    ...
    if provider_id == "lmstudio":
        from clio_cli.auth import AuthError
        from clio_cli.models import fetch_lmstudio_models
        ...
        from clio_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from clio_constants import display_clio_home as _dhh_fn

            print(
                f"    Clio will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from clio_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'clio model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_logout(args):
    ...
    from clio_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from clio_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from clio_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from clio_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from clio_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``clio slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: clio slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `clio slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from clio_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from clio_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from clio_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from clio_cli.doctor import run_doctor
...
def cmd_evolve(args):
    ...
    result = run_evolve(dry_run=dry_run, report_only=report_only)
    print(result)


def cmd_security(args):
    """Dispatch `clio security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from clio_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from clio_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from clio_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from clio_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Clio home directory to a zip file."""
    if getattr(args, "quick", False):
        from clio_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from clio_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Clio backup from a zip file."""
    from clio_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from clio_cli.banner import check_for_updates
        from clio_cli.config import recommended_update_command
...
# Critical files that every ``clio`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``clio update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "clio_cli/main.py",
    "clio_cli/config.py",
    "clio_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``clio`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="clio-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``clio update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``clio_cli/web_dist/`` (per vite.config.ts
    outDir: "../clio_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "clio_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``clio`` launcher present but ``clio_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``clio update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``clio web``).
    ...
    # build path usable on a stock `py -m clio_cli.main web` invocation.
    ...
    npm = shutil.which("npm")
    if not npm:
        _say("Node.js not found — attempting auto-install...")
        try:
            from clio_cli.dep_ensure import ensure_dependency
            installed = ensure_dependency("node", interactive=False)
            if installed:
                # Re-check npm after potential install
                npm = shutil.which("npm")
        except Exception as exc:
            _say(f"  Auto-install failed: {exc}")
        if not npm:
            if fatal:
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (clio web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``clio update`` can unconditionally call ``clio desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``clio desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $CLIO_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $CLIO_HOME."""
    from clio_constants import get_clio_home
    return get_clio_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``clio update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Clio.app/Contents/MacOS/Clio"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Clio.exe",
            release_dir / "win-ia32-unpacked" / "Clio.exe",
            release_dir / "win-arm64-unpacked" / "Clio.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "clio",
            release_dir / "linux-unpacked" / "Clio",
            release_dir / "linux-arm64-unpacked" / "clio",
            release_dir / "linux-arm64-unpacked" / "Clio",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Clio'`` desktop build failure: a corrupt zip in the
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Clio is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Clio.app/Contents/MacOS/Clio  ->  app bundle = .../Clio.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Clio Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Clio Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from clio_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["CLIO_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["CLIO_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "clio_root", None):
        env["CLIO_DESKTOP_CLIO_ROOT"] = str(Path(args.clio_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["CLIO_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  clio gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Clio` rename: unpack-electron
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Clio is
    ...
    if source_mode:
        print("→ Launching Clio Desktop from source build...")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``clio dashboard`` processes other than ourselves.

    ``clio dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``clio update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Clio Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``clio dashboard`` as
    ...
    environment variable ``CLIO_DESKTOP_CHILD_PID`` on the spawned
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "clio.*dashboard"`) keeps us consistent
            # with `clio_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `clio update`.
    ...
    print("  Preview now:  clio curator run --dry-run")
    print("  Pause it:     clio curator pause")
    print(
        "  Docs:         https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``clio update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``clio update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: clio curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``clio dashboard`` processes.

    Called at the end of ``clio update`` (default ``reason``) and also
    from ``clio dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Clio Desktop Electron app spawns this dashboard as a
    # backend child, it sets CLIO_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("CLIO_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    clio dashboard --port <port>")
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `clio update "
            f"--branch {branch}`, or update against main with `clio update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/Clioloop/Clioloop-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="clio-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"clio-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for clio-agent itself
        ...
        # GitHub ZIPs extract to clio-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"clio-agent-{branch}")
    ...
    from clio_cli.managed_uv import ensure_uv, update_managed_uv
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "clio-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Clio behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes clio completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Clio couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Clio couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Clio behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Clio couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Clio couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `clio update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/Clioloop/Clioloop-agent.git",
    "git@github.com:Clioloop/Clioloop-agent.git",
    "https://github.com/Clioloop/Clioloop-agent",
    "git@github.com:Clioloop/Clioloop-agent",
}
OFFICIAL_REPO_URL = "https://github.com/Clioloop/Clioloop-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_clio_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_clio_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Clio repository.")
        print("  This means you may miss updates from Clioloop/Clioloop-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Clioloop/Clioloop-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Clioloop/Clioloop-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``clio update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from clio_constants import get_default_clio_root

    default_home = get_default_clio_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``clio update`` is still progressing even if pip/uv itself is silent.
    """
...
def _clio_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "clio.exe",
        scripts_dir / "clio-gateway.exe",
    ]


def _detect_concurrent_clio_instances(
    ...
    The Clio Desktop Electron app spawns ``clio.EXE`` as a backend child,
    so during ``clio update`` the user-invoked process and the desktop's
    ...
    shims (``clio.exe`` / ``clio-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``clio update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``clio.exe``) is a separate process from the Python
    ...
    # setuptools-generated clio.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``clio update``
    ...
    #      clio.exe sitting *under* a non-Clio parent (e.g. a Clio
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "clio.exe"
    lines = ["✗ Another clio.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Clio Desktop, exit any open `clio` REPLs, and")
    lines.append("  stop the gateway (`clio gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `clio update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_clio_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``clio.exe``.
    ...
    when ``clio update`` runs, ``clio.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``clio.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next clio invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Clio Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Clio Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _clio_exe_shims(scripts_dir):
        ...
        print(
            "    Close Clio Desktop, exit other `clio` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `clio update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_clio_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``clio.exe.old.*`` left by prior updates.

    Called early on every clio invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``clio update`` runs ``uv pip install -e .[all]`` which
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``clio.exe`` / ``clio-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_clio_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_clio_exe(scripts_dir)
        try:
    ...
    # hours later inside ``clio update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``clio update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `clio update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `clio update --force` after closing other clio processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from clio_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$CLIO_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from clio_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `clio update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``clio update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.clio/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``clio update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``clio update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.clio/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``clio update --gateway``) the update is already
    ...
    try:
        ...
        # clio_cli.config.get_clio_home to simulate setup failure.
        from clio_cli.config import get_clio_home as _get_clio_home

        logs_dir = _get_clio_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    dropping the flag.
    """
    from clio_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``clio update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from clio_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from clio_cli.config import recommended_update_command
        from clio_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``clio update`` without requiring a reinstall.  The
    ...
    ``clio`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/clio.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v clio'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/clio, code at /usr/local/lib/clio-agent).
    fhs_link = Path("/usr/local/bin/clio")
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of CLIO_HOME before running the update.
    ...
    CLIO_HOME directories.  The ``--backup`` flag on ``clio update``
    ...
    try:
        from clio_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_clio_home so the user sees ~/.clio/...
    try:
        from clio_constants import get_clio_home, display_clio_home

        home = get_clio_home()
        try:
            display_path = f"{display_clio_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  clio import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``clio update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `clio update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Clio via pip (for PyPI installs)."""
    from clio_cli import __version__
    from clio_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "clio-agent"]
        ...
        cmd = [pipx, "upgrade", "clio-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "clio-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "clio-agent"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # though `git pull` can't touch $CLIO_HOME, this is cheap
        ...
        try:
            from clio_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in clio_cli/config.py bricked
        # every user who ran ``clio update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``clio
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``clio update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_clio_home added to clio_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # build.  ``clio desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``clio desktop`` shouldn't be forced into a full
        # Electron build by ``clio update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        ...
        # modules in this process.  Reload clio_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_clio_home() added since the last release.
        try:
            import importlib
            import clio_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit CLIO_HOME so
        # it is not affected by sync_skills()'s module-level CLIO_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's CLIO_HOME env var points at the default or a named profile.
        try:
            from clio_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'clio config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: clio config migrate")
            else:
                print()
                print("Skipped. Run 'clio config migrate' later to configure.")
        else:
        ...
        try:
            from clio_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `clio curator status`. Self-stamps after printing so it
        ...
        if gateway_mode:
            _exit_code_path = get_clio_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all clio-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. clio-gateway.service or clio-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: clio gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: clio -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Clio gateway unit files are still installed.
        # When both clio.service (from a pre-rename install) and the
        # current clio-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `clio update` surfaces the issue until the user migrates.
        try:
            from clio_cli.gateway import (
                has_legacy_clio_units,
                _find_legacy_clio_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_clio_units():
                print()
                print("⚠ Legacy Clio gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_clio_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (clio.service) fight the current")
                print("  clio-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    clio gateway migrate-legacy")
                print()
        ...
        print("  clio model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``clio -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `clio profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_clio_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → clio -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.clio)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  clio profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     clio -p {name} chat")
                else:
        ...
        from clio_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from clio_constants import get_clio_home as _hh
        ...
        from clio_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `clio profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from clio_cli.profiles import profile_exists
        ...
        from clio_cli.profiles import rename_profile
        ...
        from clio_cli.profiles import export_profile
        ...
        import tempfile
        from clio_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="clio_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  clio -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      clio -p {plan.manifest.name} chat")
        ...
        from clio_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `clio profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"clio -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from clio_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("clio_requires"):
            print(f"Requires:     Clio {data['clio_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from clio_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.clio_requires:
        print(f"  Requires: Clio {mf.clio_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``clio dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``clio dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No clio dashboard processes running.")
        return 0

    print(f"{len(pids)} clio dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No clio dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `clio update`.
    ...
    # the same logs directory as every other Clio surface.
    try:
        from clio_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "CLIO_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["CLIO_WEB_DIST"])
            if "CLIO_WEB_DIST" in os.environ
            else PROJECT_ROOT / "clio_cli" / "web_dist"
        )
    ...
    # (e.g. plugins/dashboard_auth/managed) registers BEFORE start_server's
    ...
    try:
        from clio_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
...
def cmd_completion(args, parser=None):
    ...
    from clio_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from clio_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Clio log files."""
    from clio_cli.logs import tail_log, list_logs
...
# so that in ``clio -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``clio_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``clio -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``clio`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("CLIO_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from clio_cli.mcp_startup import start_background_mcp_discovery
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if getattr(args, "oneshot", None):
        ...
        from clio_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["CLIO_DEFER_AGENT_STARTUP"] = "1"
            os.environ["CLIO_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["CLIO_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `clio --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for clio CLI."""
    # Cosmetic: make the process show up as 'clio' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from clio_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``clio.exe.old.*`` quarantine files left by previous
    # ``clio update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_clio_exe``.
    try:
    ...
    # =========================================================================
    from clio_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `clio model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.clio/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from clio_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from clio_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if CLIO_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_install.add_argument(
        ...
        action="store_true",
        help=(
            "Install without prompts (used by the desktop app): assume yes for "
            "start-now / start-on-login and, on Windows, open the UAC elevation "
            "prompt directly when admin rights are needed."
        ),
    )
    gateway_install.add_argument(
        "--system",
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy clio.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Clio gateway unit files "
            "(e.g. clio.service) left over from older installs. Profile "
            "units (clio-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="xai",
        help="Upstream provider: xai or openrouter (default: xai). See `clio proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'clio setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Clio.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $CLIO_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Clio")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from clio_cli.send_cmd import register_send_subparser
    ...
    login_parser.add_argument(
        ...
        default=None,
        help="Provider to authenticate with (default: auto-detect)",
    )
    login_parser.add_argument(
        "--manual-paste",
        action="store_true",
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Clio with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set CLIO_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.clio/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Clio profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.clio/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Clio profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from clio_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.clio/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.clio/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `clio "
    ...
    # =========================================================================
    # evolve command — skill evolution engine
    # =========================================================================
    evolve_parser = subparsers.add_parser(
        "evolve",
        help="Evolve the skill library (analyze, merge, archive, suggest)",
        description=(
            "Analyze your skill library and evolve it by detecting overlaps, "
            "merging duplicates, archiving stale skills, and suggesting improvements. "
            "Only affects user-created skills in ~/.clio/skills/."
        ),
    )
    evolve_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Analysis only — no changes written to disk",
    )
    evolve_parser.add_argument(
        "--report",
        action="store_true",
        help="Print a health report without taking any actions",
    )
    evolve_parser.set_defaults(func=cmd_evolve)

    # =========================================================================
    # security command — supply-chain audit
    # =========================================================================
    security_parser = subparsers.add_parser(
        "security",
        help="Supply-chain audit (OSV.dev) for venv, plugins, and MCP servers",
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Clio "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.clio/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Clio Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Clio setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    clio debug share              Upload debug report and print URL
    clio debug share --lines 500  Include more log lines
    clio debug share --expire 30  Keep paste for 30 days
    clio debug share --local      Print report locally (no upload)
    clio debug share --no-redact  Disable upload-time secret redaction
    clio debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'clio debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Clio home directory to a zip file",
        description="Create a zip archive of your entire Clio configuration, "
        "skills, sessions, and data (excludes the clio-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/clio-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.clio/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo clio uses to snapshot working directories before "
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Clio backup from a zip file",
        description="Extract a previously created Clio backup into your "
        "Clio home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    def cmd_pairing(args):
        from clio_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.clio/skills/.bundled_manifest) "
            "so future 'clio update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`clio update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `clio update`. Pass --sync to re-seed now."
        ),
    )
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from clio_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from clio_cli.skills_hub import skills_command
    ...
    from clio_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/clio-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `clio plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from clio_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``clio --help``, ``clio version``, ``clio logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from clio_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from clio_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_clio_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_clio_home()}/memories/\n")
        else:
            from clio_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'clio tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # clio tools disable <name...> [--platform cli]
    ...
    # clio tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from clio_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from clio_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Clio as an MCP server",
        description=(
            "Manage MCP server connections and run Clio as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'clio mcp add' to connect to a new server, or\n"
            "'clio mcp serve' to expose Clio conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Clio as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `clio mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `clio mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `clio mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from clio_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        # Hide third-party tool sessions by default, but honor explicit --source
        ...
        if action == "list":
            ...
            # Launch clio --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Clio",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Clio",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.clio/ (by default a "
        "single restore-point archive is written to ~/.clio/backups/ "
        "before apply; restorable with 'clio import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from clio_cli.claw import claw_command
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "If the local checkout is on a different branch, clio will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another clio.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Clio ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Clio provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.clio/node/ "
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Clio instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `clio update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `clio profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Clio profile distribution. SOURCE can be a git URL "
    ...
    # for `clio dashboard` cmdlines and SIGTERM them directly — the same
    # path `clio update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running clio dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running clio dashboard processes and exit",
    )
    ...
    # for one release. The Clio-Setup.exe success screen tells users to
    # run `clio desktop` from a terminal, so the canonical name needs
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Clio Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any clio CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--clio-root",
        help="Override the Clio source root used by Desktop (sets CLIO_DESKTOP_CLIO_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets CLIO_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Clio log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    clio logs                    Show last 50 lines of agent.log
    clio logs -f                 Follow agent.log in real time
    clio logs errors             Show last 50 lines of errors.log
    clio logs gateway -n 100     Show last 100 lines of gateway.log
    clio logs gui -f             Follow gui.log in real time
    clio logs desktop -f         Follow desktop.log (Electron app boot/backend)
    clio logs --level WARNING    Only show WARNING and above
    clio logs --session abc123   Filter by session ID
    clio logs --component tools  Only show tool-related lines
    clio logs --since 1h         Lines from the last hour
    clio logs --since 30m -f     Follow, starting from 30 min ago
    clio logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``clio -c Pokemon Agent Dev`` → ``clio -c 'Pokemon Agent Dev'``
    ...
    from clio_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'clio -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (clio hooks list, cron

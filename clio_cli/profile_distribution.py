"""Profile distributions — shareable, packaged Clio profiles via git.

A distribution is a Clio profile published as a git repository (or
...
* ``clio profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``clio skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``clio profile``, not a parallel tree):

    clio profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "clio-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            clio_requires=str(data.get("clio_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.clio_requires:
            out["clio_requires"] = self.clio_requires
        if self.author:
...
def check_clio_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Clio {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``clio profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Clio profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Clio distribution."
        )

    # Version check up-front so we fail fast
    check_clio_requires(manifest.clio_requires, clio_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.clio).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="clio_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `clio profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`clio profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="clio_dist_update_") as tmp:
        plan = plan_install(

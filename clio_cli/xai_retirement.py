...
Pure logic: walks a Clio config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `clio doctor` and a future `clio migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Clio config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Clio config and return retirement issues.

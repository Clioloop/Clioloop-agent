...
manual BotFather token copy-paste. Clio talks only to the managed provider onboarding
...
DEFAULT_MANAGER_BOT = "ClioSetupBot"

DEFAULT_BOT_NAME = "Clioloop"
...
def render_qr_terminal(url: str) -> str:
    ...
    # Auto-install the small ``qrcode`` dep on first use (venv-scoped, allowlisted)
    # so onboarding draws a scannable code instead of a "pip install" hint.
    try:
        from tools.lazy_deps import ensure as _ensure_dep

        _ensure_dep("tool.qrcode", prompt=False)
    except Exception:
        pass
    try:
        import io
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``clio_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``clio_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"clio_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Clio Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Clio Telegram onboarding service.")

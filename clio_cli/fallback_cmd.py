"""
clio fallback — manage the fallback provider chain.
...
Subcommands:
  clio fallback [list]   Show the current fallback chain (default when no subcommand)
  clio fallback add      Pick provider + model via the same picker as `clio model`,
                           then append the selection to the chain
  clio fallback remove   Pick an entry to delete from the chain
  clio fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.clio/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from clio_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `clio model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  clio fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `clio model`, then append the selection to the chain."""
    from clio_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `clio model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `clio fallback list` to view, or `clio fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``clio fallback [subcommand]``."""

...
- clio config          - Show current configuration
- clio config edit     - Open config in editor
- clio config set      - Set a specific value
- clio config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``clio config set`` (which rewrites
    ...
    ``config.yaml`` in place: clio never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.clio/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``clio logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  clio config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Clio makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Clio itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``clio update``, the TUI build.
...
#   on every plugin install / ``clio update``.
...
# * ``CLIO_HOME`` / ``CLIO_PROFILE`` / ``CLIO_CONFIG`` /
#   ``CLIO_ENV`` — Clio runtime location flags. Writing these into
...
#
# IMPORTANT: ``CLIO_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (CLIO_GEMINI_CLIENT_ID,
# CLIO_LANGFUSE_PUBLIC_KEY, CLIO_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Clio runtime location — never via dashboard env writer.
    # NOT a CLIO_* blanket: integration credentials (CLIO_GEMINI_*,
    # CLIO_LANGFUSE_*, CLIO_SPOTIFY_*, ...) ARE allowed.
    "CLIO_HOME", "CLIO_PROFILE", "CLIO_CONFIG", "CLIO_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Clio runtime location "
            "(CLIO_HOME, CLIO_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `clio plugins enable
    # observability/langfuse` or `clio tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "CLIO_LANGFUSE_ENV",
    "CLIO_LANGFUSE_RELEASE",
    "CLIO_LANGFUSE_SAMPLE_RATE",
    "CLIO_LANGFUSE_MAX_CHARS",
    "CLIO_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from clio_cli.colors import Colors, color
from clio_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CLIO_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_clio_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Clio is running in package-manager-managed mode.

    Two signals: the CLIO_MANAGED env var (set by the systemd service),
    or a .managed marker file in CLIO_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade clio-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Clio was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.clio/.install_method`` file (written by installers)
    2. CLIO_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``clioloop/clioloop-agent`` image stamps ``docker``
    ...
    so ``clio update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_clio_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.clio/.install_method."""
    stamp = get_clio_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Clio lives in a ``uv tool`` layout.

    ``uv tool install clio-agent`` places the install at
    ``.../uv/tools/clio-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``clio-agent`` happens to be uv-tool-installed on the machine while
    the *active* Clio is a regular pip/venv install, causing
    ``clio update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/clio-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade clio-agent"
    if method == "docker":
        return "docker pull clioloop/clioloop-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade clio-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade clio-agent"
        return "pip install --upgrade clio-agent"
    return "clio update"
...
# Long-form text for ``clio update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# clio_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Clio, it doesn't update
...
pulling a fresh image and restarting your container instead:
  ...
  docker pull clioloop/clioloop-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate clio-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm clioloop/clioloop-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/clioloop/clioloop-agent/tags
  • Your config and session history live under ``$CLIO_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``clio update`` inside Docker.
...
def format_managed_message(action: str = "modify this Clio installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("CLIO_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Clio installation is managed by NixOS "
            f"(CLIO_MANAGED={env_hint}).\n"
            "Edit services.clio-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Clio installation is managed by Homebrew "
            f"(CLIO_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade clio-agent"
        )

    return (
        f"Cannot {action}: this Clio installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Clio."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CLIO_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, clio_bin
    or None if container mode is not active, we're already inside the
    container, or CLIO_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("CLIO_DEV") == "1":
        return None

    from clio_constants import is_container
    if is_container():
        return None

    container_mode_file = get_clio_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "clio-agent")
    exec_user = info.get("exec_user", "clio")
    clio_bin = info.get("clio_bin", "/data/current-package/bin/clio")
...
# Re-export from clio_constants — canonical definition lives there.
from clio_constants import get_clio_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_clio_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_clio_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_clio_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the CLIO_UID / CLIO_GID env vars set by Docker deployments.

    Docker containers running Clio commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level CLIO_HOME once, but
    subdirectories created at runtime by ``ensure_clio_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("CLIO_UID", "").strip()
    gid_str = os.environ.get("CLIO_GID", "").strip()
    try:
...
def _chown_to_clio_uid(path) -> None:
    """Chown ``path`` to ``CLIO_UID:CLIO_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_clio_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_clio_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the clio group can
    share state with the gateway service.

    The mode can be overridden via the CLIO_HOME_MODE environment variable
    (e.g. CLIO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CLIO_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``CLIO_UID``/``CLIO_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CLIO_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_clio_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Clio runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("CLIO_CONTAINER") or os.environ.get("CLIO_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set CLIO_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CLIO_HOME if the user doesn't have one yet."""
...
def ensure_clio_home():
    """Ensure ~/.clio directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_clio_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_clio_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CLIO_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["clio-cli"],
    "fusion": {
        # Fusion v2 is the default architecture: planners -> main worker ->
        # reviewers -> judge -> synthesizer. Set version=1 only for a temporary
        # compatibility fallback to the legacy revise/finalize loop.
        "version": 2,
        "gate": True,
        "legacy_revision_loop": False,
        # Optional model overrides for the structured judge and final writer.
        # Empty = use the current main chat model.
        "judge_model": "",
        "synthesizer_model": "",
        # Rough total prompt/output budget for one Fusion invocation. If the
        # cap is reached, Fusion skips later optional stages and returns the
        # best available draft with a footer.
        "max_total_tokens": 200_000,
        # Reserved schema knobs for the v2 planner/reviewer panel. The runtime
        # currently uses its built-in role order when these are empty.
        "panel": {},
        "roles": {
            "advisors": [],
            "reviewers": [],
        },
        # One tool-bearing rework pass is allowed after judge review.
        "max_rounds": 1,
        # Legacy/reserved. Fusion now always lets the current main agent stream
        # its full-tool work normally; planner/reviewer helper output stays in
        # the background.
        "show_work": False,
        # Tool-iteration budget for each main-model work/revise turn. 0 = use the
        # agent's own max_iterations (a Fusion work turn then gets the same budget
        # as a normal turn). The budget is freshly granted on every work/revise
        # round (run_conversation resets it per turn), so it resets after each
        # review.
    ...
    "agent": {
        ...
        # the Clio-level retry loop that wraps the whole call.  Lower
        ...
        # identity slot (SOUL.md). Empty by default. The CLIO_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Clio auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Clio sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Clio
        ...
        # bundled Clio image, which drops to the `clio` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Clio sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Clio should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``clio chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.clio/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: clio sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Clio truncates the
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | managed | codex | custom
        ...
        # Invoked by ``clio kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``clio kanban decompose`` and the kanban
        ...
        # ``clio profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `clio model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `clio` (and `clio chat`) launches by default:
        ...
        # REPL and `--tui` (or CLIO_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `clio --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `clio -c` muscle memory.  Default off so existing
        # users aren't surprised.  CLIO_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `clio --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/clio`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled managed provider plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``CLIO_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``CLIO_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``CLIO_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``CLIO_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/clio``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.clio/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.clio/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, managed provider, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # response. If not, Clio feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Clio auto-pauses the goal and
    ...
    # always goes to ~/.clio/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${CLIO_SKILL_DIR} and ${CLIO_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `clio curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `clio update`, so pruning them only
        ...
        # skipped), snapshot ~/.clio/skills/ into
        # ~/.clio/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `clio curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for clio-specific overrides; everything else
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (CLIO_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # toggleable per-invocation via --accept-hooks or CLIO_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `clio doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``clio_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Clio to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via CLIO_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `clio -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `clio` launches with
        ...
        # decomposition is manual via `clio kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     clio-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Clio tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.clio/logs/.
    ...
    # curated model lists for OpenRouter and managed provider from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a clio-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `clio model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.clio/.env,
        ...
        # behavior: files must live under the Clio cache, under
        ...
        # user. Bridged to CLIO_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Clio
        # cache (~/.clio/cache/{documents,images,audio,video,screenshots})
        ...
        # to CLIO_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # CLIO_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to CLIO_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.clio/state.db.
    ...
    "sessions": {
        ...
        # default of ``clio sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.clio/sessions/session_{sid}.json`` on every turn
    ...
    # ``clio update`` behaviour.
    "updates": {
        # Run a full ``clio backup``-style zip of CLIO_HOME before every
        # ``clio update``.  Backups land in ``<CLIO_HOME>/backups/`` and
        # can be restored with ``clio import <path>``.  Off by default —
        # on large CLIO_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `clio update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<CLIO_HOME>/lsp/bin/`` on first use.
    ...
    # `clio tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.clio/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.clio/.env (or your shell) and never in config.yaml.
            ...
            # ~/.clio/bin/ on first use.  When False you must install
...
ENV_VARS_BY_VERSION: Dict[int, List[str]] = {
    3: ["FIRECRAWL_API_KEY", "BROWSERBASE_API_KEY", "BROWSERBASE_PROJECT_ID", "VIDU_API_KEY"],
...
# selection step (managed provider / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "MANAGED_BASE_URL": {
        "description": "managed provider base URL override",
        "prompt": "managed provider base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'clio model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for managed provider Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for managed provider Subscribers only, used to derive vendor hosts, e.g.  -> firecrawl-gateway.",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for managed provider Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit managed provider Subscriber access token for tool-gateway requests (optional; otherwise read from the Clio auth store)",
    ...
    "VIDU_API_KEY": {
        "description": "Vidu API key for video generation",
        "prompt": "Vidu API key",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @clio:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CLIO_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #clio)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: clio-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'clio-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Clio API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Clio API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Clio API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CLIO_TOOL_PROGRESS and CLIO_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.clio.config`` entries, then checks
    ...
    try:
        ...
        # should never break `clio update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Clio versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``clio_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'clio setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Clio won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'clio doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("CLIO_HOME", "~/.clio")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("CLIO_TOOL_PROGRESS")
            old_mode = get_env_value("CLIO_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from CLIO_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from CLIO_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("CLIO_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from CLIO_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$CLIO_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_clio_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `clio plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `clio curator status` has no
    ...
    #   3. Creates `~/.clio/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_clio_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_clio_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: clio config set <key> <value>")
    ...
    # metadata.clio.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _clio_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.clio/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.clio/config.yaml.
    ...
    (which change ``CLIO_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — clio auth) — OpenAI Codex
#   managed         (OAuth — clio auth) — managed provider
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.clio/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.clio/.env.
    ...
    interactive menu render (`clio tools`, `clio setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `clio tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Clio env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.clio/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.clio/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.clio/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.clio/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Clio — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Clio vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.clio/.env or environment."""
...
def show_config():
    ...
    print(color("│              ∞ Clio Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  clio config edit     # Edit config file", Colors.DIM))
    print(color("  clio config set <key> <value>", Colors.DIM))
    print(color("  clio setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    api_keys = [
        ...
        'VIDU_API_KEY', 'FAL_KEY', 'TELEGRAM_BOT_TOKEN', 'DISCORD_BOT_TOKEN',
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: clio config set <key> <value>")
            print()
            print("Examples:")
            print("  clio config set model anthropic/claude-sonnet-4")
            print("  clio config set terminal.backend docker")
            print("  clio config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'clio config migrate' to add them")
        ...
        print("  clio config           Show current configuration")
        print("  clio config edit      Open config in editor")
        print("  clio config set <key> <value>   Set a config value")
        print("  clio config check     Check for missing/outdated config")
        print("  clio config migrate   Update config with new options")
        print("  clio config path      Show config file path")
        print("  clio config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``clio config`` UI so users

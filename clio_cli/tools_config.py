    ...
    "tts": {
        ...
        "providers": [
            ...
            },
            {
                ...
                "badge": "subscription",
                "tag": "Studio-quality Supertonic-3 voices through your portal — no API key",
                "env_vars": [],
                "tts_provider": "openai",
                "managed_feature": "tts",
            },
        ...
        "providers": [
            {
                ...
                "badge": "subscription",
                "tag": "Web search & extract through your portal subscription — no API key",
    ...
    "browser": {
        ...
        "providers": [
            ...
            },
            {
                ...
                "badge": "subscription",
                "tag": "Cloud browser sessions through your portal subscription — no API key",
                "env_vars": [],
...
# ("managed provider Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("managed provider Subscription", "Local Browser", "Camofox") — see the comment block
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_managed_message = _hidden_managed_gateway_message(
        cat,
        config,
        f"the managed provider Subscription provider for {name}",
...
def _configure_provider(
    ...
    *,
    ...
    if provider.get("requires_managed_auth") and not managed_feature:
        ...
        if not features.provider_auth_present or not entitled:
            message = format_managed_provider_entitlement_message(
                ...
                capability=f"{provider.get('name', 'managed provider Subscription')}",
            )
            _print_warning(
                f"  {message or 'managed provider Subscription is only available after logging into managed provider.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your managed subscription.")
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your managed subscription defaults", Colors.GREEN))

...
Saves per-platform tool configuration to ~/.clio/config.yaml under
...
from clio_cli.portal_subscription import (
...
def _is_retired_managed_gateway_provider(provider: dict) -> bool:
    # A retired-name denylist guards against stale plugin/cache data
    # reintroducing old provider rows; managed_feature rows are the live
    # Omni Loop Portal Tool Gateway entries.
    name = str(provider.get("name") or "").strip().lower()
    return name in RETIRED_MANAGED_GATEWAY_NAMES


# ─── UI Helpers (shared with setup.py) ────────────────────────────────────────

from clio_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _CLIO_CORE_TOOLS (available at runtime if enabled),
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from clio_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `clio tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``clio tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from clio_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``clio tools`` checklist actually offers
    for ``platform``.
    ...
    so ``clio tools`` never claims to add or remove a toolset the user was
...
from clio_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            {
                "name": "Omni Loop Portal Subscription (Supertonic Premium TTS)",
                "badge": "Recommended · subscription",
                "tag": "Studio-quality Supertonic-3 voices through your Omni Loop Portal Subscription — no API key",
                "env_vars": [],
                "tts_provider": "openai",
                "managed_feature": "tts",
            },
            {
                "name": "Supertonic",
                "badge": "local · free",
                "tag": "Studio-quality local ONNX TTS (Supertonic-3, ~66M params, CPU)",
                "env_vars": [],
    ...
    "web": {
        ...
        # in _visible_providers(). Only self-hosted Firecrawl remains as a
        # built-in setup-flow row.
        "providers": [
            {
                "name": "Omni Loop Portal Subscription (Firecrawl)",
                "badge": "Recommended · subscription",
                "tag": "Web search & extract through your Omni Loop Portal Subscription Tool Gateway — no API key",
        ...
        "providers": [
            {
                ...
                "badge": "Recommended · required for Fusion",
                "tag": "Required for Model Fusion; self-hosted image generation through your Omni Loop Portal Subscription — no API key",
                "env_vars": [],
        ...
        "providers": [
            {
                ...
                "tag": "Video generation through your Omni Loop Portal Subscription Tool Gateway — no API key",
                "env_vars": [],
    ...
    "x_search": {
        ...
        "setup_note": (
            "Clio routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # _plugin_browser_providers() in _visible_providers(). Only local
        # setup-flow rows remain here:
        ...
        "providers": [
            ...
            {
                "name": "Omni Loop Portal Subscription (Browser Use Cloud)",
                "badge": "Recommended · subscription",
                "tag": "Cloud browser sessions through your Omni Loop Portal Subscription Tool Gateway — no API key",
                "env_vars": [],
                "browser_provider": "browser-use",
                "managed_feature": "browser",
            },
        ],
    },
    "homeassistant": {
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "CLIO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "CLIO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "CLIO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "CLIO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "CLIO_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``clio update`` and
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_clio_home()}/clio-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in clio-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/clioloop/clioloop-agent:latest"
            )
            return
        ...
        try:
            ...
            pass
        _print_info("    Installing supertonic (ONNX, CPU-only; supertonic-3 model auto-downloads on first use)...")
        try:
            result = _pip_install(["-U", "supertonic", "soundfile", "--quiet"], timeout=300)
            if result.returncode == 0:
                _print_success("    supertonic installed")
                _print_info("    Models: supertonic-3 (default) · voices: F1, M1, …")
            else:
                _print_warning("    supertonic install failed:")
                _print_info(f"      {(result.stderr or '').strip()[:300]}")
                _print_info("    Run manually: uv pip install -U supertonic soundfile")
        except subprocess.TimeoutExpired:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.clio/config.yaml")
        ...
        # Run the full `clio auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.clio/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from clio_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: clio auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `clio auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: clio auth spotify")
        except Exception as exc:
        ...
        try:
            from clio_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: clio plugins enable observability/langfuse")
        _print_info("    Restart Clio for tracing to take effect.")
        _print_info("    Verify: clio plugins list")
        ...
        try:
            ...
            _print_info("    Run later: clio auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `clio auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: clio auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: clio auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "clio-cli" (which include all _CLIO_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[clio-cli, spotify]`` after enabling Spotify via ``clio
        ...
        # (e.g. "clio-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``clio tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `clio tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"clio-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("clio-")}
    ...
    # shipped as a bundled plugin but user must opt in via `clio tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `clio tools`
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (clio-cli, clio-telegram, etc.)
    ...
    # Opening `clio tools` is the user's opt-in to reconfigure tools, so treat
...
# ("Omni Loop Portal Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Omni Loop Portal Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Retired managed gateway rows are filtered here so stale plugin/cache data
    cannot reintroduce the removed portal flow.
    """
    features = get_managed_subscription_features(config, force_fresh=force_fresh)
    ...
    for provider in cat.get("providers", []):
        if _is_retired_managed_gateway_provider(provider):
            continue
        # Pre-auth UX rows stay hidden until the user is logged in.
        if (
            provider.get("requires_managed_auth")
            and not provider.get("managed_feature")
            and not features.provider_auth_present
        ):
            continue
    ...
    # Inject plugin-registered cloud browser backends. Local Browser and
    # Camofox remain as built-in setup-flow rows.
...
def _hidden_managed_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated no-op retained for older call sites."""
    return ""
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `clio tools` gets a silent no-op
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_managed_message = _hidden_managed_gateway_message(
        cat,
        config,
        f"the Omni Loop Portal Subscription provider for {name}",
    ...
    if len(providers) == 1:
        ...
        if hidden_managed_message:
            for line in hidden_managed_message.splitlines():
        ...
        # When the user is logged into managed provider, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of managed provider.
        try:
            _managed_logged_in = bool(
                get_managed_subscription_features(
                    config,
                    force_fresh=force_fresh,
        ...
        for p in providers:
            ...
            # Mark managed-provider entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via managed provider" hint so
            ...
            sub_marker = ""
            if p.get("managed_feature"):
                if _managed_logged_in:
                    sub_marker = "  ★ Included with your subscription"
                else:
                    sub_marker = "  ★ managed (Portal login removed in Clio)"
...
def _is_provider_active(
    ...
    *,
    ...
    if video_plugin_name and not provider.get("managed_feature"):
    ...
    if managed_feature:
        ...
        if provider.get("tts_provider"):
            # The managed "Omni Loop Portal Subscription" row is the active one only when the
        ...
        if "browser_provider" in provider:
            ...
            return feature.managed_by_provider and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return feature.managed_by_provider and current == provider["web_backend"]
        return feature.managed_by_provider

    if provider.get("tts_provider"):
        # A direct (bring-your-own-key) TTS row is active only when the gateway
        # is OFF. With use_gateway on, the managed "Omni Loop Portal Subscription" row above
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    NOT prompt for env vars, run post-setup hooks, gate on managed-provider auth, or run
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on managed provider auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    # managed-provider Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid managed provider
    ...
    # "enable all tools" prompt (that lives in `clio model`).
    if managed_feature:
        from clio_cli.portal_subscription import (
        ...
        if not ensure_managed_provider_access(
            capability=f"{provider.get('name', 'the managed provider Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — managed provider access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_managed_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_managed_auth") and not managed_feature:
        features = get_managed_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.provider_auth_present or not entitled:
            message = format_managed_provider_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Omni Loop Portal Subscription')}",
            )
            _print_warning(
                f"  {message or 'Omni Loop Portal Subscription is only available after logging into the portal.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Omni Loop Portal Subscription.")
    ...
    # managed-provider sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to managed provider.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_managed_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_managed_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.provider_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through your Omni Loop Portal subscription.")
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline managed provider login + entitlement gate as _configure_provider:
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `clio tools` and `clio setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("∞ Clio Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Omni Loop Portal Subscription defaults", Colors.GREEN))
    ...
    print()
    from clio_constants import display_clio_home
    print(color(f"  Tool configuration saved to {display_clio_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'clio' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, clio mcp

...
Pure display functions with no ClioCLI state dependency.
"""
...
_PRIMARY = "\033[1;38;2;167;139;250m"  # True-color #A78BFA bold
_BOLD = "\033[1m"
_DIM = "\033[2m"
_RST = "\033[0m"
...
from clio_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def _terminal_art_path_candidates() -> List[Path]:
    root = Path(__file__).parent.parent.resolve()
    clio_home = get_clio_home()
    return [
        root / "assets" / "terminal-art.txt",
        Path.cwd() / "assets" / "terminal-art.txt",
        clio_home / "clio-agent" / "assets" / "terminal-art.txt",
    ]


def _load_terminal_art() -> List[str]:
    global _TERMINAL_ART_CACHE
    if _TERMINAL_ART_CACHE is not None:
        return _TERMINAL_ART_CACHE

    for path in _terminal_art_path_candidates():
        try:
            if path.exists():
                _TERMINAL_ART_CACHE = [
                    line.rstrip()
...
def _crop_art_line(line: str, width: int) -> str:
    if len(line) <= width:
        return line
    start = max(0, (len(line) - width) // 2)
    return line[start:start + width]


def _terminal_art_markup(max_width: int, max_height: int = 12) -> Optional[str]:
    art = _load_terminal_art()
    if not art or max_width < 70:
        return None

    height = min(max_height, len(art))
    start = max(0, (len(art) - height) // 3)
    colors = ("#C4B5FD", "#A78BFA", "#8B5CF6", "#6D28D9", "#2DD4BF")
    lines = []
    for index, line in enumerate(art[start:start + height]):
        color = colors[min(index // 3, len(colors) - 1)]
        lines.append(f"[{color}]{_crop_art_line(line, max_width)}[/]")
    return "\n".join(lines)
...
# (e.g. nix-built clio — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/Clioloop/Clioloop-agent.git"
...
def _fetch_pypi_latest(package: str = "clio-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Clio update is available.

    Two paths: if ``CLIO_REVISION`` is set (nix builds embed it), compare
    ...
    """
    clio_home = get_clio_home()
    cache_file = clio_home / ".update_check"
    embedded_rev = os.environ.get("CLIO_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # CLIO_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `clio update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/clio/update/check`
    ...
    if embedded_rev:
        ...
        # $CLIO_HOME/clio-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = clio_home / "clio-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Clio git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$CLIO_HOME/clio-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``clio_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from clio_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/Clioloop/Clioloop-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Clio checkout. Cached per-process. Release URL always points at the
    canonical Clioloop/Clioloop-agent repo (forks don't get a link).
    """
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    """Build and print a welcome banner with infinity on left and info on right.
    ...
    accent = _skin_color("banner_accent", "#7C3AED")
    dim = _skin_color("banner_dim", "#C4B5FD")
    text = _skin_color("banner_text", "#EDE9FE")
    session_color = _skin_color("session_border", "#8B8682")

    # Use skin's custom hero art if provided. Otherwise show the new terminal
    # braille art in a restrained crop when there is enough room.
    try:
        from clio_cli.skin_engine import get_active_skin
        _bskin = get_active_skin()
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else None
    except Exception:
        _bskin = None
        _hero = None
    if not _hero:
        terminal_width = shutil.get_terminal_size((120, 30)).columns
        _hero = _terminal_art_markup(min(78, max(0, terminal_width // 2 - 6))) or CLIO_INFINITY
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Omni Loop Labs[/]")

    if os.getenv("CLIO_YOLO_MODE"):
    ...
    try:
        from clio_cli.codex_runtime_switch import get_current_runtime
        from clio_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from clio_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built clio; we know an update
    ...
    # Pip-install warning — `pip install clio-agent` is not the supported
    ...
    title_color = _skin_color("banner_title", "#A78BFA")
    border_color = _skin_color("banner_border", "#5B21B6")
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else CLIO_AGENT_LOGO
        console.print(_logo)
        console.print()

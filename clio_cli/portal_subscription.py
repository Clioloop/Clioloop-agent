        ...
        *,
        ...
        for key, label in _FEATURE_DEFS:
            ...
            state = ManagedFeatureState(
                ...
                current_provider="Omni Loop Portal" if managed else local_label,
            )
...
def ensure_managed_provider_access(
    ...
    **_kwargs: Any,
    ...
    logged into the Omni Loop Portal or the plan lacks gateway access.
    """
    ...
    if info is None or not getattr(info, "logged_in", False):
        try:
            print(
                "  Omni Loop Portal login required — connect with: clio setup --portal"
            )
        except Exception:
            pass
        return False
    ...
    try:
        print(f"  Your Omni Loop Portal plan does not include {what}{upgrade}")
    except Exception:
        pass
    return False

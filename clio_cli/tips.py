...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or clio -c.",
    ...
    "/personality pirate sets a fun personality — 14 built-in options from clio-spinner to shakespeare.",
    ...
    "Ctrl+Z suspends Clio to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "clio -c resumes your most recent CLI session. clio -c \"project name\" resumes by title.",
    "clio -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "clio -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    ...
    "clio -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "clio doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "clio dump outputs a compact setup summary — great for bug reports.",
    "clio config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "clio config edit opens config.yaml in your default editor.",
    "clio config check scans for missing or stale configuration options.",
    "clio sessions browse opens an interactive session picker with search.",
    "clio sessions stats shows session counts by platform and database size.",
    "clio sessions prune --older-than 30 cleans up old sessions.",
    "clio skills search react --source skills-sh searches the skills.sh public directory.",
    "clio skills check scans installed hub skills for upstream updates.",
    "clio skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "clio skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "clio mcp add github --command npx adds MCP servers from the command line.",
    "clio mcp serve runs Clio itself as an MCP server for other agents.",
    "clio auth add lets you add multiple API keys for credential pool rotation.",
    "clio completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "clio logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "clio backup creates a zip backup of your entire Clio home directory.",
    "clio profile create coder creates an isolated profile that becomes its own command.",
    "clio profile create work --clone copies your current config and keys to a new profile.",
    "clio update syncs new bundled skills to ALL profiles automatically.",
    "clio gateway install sets up Clio as a system service (systemd/launchd).",
    "clio memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "clio webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: clio tools disables unused tools, clio skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "clio models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as clio -w).",
    ...
    "CLIO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Clio tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'clio profile create coder' creates the 'coder' command.",
    "clio profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Clio prints a resume command with session ID and stats.",
    "clio sessions export backup.jsonl exports all sessions for backup or analysis.",
    "clio -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "clio skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .clio/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: clio cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Clio runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "clio gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Clio via a local macOS server.",
    ...
    "Context files (.clio.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Clio loads project context from .clio.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "clio mcp opens an interactive picker of managed-provider-approved MCPs you can install in one keystroke.",
    "clio mcp catalog lists managed-provider-approved MCP servers shipped with the repo.",
    "clio mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — clio tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.clio/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "clio chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "clio plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "clio acp runs Clio as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "CLIO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "clio auth add managed or clio auth add openai-codex sets up OAuth-based providers.",
    ...
    "clio status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.clio/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Clio your own.",
    ...
    "Before a gateway session reset, Clio auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.clio/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at CLIO_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "CLIO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in CLIO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.clio/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'clio profile create ls' would be rejected.",
    "clio profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "clio claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "CLIO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via clio tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.clio/.env.",
    "Skills from trusted repos (Clioloop) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.clio/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Clio auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Clio config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Clio is working.',
    ...
    '/reload reloads ~/.clio/.env into the running session — pick up new API keys without restarting.',
    ...
    "CLIO_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.clio/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.clio/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'clio curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "clio curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'clio curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'clio auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Omni Loop Portal Subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'CLIO_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "CLIO_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'clio dashboard launches a local web UI at 127.0.0.1:10119 — zero data leaves localhost.',
    'clio dashboard embeds the full Clio TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.clio/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.clio/dashboard-plugins/ — no npm build required.',
    ...
    'CLIO_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'CLIO_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'CLIO_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'CLIO_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.clio/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.clio/skills/ so drop-in skills appear without restarting the session.',
    ...
    'clio -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'clio chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'clio chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'clio chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "clio chat --source tool tags programmatic chats so they don't clutter clio sessions list.",
    'clio dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'clio sessions rename <ID> "new title" renames any past session; clio sessions delete <ID> removes one.',
    'clio import restores a session export or profile archive produced by sessions export or profile export.',
    'clio fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'clio pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'clio setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'clio status --deep runs the full health sweep across every component; plain clio status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'CLIO_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'CLIO_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.clio/plugins/ — trust-gated by design.',
    "CLIO_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'CLIO_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'CLIO_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'CLIO_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'CLIO_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'CLIO_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'CLIO_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'CLIO_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'CLIO_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'CLIO_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'CLIO_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    'music_gen.provider in config.yaml routes music generation through the Apiframe (Suno V5) plugin. Set via `clio tools` → Music Generation.',
    'music_gen.model selects the Suno model version (suno-v5, suno-v5_5). Max plan includes managed music generation via Omni Loop Portal Subscription.',
    'AUXILIARY_VISION_BASE_URL + AUXILIARY_VISION_API_KEY point vision analysis at any OpenAI-compatible endpoint.',

    # --- Security ---
    'security.tirith_fail_open: false makes Clio block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from clio sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.clio/dashboard-plugins/.',
]

"""``clio logs`` — view and filter Clio log files.
...
Usage examples::
    ...
    clio logs                    # last 50 lines of agent.log
    clio logs -f                 # follow agent.log in real time
    clio logs errors             # last 50 lines of errors.log
    clio logs gateway -n 100    # last 100 lines of gateway.log
    clio logs gui -f            # follow gui.log (dashboard/pty/ws)
    clio logs desktop -f        # follow desktop.log (Electron app boot/backend)
    clio logs --level WARNING    # only WARNING+ lines
    clio logs --session abc123   # filter by session ID substring
    clio logs --component tools  # only tool-related lines
    clio logs --since 1h         # lines from the last hour
    clio logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_clio_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Clio runs — try 'clio chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_clio_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_clio_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_clio_home()}/logs/")
        return

    print(f"Log files in {display_clio_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'clio chat' to generate logs)")

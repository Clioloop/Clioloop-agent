"""Helpers for loading Clio .env files consistently across entrypoints."""
...
# load_clio_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_clio_dotenv().
# Used by setup / `clio model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# CLIO_HOME paths we've already pulled external secrets for during this
# process.  ``load_clio_dotenv()`` is called at module-import time from
# several hot modules (cli.py, clio_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_clio_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which CLIO_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `clio setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``clio_cli.config._sanitize_env_lines`` which
    already knows all valid Clio env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from clio_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_clio_dotenv(
    *,
    clio_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Clio environment files with user config taking precedence.

    Behavior:
    - `~/.clio/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(clio_home or os.getenv("CLIO_HOME", Path.home() / ".clio"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Clio reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_clio_dotenv()`` runs at import
    time from several hot modules (cli.py, clio_cli/main.py,
    run_agent.py, trajectory_compressor.py, ...), so without this guard
    the Bitwarden status line would print 3-5x per CLI startup.  Use
    ``reset_secret_source_cache()`` if you need to force a re-pull
    (tests, future ``clio secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `clio model`

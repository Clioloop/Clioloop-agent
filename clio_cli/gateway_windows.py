...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m clio_cli.main gateway run --replace`` invocation) and
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Clio profile."""
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "clio_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("CLIO_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("CLIO_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("CLIO_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["CLIO_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["CLIO_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["CLIO_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("CLIO_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("CLIO_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("CLIO_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Clio_Gateway``
    Named profile X: ``Clio_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during clio_cli boot.
    from clio_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\clio\\gateway-service\\<task_name>.cmd``
    (or ``<CLIO_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Clio installs stay self-contained).
    """
    _assert_windows()
    from clio_cli.config import get_clio_home

    script_dir = Path(get_clio_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``CLIO_HOME`` whenever it
    ...
    source checkout only if ``CLIO_HOME`` cannot be resolved yet.
    """
    from clio_cli.config import get_clio_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports CLIO_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m clio_cli.main [--profile X] gateway run``
...
def _write_task_script() -> Path:
    ...
    content = _build_gateway_cmd_script(python_path, working_dir, clio_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Clio builds and failed
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m clio_cli.main gateway run``
    ...
    log_dir = Path(get_clio_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("CLIO_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("CLIO_GATEWAY_INSTALL_START_ON_LOGIN")
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `clio -p <profile> gateway install` must be safe.
    ...
    from clio_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"clio {profile_arg} gateway start" if profile_arg else "clio gateway start"
...
def install(
    ...
    *,
    ...
    ``assume_yes`` (set by ``--non-interactive``, e.g. the desktop app) skips the
    terminal "Open the UAC prompt now?" question and opens the native UAC
    elevation dialog directly when admin rights are needed.
    """
    _assert_windows()
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: clio gateway start")
        return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: clio gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from clio_cli.config import get_clio_home
        print(f"    type {Path(get_clio_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_clio_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from clio_cli.config import get_clio_home

    clio_home = Path(get_clio_home()).resolve()
    print()
    print("Next steps:")
    print("  clio gateway status                      # Check status")
    print(f"  type {clio_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Clio gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: clio gateway status")
                    return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from clio_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout

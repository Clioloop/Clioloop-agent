"""Shell completion script generation for clio CLI.
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_clio_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
...
_clio_profiles() {{
    local profiles_dir="$HOME/.clio/profiles"
...
_clio_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_clio_profiles)" -- "$cur"))
        return
    fi
...
_clio_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.clio/profiles" ]]; then
        profiles+=($HOME/.clio/profiles/*(N/:t))
    fi
...
_clio() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_clio_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'clio command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "function __clio_profiles",
        "    echo default",
        "    if test -d $HOME/.clio/profiles",
        "        for d in $HOME/.clio/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c clio -f -s p -l profile"
        " -d 'Profile name' -xa '(__clio_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__clio_profiles)' -d 'Profile name'"
                )

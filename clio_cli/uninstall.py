...
- Keep data: Remove code but keep ~/.clio/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Clio PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing clio-agent or clio PATH entries
            new_lines = []
            skip_next = False
            ...
            for line in content.split('\n'):
                ...
                # Remove any PATH line containing clio
                if 'clio' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the clio wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "clio",
        Path("/usr/local/bin/clio"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains clio_cli reference)
                content = wrapper.read_text()
                if 'clio_cli' in content or 'clio-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(clio_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``clio`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Clio home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (clio_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``clio gateway run`` processes
    ...
    try:
        from clio_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from clio_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CLIO_HOME`` and ``CLIO_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\clio\git\cmd``, ``%LOCALAPPDATA%\clio\git\bin``,
#      ``%LOCALAPPDATA%\clio\git\usr\bin``, ``%LOCALAPPDATA%\clio\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\clio\git\`` and Node to
#      ``%LOCALAPPDATA%\clio\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``clio dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\clio\gateway-service\`` and sometimes
...
def _clio_path_markers(clio_home: Path) -> list[str]:
    """Path-entry substrings that identify Clio-owned User-PATH entries."""
    root = str(clio_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare clio-agent install dir.
    markers = [root + "\\clio-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if CLIO_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "clio".  We don't want to catch
    # unrelated entries like "cclio-foo" or "ephermeral", so we look for
    # backslash-clio as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(clio_home: Path) -> list[str]:
    """Strip Clio-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _clio_path_markers(clio_home)
            kept: list[str] = []
...
def remove_clio_env_vars_windows() -> list[str]:
    """Delete CLIO_HOME and CLIO_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CLIO_HOME", "CLIO_GIT_BASH_PATH"):
                try:
...
def _is_default_clio_home(clio_home: Path) -> bool:
    """Return True when ``clio_home`` points at the default (non-profile) root."""
    try:
        from clio_constants import get_default_clio_root
        return clio_home.resolve() == get_default_clio_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CLIO_HOME directory.

    We shell out to ``clio -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CLIO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m clio_cli.main` so we don't depend on a `clio`
    #    wrapper that may be half-removed mid-uninstall.
    clio_invocation = [_sys.executable, "-m", "clio_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                clio_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's CLIO_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.clio/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.clio/ for future reinstall
    """
    project_root = get_project_root()
    clio_home = get_clio_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie CLIO_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_clio_home(clio_home)
    ...
    print(f"  Config:  {clio_home / 'config.yaml'}")
    print(f"  Secrets: {clio_home / '.env'}")
    print(f"  Data:    {clio_home / 'cron/'}, {clio_home / 'sessions/'}, {clio_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CLIO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Clio data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Clio code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in clio_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\clio\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(clio_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Clio-owned PATH entries in User environment")

        log_info("Removing CLIO_HOME / CLIO_GIT_BASH_PATH User env vars...")
        removed_env = remove_clio_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Clio-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing clio command...")
    ...
    #     (only when they still point into this Clio home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Clio-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(clio_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Clio-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.clio/, just remove the clio-agent subdir
            if clio_home in project_root.parents or project_root.parent == clio_home:
    ...
    #     under CLIO_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(clio_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(clio_home)
    ...
    # 5. Optionally remove ~/.clio/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CLIO_HOME dirs live under
        ...
        try:
            if clio_home.exists():
                shutil.rmtree(clio_home)
                log_success(f"Removed {clio_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {clio_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {clio_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/Clioloop/Clioloop-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/Clioloop/Clioloop-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()

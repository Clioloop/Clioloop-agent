...
Design notes / invariants (same contract as ``clio_cli/goals.py``):
...
- This module has zero hard dependency on the interactive CLI, the gateway
...
def _get_session_db() -> Optional[Any]:
    """One SessionDB per CLIO_HOME (same pattern as goals._get_session_db)."""
    try:
    ...
    def complete_tick(self, last_response: str) -> Dict[str, Any]:
        ...
        if s.until and (last_response or "").strip():
            try:
                ...
                judged = judge_goal(s.until, last_response)
                verdict, reason = judged[0], judged[1]
            except Exception as exc:

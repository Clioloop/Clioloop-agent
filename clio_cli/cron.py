...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(clio\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*clio)"
    r"|(systemctl\s+(restart|stop|start)\s+.*clio)"
    r"|(p?kill\s+.*clio.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'clio cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: clio gateway install", Colors.DIM))
        print(color("                    sudo clio gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    clio gateway install    # Install as a user service")
        print("    sudo clio gateway install --system  # Linux servers: boot-time system service")
        print("    clio gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `clio gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1

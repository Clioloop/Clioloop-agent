...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "managed", "openai-codex", "xai-oauth", "qwen-oauth", "google-gemini-cli", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_clio_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:clio_pkce",
    ...
    if provider == "managed":
        # Codex-style auto-import: if a shared managed-provider credential lives at
        # <clio-root>/shared/managed_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `clio --profile <name>
        # auth add managed --type oauth` a one-tap operation for users who
        # run multiple profiles.
        shared = auth_mod._read_shared_managed_state()
        if shared:
            try:
                path = auth_mod._managed_shared_store_path()
            except RuntimeError:
                path = None
            print()
            if path:
                print(f"Found existing managed-provider OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                print("Rehydrating Omni Loop Portal session from shared credentials...")
                rehydrated = auth_mod._try_import_shared_managed_state(
                    timeout_seconds=getattr(args, "timeout", None) or 15.0,
                )
                if rehydrated is not None:
                    custom_label = (getattr(args, "label", None) or "").strip() or None
                    entry = auth_mod.persist_managed_credentials(rehydrated, label=custom_label)
        ...
        creds = auth_mod._managed_device_code_login(
        ...
        # Honor `--label <name>` so managed matches other providers' UX.  The
        # helper embeds this into providers.managed so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("managed").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_managed_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
    ...
    raise SystemExit(f"`clio auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Clio reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `clio auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `clio auth` is called bare."""

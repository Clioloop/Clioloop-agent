...
- **Managed Portal** accepts both bare open-model ids and ``vendor/model``
  OpenRouter ids; callers must preserve the selected id so the Portal can route
  to the correct upstream key.
...
- **DeepSeek** accepts ``deepseek-chat`` (V3), ``deepseek-reasoner``
  ...
  Clio revisions folded every non-reasoner input into
...
# slug used by aggregator APIs (OpenRouter, managed provider, etc.).
#
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Clio' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Clio provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``clio_cli.models.normalize_provider()``.
    ...
    # The managed Portal is itself the router: bare ids go to the open-model
    # upstream and vendor/model ids go to OpenRouter. Do not add or strip a
    # vendor prefix here or the Portal will choose the wrong upstream.
    if provider == "managed":
        return name
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from clio_cli.models import normalize_copilot_model_id

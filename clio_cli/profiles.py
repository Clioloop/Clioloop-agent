"""
Profile management for multiple isolated Clio instances.

Each profile is a fully independent CLIO_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.clio/profiles/<name>/`` by default.

The "default" profile is ``~/.clio`` itself — backward compatible,
zero migration needed.

Usage::

    clio profile create coder          # fresh profile + bundled skills
    clio profile create coder --clone  # also copy config, .env, SOUL.md, skills
    clio profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    clio -p coder chat                 # or via flag
    clio profile use coder             # set as sticky default
    clio profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See clio_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `clio profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `clio
...
# `clio skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Clio infrastructure directories that only the default profile
         (``~/.clio``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_clio_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.clio) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "clio-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "clio", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the clio root, NOT to the current CLIO_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where CLIO_HOME points outside
    ``~/.clio``, profiles live under ``CLIO_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_clio_home() / "profiles"


def _get_default_clio_home() -> Path:
    """Return the default (pre-profile) CLIO_HOME path.

    In standard deployments this is ``~/.clio``.
    In Docker/custom deployments where CLIO_HOME is outside ``~/.clio``
    (e.g. ``/opt/data``), returns CLIO_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_clio_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Clio installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its CLIO_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_clio_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, clio subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _CLIO_SUBCOMMANDS:
        return f"'{canon}' conflicts with a clio subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``clio profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Clio config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_clio_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``clio update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.clio)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.clio/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `clio update`'s
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `clio profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `clio -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches CLIO_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``clio profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "CLIO_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.clio).\n"
            "To remove everything, use: clio uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from clio_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set CLIO_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("CLIO_HOME")
    try:
        os.environ["CLIO_HOME"] = str(profile_dir)
        from clio_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["CLIO_HOME"] = old_home
        elif "CLIO_HOME" in os.environ:
            del os.environ["CLIO_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.clio/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: clio profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from CLIO_HOME.

    Returns ``"default"`` if CLIO_HOME is not set or points to ``~/.clio``.
    Returns the profile name if CLIO_HOME points into ``~/.clio/profiles/<name>``.
    Returns ``"custom"`` if CLIO_HOME is set to an unrecognized path.
    """
    from clio_constants import get_clio_home
    clio_home = get_clio_home()
    resolved = clio_home.resolve()

    default_resolved = _get_default_clio_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.clio itself — its parent is ~/ and its
        # directory name is ".clio", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: clio profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.clio itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.clio). "
            "Specify a different name: clio profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="clio_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"clio_{old_name}"
    legacy_old_host = f"clio.{old_name}"
    new_host = f"clio_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_clio_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("clio_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a CLIO_HOME path string.

    Called early in the CLI entry point, before any clio modules
    are imported, to set the CLIO_HOME environment variable.
    """

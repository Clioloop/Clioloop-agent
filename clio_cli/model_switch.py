...
- ``clio_cli.providers``        -- canonical provider identity + overlays
- ``clio_cli.model_normalize``  -- per-provider name formatting
...
# Match only the real Omni Loop Labs Clio 3 / Clio 4 chat families.
# The previous substring check (`"clio" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``clio-brain:qwen3-14b-ctx16k`` that just
# happen to carry "clio" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   Clioloop/Clio-3-Llama-3.1-70B, clio-4-405b, openrouter/clio3:70b
# Negative examples it must NOT match:
#   clio-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_MANAGED_CLIO_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])clio[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_managed_clio_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real managed provider Clio 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_MANAGED_CLIO_NON_AGENTIC_RE.search(model_name))


def _check_clio_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a managed provider Clio 3/4 chat model."""
    if is_managed_clio_non_agentic(model_name):
        return _CLIO_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``clio config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from clio_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "managed")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "managed")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, managed provider, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'clio model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'clio doctor' — config issues detected:"
        ...
        from clio_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from clio_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        if (is_aggregator(target_provider) or target_provider == "managed") and not resolved_alias:
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from clio_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in clio_cli.runtime_provider.resolve_runtime_provider;
    ...
    clio_warn = _check_clio_model_warning(new_model)
    if clio_warn:
        warnings.append(clio_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from clio_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from clio_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by clio provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "managed" pulls from the remote model-catalog manifest published at
    # https://github.com/Clioloop/Clioloop-agent/tree/main/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Clio release. Falls back to the in-repo
    # _PROVIDER_MODELS["managed"] snapshot when the manifest is unreachable.
    curated["managed"] = get_curated_managed_model_ids()
    ...
    # --- 1. Check Clio-mapped providers ---
    from clio_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from clio_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for clio_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(clio_id)
        if (
            _alias_target
            and _alias_target != clio_id
        ...
        pconfig = PROVIDER_REGISTRY.get(clio_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (CLIO_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from clio_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(clio_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `clio model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(clio_id)
        if not model_ids:
            model_ids = curated.get(clio_id, [])
    ...
    # --- 2. Check Clio-only providers (managed, openai-codex, copilot, opencode-go) ---
    from clio_cli.providers import CLIO_OVERLAYS
    from clio_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Clio provider ID.
    # CLIO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Clio IDs ("copilot").
    _mdev_to_clio = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in CLIO_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Clio slug — e.g. "github-copilot" → "copilot"
        clio_slug = _mdev_to_clio.get(pid, pid)
        if clio_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(clio_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, clio_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or clio_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(clio_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", clio_slug, exc)
        ...
        # configured.
        if not has_creds and clio_slug == "anthropic":
            try:
                ...
                if (clio_creds and clio_creds.get("accessToken")) or \
        ...
        if clio_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(clio_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(clio_slug)
                model_ids = _ids if _ids else (curated.get(clio_slug, []) or curated.get(pid, []))
            except Exception:
            ...
            try:
                from clio_cli.models import (
                    ...
                    check_managed_free_tier as _managed_free,
                    union_with_portal_free_recommendations as _union_free,
                    union_with_portal_paid_recommendations as _union_paid,
                )
                from clio_cli.auth import get_provider_auth_state as _managed_state

                _pricing = _managed_pricing("managed") or {}
                _portal = ""
                try:
                    _st = _managed_state("managed") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _managed_free(force_fresh=True):
        ...
        results.append({
            "slug": clio_slug,
            "name": get_label(clio_slug),
            "is_current": clio_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "clio",
        ...
        seen_slugs.add(clio_slug.lower())
        _record_builtin_endpoint(clio_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or CLIO_OVERLAYS (keeps /model in sync
    # with `clio model`).
    try:
        from clio_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Clio's canonical write key (matches
            ...
            # Clio writes ``models:`` as a dict keyed by model id
            # (see clio_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Clio's own writer uses; a hyphen variant is accepted
                ...
                # honor that for the whole grouped row.
                if not discover:
                    groups[group_key]["discover_models"] = False

            # The singular ``model:`` field only holds the currently
            # active model. Clio's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, clio_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    """
    from clio_cli.models import fetch_openrouter_models
    ...
    filtered.sort(key=lambda row: 0 if str(row.get("slug", "")).lower() == "managed" else 1)
    return filtered

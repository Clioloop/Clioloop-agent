"""clio hooks — inspect and manage shell-script hooks.

Usage::

    clio hooks list
    clio hooks test <event> [--for-tool X] [--payload-file F]
    clio hooks revoke <command>
    clio hooks doctor

Consent records live under ``~/.clio/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.clio/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``clio hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: clio hooks {list|test|revoke|doctor}")
        print("Run 'clio hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.clio/config.yaml.")
        print("See `clio hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `clio hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `clio hooks test` and `clio hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Clio wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `clio hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `clio hooks doctor` would execute

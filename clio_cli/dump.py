"""
Dump command for clio CLI.

Outputs a compact, plain-text summary of the user's Clio setup
...
from clio_cli.config import get_clio_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.clio_build_sha`` by the Dockerfile's
    ``CLIO_GIT_SHA`` build-arg (see ``clio_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``clio dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from clio_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(clio_home: Path) -> int:
...
def _cron_summary(clio_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = clio_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"clio_home:      {display_clio_home()}")
    ...
    toolsets = config.get("toolsets", ["clio-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(clio_home)}")
    lines.append(f"  skills:             {_count_skills(clio_home)}")

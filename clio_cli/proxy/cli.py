"""CLI handlers for the ``clio proxy`` subcommand."""
...
from clio_cli.proxy.adapters import ADAPTERS, get_adapter
from clio_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "clio proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"clio auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Clio proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Clio proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: clio proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``clio proxy <subcommand>``."""
    ...
    print(
        "clio proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  clio proxy start [--provider xai|openrouter] [--host 127.0.0.1] [--port 9645]\n"

...
the existing module-level functions in clio_cli.gateway and
clio_cli.gateway_windows directly. This protocol is a thin facade
...
``clio gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``clio gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged clio
    ...
    detection always fails. Since every Clio runtime call inside the
    container drops to clio via ``s6-setuidgid``, that silent failure
...
# in ``clio_cli.gateway`` (systemd/launchd) and ``clio_cli.gateway_windows``
...
# active (set via the ``clio -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in clio_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from clio_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from clio_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from clio_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from clio_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in clio_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from clio_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from clio_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from clio_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from clio_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``clio_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from clio_cli import gateway_windows
        from clio_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `clio profile create`
# runs inside the container (Phase 4). Static services (main-clio, dashboard)
...
# UID/GID of the in-image ``clio`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the clio user.
    ...
    (the ``clio`` user — UID 10000 — running every Clio runtime
    ...
    is dispatched as the clio user.
    ...
    with clio ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           clio:clio, 0755 (parent must already exist)
    ``svc_dir/event/``                     clio:clio, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 clio:clio, 0755
    ``svc_dir/supervise/event/``           clio:clio, 03730
    ``svc_dir/supervise/control``          clio:clio, 0660    (FIFO)
    ...
    slot's supervise/ was clio-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _CLIO_UID, _CLIO_GID)
        except PermissionError:
            # Running as the clio user already — directory is clio-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _CLIO_UID, _CLIO_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is clio-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _CLIO_UID, _CLIO_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``clio -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`clio profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-clio, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources CLIO_HOME (and any extra env) via with-contenv —
             so e.g. ``-e CLIO_HOME=/data/clio`` is honored at run
          ...
          4. Drops to the clio user and exec's
             ``clio -p <profile> gateway run`` (or just ``clio
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``clio gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root CLIO_HOME profile" (the implicit profile that exists at
        the top of $CLIO_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``clio_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$CLIO_HOME/profiles/default/`` — a
        ...
        ``clio_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        # guard.
        lines.append("export CLIO_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            gateway_cmd = "clio gateway run"
        else:
            gateway_cmd = f"clio -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid clio {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${CLIO_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the CLIO_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e CLIO_HOME=/data/clio``
        gets its logs under /data/clio/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``clio logs`` reads and what
        ...
        return (
            ...
            f': "${{CLIO_HOME:=/opt/data}}"\n'
            f'log_dir="$CLIO_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R clio:clio "$log_dir" 2>/dev/null || true\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid clio s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with clio ownership
            ...
            # the clio user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is clio-owned (see ``_seed_supervise_skeleton``), and on

...
Usage:
    clio pairing list              # Show all pending + approved users
    clio pairing approve <platform> <code>  # Approve a pairing code
    clio pairing revoke <platform> <user_id> # Revoke user access
    clio pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle clio pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: clio pairing {list|approve|revoke|clear-pending}")
        print("Run 'clio pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.clio/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'clio pairing list' to see pending codes.\n")

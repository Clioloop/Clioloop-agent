...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for managed provider, ``free_tier``/``unavailable_models`` so the GUI picker can
    ...
    _promote_managed_provider(rows)
...
def _promote_managed_provider(rows: list[dict]) -> None:
    """Keep Omni first in provider pickers while preserving all other order."""
    rows.sort(key=lambda row: 0 if str(row.get("slug", "")).lower() == "managed" else 1)
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from clio_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `clio model` to configure ({auth_type})"
        )
...
def _apply_pricing(rows: list[dict]) -> None:
    """Enrich each provider row with per-model pricing + managed provider tier gating.

    Mutates ``rows`` in-place. For every row whose provider supports live
    pricing (openrouter / managed / novita) adds::

        row["pricing"] = {model_id: {"input": "$3.00", "output": "$15.00",
                                     "cache": "$0.30" | None, "free": bool}}

    For managed provider additionally adds::
    ...
    # Resolve managed provider free-tier once (cached in models.py for the TTL window).
    managed_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        if slug == "managed":
            try:
                if managed_free_tier is None:
                    managed_free_tier = check_managed_free_tier(force_fresh=True)
                row["free_tier"] = bool(managed_free_tier)
                if managed_free_tier:
                    _selectable, unavailable = partition_managed_models_by_tier(

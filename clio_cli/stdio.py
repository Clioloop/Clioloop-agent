...
on Japanese locales, etc.).  Clio's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``clio_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Clio is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CLIO_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CLIO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Clio-managed Git install directories so
    ...
    # already-running shells don't see the change — which means clio
    ...
    # install without Clio-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Clio's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Clio's
    ``clio config edit`` both honor ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Clio (User env var in Windows Settings, or
    export in a PowerShell profile) and Clio picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Clio-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\clio\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``clio`` from right after install) retain their old PATH.

    Any subprocess Clio spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Clio-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "clio", "git", "cmd"),
        os.path.join(local_appdata, "clio", "git", "bin"),
        os.path.join(local_appdata, "clio", "git", "usr", "bin"),
        # Clio venv Scripts directory — host of the clio.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes clio, but harmless to include.
        os.path.join(local_appdata, "clio", "clio-agent", "venv", "Scripts"),

...
``$CLIO_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-clio-setup
(the stage2 hook) has chowned the volume and seeded $CLIO_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$CLIO_HOME``,
    not under ``profiles/``). The dispatcher in ``clio_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``clio gateway start`` (no ``-p``) targets. Without it,
    bare ``clio gateway start`` inside the container would land on
    ...
    ``$CLIO_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        clio_home: The container's CLIO_HOME (typically /opt/data).
            Profiles live under ``<clio_home>/profiles/<name>/``;
            the default profile lives at ``<clio_home>`` itself.
    ...
    # ``clio gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(clio_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(clio_home)
    ...
    profiles_root = clio_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `clio profile create` (config.yaml
            # is not — that comes later via `clio setup`). Use it as the
            ...
            # via ``clio -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(clio_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = clio_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("CLIO_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "clio":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `clio -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit clio
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the clio user) won't hit EACCES. See
...
def _write_reconcile_log(
    clio_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $CLIO_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    clio_home = Path(os.environ.get("CLIO_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        clio_home=clio_home, scandir=scandir,
    )
    for a in actions:
        print(

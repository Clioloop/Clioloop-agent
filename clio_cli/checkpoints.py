"""`clio checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.clio/checkpoints/``.  Actions:

    clio checkpoints               # same as `status`
    clio checkpoints status        # total size, project count, breakdown
    clio checkpoints list          # per-project checkpoint counts + workdir
    clio checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    clio checkpoints clear [-f]    # nuke the entire base (asks first)
    clio checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    clio checkpoints
    clio checkpoints prune --retention-days 3 --max-size-mb 200
    clio checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: clio checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``clio checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `clio checkpoints` → status

...
from clio_cli.auth import AuthError, resolve_provider
from clio_cli.colors import Colors, color
from clio_cli.config import get_env_path, get_env_value, get_clio_home, load_config
from clio_cli.models import provider_label
from clio_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``clio config``'s
...
def show_status(args):
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: clio model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: clio auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: clio auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: clio model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from clio_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from clio_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_clio_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_clio_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'clio doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'clio setup' to configure", Colors.DIM))
    print()

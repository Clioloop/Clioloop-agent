...
is persisted in ~/.clio/auth.json with cross-process file locking.
...
managed-provider authentication paths:
...
from clio_cli.config import get_clio_home, get_config_path, read_raw_config
from clio_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# Omni Loop Portal Subscription — Clioloop's managed model provider. One device login gives
# access to 300+ models proxied through the portal (see portal/ in the repo).
# Override per-install with CLIO_PORTAL_BASE_URL (e.g. http://localhost:4280
# when running a local portal); the inference URL then derives automatically.
DEFAULT_MANAGED_PORTAL_URL = "https://portal.clioloop.com"
DEFAULT_MANAGED_INFERENCE_URL = f"{DEFAULT_MANAGED_PORTAL_URL}/api/v1"
DEFAULT_MANAGED_CLIENT_ID = "clio-cli"
MANAGED_INFERENCE_INVOKE_SCOPE = "inference:invoke"
DEFAULT_MANAGED_SCOPE = MANAGED_INFERENCE_INVOKE_SCOPE
MANAGED_DEVICE_CODE_SOURCE = "device_code"
MANAGED_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
MANAGED_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
SPOTIFY_DOCS_URL = "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://github.com/Clioloop/Clioloop-agent/tree/main/docs/guides/oauth-over-ssh"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "managed": ProviderConfig(
        ...
        portal_base_url=DEFAULT_MANAGED_PORTAL_URL,
        inference_base_url=DEFAULT_MANAGED_INFERENCE_URL,
        client_id=DEFAULT_MANAGED_CLIENT_ID,
        scope=DEFAULT_MANAGED_SCOPE,
    ),
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from clio_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.clio/.env file
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``clio auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `clio model` to re-authenticate."

    if error.code == "subscription_required":
        if error.provider == "managed":
            return _format_managed_entitlement_auth_error(error)
...
def _format_managed_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        account_info = get_managed_provider_account_info(force_fresh=True)
        message = format_managed_provider_entitlement_message(
            account_info,
            capability="Omni Loop Portal model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in managed provider, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("CLIO_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.clio/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_clio_home() / "auth.json"
    # Seat belt: if pytest is running and CLIO_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch CLIO_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".clio" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set CLIO_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom CLIO_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_clio_root()
    except Exception:
        return None
    profile_home = get_clio_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.clio/auth.json`` even when CLIO_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_clio_root()`` for a profile-shaped CLIO_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".clio" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared managed-provider store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    ``_managed_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared managed provider lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "managed_portal" in systems:
            providers["managed"] = systems["managed_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "managed" if providers else None}
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``managed``) that were only authenticated at
    global scope. Once the user runs ``clio auth login <provider>`` inside
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``clio auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or managed-provider device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Clio.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `clio logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from clio_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'clio doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'clio model' for available providers, or run 'clio doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'clio model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.clio/.env.",
...
# Allowlist of hosts the managed provider proxy is willing to forward inference
...
# (MANAGED_INFERENCE_BASE_URL) bypass validation — that's the documented
# dev/staging escape hatch and the env source is already trusted (the
# user set it themselves).
_ALLOWED_MANAGED_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "",
})


def _validate_managed_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``MANAGED_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        logger.warning(
            "managed: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_MANAGED_INFERENCE_HOSTS:
        logger.warning(
            "managed: refusing inference URL host %r from Portal response "
...
def _managed_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = MANAGED_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if MANAGED_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _managed_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_managed_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "managed provider access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: clio auth add managed",
        provider="managed",
        code=reason,
...
def _log_managed_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.info("managed provider inference auth: using NAS invoke JWT")
    _oauth_trace(
        "managed_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _managed_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_managed_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _managed_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_managed_invoke_jwt(
    state: Dict[str, Any],
    *,
    access_token: Any = None,
    sequence_id: Optional[str] = None,
    ...
    _set_managed_agent_key_from_invoke_jwt(state)
    _log_managed_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_MANAGED_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _managed_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _MANAGED_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("CLIO_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `clio model` walks users into a broken Qwen setup flow.
...
#
# Tokens live in ~/.clio/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `clio auth list` / `clio status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.clio/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("CLIO_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set CLIO_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("CLIO_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("CLIO_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("CLIO_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but Clio times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``CLIO_LOG_LEVEL=INFO`` (or check
            # ``~/.clio/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>clio auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `clio auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `clio auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `clio auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `clio auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.clio/.env, and return it.
    ...
    print("       App name:     anything (e.g. clio-agent)")
    ...
    # Persist so subsequent `clio auth spotify` runs skip the wizard.
    save_env_value("CLIO_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("CLIO_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved CLIO_SPOTIFY_CLIENT_ID to ~/.clio/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "CLIO_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Clio:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Clio is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.clio/auth.json (not ~/.codex/)
#
# Clio maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Clio auth store (~/.clio/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `clio auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `clio auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `clio auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `clio auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``clio setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``clio auth add openai-codex``
      ...
      ``clio auth add`` workaround for #33000 would silently leave that
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Clio auth store (~/.clio/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Clio auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run clio auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `clio auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Clio auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Clio's own Codex token store.
    ...
    try:
        ...
        if pool_token:
            base_url = (
                os.getenv("CLIO_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("CLIO_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Clio processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.clio/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `clio model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `clio model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `clio model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `clio model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.clio/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `clio model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``CLIO_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `clio model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Clio (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `clio model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``clio model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("CLIO_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("CLIO_XAI_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# managed provider — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared managed provider token store — lets OAuth credentials persist across profiles
# so a new `clio --profile <name> auth add managed --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${CLIO_SHARED_AUTH_DIR}/managed_auth.json, defaulting to
# ``<clio-root>/shared/managed_auth.json`` where ``<clio-root>`` is what
# ``get_default_clio_root()`` returns — ``~/.clio`` on Linux/macOS,
# ``%LOCALAPPDATA%\clio`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's CLIO_HOME so named profiles (which
# typically live under ``<clio-root>/profiles/<name>/``) all see the
# same file.
#
...
MANAGED_SHARED_STORE_FILENAME = "managed_auth.json"
_managed_shared_lock_holder = threading.local()


def _managed_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared managed provider token store.

    Honors ``CLIO_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<clio-root>/shared/``, where ``<clio-root>`` is what
    :func:`clio_constants.get_default_clio_root` returns — so
    Linux/macOS classic installs land at ``~/.clio/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\clio\\shared\\``, and
    Docker / custom ``CLIO_HOME`` deployments at
    ``<CLIO_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("CLIO_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from clio_constants import get_default_clio_root
    return get_default_clio_root() / "shared"


def _managed_shared_store_path() -> Path:
    path = _managed_shared_auth_dir() / MANAGED_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Clio root, refuse rather than silently corrupt cross-profile
    # state. Tests must set CLIO_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from clio_constants import get_default_clio_root
        real_home_shared = (
            get_default_clio_root() / "shared" / MANAGED_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared managed provider auth store during test run: "
                f"{path}. Set CLIO_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _managed_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared managed-provider OAuth store.
    ...
    ``_try_import_shared_managed_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _managed_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No CLIO_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _managed_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared managed provider auth lock",
    ):
        yield


def _merge_shared_managed_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local managed provider state."""
    shared = _read_shared_managed_state()
    if not shared:
        return False
...
def _write_shared_managed_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the managed-provider OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_MANAGED_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_MANAGED_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_MANAGED_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_MANAGED_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _managed_shared_store_lock():
            path = _managed_shared_store_path()
            path.parent.mkdir(parents=True, exist_ok=True)
            # secure_parent_dir refuses to chmod / or top-level dirs (#25821).
            secure_parent_dir(path)
            tmp = path.with_name(f"{path.name}.tmp.{os.getpid()}.{uuid.uuid4().hex}")
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed managed provider
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "managed_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared managed provider auth store: %s", exc)


def _read_shared_managed_state() -> Optional[Dict[str, Any]]:
    """Return the shared managed-provider OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared managed provider auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_managed_state(reason: str) -> None:
    """Remove the shared managed-provider OAuth store after a terminal token failure."""
    try:
        with _managed_shared_store_lock():
            path = _managed_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("managed_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared managed provider auth store: %s", exc)


def _is_terminal_managed_refresh_error(exc: Exception) -> bool:
    """True when retrying the same managed provider refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "managed"
...
def _quarantine_managed_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    _clear_shared_managed_state(reason)
    invalidate_managed_auth_status_cache()


def _quarantine_managed_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded managed provider pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("managed")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {MANAGED_DEVICE_CODE_SOURCE, f"manual:{MANAGED_DEVICE_CODE_SOURCE}"}
    for entry in entries:
    ...
    if removed:
        pool["managed"] = retained
        _oauth_trace(
            "managed_pool_device_code_quarantined",
            reason=reason,
...
def _try_import_shared_managed_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate managed-provider OAuth state from the shared store.
    ...
    for ``persist_managed_credentials()``.
    ...
    """
    try:
        with _managed_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_managed_state()
            if not shared:
                return None

            # Build a full state dict so refresh_managed_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_MANAGED_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_MANAGED_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_MANAGED_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_MANAGED_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_managed_state(updated_state)

            refreshed = refresh_managed_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_managed_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "managed_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_managed_refresh_error(exc):
            _clear_shared_managed_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared managed provider import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "managed_shared_import_failed",
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="managed", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="managed", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the managed provider
    ...
    # called POST /api/oauth/token with Clio's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Clio's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "managed provider detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Clio install sharing "
            "~/.clio/auth.json) called POST /api/oauth/token with Clio's "
            "refresh token without persisting the rotated token back.\n"
            "managed provider refresh tokens are single-use — only Clio may call the "
            "refresh endpoint. For health checks, use `clio auth status` "
            "instead.\n"
            "Re-authenticate with: clio auth add managed"
        )
        relogin = True

    raise AuthError(description, provider="managed", code=code, relogin_required=relogin)


def fetch_managed_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the managed provider inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="managed", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Clio models — they're not reliable for agentic tool-calling
            if "clio" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _managed_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
def resolve_managed_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """Resolve a refresh-aware managed provider access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "managed")

        if not state:
            raise AuthError(
                "Clio is not logged into managed provider.",
                provider="managed",
                relogin_required=True,
            )
        ...
        client_id = str(state.get("client_id") or DEFAULT_MANAGED_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)

        with _managed_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_managed_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for managed provider login.",
                    provider="managed",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state(auth_store, "managed", state)
                    _save_auth_store(auth_store)
                return access_token
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_managed_refresh_error(exc):
                        _quarantine_managed_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_managed_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
            ...
            _save_provider_state(auth_store, "managed", state)
            _save_auth_store(auth_store)
            _write_shared_managed_state(state)
            return state["access_token"]


def refresh_managed_oauth_pure(
    access_token: str,
    refresh_token: str,
    client_id: str,
    portal_base_url: str,
    ...
    *,
    ...
    scope: str = DEFAULT_MANAGED_SCOPE,
    obtained_at: Optional[str] = None,
    ...
    """Refresh managed-provider OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_MANAGED_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_MANAGED_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_MANAGED_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_MANAGED_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _managed_invoke_jwt_status(
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for managed provider.",
                    provider="managed",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
            state["scope"] = refreshed.get("scope") or state.get("scope")
            refreshed_url = _validate_managed_inference_url_from_network(refreshed.get("inference_base_url"))
            if refreshed_url:
        ...
        _assert_managed_inference_jwt_usable(state)
        _select_managed_invoke_jwt(state)

    return state


def refresh_managed_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh managed-provider OAuth from a state dict. Thin wrapper around refresh_managed_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_managed_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "clio-cli"),
        state.get("portal_base_url", DEFAULT_MANAGED_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_MANAGED_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_MANAGED_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_managed_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist managed-provider OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    managed-provider credentials are read at runtime from two independent locations:

    - ``providers.managed``: singleton state read by
      ``resolve_managed_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.managed``: used by the runtime ``pool.select()`` path.

    Historically ``clio auth add managed`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.managed``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.managed`` then calls ``load_pool("managed")`` so
    ...
    ``clio auth add managed --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("managed")`` instead of the
    ...
    # these credentials via `clio auth add managed --type oauth`. Best-
    # effort: any I/O failure is logged and swallowed (the per-profile
    # auth.json is still the source of truth).
    _write_shared_managed_state(state)

    pool = load_pool("managed")
    return next(
        (e for e in pool.entries() if e.source == MANAGED_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_managed_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.managed changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("managed")
    except Exception as exc:
        logger.debug("Failed to sync managed-provider credential pool from auth store: %s", exc)


def resolve_managed_runtime_credentials(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """
    Resolve managed-provider inference credentials for runtime use.
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Clio is not logged into managed provider.",
                            provider="managed", relogin_required=True)
        ...
        inference_base_url = (
            ...
            or os.getenv("MANAGED_INFERENCE_BASE_URL")
            or DEFAULT_MANAGED_INFERENCE_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_MANAGED_CLIENT_ID)
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed managed provider auth-status cache warm during read paths.
            if (
                _managed_effective_provider_state(state)
                == _managed_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "managed_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
            ...
            # _write_shared_managed_state.
            _write_shared_managed_state(state)

        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "managed_runtime_credentials_start",
            sequence_id=sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        )
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for managed provider login.",
                                provider="managed", relogin_required=True)

            invoke_jwt_status = _managed_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            if force_refresh or invoke_jwt_status is not None:
                with _managed_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    if _merge_shared_managed_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        invoke_jwt_status = _managed_invoke_jwt_status(
                            access_token,
            ...
            _assert_managed_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_managed_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_managed_runtime_credentials_final")

    if state_persisted:
        _sync_managed_pool_from_auth_store()

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a managed provider inference API key",
                        provider="managed", code="server_error")
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": MANAGED_AUTH_PATH_INVOKE_JWT,
        "auth_path": MANAGED_AUTH_PATH_INVOKE_JWT,
    }
...
def _empty_managed_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_managed_pool_status() -> Dict[str, Any]:
    """Best-effort status from the credential pool.

    This is a fallback only. The auth-store provider state is the runtime source
    of truth because it is what ``resolve_managed_runtime_credentials()`` refreshes.
    """
    try:
        from agent.credential_pool import load_pool
        ...
        if not pool or not pool.has_credentials():
            return _empty_managed_auth_status()
...
# ── Process-level memo for get_managed_auth_status() ──
# get_managed_auth_status() validates state by calling resolve_managed_runtime_credentials(),
# which does a synchronous OAuth refresh POST to . That can take
# ~350ms even on the failure path, and read-only UI surfaces (`clio tools`, status panels,
# subscription-feature checks) call it many times per render — `clio tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `clio auth login/logout/add/remove` invalidate naturally on the next call.
_MANAGED_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_managed_auth_status_cache: Optional[Tuple[float, Optional[float], Dict[str, Any]]] = None
...
def invalidate_managed_auth_status_cache() -> None:
    """Clear the get_managed_auth_status() process-level memo.

    Call this from any code path that mutates managed-provider auth state without going
    through resolve_managed_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _managed_auth_status_cache
    _managed_auth_status_cache = None


def get_managed_auth_status() -> Dict[str, Any]:
    """Status snapshot for managed provider auth.
    ...
    ``invalidate_managed_auth_status_cache()`` explicitly.
    """
    global _managed_auth_status_cache
    now = time.monotonic()
    mtime = _auth_file_mtime()
    cached = _managed_auth_status_cache
    if cached is not None:
        cached_at, cached_mtime, cached_status = cached
        if (
            cached_mtime == mtime
            and (now - cached_at) < _MANAGED_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_managed_auth_status()
    _managed_auth_status_cache = (now, mtime, dict(status))
    return status


def _compute_managed_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_managed_auth_status(). See that function."""
    state = get_provider_auth_state("managed")
    if state:
        base_status = {
        ...
        try:
            creds = resolve_managed_runtime_credentials()
            refreshed_state = get_provider_auth_state("managed") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_managed_pool_status()


def get_codex_auth_status() -> Dict[str, Any]:
    """Status snapshot for Codex auth.
    
    Checks the credential pool first (where `clio auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `clio auth` and
    # `clio model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("CLIO_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("CLIO_COPILOT_ACP_ARGS", "").strip()
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target == "managed":
        return get_managed_auth_status()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``clio doctor`` runs the live
    ...
    try:
        from clio_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Clio' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `clio doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set CLIO_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `clio logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"managed", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_MANAGED_PORTAL_URL).rstrip("/")
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from clio_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'clio model' or 'clio setup' instead."""
    print("The 'clio login' command has been removed.")
    print("Use 'clio auth' to manage credentials,")
    print("'clio model' to select a provider, or 'clio setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.clio/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Clio-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Clio auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Clio will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("CLIO_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Clio will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Clio gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Clio creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Clio auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    from clio_constants import display_clio_home as _dhh
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Clio auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Clio creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=clio-agent` lets xAI attribute Clio-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Clio — please report at "
            "https://github.com/Clioloop/Clioloop-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Clio with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Clio auth store (~/.clio/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Clio login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the managed provider / xAI-OAuth / Codex-OAuth quarantine pattern
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `clio model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``clio status`` that just want to know
...
def _managed_device_code_login(
    *,
    ...
    """Run the managed provider device-code flow and return full OAuth state without persisting."""
    ...
    # When the portal URL is overridden (CLIO_PORTAL_BASE_URL, --portal-url)
    # derive the inference URL from it so a custom/local portal "just works"
    # without a second env var.
    _default_inference = (
        pconfig.inference_base_url
        if portal_base_url == pconfig.portal_base_url.rstrip("/")
        else f"{portal_base_url}/api/v1"
    )
    requested_inference_url = (
        inference_base_url
        or os.getenv("MANAGED_INFERENCE_BASE_URL")
        or _default_inference
    ).rstrip("/")
    ...
    print(f"Starting Clio login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        return refresh_managed_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `clio model` again to finish setup.")
            raise SystemExit(1)
        raise


def _login_managed(args, pconfig: ProviderConfig) -> None:
    """managed provider device authorization flow."""
    ...
    try:
        ...
        # Reuse path: `clio model` passes reuse_existing=True when the user
        # picked "Use existing login" — refresh and reuse the stored OAuth
        # state instead of launching a new device-code flow.
        if getattr(args, "reuse_existing", False):
            existing = None
            try:
                resolve_managed_access_token(timeout_seconds=timeout_seconds)
                existing = get_provider_auth_state("managed")
            except Exception as exc:
                logger.debug("managed reuse_existing failed, falling back to login: %s", exc)
            if existing and existing.get("access_token") and existing.get("inference_base_url"):
                auth_state = dict(existing)
            else:
                print("Could not reuse the existing Omni Loop Portal Subscription login — starting a fresh one.")

        # Codex-style auto-import: before launching a fresh device-code
        # flow, check the shared store for an existing managed-provider credential
        # from any other profile. If present, offer to rehydrate it.
        shared = None if auth_state is not None else _read_shared_managed_state()
        if shared:
            try:
                shared_path = _managed_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing managed-provider OAuth credentials at {shared_path}")
            else:
                print("Found existing shared managed-provider OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating managed provider session from shared credentials...")
                auth_state = _try_import_shared_managed_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        if auth_state is None:
            auth_state = _managed_device_code_login(
        ...
        # overwrites it to "managed".  If the user picks "Skip (keep current)"
        ...
        with _auth_store_lock():
            auth_store = _load_auth_store()
            _save_provider_state(auth_store, "managed", auth_state)
            saved_to = _save_auth_store(auth_store)
        ...
        _write_shared_managed_state(auth_state)
        _sync_managed_pool_from_auth_store()
        ...
        # leave the config in a half-updated state (provider=managed but model
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    try:
                        ...
                        _account_info = get_managed_provider_account_info(force_fresh=True)
                        unavailable_message = (
                            format_managed_provider_entitlement_message(
                                _account_info,
                                capability="paid managed provider models",
                            )
                            or ""
                        )
                    except Exception:
                    ...
                    # as free so users on older Clio builds still see
                    # newly-launched free models without a CLI release.
                    model_ids, pricing = union_with_portal_free_recommendations(
                        model_ids, pricing, _portal_for_recs,
                    )
                    model_ids, unavailable_models = partition_managed_models_by_tier(
            ...
            if model_ids:
                ...
                print("No curated models available for managed provider.")
        except Exception as exc:
        ...
        # them to managed provider with a mismatched model.  The managed-provider OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "managed".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. managed-provider credentials saved for future use.")
            print("  Run `clio model` again to switch to managed provider.")
            return

        config_path = _update_config_for_provider(
            "managed", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=managed)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Clio will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `clio model` or configure an API key to use Clio.")
        else:

"""
Backup and import commands for clio CLI.

`clio backup` creates a zip archive of the entire ~/.clio/ directory
(excluding the clio-agent repo and transient files).

`clio import` restores from a backup zip, overlaying onto the current
CLIO_HOME root.
"""
...
from clio_constants import get_default_clio_root, get_clio_home, display_clio_home
...
_EXCLUDED_DIRS = {
    "clio-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to clio root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside CLIO_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Clio home directory."""
    clio_root = get_default_clio_root()

    if not clio_root.is_dir():
    ...
    for dirpath, dirnames, filenames in os.walk(clio_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(clio_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(clio_root)
    ...
    print(f"\nRestore with: clio import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Clio backup.
    ...
    # Look for telltale files that a clio home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Clio backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.clio/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a clio dir name
        if prefix in {".clio", "clio"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_clio_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (clio_root / "config.yaml").exists()
        has_env = (clio_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Clio configuration.")
        ...
        for member in members:
            ...
            target = clio_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(clio_root.resolve())
            except ValueError:
        ...
        profiles_dir = clio_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # clio_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: clio profile list  (after installing clio)")

        # Guidance
        print()
        if not (clio_root / "clio-agent").is_dir():
            print("Note: The clio-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: clio update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  clio -p {pname} gateway install")

        print("Done. Your Clio configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and clio backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to CLIO_HOME).
...
# — `clio update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(clio_home: Optional[Path] = None) -> Path:
    home = clio_home or get_clio_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    clio_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(clio_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside CLIO_HOME. Kept in sync with
...
def _count_cron_jobs(path: Path) -> Optional[int]:
    ...
    A legacy bare-list shape (``[...]``) is also honored.
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``clio update``.
    ...
    Args:
        ...
        clio_home: Override for the Clio home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(clio_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for clio backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_clio_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, clio_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``clio_root`` to ``out_path``.
...
def _pre_update_backup_dir(clio_home: Optional[Path] = None) -> Path:
    home = clio_home or get_clio_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of CLIO_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<CLIO_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``clio update``) should continue even if the backup fails.
    """
    clio_root = clio_home or get_default_clio_root()
    if not clio_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(clio_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, clio_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `clio claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of CLIO_HOME under ``backups/`` before a
    ``clio claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``clio import <archive>``.  Writes to
    ``<CLIO_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `clio import` and the

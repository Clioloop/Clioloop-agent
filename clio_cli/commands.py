...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Clio config/state", "Session",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "Kill all running background processes", "Session"),
    CommandDef("pause", "Pause new work globally; '/pause off' resumes", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Clio works on across turns until achieved", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("subgoal", "Add or manage extra criteria on the active goal", "Session",
               args_hint="[text | remove N | clear]"),
    CommandDef("status", "Show session info", "Session"),
    CommandDef("context", "Show detailed context-window usage and category breakdown", "Session",
               aliases=("ctx",), args_hint="[all]", subcommands=("all",)),
    CommandDef("whoami", "Show your slash command access (admin / user)", "Info"),
    CommandDef("profile", "Show active profile name and home directory", "Info"),
    CommandDef("bots", "List profile-backed Clio Bots and their handles", "Info"),
    CommandDef(
        "botroom",
        "Manage bounded multi-Bot rooms",
        "Session",
        gateway_only=True,
        args_hint="[list|create|show|send|delete]",
        subcommands=("list", "create", "show", "send", "delete"),
    ),
    ...
    CommandDef("fusion",
               "Model Fusion (Pro): advisors/reviewers assist while the current main model works",
               "Configuration", args_hint="[off|status]",
    ...
    CommandDef("diff", "Show git changes in the working directory", "Info",
               args_hint="[staged|all] [--stat] [path...]",
               subcommands=("staged", "all")),
    CommandDef("worktree", "Audit or safely reclaim managed git worktrees and merged branches", "Info",
               cli_only=True, args_hint="[list|prune] [--dry-run] [--repo PATH]",
               subcommands=("list", "prune")),
    CommandDef("verbose", "Cycle tool progress display: off -> new -> all -> verbose",
               "Configuration", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("focus", "Toggle focus view — show only prompts and final responses",
    ...
    CommandDef("busy", "Control what Enter does while Clio is working", "Configuration",
    ...
    CommandDef("learn", "Learn a reusable skill from a directory, URL, chat, or notes",
    ...
    CommandDef("verify", "Run the detected project build/test verification recipe",
               "Tools & Skills", cli_only=True, args_hint="[build|test|all]",
               subcommands=("build", "test", "all")),
    ...
    CommandDef("evolve", "Skill evolution: merge overlapping skills, archive stale ones, suggest improvements",
               "Tools & Skills", args_hint="[--dry-run|--report]"),
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.clio/skills/ for newly installed or removed skills",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`clio_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/clio`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Clio
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``clio skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``clio skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``clio skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/clio <verb>`` subcommand.
    ...
    can still reach them via ``/clio <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/clio`` is always reserved as the first entry so the
    legacy ``/clio <subcommand>`` form keeps working for anything that
    ...
    # Reserve /clio as the catch-all top-level command.
    entries.append(("clio", "Talk to Clio or run a subcommand", "[subcommand] [args]"))
    seen.add("clio")
...
def slack_app_manifest(request_url: str = "https://clio-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /clio handler.

    Maps both canonical names and aliases so /clio bg do stuff works
    the same as /clio background do stuff.

    Plugin-registered slash commands are included so ``/clio <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from clio_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]

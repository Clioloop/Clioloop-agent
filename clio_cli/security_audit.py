...
1. The Clio venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.clio/plugins``
...
def _discover_plugins(clio_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.clio/plugins``.
...
def run_audit(
    *,
    ...
    home = clio_home or Path(get_clio_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, clio_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(clio_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `clio security audit`."""
    home = Path(get_clio_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, clio_home=home
    )
    if total == 0:

...
Invoked by ``clio kanban decompose [task_id | --all]`` and the
...
* Mirrors the shape of ``clio_cli/kanban_specify.py``: lazy aux
...
* ``faint=false`` collapses to the same effect as ``kanban specify``:
...
_SYSTEM_PROMPT = """You are the Kanban decomposer for the Clio Agent board.
...
def _profile_author() -> str:
    """Mirror of ``clio_cli.kanban._profile_author``."""
    return (
        os.environ.get("CLIO_PROFILE")
...
def decompose_task(
    ...
    *,
    ...
    faint=true with empty task list) — those surface via ``ok=False``.
    """
    ...
    faint = bool(parsed.get("faint", parsed.get("fanout")))
    ...
    if not faint:
        ...
        if title_val is None and body_val is None:
            return DecomposeOutcome(
                task_id, False, "decomposer returned faint=false with no title/body",
            )
        with kb.connect_closing() as conn:
        ...
        return DecomposeOutcome(
            task_id, True, "single task (no faint)",
            faint=False, new_title=title_val,
        )

    raw_tasks = parsed.get("tasks") or []
    if not isinstance(raw_tasks, list) or not raw_tasks:
        return DecomposeOutcome(
            task_id, False, "decomposer returned faint=true with empty tasks list",
        )
    ...
    return DecomposeOutcome(
        ...
        faint=True, child_ids=child_ids,
    )

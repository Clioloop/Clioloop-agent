...
Write path is the CLI (``clio cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_clio_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`clio cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `clio cron notepad {job_id} delete "

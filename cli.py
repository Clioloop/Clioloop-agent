#!/usr/bin/env python3
"""
Clio Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Clio Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills clio-agent-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: clio_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See clio_bootstrap.py for full rationale.
try:
    import clio_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when clio_bootstrap isn't registered in the venv
    # yet — happens during partial ``clio update`` where git-reset landed
...
os.environ["CLIO_QUIET"] = "1"  # Our own modules

import yaml

from clio_cli.fallback_config import get_fallback_chain
...
try:
    from clio_cli.pt_input_extras import (
...
from clio_cli.banner import _format_context_length, format_banner_version_label
...
from clio_cli.browser_connect import (
...
_clio_home = get_clio_home()
_project_env = Path(__file__).parent / '.env'
load_clio_dotenv(clio_home=_clio_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.clio/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("CLIO_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.clio/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If CLIO_IGNORE_USER_CONFIG=1 is set (via ``clio chat --ignore-user-config``),
    the user config at ``~/.clio/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({CLIO_HOME}/config.yaml)
    user_config_path = _clio_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("CLIO_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "clio-spinner": "You are a clio-spinner assistant! Use cute expressions like (◕‿◕), ★, ♪, and ~! Add sparkles and be super enthusiastic about everything! Every response should feel warm and adorable desu~! ヽ(>∀<☆)ノ",
                "catgirl": "You are Neko-chan, an anime catgirl AI assistant, nya~! Add 'nya' and cat-like expressions to your speech. Use kaomoji like (=^･ω･^=) and ฅ^•ﻌ•^ฅ. Be playful and curious like a cat, nya~!",
                "pirate": "Arrr! Ye be talkin' to Captain Clio, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Clio - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see clio_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from clio_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and ClioCLI.__init__ checks "default" first.
    ...
    # Normalize config key: the new config system (clio_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && clio` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _CLIO_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_CLIO_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["CLIO_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.clio/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from clio_logging import setup_logging
...
try:
    from clio_cli.config import print_config_warnings
...
try:
    from clio_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``clio --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from clio_cli.mcp_startup import wait_for_mcp_discovery
...
from clio_cli.banner import build_welcome_banner
from clio_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from clio_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("CLIO_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("CLIO_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _run_cleanup():
    ...
    try:
        from clio_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run clio -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"clio-{short_id}"
    branch_name = f"clio/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`clio_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from clio_cli.config import load_config as _load_full_config
        from clio_constants import get_clio_home as _get_clio_home
        _clio_home_maint = _get_clio_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_clio_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`clio_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``clio/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("clio-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``clio/clio-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``clio -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("clio/clio-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. CLIO_LIGHT / CLIO_TUI_LIGHT env (true/false) — explicit override
#   2. CLIO_TUI_THEME=light|dark — explicit theme
#   3. CLIO_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Clio assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("CLIO_LIGHT", "CLIO_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("CLIO_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("CLIO_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from clio_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_clio_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._clio_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from clio_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Clio output it's common to display cron schedules verbatim.
    #
...
from clio_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_clio_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_clio_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._clio_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_clio_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._clio_bp_start = now
        ...
        _vt100_mod._clio_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``ClioCLI._busy_command()``.
        """
        yield self

# ASCII Art - CLIO-AGENT logo (full width, single line - requires ~95 char terminal)
CLIO_AGENT_LOGO = """[bold #A78BFA]  _____ _      _____ ____             _____ ______ _   _ _______[/]
[bold #A78BFA] / ____| |    |_   _/ __ \\      /\\   / ____|  ____| \\ | |__   __|[/]
[#7C3AED]| |    | |      | || |  | |    /  \\ | |  __| |__  |  \\| |  | |   [/]
[#7C3AED]| |    | |      | || |  | |   / /\\ \\| | |_ |  __| | . ` |  | |   [/]
[#5B21B6]| |____| |____ _| || |__| |  / ____ \\ |__| | |____| |\\  |  | |   [/]
[#5B21B6] \\_____|______|_____\\____/  /_/    \\_\\_____|______|_| \\_|  |_|   [/]"""

# ASCII Art - Clio Caduceus (compact, fits in left panel)
CLIO_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    border_color = _skin.get_color("banner_border", "#6D28D9") if _skin else "#6D28D9"
    title_color = _skin.get_color("banner_title", "#C4B5FD") if _skin else "#C4B5FD"
    dim_color = _skin.get_color("banner_dim", "#A78BFA") if _skin else "#A78BFA"

    if skin_name == "default":
        line1 = "∞ CLIO - AI Agent Framework"
        tiny_line = "∞ CLIO"
    else:
        agent_name = _skin.get_branding("agent_name", "Clio Agent") if _skin else "Clio Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("CLIO_FAST_STARTUP_BANNER") == "1":
        from clio_cli import __release_date__ as _release_date
        from clio_cli import __version__ as _version

        version_line = f"Clio Agent v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Omni Loop Labs[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from clio_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.clio/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.clio/config.yaml on first use)
...
class ClioCLI:
    """
    Interactive CLI for the Clio Agent.
    ...
    def __init__(
        ...
        Args:
            ...
            max_turns: Maximum tool-calling iterations shared with subagents (default: unlimited; positive values stay finite)
        ...
        # Read max_tokens from config (env var override: CLIO_MAX_TOKENS)
        _env_mt = os.environ.get("CLIO_MAX_TOKENS")
        if _env_mt:
            try:
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from clio_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("CLIO_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        # by `clio chat --ignore-rules` in clio_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("CLIO_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("CLIO_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Clio processes for this CLIO_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.clio/checkpoints/.  Opt-in via
        ...
        self._history_file = _clio_home / ".clio_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from clio_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"∞ {self.model if getattr(self, 'model', None) else 'Clio'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from clio_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from clio_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from clio_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from clio_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "∞ Clio")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - ClioCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from clio_cli.runtime_provider import (
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: clio setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: clio setup")
            return False
        ...
        # `clio chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `clio auth add openai-codex`
        # without `clio model`), fall back to the provider's first catalog
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from clio_cli.models import resolve_fast_mode_overrides
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (clio sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (clio sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``clio doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``clio doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Clio needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a managed provider Clio LLM (not agentic)
        from clio_cli.model_switch import is_managed_clio_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_managed_clio_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Omni Loop Labs Clio 3 & 4 models are NOT agentic and are not "
                "designed for use with Clio Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Clio: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Clio: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.clio/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from clio_cli.clipboard import save_clipboard_image

        img_dir = get_clio_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: clio --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Clio config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_clio_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from clio_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run clio chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'clio setup' to configure[/]")
        except Exception:
    ...
    def show_tools(self):
        """Display available tools with clio-spinner ASCII art."""
        tools = get_tool_definitions(enabled_toolsets=self.enabled_toolsets, quiet_mode=True)
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in clio_cli/colors.py
        ...
        from clio_cli.tools_config import _get_platform_tools
        from clio_cli.config import load_config
    ...
    def show_toolsets(self):
        """Display available toolsets with clio-spinner ASCII art."""
    ...
    def _handle_profile_command(self):
        ...
        from clio_constants import display_clio_home
        from clio_cli.profiles import get_active_profile_name

        display = display_clio_home()
    ...
    def show_config(self):
        """Display current configuration with clio-spinner ASCII art."""
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from clio_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("CLIO_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        """
        from clio_state import format_session_db_unavailable
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `clio gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `clio sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from clio_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `clio sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.clio/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``clio --resume <id>``
        ...
        saved_dir = get_clio_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"clio_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: clio --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from clio_cli.curses_ui import curses_single_select
    ...
    # ----- /fusion: 3-step model picker (reuses the /model picker modal) -----

    def _handle_fusion_command(self, cmd_original: str) -> None:
        """Handle /fusion — configure Model Fusion (Max plan, managed provider).

        Forms:
          /fusion                  open the 3-step picker (advisor 1, advisor 2, main)
          /fusion <m1> <m2> <judge>  set the two advisors and main model directly
          /fusion off              turn fusion mode off
          /fusion status           show the current configuration
        """
        import shlex
        try:
            tokens = shlex.split(cmd_original)[1:]
        except ValueError:
            tokens = cmd_original.split()[1:]
        ...
        def _emit(text: str) -> None:
            for line in text.split("\n"):
                _cprint("  " + line.replace("*", ""))

        if sub == "off":
            if self.agent is not None:
                cfg = get_fusion_config(self.agent)
                cfg.enabled = False
                set_fusion_config(self.agent, cfg)
            _cprint("  🔮 Fusion mode off — replies use your normal model again.")
            return

        if sub == "status":
            if self.agent is None:
                _cprint("  🔮 Fusion is not configured.")
            else:
                _emit(status_notice(self.agent))
            return

        # Everything below configures/enables fusion — gate first.
        ...
        # `m1 m2 judge`. Anything incomplete opens the interactive picker.
        parsed = parse_fusion_args(tokens)
        if parsed is not None:
            if self.agent is not None:
                set_fusion_config(self.agent, parsed)
            _emit(activation_notice(parsed))
            return

        # No args (or "on"): open the interactive picker.
        self._open_fusion_picker()

    # Count options for the "how many models?" phases (1..5).
    _FUSION_MAX_PER_GROUP = 5

    def _open_fusion_picker(self) -> None:
        """Open the phased fusion picker over the managed-portal catalog.

        Flow: how-many-planners → pick planners → how-many-reviewers → pick
        reviewers → pick the main model.
        """
        from agent.fusion_engine import model_open_tag
        try:
            from clio_cli.models import provider_model_ids
            ids = provider_model_ids("managed")
        except Exception as exc:
            _cprint(f"  ✗ Couldn't load the Omni Loop Portal model list: {exc}")
            return
        if not ids:
            _cprint("  ✗ No models available on your Omni Loop Portal plan.")
            return
        try:
            from clio_cli.models import get_pricing_for_provider
            managed_pricing = get_pricing_for_provider("managed")
        except Exception:
            managed_pricing = None
        # Open-weight models first (bare ids), then OpenRouter models — each
        # sorted — so the full catalog is browsable and clearly grouped.
        opens = sorted(m for m in ids if "/" not in m)
        oroute = sorted(m for m in ids if "/" in m)
        ordered = opens + oroute
        labels = [f"{m}  {model_open_tag(m, managed_pricing)}" for m in ordered]
        ...
        state = {
            ...
            "reviewers": [],
            "fusion_ids": ordered,
            "fusion_model_labels": labels,
            "selected": 0,
            "current_model": self.model or "",
            "current_provider": self.provider or "",
        }
        self._model_picker_state = state
        self._refresh_fusion_picker_view(state)
        self._invalidate(min_interval=0.0)

    @staticmethod
    def _fusion_is_count_phase(phase: str) -> bool:
        return phase in ("planner_count", "reviewer_count")

    def _refresh_fusion_picker_view(self, state: dict) -> None:
        """Set the visible list + title for the current phase and reset cursor."""
        phase = state.get("phase", "planner_count")
        if self._fusion_is_count_phase(phase):
            state["model_list"] = [
                f"{n} model{'s' if n != 1 else ''}"
                for n in range(1, self._FUSION_MAX_PER_GROUP + 1)
            ]
        else:
            state["model_list"] = list(state.get("fusion_model_labels") or [])
        state["selected"] = 0
        self._set_fusion_picker_title(state)
        self._invalidate(min_interval=0.0)

    def _set_fusion_picker_title(self, state: dict) -> None:
        from agent.fusion_engine import label_model
        phase = state.get("phase", "planner_count")
        advisors = state.get("advisors") or []
        reviewers = state.get("reviewers") or []
        if phase == "planner_count":
            title = "🔮 Fusion: how many models should plan? (1–5)"
        elif phase == "planner_pick":
            title = f"🔮 Fusion: pick planner {len(advisors) + 1} of {state.get('planner_count')}"
        elif phase == "reviewer_count":
            title = "🔮 Fusion: how many models should review? (1–5)"
        elif phase == "reviewer_pick":
            title = f"🔮 Fusion: pick reviewer {len(reviewers) + 1} of {state.get('reviewer_count')}"
        else:  # judge_pick
            title = "🔮 Fusion: pick the main model (does the work + fuses)"
        if advisors:
            title += "   planners: " + " · ".join(label_model(p) for p in advisors)
        if reviewers:
            title += "   reviewers: " + " · ".join(label_model(p) for p in reviewers)
        state["provider_data"] = {"name": title, "slug": "managed"}

    def _fusion_back(self, state: dict) -> bool:
        """Step one move backwards; return False if already at the first phase."""
        phase = state.get("phase")
        if phase == "judge_pick":
            if state.get("reviewers"):
                state["reviewers"].pop()
            state["phase"] = "reviewer_pick"
        elif phase == "reviewer_pick":
            if state.get("reviewers"):
                state["reviewers"].pop()
            else:
                state["phase"] = "reviewer_count"
        elif phase == "reviewer_count":
            if state.get("advisors"):
                state["advisors"].pop()
            state["phase"] = "planner_pick"
        elif phase == "planner_pick":
            if state.get("advisors"):
                state["advisors"].pop()
            else:
                state["phase"] = "planner_count"
        else:  # planner_count — first phase
            return False
        return True

    def _fusion_picker_select(self, selected: int) -> None:
        state = self._model_picker_state
        if not state:
            return
        ids = state.get("fusion_ids") or []
        ...
        if selected >= cancel_idx:
            self._close_model_picker()
            return
        if not (0 <= selected < len(model_list)):
            self._close_model_picker()
            return

        if self._fusion_is_count_phase(phase):
            n = max(1, min(self._FUSION_MAX_PER_GROUP, selected + 1))
            if phase == "planner_count":
                state["planner_count"] = n
                state["phase"] = "planner_pick"
            else:
                state["reviewer_count"] = n
                state["phase"] = "reviewer_pick"
            self._refresh_fusion_picker_view(state)
            return
        ...
        if phase == "planner_pick":
            state["advisors"].append(model_id)
            if len(state["advisors"]) >= state["planner_count"]:
                state["phase"] = "reviewer_count"
            self._refresh_fusion_picker_view(state)
            return
        if phase == "reviewer_pick":
            state["reviewers"].append(model_id)
            if len(state["reviewers"]) >= state["reviewer_count"]:
                state["phase"] = "judge_pick"
            self._refresh_fusion_picker_view(state)
            return
        # judge_pick — finalize.
        self._finalize_fusion(
            list(state.get("advisors") or []),
            list(state.get("reviewers") or []),
            model_id,
        )

    def _finalize_fusion(self, advisors: list, reviewers: list, judge: str) -> None:
        from agent.fusion_engine import (
            FusionConfig, set_fusion_config, activation_notice,
        )
        cfg = FusionConfig(
            advisors=list(advisors), reviewers=list(reviewers),
            judge=judge, enabled=True,
        )
        self._close_model_picker()
        if self.agent is not None:
            set_fusion_config(self.agent, cfg)
        for line in activation_notice(cfg).split("\n"):
            _cprint("  " + line.replace("*", ""))
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        # Copilot, and managed-provider-enforced caps win over the raw models.dev entry
        # (e.g. gpt-5.5 is 1.05M on openai but 272K on Codex OAuth).
        mi = result.model_info
        try:
            from clio_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # Model IDs to offer. For the managed Omni Loop Portal provider,
            # always use the FULL live per-account catalog so every entitled
            # model shows (the curated snapshot is capped/partial). Other
            # providers keep their curated list, falling back to the live
            # catalog only when empty.
            model_ids = list(provider_data.get("models", []) or [])
            if slug == "managed":
                try:
                    from clio_cli.models import provider_model_ids
                    live = provider_model_ids("managed")
                    if live:
                        # open-weight first, then OpenRouter — grouped + sorted,
                        # mirroring the Fusion picker.
                        opens = sorted(m for m in live if "/" not in m)
                        oroute = sorted(m for m in live if "/" in m)
                        model_ids = opens + oroute
                except Exception:
                    pass
            elif not model_ids:
                try:
                    ...
                    pass
            # Tag managed entries (open) / (openrouter) so the distinction is
            # visible, like the Fusion picker. Other providers render plain ids.
            if slug == "managed":
                from agent.fusion_engine import model_open_tag
                try:
                    from clio_cli.models import get_pricing_for_provider
                    ...
                    managed_pricing = None
                model_list = [f"{m}  {model_open_tag(m, managed_pricing)}" for m in model_ids]
            else:
        ...
        if stage == "model":
            if state.get("fusion"):
                self._fusion_picker_select(selected)
                return
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from clio_cli.model_switch import switch_model, parse_model_flags
        from clio_cli.providers import get_label
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to clio_cli.curator so the CLI and the `clio curator`
        ...
        try:
            from clio_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from clio_cli.kanban import run_slash
    ...
    def _handle_evolve_command(self, cmd: str):
        """Handle /evolve slash command — delegate to the evolve engine."""
        import shlex

        tokens = shlex.split(cmd)[1:] if cmd else []

        dry_run = "--dry-run" in tokens
        report_only = "--report" in tokens
        ...
        try:
            ...
            print(f"(._.) evolve error: {exc}")

    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to clio_cli.skills_hub."""
        from clio_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_clio_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_clio_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in clio_cli/commands.py instead of touching every dispatch site.
        from clio_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from clio_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from clio_cli.skin_engine import get_active_skin
            ...
            from clio_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from clio_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_clio_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from clio_cli.commands import COMMANDS
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``clio bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: clio bundles create "
        ...
        _cprint(
            ...
            f"Manage with `clio bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Clio-managed isolated debug "
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from clio_cli.goals import GoalManager
            from clio_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Clio keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from clio_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_clio_home()}/skins/\n")
            return
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``CLIO_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``CLIO_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Clio is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Clio is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Clio is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from clio_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from clio_cli.partial_compress import (
    ...
    def _handle_debug_command(self):
        ...
        from clio_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Clio Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``clio update`` so the user sees update output directly and gets
        ...
        when cancelled.
        """
        from clio_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update Clio Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Clio Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="∞  Update Clio Agent",
            detail="This will exit the current session and run `clio update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by clio_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from clio_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.clio/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_clio_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "clio_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "clio_voice",
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        try:
            ...
            def run_agent():
                ...
                try:
                    from agent.fusion_engine import fusion_is_active, run_fusion_turn
                    if fusion_is_active(self.agent):
                        def _fusion_cprint(ev):
                            # ev is a FusionEvent (str() yields the label). The
                            # main model's work + final answer print through the
                            # normal stream; here we print the phase label and,
                            # for the reviewer decision, the critique/approval
                            # notes so the reviewer step is visible in the CLI.
                            _cprint(f"  {getattr(ev, 'text', ev)}")
                            _detail = getattr(ev, "detail", "")
                            if _detail and getattr(ev, "phase", "") in ("critique", "approved"):
                                for _line in str(_detail).splitlines():
                                    _cprint(f"    {_line}")
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _clio_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard CLIO_HOME, so no -p needed.
            try:
                from clio_cli.profiles import get_active_profile_name
            ...
            print(f"  clio --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  clio -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from clio_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from clio_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from clio_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Clio Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Clio Agent! Type your message or /help for commands."
        ...
        try:
            from clio_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            _redact_raw = os.getenv("CLIO_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(CLIO_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Clio migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from clio_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in clio_cli.pt_input_extras
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_clio_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Clio binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from clio_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Clio Agent")
        ...
        # Dynamic prompt: shows Clio symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Clio needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Clio needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Clio needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = ClioCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_clio_osd_patched", False):
                ...
                _pt_renderer._clio_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``CLIO_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("CLIO_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the clio process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: clio setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                if getattr(self, '_delete_session_on_exit', False):
                    try:
                        from clio_constants import get_clio_home as _ghh
...
def _run_kanban_goal_loop_q(cli: "ClioCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``CLIO_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("CLIO_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from clio_cli import kanban_db as _kb
    from clio_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Clio Agent CLI - Interactive AI Assistant
    ...
    os.environ["CLIO_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Clio Gateway (messaging platforms)...")
    ...
    # Default to clio-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside ClioCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  CLIO_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("CLIO_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``clio chat -q "work kanban task <id>"``;
        ...
        _kanban_task_id = os.environ.get("CLIO_KANBAN_TASK", "").strip()
        if _kanban_task_id:
            try:
                from clio_cli import kanban_db as _kb
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    if os.environ.get("CLIO_KANBAN_GOAL_MODE") == "1":
                        try:
                    ...
                    if isinstance(result, dict) and result.get("failed"):
                        _exit_code = 1
                        if os.environ.get("CLIO_KANBAN_TASK") and result.get(
                            "failure_reason"
                        ) in ("rate_limit", "billing"):
                            try:
                                from clio_cli.kanban_db import (
            ...
            # Single-query mode (`clio chat -q "…"`): skip the welcome

...
## Client ID
- Need to register a "Desktop app" OAuth client on a Omni Loop Labs GCP project
...
## Token Lifecycle
- Store at `~/.clio/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `clio_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `clio_cli/models.py` — Add Gemini model catalog
4. `clio_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `clio_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `clio_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Omni Loop Labs GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via CLIO_GEMINI_CLIENT_ID env var
...
- clio-agent Copilot OAuth: `clio_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
